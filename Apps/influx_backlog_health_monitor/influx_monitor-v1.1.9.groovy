 ...
 *  v1.1.9 - Restored count, color HTML styling, and trend arrows; prepended formatted time; set label to 'Influx Backlog: '.
 *  ------------------------------------------------------------------------------------------------
 */

private static String appVersion() { return "1.1.9" }
...
def processManagerUpdate() {
    ...
    // Colored Bracket Volume Tag based on count thresholds
    String volumeTag = ""
    if (currentVal <= 100) {
        volumeTag = "<font color='#4CAF50'>[N]</font> "
    } else if (currentVal >= 101 && currentVal <= 1000) {
        volumeTag = "<font color='#FF9800'>[L]</font> "
    } else if (currentVal >= 1001 && currentVal <= 2500) {
        volumeTag = "<font color='#FF5722'>[H]</font> "
    } else if (currentVal > 2500) {
        volumeTag = "<font color='#F44336'>[E]</font> "
    }

    // Trend calculation with arrows or 'steady'
    String trendMessage = ""
    if (currentVal > previousVal) {
        def diff = (currentVal - previousVal).round(2)
        trendMessage = " <font color='#FF5722'>▲ +${diff}</font>"
    } else if (currentVal < previousVal) {
        def diff = (previousVal - currentVal).round(2)
        trendMessage = " <font color='#4CAF50'>▼ -${diff}</font>"
    } else {
        trendMessage = " steady"
    }
    
    // Construct output: 9:33a [N] Influx Backlog: 45 steady
