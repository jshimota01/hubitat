 ...
 * Notes: Validates InfluxDB v2 connections, tracks hub event backlog variables via scheduled polling, and pushes dashboard notifications
 * Category: Utility
 **/
/**
...
/**
 *  Purpose:
 *  Validates InfluxDB v2 HTTP endpoint connections and manages automated backlog status updates, macro-level scheduled variable tracking,
 *  and dashboard notification tile pushes without event subscription chatter.
 *  
 *  Changelog:
 *  v1.4.0    09/24/26    jshimota    Reverted variable tracking to pure scheduled polling. Removed real-time event subscriptions and dynamic schedule boosting to eliminate tile chatter and thread overhead.
...
def timeStamp() { return "2026/09/24 10:25 AM" }
...
definition(
    ...
    iconX2Url: "",
    iconX3Url: "",
    importUrl: "https://raw.githubusercontent.com/jshimota01/hubitat/main/Apps/influxdb_backlog_health_monitor/influx_backlog_health_monitor_v1.4.0.groovy",
    singleThreaded: true
)
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>Section 2. Notification Manager</b>") {
            ...
            if (getSettingBool("prefEnableManager", false)) {
                input name: "prefUpdateInterval", type: "enum", title: "Scheduled Polling Interval", options: ["1":"1 Minute", "5":"5 Minutes", "15":"15 Minutes", "30":"30 Minutes", "60":"1 Hour"], defaultValue: "15", required: true
...
private void applySchedule(String intervalMinutes) {
    ...
    unschedule("processManagerUpdate")
    logDebug "Setting polling schedule to execute processManagerUpdate every ${intervalMinutes} minute(s)."
