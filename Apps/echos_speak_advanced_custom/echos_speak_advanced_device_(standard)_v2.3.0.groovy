...
/**
 *  Purpose:
 *  Child Device Driver for managing standard audio Amazon Echo devices bridged via Echos Speak Advanced Parent App.
 *
 *  Changelog:
...
@Field static final List<String> alexaVoices = ["", "Ivy", "Joanna", "Joey", "Justin", "Kendra", "Kimberly", "Matthew", "Salli"]
...
@Field static final List<Map> deviceModelTable = [
    ...
    [deviceType: "A2DS1Q2TPDJ48U", family: "ECHO", generation: "5", modelName: "Echo Dot with Clock", imageFilename: "echo_dot_clock_gen4", validated: true],
...
def timeStamp() { return "2026/09/22 03:45 PM" }

metadata {
    definition (
        name: "Echos Speak Advanced Device",
        namespace: "jshimota",
        ...
        command "playText", [
            ...
            [name: "voice", type: "ENUM", constraints: alexaVoices, description: "Optional Alexa SSML Voice Profile"],
...
void playText(String text, String voice = null, Object volume = null) {
    if (!text) return
    logInfo "GUI Action: [playText] triggered -> Routing text through speak(): \"${text}\"${voice ? " | Voice: ${voice}" : ""}${volume != null ? " | Volume: ${volume}%" : ""}"
    speak(text, voice, volume, false)
}

void playTextAndRestore(String text, String voice = null, Object volume = null) {
    if (!text) return
    logInfo "GUI Action: [playTextAndRestore] triggered -> Routing text through speak() with volume (${volume ?: 'current'}%): \"${text}\"${voice ? " | Voice: ${voice}" : ""}"
    speak(text, voice, volume, false)
}
...
void speak(String text, String voice = null, Object volume = null, Object ignoreDnd = false) {
    ...
    String selectedVoice = (voice && voice.trim() != "" && voice != "Default") ? voice.trim() : null
    logInfo "GUI Action: [speak] command triggered -> Text: \"${text}\"${selectedVoice ? " | Voice: ${selectedVoice}" : " | Voice: Default"}${volume != null ? " | Volume: ${volume}%" : ""}${ignoreDnd ? " | Ignore DND: true" : ""}"
    ...
    processOutputPayload(text, "speak", [text: text, voice: selectedVoice])
...
private void processOutputPayload(String text, String commandType, Map payload) {
    ...
    if (parent) {
        ...
        Boolean success = parent.sendBridgeCommand(device.deviceNetworkId, commandType, payload)
        if (!success) {
            logError "${device.displayName} - ${commandType} payload dispatch failed on bridge server."
        }
    } else {
        logError "${device.displayName} - Parent application reference missing."
    }
...
void voiceCmdAsText(String text) {
    ...
    if (parent) {
        Boolean success = parent.sendBridgeCommand(device.deviceNetworkId, "voiceCmdAsText", [text: text])
        if (!success) {
            logError "${device.displayName} - voiceCmdAsText command failed on bridge server."
        }
    } else {
        logError "${device.displayName} - Parent application reference missing."
    }
...
void setDisplayBrightness(level) {
    ...
    if (!hasServerCapability("displayBrightness")) {
    ...
    if (parent) {
        ...
        if (success == true) {
            ...
            logError "${device.displayName} - setDisplayBrightness command failed on bridge server."
        }
    } else {
        logError "${device.displayName} - Parent application reference missing."
    }
...
void setDisplayPowerOn() {
    ...
    if (parent) {
        ...
        if (success == true) {
            ...
            logError "${device.displayName} - setDisplayPowerOn command failed on bridge server."
        }
    } else {
        logError "${device.displayName} - Parent application reference missing."
    }
...
void setDisplayPowerOff() {
    ...
    if (parent) {
        ...
        if (success == true) {
            ...
            logError "${device.displayName} - setDisplayPowerOff command failed on bridge server."
        }
    } else {
        logError "${device.displayName} - Parent application reference missing."
    }
...
void setAdaptiveBrightnessOn() {
    ...
    if (parent) {
        ...
        if (success == true) {
            ...
            logError "${device.displayName} - setAdaptiveBrightnessOn command failed on bridge server."
        }
    } else {
        logError "${device.displayName} - Parent application reference missing."
    }
...
void setAdaptiveBrightnessOff() {
    ...
    if (parent) {
        ...
        if (success == true) {
            ...
            logError "${device.displayName} - setAdaptiveBrightnessOff command failed on bridge server."
        }
    } else {
        logError "${device.displayName} - Parent application reference missing."
    }
...
void setLevel(level, duration = null) {
    if (hasServerCapability("displayBrightness")) {
        logInfo "GUI Action: [setLevel] triggered -> Display Brightness Level: ${level}%"
...
void setVolume(volumeLevel) {
    ...
    if (parent) {
        Boolean success = parent.sendBridgeCommand(device.deviceNetworkId, "volume", [level: vol])
        if (!success) {
            logError "${device.displayName} - setVolume command failed on bridge server."
        }
    } else {
        logError "${device.displayName} - Parent application reference missing."
    }
...
void mute() {
    ...
    if (parent) {
        Boolean success = parent.sendBridgeCommand(device.deviceNetworkId, "volume", [level: 0])
        if (!success) {
            logError "${device.displayName} - mute command failed on bridge server."
        }
    } else {
        logError "${device.displayName} - Parent application reference missing."
    }
...
void setDoNotDisturbOn() {
    ...
    if (parent) {
        Boolean success = parent.sendBridgeCommand(device.deviceNetworkId, "dnd", [state: true])
        if (success == true) {
            sendEvent(name: "doNotDisturb", value: "enabled")
        } else {
            logError "${device.displayName} - setDoNotDisturbOn command failed on bridge server."
        }
    } else {
        logError "${device.displayName} - Parent application reference missing."
    }
...
void setDoNotDisturbOff() {
    ...
    if (parent) {
        Boolean success = parent.sendBridgeCommand(device.deviceNetworkId, "dnd", [state: false])
        if (success == true) {
            sendEvent(name: "doNotDisturb", value: "disabled")
        } else {
            logError "${device.displayName} - setDoNotDisturbOff command failed on bridge server."
        }
    } else {
        logError "${device.displayName} - Parent application reference missing."
    }
