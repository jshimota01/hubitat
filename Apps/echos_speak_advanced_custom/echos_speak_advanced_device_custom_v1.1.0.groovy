...
def timeStamp() { return "2026/09/09 11:30 AM" }
...
metadata {
    definition (
        ...
        command "speak", [[name: "Text", type: "STRING", description: "Enter text for Echo to speak"]]
        ...
        command "sendCmdToToDevice", [[name: "Command", type: "STRING", description: "Raw command string"], [name: "Value", type: "STRING", description: "Raw value string"]]

        // Custom Commands - Automation & Actions
        command "setDoNotDisturb", [[name: "State", type: "ENUM", constraints: ["true", "false"], description: "Toggle Do Not Disturb mode"]]
        command "setAlarm", [[name: "Time", type: "STRING", description: "Format: YYYY-MM-DD HH:MM:SS"]]
        command "createReminder", [[name: "Text", type: "STRING", description: "Reminder text"], [name: "Time", type: "STRING", description: "Format: YYYY-MM-DD HH:MM:SS"]]
        command "setAlarmVolume", [[name: "Volume", type: "NUMBER", description: "Alarm volume level (0-100)"]]
        ...
        command "playTextAndRestore", [[name: "Text", type: "STRING", description: "Text to speak"], [name: "Volume", type: "NUMBER", description: "Temporary volume level (0-100)"]]
        command "playTextAndRestoreAll", [[name: "Text", type: "STRING", description: "Text to speak on all devices"], [name: "Volume", type: "NUMBER", description: "Temporary volume level (0-100)"]]
...
/* =========================================================================================
   BIGDECIMAL HELPER ENGINE
   ========================================================================================= */

private BigDecimal bd(Object value, Integer scale = null) {
    if (value == null || value.toString().trim() == "" || value.toString() == "null") return null
    try {
        BigDecimal result = value.toString().trim().toBigDecimal()
        if (scale != null) {
            result = result.setScale(scale, BigDecimal.ROUND_HALF_UP)
        }
        return result
    } catch (Exception e) {
        logWarn "bd(): Unable to parse value '${value}' as BigDecimal"
        return null
    }
}
...
void speak(String text) {
    ...
    if (!text || text.trim() == "" || text == "null") {
        logWarn "speak(): Empty or null text string provided; ignoring command."
        return
    }
    String cleanText = text.trim()
    logInfo "speak(): ${cleanText}"
    updateAttribute("lastSpokenText", cleanText)
    parent?.sendBridgeCommand(device.deviceNetworkId, "speak", [text: cleanText])
}

void speak(String text, Object volume, String voice = null) {
    checkAndLogVersionDemarcation()
    if (!text || text.trim() == "" || text == "null") {
        logWarn "speak(): Empty or null text string provided; ignoring command."
        return
    }
    if (volume != null && volume.toString().trim() != "") setVolume(volume)
    speak(text)
}

void playText(String text) { speak(text) }
void playText(String text, Object volume) { speak(text, volume) }

void playTextAndRestore(String text, Object volume = null) {
    checkAndLogVersionDemarcation()
    if (!text || text.trim() == "" || text == "null") {
        logWarn "playTextAndRestore(): Empty or null text string provided; ignoring command."
        return
    }
    speak(text, volume)
}

void playTextAndRestoreAll(String text, Object volume = null) {
    checkAndLogVersionDemarcation()
    if (!text || text.trim() == "" || text == "null") {
        logWarn "playTextAndRestoreAll(): Empty or null text string provided; ignoring command."
        return
    }
    speak(text, volume)
}
...
void playAnnouncement(String text) {
    checkAndLogVersionDemarcation()
    if (!text || text.trim() == "" || text == "null") {
        logWarn "playAnnouncement(): Empty or null text string provided; ignoring command."
        return
    }
    String cleanText = text.trim()
    logInfo "playAnnouncement(): ${cleanText}"
    updateAttribute("lastAnnouncement", cleanText)
    parent?.sendBridgeCommand(device.deviceNetworkId, "announce", [text: cleanText])
}

void playAnnouncementAll(String text) {
    checkAndLogVersionDemarcation()
    if (!text || text.trim() == "" || text == "null") {
        logWarn "playAnnouncementAll(): Empty or null text string provided; ignoring command."
        return
    }
    String cleanText = text.trim()
    logInfo "playAnnouncementAll(): ${cleanText}"
    updateAttribute("lastAnnouncement", cleanText)
    parent?.sendBridgeCommand(device.deviceNetworkId, "announce", [text: cleanText, target: "all"])
}

void playAnnouncementIncludeVis(String text) {
    checkAndLogVersionDemarcation()
    if (!text || text.trim() == "" || text == "null") {
        logWarn "playAnnouncementIncludeVis(): Empty or null text string provided; ignoring command."
        return
    }
    String cleanText = text.trim()
    logInfo "playAnnouncementIncludeVis(): ${cleanText}"
    updateAttribute("lastAnnouncement", cleanText)
    parent?.sendBridgeCommand(device.deviceNetworkId, "announce", [text: cleanText, display: true])
}

void sendCmdToToDevice(String cmd, String val = "") {
    checkAndLogVersionDemarcation()
    if (!cmd || cmd.trim() == "" || cmd == "null") {
        logWarn "sendCmdToToDevice(): Command string parameter is required; ignoring command."
        return
    }
    logInfo "sendCmdToToDevice(): cmd=${cmd}, val=${val}"
    parent?.sendBridgeCommand(device.deviceNetworkId, "sendCmd", [cmd: cmd.trim(), val: (val ?: "").trim()])
}

void voiceCmdAsText(String cmd) {
    checkAndLogVersionDemarcation()
    if (!cmd || cmd.trim() == "" || cmd == "null") {
        logWarn "voiceCmdAsText(): Command text parameter is required; ignoring command."
        return
    }
    String cleanCmd = cmd.trim()
    logInfo "voiceCmdAsText(): ${cleanCmd}"
    parent?.sendBridgeCommand(device.deviceNetworkId, "voiceCmd", [command: cleanCmd])
}
...
void setDoNotDisturb(String state) {
    ...
    if (!state || state.trim() == "" || state == "null") {
        logWarn "setDoNotDisturb(): State parameter is required; ignoring command."
        return
    }
    String cleanState = state.trim().toLowerCase()
    logInfo "setDoNotDisturb(): ${cleanState}"
    updateAttribute("dnd", cleanState)
    updateAttribute("doNotDisturb", cleanState)
    parent?.sendBridgeCommand(device.deviceNetworkId, "dnd", [state: (cleanState == "true")])
}

void setAlarm(String timeStr) {
    checkAndLogVersionDemarcation()
    if (!timeStr || timeStr.trim() == "" || timeStr == "null") {
        logWarn "setAlarm(): Time string parameter is required; ignoring command."
        return
    }
    String cleanTime = timeStr.trim()
    logInfo "setAlarm(): ${cleanTime}"
    parent?.sendBridgeCommand(device.deviceNetworkId, "setAlarm", [time: cleanTime])
}

void createReminder(String text, String timeStr) {
    checkAndLogVersionDemarcation()
    if (!text || text.trim() == "" || !timeStr || timeStr.trim() == "") {
        logWarn "createReminder(): Both text and time string parameters are required; ignoring command."
        return
    }
    logInfo "createReminder(): text=${text}, time=${timeStr}"
    parent?.sendBridgeCommand(device.deviceNetworkId, "createReminder", [text: text.trim(), time: timeStr.trim()])
}

void setAlarmVolume(Object level) {
    checkAndLogVersionDemarcation()
    if (level == null || level.toString().trim() == "") {
        logWarn "setAlarmVolume(): Volume level parameter is required; ignoring command."
        return
    }
    BigDecimal targetVol = bd(level, 0)
    if (targetVol == null) return
    logInfo "setAlarmVolume(): ${targetVol}"
    updateAttribute("alarmVolume", targetVol)
    parent?.sendBridgeCommand(device.deviceNetworkId, "setAlarmVolume", [level: targetVol])
}

/* =========================================================================================
   MEDIA, LEVEL & VOLUME CONTROLS
   ========================================================================================= */

void setVolume(Object level) {
    checkAndLogVersionDemarcation()
    if (level == null || level.toString().trim() == "") {
        logWarn "setVolume(): Volume level parameter is required; ignoring command."
        return
    }
    BigDecimal targetVol = bd(level, 0)
    if (targetVol == null) return
    logInfo "setVolume(): ${targetVol}"
    updateAttribute("volume", targetVol)
    updateAttribute("level", targetVol)
    parent?.sendBridgeCommand(device.deviceNetworkId, "volume", [level: targetVol])
}

void setLevel(Object level, Object duration = null) {
    checkAndLogVersionDemarcation()
    if (level == null || level.toString().trim() == "") {
        logWarn "setLevel(): Level parameter is required; ignoring command."
        return
    }
    logInfo "setLevel(): level=${level}, duration=${duration}"
    setVolume(level)
}

void volumeUp() {
    checkAndLogVersionDemarcation()
    BigDecimal curVol = bd(device.currentValue("volume"), 0) ?: 20G
...
void parseDeviceData(Map data) {
    ...
    String resolvedIp = (data.ipAddress && !data.ipAddress.contains("X") && data.ipAddress != "null") ? data.ipAddress : "Masked by Amazon"
    String resolvedMac = (data.macAddress && !data.macAddress.contains("X") && data.macAddress != "null") ? data.macAddress : "Masked by Amazon"
    
    updateAttribute("ipAddress", resolvedIp)
    updateAttribute("macAddress", resolvedMac)
    ...
    if (data.volume != null) {
        BigDecimal volVal = bd(data.volume, 0)
        updateAttribute("volume", volVal)
        updateAttribute("level", volVal)
    }
    
    if (data.alarmVolume != null) {
        updateAttribute("alarmVolume", bd(data.alarmVolume, 0))
    }
