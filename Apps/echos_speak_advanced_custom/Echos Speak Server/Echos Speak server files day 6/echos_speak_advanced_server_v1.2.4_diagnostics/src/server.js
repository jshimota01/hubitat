...
app.get('/api/debug/media/:serialNumber', (req, res) => {
  const status = authManager.getStatus();
  ...
  logger.info('[Server] GET /api/debug/media requested:', {
    serialNumber,
  });

  const started = Date.now();

  require('./alexaWrapper').getMediaDiagnostics(
    serialNumber,
    (err, diagnostics) => {
      if (err) {
        logger.error('[Server] Media diagnostics failed:', {
          serialNumber,
          error: err.message,
      ...
      logger.info('[Server] Media diagnostics complete:', {
        serialNumber,
        elapsedMs: Date.now() - started,
        hasMediaState: !!diagnostics.mediaState,
        hasPlayerInfo: !!diagnostics.playerInfo,
        hasMediaSessions: !!diagnostics.mediaSessions,
      });
