    ...
    switch (command) {
      ...
      case 'pause':
        invokeAlexa(command, (done) => {
          // Amazon's /api/np/command endpoint expects the media
          // directive names, not the short dispatcher command names.
          // alexa-remote2 v8.1.1 signature:
          // sendCommand(serialOrName, command, value, callback)
          const mediaCommand = command === 'play'
            ? 'PlayCommand'
            : 'PauseCommand';
      ...
      case 'nextTrack':
        invokeAlexa('nextTrack', (done) => {
          // Amazon's /api/np/command endpoint expects NextCommand.
          alexa.sendCommand(
            serialNumber,
            'NextCommand',
            null,
      ...
      case 'previousTrack':
        invokeAlexa('previousTrack', (done) => {
          // Amazon's /api/np/command endpoint expects PreviousCommand.
          alexa.sendCommand(
            serialNumber,
            'PreviousCommand',
            null,
