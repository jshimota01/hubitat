...
class CommandDispatcher {
  ...
  dispatch(serialNumber, command, payload = {}, callback) {
    ...
    switch (command) {
      ...
      // ============================================================
      // Fire TV video-control test commands
      // ============================================================
      // These deliberately use Alexa's Fire TV routine sequence path rather
      // than the generic /api/np/command media transport. This lets us test
      // whether the authenticated Alexa session can directly control an
      // active on-screen Fire TV video session.
      case 'fireTVPauseVideo':
      case 'fireTVResumeVideo':
      case 'fireTVStartVideo':
      case 'fireTVStopVideo': {
        if (targetDevice.deviceFamily !== 'FIRE_TV') {
          return callback(new Error(
            `Device '${serialNumber}' is not a Fire TV endpoint.`
          ));
        }

        if (!targetDevice.deviceAccountId) {
          return callback(new Error(
            `Fire TV endpoint '${serialNumber}' has no deviceAccountId.`
          ));
        }

        const sequenceCommand =
          command === 'fireTVPauseVideo' ? 'fireTVPauseVideo' :
          command === 'fireTVResumeVideo' || command === 'fireTVStartVideo'
            ? 'fireTVResumeVideo'
            : 'deviceStop';

        const operation = `fireTV/${command}`;

        invokeAlexa(operation, (done) => {
          if (sequenceCommand === 'deviceStop') {
            alexa.sendSequenceCommand(
              serialNumber,
              'deviceStop',
              null,
          ...
          // The Fire TV sequence requires the Amazon account/device target
          // embedded in the operation payload. alexa-remote2 builds the
          // remaining sequence envelope from the command name.
          alexa.sendSequenceCommand(
            serialNumber,
            sequenceCommand,
            null,
      ...
      case 'mute': {
        // Alexa's supported sequence path exposes volume control, not a
        // separate documented mute sequence.  Mute is therefore implemented
        // as volume 0.  The Hubitat child driver preserves the previous
        // volume and sends it back for unmute.
        invokeAlexa('mute', (done) => {
          alexa.sendSequenceCommand(
            serialNumber,
            'volume',
            0,
      ...
      case 'unmute': {
        if (payload.level === undefined) {
          return callback(
            new Error("Missing 'level' in payload for unmute command.")
          );
        }

        const restoreVol = Number(payload.level);

        if (!Number.isInteger(restoreVol) || restoreVol < 0 || restoreVol > 100) {
          return callback(
            new Error("Invalid 'level'. Unmute volume must be a number from 0 to 100.")
          );
        }

        invokeAlexa('unmute', (done) => {
          alexa.sendSequenceCommand(
            serialNumber,
            'volume',
            restoreVol,
      ...
      case 'textCommand': {
        if (!payload.text || typeof payload.text !== 'string') {
          return callback(
            new Error("Missing 'text' in payload for textCommand.")
          );
        }

        // This is deliberately different from /api/np/command.
        // TextCommand uses Amazon's tell-alexa behavior path, effectively
        // submitting a typed utterance to Alexa. This gives us a controlled
        // way to test whether Alexa itself routes "pause" to an active
        // VideoApp session, rather than using the generic media transport.
        invokeAlexa('textCommand', (done) => {
          alexa.sendSequenceCommand(
            serialNumber,
            'textCommand',
            payload.text,
      ...
      case 'pause':
        invokeAlexa(command, (done) => {
          // alexa-remote2 v8.1.1 expects the SHORT command name here.
          // It converts play/pause to PlayCommand/PauseCommand internally.
          // Signature: sendCommand(serialOrName, command, value, callback)
          alexa.sendCommand(
            serialNumber,
            command,
            null,
      ...
      case 'nextTrack':
        invokeAlexa('nextTrack', (done) => {
          // alexa-remote2 converts 'next' to NextCommand internally.
          alexa.sendCommand(
            serialNumber,
            'next',
            null,
      ...
      case 'previousTrack':
        invokeAlexa('previousTrack', (done) => {
          // alexa-remote2 converts 'previous' to PreviousCommand internally.
          alexa.sendCommand(
            serialNumber,
            'previous',
            null,
