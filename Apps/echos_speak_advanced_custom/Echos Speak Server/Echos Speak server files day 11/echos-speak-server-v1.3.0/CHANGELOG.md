...
- Hardened expensive read operations against duplicate concurrent requests for the same device.
- Coalesced concurrent `getDeviceFeatures` requests per serial number so only one Amazon feature/state query is in flight at a time; all callers receive the same completed result.
- Coalesced concurrent `getPlaybackState` requests per serial number to prevent duplicate `getPlayerInfo()` calls when Hubitat refresh paths overlap.
- Replaced recursive traversal of Amazon smart-home payloads with iterative stack-based traversal to remove call-stack depth risk on unexpectedly deep payloads.
- Replaced recursive traversal of returned sensor capability states with iterative traversal for the same reason.
- Corrected WHA audio-notification family comparison fallback typo.
- Preserved existing command behavior, capability mapping, timeouts, and Amazon API usage.
