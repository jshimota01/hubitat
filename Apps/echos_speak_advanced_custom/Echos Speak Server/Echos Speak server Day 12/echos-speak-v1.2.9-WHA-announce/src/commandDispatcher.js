...
class CommandDispatcher {
  ...
  getDeviceFeatures(serialNumber, callback) {
    ...
    const readSensors = () => {
      const wantsTemperature = capabilities.temperature;
      const wantsMotion = capabilities.motion;
  ...
  getSensorState(
    ...
    const collectAppliances = (value) => {
      ...
      if (Array.isArray(value)) {
        value.forEach(collectAppliances);
      } else {
        Object.keys(value).forEach(
          key => collectAppliances(value[key])
        );
      }
    };
              ...
              const collectStates = value => {
                ...
                if (typeof value === 'object') {
                  if (
                    ...
                    value.capabilityStates.forEach(
                      sensorState => {
                        if (
                          typeof sensorState ===
                          'string'
  ...
  dispatch(serialNumber, command, payload = {}, callback) {
    ...
    const isMultiTarget = Array.isArray(serialNumber);

    const targetDevice = isMultiTarget
      ? null
      : (
          alexa.serialNumbers[serialNumber] ||
          Object.values(alexa.serialNumbers).find(
    ...
    // Multi-target serial arrays are intentionally supported by commands
    // such as raw/announcement and must not be coerced into a comma-separated
    // device identifier during the single-device lookup above.
    const multiTargetCommandAllowed =
      isMultiTarget &&
      command === 'raw';
    ...
    switch (command) {
      ...
      case 'announce':
        ...
        invokeAlexa('announce', (done) => {
          // WHA endpoints cannot execute AlexaAnnouncement directly.
          // When the target is a WHA endpoint, target its physical cluster
          // members as one multi-device announcement sequence.
          let announcementTarget = serialNumber;

          if (
            targetDevice &&
            targetDevice.deviceFamily === 'WHA' &&
            ...
            logger.info(
              `[CommandDispatcher] WHA announce expanded to physical members`,
              {
