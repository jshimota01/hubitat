 ...
 *  v1.1.3 (2026-09-08) - Refactored REST API response wrappers and session expiration fallback state tracking.
 *  v1.1.2 (2026-09-08) - Added device discovery filtering logic for Amazon Echo device family types.
 *  v1.1.1 (2026-09-08) - Updated child device instantiation routine to match custom Groovy driver schema.
...
def timeStamp() { return "2026/09/09 12:55 PM" }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 1: Synology NAS REST API Bridge Configuration</b>", hideable: true, hidden: false) {
            ...
            input name: "bridgePort", type: "number", title: "<b>REST API Port*</b>", defaultValue: 8093, required: true, submitOnChange: true, width: 6
            ...
            paragraph "<div style='background-color:#EBF5FB; border-left:4px solid #3498DB; padding:8px; border-radius:4px; font-size:12px; margin-top:6px; margin-bottom:10px;'>" +
                      ...
                      "<a href='http://${getBridgeIp()}:8094' target='_blank' style='color:#2980B9; font-weight:bold; text-decoration:underline;'>" +
                      "http://${getBridgeIp()}:8094</a> to renew your session.</div>"
        }
        ...
        section("<b>SECTION 2: Amazon Echo Device Discovery</b>", hideable: true, hidden: false) {
            ...
            if (devMap) {
                ...
                paragraph "<div style='background-color:#FEF9E7; border-left:4px solid #F39C12; padding:8px; border-radius:4px; font-size:12px;'>" +
                          "⚠️ <b>No Devices Found / Auth Required:</b> Verify that <code>server.js</code> is running and authenticated via port 8094.</div>"
            }
        }
...
private int getBridgePort() {
    return settings.bridgePort ? settings.bridgePort.toInteger() : 8093
}
...
private Boolean testBridgeConnection() {
    String url = "http://${getBridgeIp()}:${getBridgePort()}/status"
    Map params = [
        uri: url,
        timeout: 5,
    ...
    try {
        httpGet(params) { resp ->
            if (resp.status == 200 && resp.data?.status == "online" && resp.data?.authenticated == true) {
                state.bridgeConnected = true
                return true
            }
        }
    } catch (Exception e) {
        logDebug "Bridge status check failed or unauthenticated at ${url}: ${e.message}"
    }
