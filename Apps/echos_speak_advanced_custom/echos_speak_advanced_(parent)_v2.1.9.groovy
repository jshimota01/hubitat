 ...
 *
 *  Changelog:
 *  v2.1.9 (2026-09-24) - Diagnostic Suite Async Callback & UI State Refresh Refactor:
 *                          - Converted btnTestWatchdog inside appButtonHandler() to execute a direct self-contained httpGet() call to /auth/status rather than relying on fetchAuthStatusData().
 *                          - Refactored httpGet() response handlers for both REST and Watchdog test buttons to ensure state properties are updated prior to mainPage() rendering.
 *                          - Added explicit completion and exception log messages inside both diagnostic HTTP response closures.
 *  v2.1.8 (2026-09-24) - GUI Section 1B Diagnostic Button Binding Fix:
 *                          - Added submitOnChange: true to btnTestBridge and btnTestWatchdog inputs to ensure button clicks trigger appButtonHandler() and update the UI in real time.
 *  v2.1.7 (2026-09-22) - Authoritative & Asynchronous Volume State Preservation Refactor:
 *                          - Refactored getRawAmazonEndpointMap() cache builder block to preserve currentVolume, volume, and volumeCommandTarget alongside transaction metadata.
 *  v2.1.6 (2026-09-22) - Asynchronous Transaction State Preservation Fix:
 *                          - Updated getRawAmazonEndpointMap() cache builder to preserve volumeCommandId, volumeCommandState, and volumeCommandError.
 *                          - Ensures asynchronous server-side volume transaction completion metadata reaches child drivers during trailing reconciliation polls.
 *  v2.1.5 (2026-09-22) - Log Demotion Optimization:
 *                          - Demoted bridge command acceptance messages from logInfo to logDebug to keep runtime info logs clean during rapid volume adjustments.
 *  v2.1.4 (2026-09-22) - Explicit Scheduler Wrapper & 5000 ms Window Alignment:
 *                          - Added dedicated public scheduledTrailingSync() method for clean Hubitat quartz scheduler binding.
 *                          - Set trailing reconciliation delay to 5000 ms (runIn(5, ...)) to guarantee Amazon API callbacks complete on the server before Hubitat executes GET /api/devices.
 *  v2.1.3 (2026-09-22) - Trailing Asynchronous State Reconciliation:
 *                          - Added automatic trailing syncDeviceStates() trigger inside sendBridgeCommand() when commands are accepted as PENDING.
 ...
 *
 *				[KEEP] Prior change history is found in changelog_parent.txt (v0.0.0 - v1.8.14)
 **/
...
def timeStamp() { return "2026/09/24 08:15 AM" }
...
def mainPage() {
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("🌐 <b>CATEGORY A: NETWORK, BRIDGE & DIAGNOSTICS</b>", hideable: true, hidden: startCollapsed) {
            ...
            input name: "btnTestBridge", type: "button", title: "<b>Test REST API Bridge Network</b>", submitOnChange: true, width: 6
            input name: "btnTestWatchdog", type: "button", title: "<b>Test Amazon Session & Watchdog Health</b>", submitOnChange: true, width: 6
        ...
        httpPostJson(params) { resp ->
            ...
            if (resp.status == 200) {
                ...
                if (cmdStatus == "pending") {
                    logDebug "[${serialNumber}] Command '${command}' accepted by bridge (status: PENDING)"
                    
                    if (command in ["volume", "speak", "announce", "playText"]) {
                        logDebug "Scheduling trailing state synchronization in 5 seconds to capture transaction result..."
                        runIn(5, "scheduledTrailingSync", [overwrite: true])
                    }
                } else {
...
void scheduledTrailingSync() {
    logDebug "scheduledTrailingSync() firing trailing state sync following asynchronous transaction window..."
    syncDeviceStates()
}
            ...
            httpGet(params) { resp ->
                if (resp.status == 200 && resp.data) {
                    ...
                    if (isAuth) {
                        ...
                        logInfo "Amazon Session Watchdog test PASSED. State: ${authState}"
                    } else {
                        state.lastWatchdogTestResult = "🔴 <b>Last Tested: ${nowTime}</b> — FAILED: Session State: <b>${authState}</b> | Error: ${lastErr}."
                        logWarn "Amazon Session Watchdog test FAILED. State: ${authState} | Error: ${lastErr}"
                    }
                } else {
                    state.lastWatchdogTestResult = "🔴 <b>Last Tested: ${nowTime}</b> — FAILED: Server responded with HTTP ${resp.status}."
                    logWarn "Amazon Session Watchdog test FAILED (HTTP ${resp.status})."
                }
            }
        } catch (Exception e) {
            state.lastWatchdogTestResult = "🔴 <b>Last Tested: ${nowTime}</b> — FAILED: Exception: ${e.message}"
            logError "Amazon Session Watchdog test EXCEPTION: ${e.message}"
        }
    }
                ...
                resp.data.devices.each { dev ->
                    ...
                    if (sNum) {
                        ...
                        cache[sNum] = [
                            ...
                            // Authoritative volume state returned by the bridge.
                            currentVolume: dev.currentVolume,
                            volume: dev.volume,

                            // Asynchronous volume transaction state returned by the bridge.
                            volumeCommandId: dev.volumeCommandId,
                            volumeCommandState: dev.volumeCommandState,
                            volumeCommandTarget: dev.volumeCommandTarget,
                            volumeCommandError: dev.volumeCommandError
                        ]
