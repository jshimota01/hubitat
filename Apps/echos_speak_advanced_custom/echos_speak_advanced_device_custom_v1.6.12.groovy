...
static String version() { return '1.6.12' }
...
void parseDeviceFeatures(Map resp) {
    if (!resp) return
    logTrace "--> parseDeviceFeatures() Canonical payload received for ${device.displayName}: ${resp}"

    Map caps  = (resp.capabilities instanceof Map) ? resp.capabilities : [:]
    Map st    = (resp.state instanceof Map)        ? resp.state        : [:]
    Map errs  = (resp.errors instanceof Map)       ? resp.errors       : [:]
    ...
    if (caps.displayBrightness == true || caps.display == true) {
        if (st.displayBrightness != null) {
            int brightVal = st.displayBrightness.toInteger().clamp(0, 100)
