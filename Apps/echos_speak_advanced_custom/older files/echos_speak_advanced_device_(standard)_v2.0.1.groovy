...
def timeStamp() { return "2026/09/21 11:15 AM" }
...
void playTextAndRestore(String text, Object speakVolume = null) {
    if (!text) return
    if (!hasServerCapability("speechSynthesis")) {
        logWarn "${device.displayName} - playTextAndRestore() suppressed: Server capability contract indicates speechSynthesis is unsupported."
        return
    }
