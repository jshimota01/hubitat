    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("⏱️ <b>CATEGORY C: POLLING & REFRESH SCHEDULES</b>", hideable: true, hidden: startCollapsed) {
            paragraph "<div style='border-left:3px solid #34495E; padding-left:10px; margin-left:5px;'>" +
                      ...
                      "<div style='color:#555; font-size:12px; margin-top:4px; margin-bottom:8px;'>" +
                      "Configure background synchronization interval for pulling volume, media playback, and availability states.</div></div>"
...
private void fetchDeviceFeatures(String serialNumber) {
    if (!serialNumber) return
    String url = "http://${getBridgeIp()}:${getBridgePort()}/api/command"
    logTrace "--> fetchDeviceFeatures() Requesting live feature state for [${serialNumber}]"
    ...
    try {
        httpPostJson(params) { resp ->
            if (resp.status == 200 && resp.data) {
                ...
                if (child) {
                    child.parseDeviceFeatures(resp.data)
                }
            }
        }
    } catch (Exception e) {
            ...
            if (resp.status == 200 && resp.data?.devices) {
                ...
                resp.data.devices.each { devData ->
                    ...
                    if (child) {
                        ...
                        List caps = (devData.capabilities instanceof List) ? devData.capabilities.collect { it.toString() } : []
                        Boolean hasTargetCaps = caps.any { it in ["DISPLAY_BRIGHTNESS_ADJUST", "DISPLAY_POWER_TOGGLE", "DISPLAY_ADAPTIVE_BRIGHTNESS", "MOTION_DETECTION", "TEMPERATURE_SENSOR"] }
                        if (hasTargetCaps) {
