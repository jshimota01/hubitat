...
def timeStamp() { return "2026/09/12 09:10 AM" }
...
private void ensureServerStatusChildDevice() {
    ...
    if (!statusDev) {
        ...
        addChildDevice("jshimota", "Echos Speak Server Status Tile Driver", dni, [label: "Echos Speak Server", isComponent: false])
    }
}
...
private void syncChildDevices() {
    ...
    selected.each { dni ->
        ...
        if (!existing) {
            ...
            addChildDevice("jshimota", "Echos Speak Advanced Device", dni, [label: devName, isComponent: false])
        }
    }
