...
def timeStamp() { return "2026/09/14 12:42 PM" }
...
def mainPage() {
    ...
    // Initial collapse flag: true on first page open, false during active session edits
...
void updated() {
    ...
    // Reset pageOpened state so next launch starts fully collapsed
    ...
    if (settingsChanged && previousSnapshot != null) {
        ...
        logDebug "Previous Snapshot Data: ${state.lastSettingsJson ?: 'None'}"
...
def appButtonHandler(btn) {
    ...
    // Force page refresh to immediately render updated state test outputs
    return mainPage()
}
