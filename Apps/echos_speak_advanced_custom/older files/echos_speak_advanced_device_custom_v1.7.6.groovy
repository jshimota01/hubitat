 ...
 *                          - Enforced dynamic capability addition/removal for all server normalized capability fields.
 ...
 *                          - Implemented runtime addCapability() / removeCapability() guards in parseDeviceFeatures() based on state.capabilitiesMap.
 *                          - Added automatic stripping of capability-dependent attributes and platform capabilities for non-supporting hardware.
 ...
 *                          - Removed duplicate/empty state.capabilities entry.
 *                          - Standardized capability-guard checks exclusively on state.capabilitiesMap.
 *                          - Added automatic removal of state.capabilities during driver init/reset.
...
static String version() { return '1.7.6' }
def timeStamp() { return "2026/09/19 11:35 AM" }
...
private void initialize(Boolean isInstall = false) {
    ...
    if (state.containsKey("capabilities")) {
        logDebug "Removing legacy/empty state.capabilities entry"
...
/* =========================================================================================
   CANONICAL DYNAMIC CAPABILITY & FEATURE PARSER (Server v1.2.8 / v1.3.0 Contract)
   ========================================================================================= */
...
void parseDeviceFeatures(Map resp) {
    ...
    logTrace "--> parseDeviceFeatures() Server Contract payload received for ${device.displayName}: ${payload}"
    ...
    // Store server normalized capability map directly
    state.capabilitiesMap = caps

    // 1. temperatureMeasurement -> Hubitat TemperatureMeasurement
    if (caps.temperatureMeasurement == true) {
        addCapability("TemperatureMeasurement")
        ...
        removeCapability("TemperatureMeasurement")
        device.deleteCurrentState("temperature")
    }

    // 2. motionSensor -> Hubitat MotionSensor
    if (caps.motionSensor == true) {
        addCapability("MotionSensor")
        if (st.motion != null) {
        ...
        removeCapability("MotionSensor")
        device.deleteCurrentState("motion")
    }

    // 3. displayBrightness -> Hubitat SwitchLevel (Intentional Terminology Translation)
    if (caps.displayBrightness == true) {
        addCapability("SwitchLevel")
        ...
        removeCapability("SwitchLevel")
        device.deleteCurrentState("displayBrightness")
        
        // Re-align level attribute for audio volume if display brightness capability is absent
    ...
    // 4. display -> Hubitat Switch (Intentional Terminology Translation)
    if (caps.display == true) {
        addCapability("Switch")
        ...
        removeCapability("Switch")
        device.deleteCurrentState("displayPower")
        device.deleteCurrentState("switch")
    }

    // 5. musicPlayer -> Hubitat MusicPlayer
    if (caps.musicPlayer == true) {
        addCapability("MusicPlayer")
    } else {
        removeCapability("MusicPlayer")
    }

    // 6. audioVolume -> Hubitat AudioVolume
    if (caps.audioVolume == true) {
        addCapability("AudioVolume")
    } else {
        removeCapability("AudioVolume")
    }

    // 7. speechSynthesis -> Hubitat SpeechSynthesis
    if (caps.speechSynthesis == true) {
        addCapability("SpeechSynthesis")
    } else {
        removeCapability("SpeechSynthesis")
    }

    // 8. audioNotification -> Hubitat AudioNotification
    if (caps.audioNotification == true) {
        addCapability("AudioNotification")
    } else {
        removeCapability("AudioNotification")
    }

    // 9. notification -> Hubitat Notification
    if (caps.notification == true) {
        addCapability("Notification")
    } else {
        removeCapability("Notification")
    }

    // 10. adaptiveBrightness (Echo Feature Flag - state tracking)
