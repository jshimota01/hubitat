 ...
 *
 *  Changelog:
 *  v1.6.3 (2026-09-17) - Nomenclature Standard Alignment (firmwareVer -> softwareVer):
 *                         - Renamed firmwareVer attribute to softwareVer to align directly with Amazon Alexa App terminology ("Device Software Version").
 *                         - Updated initialize() and parseDeviceData() mapping to process devData.softwareVer and devData.softwareVersion.
 *  v1.6.2 (2026-09-17) - Current States Rendering & Empty String Event Fix:
 *                         - Replaced all empty string "" defaults in initialize() with "None" across string attributes (lastSpokenText, lastAnnouncementText, icon, trackImage, trackImageHtml).
 *                         - Solves Hubitat platform event suppression where empty string values are discarded before rendering under Current States.
 *                         - Guarantees 100% visibility for all 23 declared attributes under Current States upon driver initialization or reset.
 *  v1.6.1 (2026-09-17) - Sensible Attribute Default Seeding in initialize():
 *                         - Updated initialize() to explicitly seed default values across all 23 declared attributes upon reset/initialization.
 *                         - Puts serialNumber, family, model, generation, and modelNumber from parent cache or DNI into Current States immediately.
 *                         - Enforces default audio (volume: 50, mute: unmuted), state (onlineStatus: online, doNotDisturb: disabled, alexaWakeWord: ALEXA), and transport attributes upon boot.
...
def timeStamp() { return "2026/09/17 12:30 PM" }
...
private void initialize(Boolean isInstall = false) {
    ...
    // Seed Default Values across all 23 declared attributes (using "None" for string properties to prevent empty string suppression)
