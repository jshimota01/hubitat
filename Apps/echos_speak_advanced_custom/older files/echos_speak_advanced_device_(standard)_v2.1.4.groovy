 ...
 *  v2.1.3 (2026-09-22) - Explicit Asynchronous Transaction Completion Debug Logging:
 *                          - Updated parseDeviceData() to emit a logDebug entry upon processing a successful server volume transaction (volumeCommandState == "success").
 *                          - Preserved state.lastVolumeCommandId deduplication logic to avoid repetitive log spam.
...
def timeStamp() { return "2026/09/22 01:18 PM" }
...
void parseDeviceData(Map devData) {
    ...
    // AUTHORITATIVE VOLUME STATE SYNC (PARSED FIRST TO SUPPLY TRANSACTION LOGGING)
    int vol = 0
    Boolean hasVolData = false
    if (devData.currentVolume != null || devData.volume != null) {
        vol = (devData.currentVolume != null ? devData.currentVolume : devData.volume).toInteger()
        hasVolData = true
        sendEvent(name: "volume", value: vol, unit: "%")
        sendEvent(name: "level", value: vol, unit: "%")
    }
    ...
    if (devData.volumeCommandId != null) {
        ...
        if (cmdState == "failed") {
            ...
            if (cmdId != lastKnownId) {
                state.lastVolumeCommandId = cmdId
                logDebug "Volume transaction [${cmdId}] verified SUCCESS by server -> Target: ${hasVolData ? vol : 'N/A'}%"
            }
