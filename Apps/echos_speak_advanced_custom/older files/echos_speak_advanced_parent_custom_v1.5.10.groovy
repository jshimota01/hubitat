 ...
 *
 *  Changelog:
 *  v1.5.10 (2026-09-18) - Platform Environment Optimization & C-7 Alignment:
 *                          - Verified bridge dispatch pipelines for Hubitat C-7 platform environment (v2.4.4.156).
 *  v1.5.9 (2026-09-18) - Version Demarcation Sync (Child v1.6.14 Integer Clamp Hotfix):
 *                         - Updated version tracking to align with Child Driver v1.6.14 method exception resolution.
 *  v1.5.8 (2026-09-18) - Canonical Feature Response Unwrap Fix:
 *                         - Updated fetchDeviceFeatures() to unwrap resp.data.result payload returned by POST /api/command getDeviceFeatures endpoint.
 *                         - Ensured child parseDeviceFeatures receives the inner result object containing capabilities, state, and errors maps.
 *  v1.5.7 (2026-09-18) - Server Contract Integration & Capability Dispatch Refactor:
 *                         - Standardized fetchDeviceFeatures() to consume official {capabilities, state, errors} payload structure from server POST /api/command getDeviceFeatures.
 *                         - Forwarded raw canonical feature map down to child devices for dynamic capability-aware attribute exposure.
 *  v1.5.6 (2026-09-18) - GUI Family Ordering Refinement:
 *                         - Reordered category family labels in Section 2 for Categories 2 (SHOW / KNIGHT) and 8 (APP / REAVER) for human clarity.
 *  v1.5.5 (2026-09-18) - Device Family Discovery Matrix Expansion:
 *                         - Mapped the 'ROOK' device family into Category 1 (Echos) family filtering matrix.
 *  v1.5.4 (2026-09-17) - Capability-Aware Command Routing & Feature State Polling (Server v1.2.8):
 *                         - Expanded sendBridgeCommand() to map setDisplayBrightness, setDisplayPowerOn/Off, setAdaptiveBrightnessOn/Off.
 *                         - Integrated getDeviceFeatures polling via POST /api/command during syncDeviceStates().
 *                         - Added return response verification for immediate authoritative UI updates.
 *  v1.5.1 (2026-09-17) - Child Driver v1.6.3 Nomenclature Alignment (softwareVer):
 *                         - Updated getDiscoveredDeviceMap() cache mapping from firmwareVer to softwareVer.
 *                         - Aligned parent version tracking with child driver v1.6.3 softwareVer attribute update.
 *  v1.5.0 (2026-09-17) - Child Driver v1.6.0 Attribute Matrix & Discovery Cache Alignment:
 *                         - Aligned parent version tracking with child driver v1.6.0 attribute schema matrix refactor.
 *                         - Expanded getDiscoveredDeviceMap() cache to store family, model/style, generation, model number, and serial details for child provisioning.
 *  v1.4.10 (2026-09-17) - Child Driver v1.5.15 Version Alignment:
 *                         - Aligned version tracking with child driver v1.5.15 2-argument speak() overload resolution.
 *  v1.4.9 (2026-09-17) - Child Driver v1.5.14 Alignment & DND Command Standardization:
 *                        - Aligned parent version demarcation to match child driver v1.5.14 setDoNotDisturbOn / setDoNotDisturbOff command standardization.
 *                        - Verified sendBridgeCommand dispatch for bridge "dnd" payload structure ([state: true/false]).
 *  v1.4.8 (2026-09-16) - Periodic Media State Polling Sync Integration:
 *                        - Updated syncDeviceStates() background poll loop to query live GET /api/media/{serialNumber} endpoints and parse track details to child devices.
 *  v1.4.7 (2026-09-16) - Live Media State Query Endpoint Addition:
 *                        - Added fetchDeviceMediaState(String serialNumber) method to execute GET /api/media/{serialNumber} queries for child driver togglePlayback().
 *  v1.4.6 (2026-09-16) - Child Driver v1.5.9 Alignment & Version Sync:
 *                        - Aligned parent version demarcation to match child driver v1.5.9 transactional DND bypass refactor.
 *  v1.4.5 (2026-09-16) - DND Command Architecture Alignment:
 *                        - Aligned parent version tracking with child driver v1.5.8 DND interface streamlining (setDNDOn, setDNDOff, toggleDND).
 *  v1.4.4 (2026-09-16) - Mute / Unmute Command Mapping Verification:
 *                        - Verified generic REST bridge endpoint forwarding for mute and unmute payload operations.
 *  v1.4.3 (2026-09-15) - Signature Guard & Child Refresh Helper Implementation:
 *                        - Updated refreshDeviceData(String deviceId = null) signature to support optional device ID parameters.
 *                        - Added refreshServerStatusData() and forceServerSessionRevalidation() helper methods for child status tile triggers.
 *  v1.4.2 (2026-09-14) - LazyMap Signature Fix for Health Card:
 *                        - Changed renderHealthCard(Map auth) signature to dynamic renderHealthCard(auth) to prevent LazyMap MissingMethodException during UI rendering.
 ...
 *
 *				[KEEP] Prior change history is found in changelog_parent.txt (v0.0.0 - v1.3.9)
 **/
...
static String version() { return '1.5.10' }
def timeStamp() { return "2026/09/18 01:32 PM" }
...
def mainPage() {
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        /* CATEGORY D: DIAGNOSTICS & APP PREFERENCES */
...
def refreshDeviceData(String deviceId = null) {
    logTrace "refreshDeviceData() triggered for device [${deviceId ?: 'ALL'}]"
    syncDeviceStates()
}
                ...
                resp.data.devices.each { dev ->
                    ...
                    if (sNum) {
                        ...
                        if (isFamilyAllowed(fam)) {
                            ...
                            logDebug "Discovered Device Endpoint: ${name} [${sNum}] | Family: ${fam} | Capabilities (${caps.size()}): ${caps}"
                        }
                    }
                }
            ...
            if (resp.status == 200 && resp.data?.devices) {
                ...
                resp.data.devices.each { devData ->
                    ...
                    if (child) {
                        ...
                        Map mediaData = fetchDeviceMediaState(devData.serialNumber)
                        if (mediaData && mediaData.success == true && mediaData.result) {
                            child.parseDeviceData(mediaData.result)
                        }

                        fetchDeviceFeatures(devData.serialNumber)
                    }
                }
            }
        }
