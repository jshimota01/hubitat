...
static String version() { return '1.5.12' }
...
def refresh() {
    logTrace "refresh() called -> Pulling live device and media state..."
...
void playAnnouncementAndRestore(String text, Object announceVolume = null, Object ignoreDnd = false) {
    if (!text) return
    logTrace "--> playAnnouncementAndRestore() Initiated -> Text: '${text}' | Temporary Vol: ${announceVolume} | Ignore DND: ${ignoreDnd}"
...
void toggleDND() {
    ...
    logTrace "--> toggleDND() Initiated -> Current State: '${currentDnd}'"
