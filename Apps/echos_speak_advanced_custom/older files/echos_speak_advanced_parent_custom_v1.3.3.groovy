...
def timeStamp() { return "2026/09/14 12:04 PM" }
...
def mainPage() {
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("🌐 <b>CATEGORY A: NETWORK, BRIDGE & DIAGNOSTICS</b>", hideable: true, hidden: false) {
        ...
        section("📱 <b>CATEGORY B: DEVICE DISCOVERY & FILTERING</b>", hideable: true, hidden: false) {
            paragraph "<div style='border-left:3px solid #34495E; padding-left:10px; margin-left:5px;'>" +
        ...
        section("⏱️ <b>CATEGORY C: POLLING & REFRESH SCHEDULES</b>", hideable: true, hidden: false) {
            paragraph "<div style='border-left:3px solid #34495E; padding-left:10px; margin-left:5px;'>" +
    ...
    if (btn == "btnRefreshDevices") {
        ...
        try {
            httpGet([uri: url, timeout: 5, contentType: "application/json", ignoreSSLIssues: true]) { resp ->
                if (resp.status == 200) {
                    state.lastTestResult = "🟢 <b>Bridge Network Test PASSED:</b> Connected successfully to <code>http://${getBridgeIp()}:${getBridgePort()}</code> (HTTP 200 OK)."
                } else {
                    state.lastTestResult = "🔴 <b>Bridge Network Test FAILED:</b> Server responded with HTTP ${resp.status}."
                }
            }
        } catch (Exception e) {
            state.lastTestResult = "🔴 <b>Bridge Network Test FAILED:</b> Exception: ${e.message}"
        }
        ...
        if (isAuth) {
            state.lastTestResult = "🟢 <b>Amazon Session Test PASSED:</b> Amazon Session State: <b>${authState}</b> | Last Check: ${authData.lastValidated ?: 'Now'}."
        } else {
            state.lastTestResult = "🔴 <b>Amazon Session Test FAILED:</b> State: <b>${authState}</b> | Error: ${lastErr}."
        }
    }
}
