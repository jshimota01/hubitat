...
def timeStamp() { return "2026/09/08 02:30 PM" }
...
def deviceDiscoveryPage() {
    dynamicPage(name: "deviceDiscoveryPage", title: "Device Discovery", nextPage: "mainPage") {
        section("<b>Select Echo Devices</b>") {
            ...
            actionName "executeDeviceDiscovery", title: "Save & Create Selected Child Devices"
        }
    }
}
...
Map getDiscoveredDeviceMap() {
    ...
    try {
        httpGet([uri: "${getBridgeUrl()}/api/devices", timeout: 10]) { resp ->
...
void handleDiscoveryResponse(response, data) {
    ...
    json.devices.each { dev ->
        // ONLY import devices explicitly selected in the dropdown
