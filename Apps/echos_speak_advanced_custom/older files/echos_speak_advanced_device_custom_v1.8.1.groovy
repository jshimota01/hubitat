 ...
 *                          - Guarded speak() with speechSynthesis.
 *                          - Guarded playAnnouncement() with audioNotification.
 *                          - Guarded setVolume(), volumeUp(), volumeDown(), mute(), unmute() with audioVolume.
 *                          - Guarded play(), pause(), stop(), nextTrack(), previousTrack(), togglePlayback() with musicPlayer.
...
def timeStamp() { return "2026/09/19 02:15 PM" }
...
void parseDeviceFeatures(Map resp) {
    ...
    logTrace "--> parseDeviceFeatures() Processing server normalized feature payload for ${device.displayName}: ${payload}"
    ...
    // Construct Complete 10-Key Normalized Capability Map on state.capabilitiesMap
    Map normCaps = [
        temperatureMeasurement: (caps.temperatureMeasurement == true),
        motionSensor:          (caps.motionSensor == true),
        displayBrightness:     (caps.displayBrightness == true),
        ...
        display:               (caps.display == true),
        adaptiveBrightness:    (caps.adaptiveBrightness == true)
    ]

    state.capabilitiesMap = normCaps

    // 1. Temperature State Parsing
    if (normCaps.temperatureMeasurement == true) {
    ...
    // 2. Motion State Parsing
    if (normCaps.motionSensor == true) {
        if (st.motion != null) {
    ...
    // 3. Display Brightness State Parsing
    if (normCaps.displayBrightness == true) {
        ...
        // Re-align level attribute to audio volume if display brightness is absent
        int currentVol = device.currentValue("volume") != null ? device.currentValue("volume").toInteger() : 50
        sendEvent(name: "level", value: currentVol, unit: "%")
    }

    // 4. Display Power State Parsing
    if (normCaps.display == true) {
    ...
    // 5. Adaptive Brightness State Parsing
    if (normCaps.adaptiveBrightness == true) {
