 ...
 *                          - Accepted static driver capability model to overcome Hubitat runtime capability registration limitations.
...
def timeStamp() { return "2026/09/19 03:25 PM" }
...
void parseDeviceFeatures(Map resp) {
    ...
    // Diagnostic logging for live state attributes
    ...
    if (st.temperature != null && st.temperature.toString().isNumber()) {
        ...
        logDebug "${device.displayName} - Publishing temperature event: ${finalTemp}°${locationScale}"
    ...
    if (st.motion != null) {
        ...
        logDebug "${device.displayName} - Publishing motion event: ${cleanMotion}"
