...
static String version() { return '1.8.10' }
def timeStamp() { return "2026/09/20 03:00 PM" }
...
def refreshDeviceData(String deviceId = null) {
    if (deviceId) {
        ...
        // 1. Verify existence against raw Amazon endpoint map
        ...
        if (devData) {
            ...
            if (currentAmazonName && child.label != currentAmazonName) {
                logInfo "Targeted Sync: Updating child device label for [${deviceId}]: '${child.label}' -> '${currentAmazonName}'"
...
private Map getRawAmazonEndpointMap() {
    String url = "http://${getBridgeIp()}:${getBridgePort()}/api/devices"
...
private Map getDiscoveredDeviceMap() {
    ...
    rawMap.each { sNum, dev ->
        ...
        if (isFamilyAllowed(fam)) {
            ...
            logDebug "Discovered Device Endpoint (Filtered UI): ${name} [${sNum}] | Family: ${fam} | Sensors: ${hasSensors}"
        }
    }
...
def syncDeviceStates() {
    ...
    // 4. Update Existing Children and Dispatch Live State Updates (omits periodic fetchDeviceFeatures call)
