 ...
 *                          - Renamed driver definition to 'Echos Speak Advanced W/Sensors Device'.
 ...
 *  v1.8.6 (2026-09-19) - Static Dual-Driver Architecture Separation (Sensors):
 *                          - Created dedicated sensor driver variant declaring static TemperatureMeasurement and MotionSensor capabilities.
 *                          - Emits native Hubitat temperature and motion events directly from payload.state updates.
 *                          - Preserved complete feature guarding and application capabilitiesMap tracking.
...
void parseDeviceFeatures(Map resp) {
    ...
    // 1. Temperature Attribute Publishing (Driven strictly by payload.state.temperature)
    if (st.temperature != null && st.temperature.toString().isNumber()) {
        ...
        logDebug "${device.displayName} - Publishing native temperature event: ${finalTemp}°${locationScale}"
        sendEvent(name: "temperature", value: finalTemp, unit: "°${locationScale}")
    } else {
        logDebug "${device.displayName} - payload.state.temperature is null or missing in current payload."
    }

    // 2. Motion Attribute Publishing (Driven strictly by payload.state.motion)
    if (st.motion != null) {
        ...
        logDebug "${device.displayName} - Publishing native motion event: ${cleanMotion}"
        sendEvent(name: "motion", value: cleanMotion)
    } else {
        logDebug "${device.displayName} - payload.state.motion is null or missing in current payload."
    }

    // 3. Display Brightness Attribute Publishing
    ...
    // 4. Display Power & Switch Attribute Publishing
    ...
    // 5. Adaptive Brightness Attribute Publishing
