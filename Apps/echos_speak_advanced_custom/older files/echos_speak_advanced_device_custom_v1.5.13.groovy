...
static String version() { return '1.5.13' }
def timeStamp() { return "2026/09/16 08:30 PM" }
...
void setDNDOn() {
    logInfo "GUI Action: [setDNDOn] button clicked."
    logTrace "--> setDNDOn() Initiated"
    sendEvent(name: "doNotDisturb", value: "enabled")
    parent?.sendBridgeCommand(device.deviceNetworkId, "dnd", [state: true])
    logTrace "<-- setDNDOn() Completed"
}

void setDNDOff() {
    logInfo "GUI Action: [setDNDOff] button clicked."
    logTrace "--> setDNDOff() Initiated"
    sendEvent(name: "doNotDisturb", value: "disabled")
    parent?.sendBridgeCommand(device.deviceNetworkId, "dnd", [state: false])
    logTrace "<-- setDNDOff() Completed"
}

void toggleDND() {
    logInfo "GUI Action: [toggleDND] button clicked."
    logTrace "--> toggleDND() Initiated"
    String currentDnd = device.currentValue("doNotDisturb") ?: "disabled"
    logDebug "toggleDND() current state: '${currentDnd}'"
    if (currentDnd == "enabled") {
        setDNDOff()
    } else {
        setDNDOn()
    }
    logTrace "<-- toggleDND() Completed"
}
...
void parseDeviceData(Map devData) {
    ...
    // Parse Standard Device Attributes
    if (devData.currentVolume != null) {
        int vol = devData.currentVolume.toInteger()
        logDebug "parseDeviceData() updating volume: ${vol}%"
        sendEvent(name: "volume", value: vol, unit: "%")
        sendEvent(name: "level", value: vol, unit: "%")
    ...
    if (devData.muted != null) {
        ...
        logDebug "parseDeviceData() updating mute: ${isMuted ? 'muted' : 'unmuted'}"
        sendEvent(name: "mute", value: isMuted ? "muted" : "unmuted")
    }

    if (devData.dnd != null) {
        Boolean isDnd = devData.dnd.toBoolean()
        logDebug "parseDeviceData() updating doNotDisturb: ${isDnd ? 'enabled' : 'disabled'}"
    ...
    if (devData.online != null) {
        ...
        logDebug "parseDeviceData() updating deviceStatus: ${status}"
        sendEvent(name: "deviceStatus", value: status)
    }

    // Parse MusicPlayer Media Track Attributes
    if (devData.state != null || devData.playing != null) {
        ...
        logDebug "parseDeviceData() updating transportStatus: ${transport}"
    ...
    if (title || artist) {
        ...
        logDebug "parseDeviceData() updating trackDescription: \"${desc}\""
