...
def timeStamp() { return "2026/09/21 07:15 PM" }
...
void playText(String text) {
    if (!text) return
    parent?.sendBridgeCommand(device.deviceNetworkId, "playText", [text: text])
}

void playTextAndRestore(String text) {
    if (!text) return
    parent?.sendBridgeCommand(device.deviceNetworkId, "playTextAndRestore", [text: text])
}
