 ...
 *                         - Routes child device creation to "Echos Speak Advanced Device (Custom - Sensors)" or "Echos Speak Advanced Device (Custom)".
...
static String version() { return '1.8.8' }
def timeStamp() { return "2026/09/20 11:15 AM" }
                ...
                resp.data.devices.each { dev ->
                    ...
                    if (sNum) {
                        ...
                        if (isFamilyAllowed(fam)) {
                            Boolean hasSensors = caps.contains("TEMPERATURE_SENSOR") || caps.contains("MOTION_DETECTION")
                            devices[sNum] = "${name} (${fam}) ${hasSensors ? '[Sensors]' : '[Standard]'}"
                            logDebug "Discovered Device Endpoint: ${name} [${sNum}] | Family: ${fam} | Sensors: ${hasSensors} | Capabilities (${caps.size()}): ${caps}"
                        }
                    }
                }
...
private void syncChildDevices() {
    ...
    selected.each { dni ->
        ...
        if (!existing) {
            ...
            // Evaluates OR condition: either temperature or motion triggers sensor driver routing
            Boolean hasTemperature = rawCapabilities.contains("TEMPERATURE_SENSOR")
            Boolean hasMotion      = rawCapabilities.contains("MOTION_DETECTION")
            Boolean useSensorDriver = hasTemperature || hasMotion

            String driverName = useSensorDriver
                ? "Echos Speak Advanced Device (Custom - Sensors)"
                : "Echos Speak Advanced Device (Custom)"

            logInfo "Creating child device [${devName}] (${dni}) using driver: ${driverName} (Sensors: ${useSensorDriver})"
...
def syncDeviceStates() {
    ...
    // 1. Fetch Authoritative Amazon Endpoint List
    Map currentDiscoveredMap = getDiscoveredDeviceMap()
    Set<String> validAmazonDnis = currentDiscoveredMap ? currentDiscoveredMap.keySet() : ([] as Set<String>)
    ...
    // 2. Reconcile settings.selectedEchoes against Authoritative Amazon Endpoints
    def rawSelected = settings.selectedEchoes
    ...
    if (currentSelected.size() != reconciledSelected.size()) {
        ...
        logWarn "Amazon Endpoint Reconciliation: Stripping deleted or invalid device DNIs from settings.selectedEchoes: ${removedDnis}"
    ...
    // 4. Update Existing Children and Dispatch Live State & Feature Updates
    reconciledSelected.each { dni ->
        def child = getChildDevice(dni)
        Map devData = state.discoveredDeviceCache?.get(dni) ?: [:]

        if (child && devData) {
            // Reconcile Hubitat Device Label if renamed on Amazon
            String currentAmazonName = devData.accountName
            if (currentAmazonName && child.label != currentAmazonName) {
                logInfo "Updating child device label for [${dni}]: '${child.label}' -> '${currentAmazonName}'"
                child.setLabel(currentAmazonName)
            }

            // Dispatch Hardware & Topology Data
            ...
            // Fetch and Dispatch Capabilities Contract
