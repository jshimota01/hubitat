/**
 * Echos Speak Advanced Standard Device Driver
...
/**
 *  Purpose:
 *  Child Device Driver for managing standard Amazon Echo devices bridged via Echos Speak Advanced Parent App.
 *
 *  Changelog:
 *  v2.0.8 (2026-09-21) - PlayText Pipeline Correction:
 *                          - Corrected playText and playTextAndRestore to route internally through existing speak() pipeline.
 *                          - Eliminated invalid outbound sendBridgeCommand dispatches that caused server HTTP 400 Bad Request errors.
 *                          - Preserved playTextAndRestore volume backup/restore behavior utilizing speak() timer execution.
 *  v2.0.7 (2026-09-21) - Custom Notification Command Restoration:
 *                          - Added playText and playTextAndRestore as custom commands without restoring capability "AudioNotification".
 *                          - Mapped execution directly via sendBridgeCommand.
 *  v2.0.6 (2026-09-21) - Custom Playback Transport Command Restoration:
 *                          - Commented out capability "MusicPlayer" and capability "AudioNotification" to purge platform command clutter.
 *                          - Declared play, pause, stop, previousTrack, and nextTrack as explicit custom commands routing directly to sendBridgeCommand.
 *                          - Preserved all existing changelog history and operational feature logic.
 *  v2.0.5 (2026-09-21) - Server-Provided Track Artwork Direct Integration:
 *                          - Mapped server normalized trackImage (URL) and trackImageHtml fields in parseDeviceData().
 *                          - Preserved raw server-rendered HTML string formatting and legacy URL fallback construction.
 *  v2.0.4 (2026-09-21) - Groovy Multimethod Compilation Fix (Hubitat 2.4.4.156):
 *                          - Renamed private initialize() implementation to initializeDriverLifecycle() to eliminate public/private method name collisions.
 *                          - Disambiguated public initialize() GUI command execution path.
 *  v2.0.3 (2026-09-21) - Explicit GUI Initialize Command Addition:
 *                          - Added explicit command 'initialize' to metadata for manual baseline identity and capability reconciliation.
 *                          - Decoupled baseline driver re-initialization (identity, defaults, server capability re-query) from operational status refresh.
 *                          - Preserved capability 'Refresh' and 'aRefresh' for runtime state updates.
 *  v2.0.2 (2026-09-21) - Target GUI Command Cleanup:
 *                          - Removed redundant text/track URL playback commands from GUI (playText, playTextAndRestore, sendRawCommand).
 *                          - Removed associated orphaned implementation code where no longer used.
 *                          - Renamed Replay Text to Replay Last Spoken Text (replayLastSpokenText).
 *                          - Preserved MusicPlayer capability and all retained playback functionality.
 *                          - No change to Speak/Voice behavior.
 *  v2.0.1 (2026-09-21) - Hubitat Capability Isolation & Transport Cleanup:
 *                          - Removed capability "Notification" and deviceNotification() method alias to prevent Echo devices from appearing in Hubitat notification selectors.
 *                          - Removed redundant transportStatus attribute in favor of status.
 *                          - Fixed stop() and pause() execution pipelines to guarantee immediate status update to 'stopped' or 'paused'.
 *  v2.0.0 (2026-09-21) - Major Architecture Release & Driver Naming Standardization:
 *                          - Renamed driver definition to 'Echos Speak Advanced Standard Device'.
 *                          - Promoted major version to v2.0.0 following completion of core validation suite.
 *                          - Preserved full historical changelog without deletion or consolidation.
 *  v1.8.7 (2026-09-21) - Display Command Removal (Standard Driver):
 *                          - Removed display GUI custom commands (setDisplayBrightness, setDisplayPowerOn, setDisplayPowerOff, setAdaptiveBrightnessOn, setAdaptiveBrightnessOff).
 *                          - Display-capable hardware is routed by Parent to the extended Sensors driver.
 *                          - Retained attribute declarations and feature parsing telemetry.
 *  v1.8.6 (2026-09-19) - Static Dual-Driver Architecture Separation (Standard):
 *                          - Standardized standard Echo driver variant omitting TemperatureMeasurement and MotionSensor capabilities.
 *                          - Preserved state.capabilitiesMap parsing and feature guarding logic.
 *  v1.8.5 (2026-09-19) - Static Platform Capability Declaration Restoration:
 *                          - Restored static capability "TemperatureMeasurement" and capability "MotionSensor" to metadata definition.
 *  v1.8.4 (2026-09-19) - Full Changelog History Restoration:
 *                          - Restored complete historical version log entries in driver header from v1.6.15 down to v0.0.0.
 *  v1.8.3 (2026-09-19) - Sensor Reading vs. Feature Flag Isolation & Diagnostic Logging:
 *                          - Completely decoupled payload.state attribute event emission from payload.capabilities map flags.
 *                          - Added explicit logDebug trace outputs for payload.state.temperature and payload.state.motion.
 *                          - Retained strict hasServerCapability() guards across all command methods.
 *  v1.8.2 (2026-09-19) - Feature Parsing & Attribute Recovery Fix:
 *                          - Restored robust parseDeviceFeatures() logic from pre-v1.7.4 baseline.
 *                          - Added flexible server capability key matching (handling temperature/temperatureMeasurement, motion/motionSensor, etc.).
 *                          - Eliminated aggressive deleteCurrentState() calls that scrubbed current state attributes when payload values were omitted.
 *                          - Retained strict hasServerCapability() guards across all command pipelines.
 *  v1.8.1 (2026-09-19) - Strict Comprehensive Capability Map Guarding:
 *                          - Enforced hasServerCapability() guards across all speech, announcement, audio volume, and media transport dispatches.
 *  v1.8.0 (2026-09-19) - Top-of-GUI Refresh Command Addition:
 *                          - Added command "aRefresh" to metadata so it sorts first alphabetically in the Hubitat Command GUI.
 *                          - Delegated aRefresh() execution directly to the core refresh() routine.
 *  v1.7.9 (2026-09-19) - Legacy Device Status Removal & State Consolidation:
 *                          - Completely removed legacy deviceStatus attribute declaration and refresh updates.
 *                          - Retained onlineStatus as the sole permanent connectivity attribute on the GUI.
 *                          - Consolidated all capability state variables exclusively onto state.capabilitiesMap.
 *                          - Added cleanup routine in initialize() to purge legacy state.capabilities entries.
 *  v1.7.8 (2026-09-19) - Authoritative 10-Key Capability Map & Dynamic Call Removal:
 *                          - Permanently removed all invalid addCapability() and removeCapability() calls.
 *                          - Enforced exact 10-key normalized state.capabilities map containing explicit true/false booleans.
 *                          - Maintained static metadata declarations for universal Echo capabilities including Refresh.
 *                          - Corrected server capability field names to temperatureMeasurement and motionSensor.
 *  v1.7.7 (2026-09-19) - Method Exception Fix & Static Capability Alignment:
 *                          - Removed non-existent addCapability() and removeCapability() platform calls causing MissingMethodException.
 *                          - Kept optional/hardware-dependent capabilities omitted from static metadata.
 *                          - Retained attribute suppression via deleteCurrentState() for non-supporting Echo devices.
 *  v1.7.6 (2026-09-19) - Mandatory Hubitat Capability Isolation:
 *                          - Restored capability "Refresh" to static metadata to maintain mandatory Hubitat platform compliance.
 *                          - Kept Hubitat refresh() completely decoupled from the server's normalized capability contract.
 *  v1.7.5 (2026-09-19) - Authoritative Server Capability Contract Alignment:
 *                          - Refactored parseDeviceFeatures() to consume server normalized capability fields directly.
 *                          - Implemented explicit displayBrightness -> SwitchLevel and display -> Switch Hubitat capability translations.
 *                          - Preserved visible state.capabilitiesMap keys to match server contract exactly.
 *  v1.7.4 (2026-09-19) - Dynamic Capability Registration & Overwatch Audit:
 *                          - Removed static TemperatureMeasurement, MotionSensor, SwitchLevel, and Switch metadata declarations.
 *                          - Added automatic stripping of capability-dependent attributes for non-supporting hardware.
 *  v1.7.3 (2026-09-19) - Announcement Pipeline Streamlining:
 *                          - Completely removed deprecated playAnnouncementAll command declaration and implementation.
 *                          - Unified all announcement dispatches strictly under playAnnouncement and playAnnouncementAndRestore.
 *  v1.7.2 (2026-09-18) - Capability State Optimization:
 *                          - Standardized capability-guard checks exclusively on state.capabilitiesMap.
 *  v1.7.1 (2026-09-18) - Icon Filename & HTML Icon Image Separation:
 *                          - Renamed icon attribute to iconFileName (ensuring .png extension is always appended).
 *                          - Added iconImage attribute containing formatted HTML <img> tag for dashboard tile rendering.
 *  v1.7.0 (2026-09-18) - Shared Icon Repository Resolution Engine:
 *                          - Updated parseDeviceData() to fetch iconBasePath from parent.getIconBasePath().
 *                          - Automatically constructs fully qualified image URLs.
 *
 *                [KEEP] Prior change history is found in changelog_device.txt (v0.0.0 - v1.6.15)
 **/
...
@Field static final List<Map> deviceModelTable = [
    ...
    [deviceType: "A2LLN0UXRW4N50", family: "KNIGHT", generation: "1",
     modelName: "Echo Show 11", imageFilename: "echo_show_11_gen1", validated: true],
...
def timeStamp() { return "2026/09/21 07:23 PM" }

metadata {
    definition (
        name: "Echos Speak Advanced Standard Device",
        namespace: "jshimota",
        ...
        // BASE CUSTOM COMMANDS (aRefresh declared first for GUI placement)
        command "aRefresh"
        command "initialize"
        command "play"
        command "pause"
        ...
        command "playTextAndRestore", [[name: "text*", type: "STRING", description: "Play text as speech and restore previous volume"]]
...
/* =========================================================================================
   HUBITAT LIFECYCLE ROUTINES & INITIALIZE PIPELINE
   ========================================================================================= */
...
private void initializeDriverLifecycle(Boolean isInstall = false) {
    ...
    // Cleanup deprecated legacy state key
    ...
    // Cleanup legacy deviceStatus & transportStatus attributes
...
void playTextAndRestore(String text) {
    if (!text) return
    int currentVol = device.currentValue("volume") != null ? device.currentValue("volume").toInteger() : 75
    logInfo "GUI Action: [playTextAndRestore] triggered -> Routing text through speak() with volume restore (${currentVol}%): \"${text}\""
    speak(text, null, currentVol, false)
}
...
/* =========================================================================================
   CANONICAL DYNAMIC CAPABILITY & FEATURE PARSER (Server Normalized Contract)
   ========================================================================================= */
...
void parseDeviceFeatures(Map resp) {
    ...
    logDebug "${device.displayName} - parseDeviceFeatures() Raw payload.state: ${st}"

    // Populate internal 10-key tracking map (state.capabilitiesMap)
    ...
    // 1. Display Brightness Attribute Publishing
    ...
    // 2. Display Power & Switch Attribute Publishing
    ...
    // 3. Adaptive Brightness Attribute Publishing
...
void parseDeviceData(Map devData) {
    ...
    // 5. Visual Artwork & Icons (Shared Base Path Resolution)
    ...
    // Playback Artwork Handling (Direct Server Contract Mapping)
