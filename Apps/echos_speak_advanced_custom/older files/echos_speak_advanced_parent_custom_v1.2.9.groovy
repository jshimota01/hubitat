 ...
 *                        - Updated child device creation target to 'Echos Speak Server Status Tile Driver (Custom)'.
...
def timeStamp() { return "2026/09/12 08:50 AM" }
...
private void updateAppLabel() {
    ...
    if (showCount) {
        int count = (getChildDevices()?.size() ?: 1) - 1
        baseLabel += " - [<span style='color:#27AE60; font-weight:bold;'>${Math.max(0, count)} Device(s)</span>]"
    }
...
void updated() {
    ...
    if (settingsChanged || codeVersionChanged) {
        ...
        logDebug "App saved without setting or version changes. Skipping re-initialization."
    }
    updateAppLabel()
}
...
private void ensureServerStatusChildDevice() {
    ...
    def statusDev = getChildDevice(dni)
    if (!statusDev) {
        logInfo "Provisioning Echos Speak Server Status child device [${dni}]..."
        addChildDevice("jshimota", "Echos Speak Server Status Tile Driver (Custom)", dni, [label: "Echos Speak Server", isComponent: false])
    }
}
...
private Map fetchAuthStatusData() {
    ...
    def statusDev = getChildDevice("ECHOS_SPEAK_SERVER_STATUS")
    if (statusDev) {
...
private void renderHealthCard(Map auth) {
    ...
    int activeChildCount = (getChildDevices()?.size() ?: 1) - 1
    ...
    paragraph "<div style='background-color:#F8F9FA; border-left:5px solid ${borderColor}; padding:10px; border-radius:4px; font-size:12px; margin-bottom:10px;'>" +
              ...
              "<b>Active Echo Devices:</b> <span style='color:#2980B9; font-weight:bold;'>${Math.max(0, activeChildCount)}</span>" +
              "</div>"
}
...
private void syncChildDevices() {
    ...
    List currentChildren = getChildDevices().findAll { it.deviceNetworkId != "ECHOS_SPEAK_SERVER_STATUS" }
