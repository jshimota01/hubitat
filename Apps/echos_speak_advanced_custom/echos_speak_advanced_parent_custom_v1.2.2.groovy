...
def timeStamp() { return "2026/09/12 12:15 AM" }
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 2: Amazon Echo Device Discovery</b>", hideable: true, hidden: false) {
            ...
            paragraph "<div style='background-color:#E8F8F5; border-left:4px solid #1ABC9C; padding:8px; border-radius:4px; font-size:12px; margin-bottom:10px;'>" +
                      "🛡️ <b>Server Hardware Masking:</b> Devices in the <b>'VOX'</b> family are masked directly on the Synology NAS server layer and excluded from discovery scans.</div>"

            Map devMap = getDiscoveredDeviceMap()
            if (devMap) {
                input name: "selectedEchoes", type: "enum", title: "<b>Discovered Echo Devices (${devMap.size()} Available)</b>", options: devMap, multiple: true, required: false
            } else {
                paragraph "<div style='background-color:#FEF9E7; border-left:4px solid #F39C12; padding:8px; border-radius:4px; font-size:12px;'>" +
                          "⚠️ <b>No Devices Found / Auth Required:</b> Verify that <code>server.js</code> is running and authenticated via port 4001.</div>"
            }
        }

        section("<b>SECTION 3: Device Family Filtering Matrix</b>", hideable: true, hidden: true) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
                      "<b>Category Filter Toggles:</b> Include or exclude specific Amazon device family categories from appearing in the discovery list above.</div>"

            input name: "catIncludeEchos", type: "bool", title: "<b>Category 1: Standard Echo & Dot Hardware (ECHO / KNIGHT)</b>", defaultValue: true, submitOnChange: true, width: 6
            ...
            input name: "catIncludeGroups", type: "bool", title: "<b>Category 7: Multi-Room Audio Speaker Groups (GROUP)</b>", defaultValue: false, submitOnChange: true, width: 6
            input name: "catIncludeWholesite", type: "bool", title: "<b>Category 8: Everywhere / All-Device Synthetics (EVERYWHERE)</b>", defaultValue: false, submitOnChange: true, width: 6
        }
...
private Boolean isFamilyAllowed(String family) {
    ...
    if (fam == "VOX") return false // Explicitly masked at server level
    ...
    Boolean incGroups    = getSettingBool("catIncludeGroups", false)
    Boolean incEverywhere = getSettingBool("catIncludeWholesite", false)

    if (fam in ["ECHO", "KNIGHT"] && incEcho) return true
    if (fam in ["SHOW"] && incShow) return true
    if (fam in ["FIRE_TV", "AFT"] && incFireTV) return true
    if (fam in ["TABLET"] && incTablets) return true
    if (fam in ["AUTO"] && incAuto) return true
    if (fam in ["3RD_PARTY", "THIRD_PARTY"] && inc3rdParty) return true
    if (fam in ["GROUP", "WHA"] && incGroups) return true
    if (fam in ["EVERYWHERE"] && incEverywhere) return true

    // Default true for unclassified family types if main toggle enabled
    return incEcho
}
...
private Map getDiscoveredDeviceMap() {
    ...
    try {
        httpGet(params) { resp ->
            ...
            if (resp.status == 200 && resp.data?.devices) {
                ...
                logDebug "Discovered ${resp.data.devices.size()} total devices from Bridge."
                
                resp.data.devices.each { dev ->
                    String fam = dev.deviceFamily ?: "Echo"
                    if (isFamilyAllowed(fam)) {
                        devices[dev.serialNumber] = "${dev.accountName} (${fam})"
                    }
                }
            }
        }
    ...
    logTrace "<-- getDiscoveredDeviceMap() Exit -> Formatted Devices Count: [${devices.size()}]"
    return devices
}
