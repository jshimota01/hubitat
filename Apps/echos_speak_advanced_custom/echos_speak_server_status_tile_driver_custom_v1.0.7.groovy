...
def timeStamp() { return "2026/09/12 09:45 AM" }
...
metadata {
    ...
    preferences {
        input name: "tileFontSize", type: "enum", title: "<b>HD+ Tile Base Text Size (em)</b>", description: "Adjust overall scaling for HD+ dashboard tile layout", options: ["0.6":"0.6 em (Tiny)", "0.7":"0.7 em (Small)", "0.8":"0.8 em (Default)", "0.9":"0.9 em (Medium)", "1.0":"1.0 em (Large)", "1.1":"1.1 em (Extra Large)"], defaultValue: "0.8", required: true
...
void parseServerStatusData(Map statusData) {
    ...
    String baseEm = settings.tileFontSize ?: "0.8"

    // Responsive Color & Icon Mapping
    ...
    // Fully Responsive, EM-Scaled HD+ Dashboard HTML Tile Output (htmlTile)
    String tileHtml = "<div style='font-size:${baseEm}em; background-color:#1A252F; color:#FFFFFF; padding:0.6em; border-radius:0.5em; font-family:system-ui, -apple-system, sans-serif; border-left:0.4em solid ${badgeColor}; box-sizing:border-box; width:100%; overflow:hidden;'>" +
                      "<div style='display:flex; justify-content:space-between; align-items:center; border-bottom:0.08em solid #34495E; padding-bottom:0.4em; margin-bottom:0.4em; gap:0.4em;'>" +
                      "<span style='font-size:1.05em; font-weight:700; color:#3498DB; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;'>Echos Speak Server</span>" +
                      "<span style='font-size:0.85em; font-weight:700; color:${badgeColor}; white-space:nowrap; flex-shrink:0;'>${statusIcon} ${rawState}</span>" +
                      "</div>" +
                      "<div style='display:flex; justify-content:space-between; gap:0.4em; color:#BDC3C7; font-size:0.9em; line-height:1.3em;'>" +
                      "<div style='white-space:nowrap;'><b>Age:</b> <span style='color:#FFF;'>${ageStr}</span></div>" +
                      "<div style='white-space:nowrap;'><b>Check:</b> <span style='color:#FFF;'>${nextValStr}</span></div>" +
                      "</div>" +
                      (lastErr != "None" ? "<div style='color:#E74C3C; margin-top:0.4em; font-size:0.75em; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;'>⚠️ ${lastErr}</div>" : "") +
                      "</div>"
