...
static String version() { return '1.7.8' }
def timeStamp() { return "2026/09/19 11:36 AM" }
...
private Boolean hasServerCapability(String capKey) {
    Map caps = state.capabilitiesMap ?: state.capabilities ?: [:]
...
void parseDeviceFeatures(Map resp) {
    ...
    // 1. Construct Complete 10-Key Normalized Capability Map (Do NOT omit false values)
    Map normCaps = [
    ...
    state.capabilities = normCaps
    state.capabilitiesMap = normCaps

    // 2. Temperature State Parsing
    ...
    // 4. Display Brightness State Parsing
    ...
    // 6. Adaptive Brightness State Parsing
