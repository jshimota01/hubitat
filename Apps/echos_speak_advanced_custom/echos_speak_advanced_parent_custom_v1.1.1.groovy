...
def timeStamp() { return "2026/09/09 10:20 AM" }
...
def bridgeConfigPage() {
    ...
    dynamicPage(name: "bridgeConfigPage", title: "Bridge & Asset Configuration", nextPage: "mainPage") {
        ...
        section("<b>Custom Icon & Image Asset Host</b>") {
            paragraph "Base URL location used for device icons and media tiles when creating or updating devices (e.g., local Synology Web Station or GitHub repository):"
        ...
        httpGet([uri: targetUrl, timeout: 10]) { resp ->
            ...
            if (resp.data && resp.data.devices) {
                resp.data.devices.each { dev ->
                    ...
                    devMap[dev.serialNumber] = "${statusTag} ${dev.accountName} (${dev.deviceFamily ?: 'Echo'})"
                }
            } else {
...
void handleDiscoveryResponse(response, data) {
    ...
    json.devices.each { dev ->
        if (chosenSerials.contains(dev.serialNumber)) {
            ...
            // Build icon URL using baseImgUrl if deviceImageUrl is missing or stock Amazon URL
            String finalIconUrl = dev.deviceImageUrl
            if (!finalIconUrl || finalIconUrl.contains("amazon-echo-1._CB346614488_.png")) {
                String familyStyle = (dev.deviceStyle ?: 'echo').toLowerCase().replaceAll("[^a-zA-Z0-9]", "_")
                finalIconUrl = "${baseImgUrl}${familyStyle}.png"
            }
