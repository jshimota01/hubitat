 ...
 *
 *  Changelog:
 *  v2.1.2 (2026-09-22) - Internal Server Volume Transaction Sync:
 *                          - Added internal transient state tracking (state.lastVolumeCommandId) for volumeCommandId, volumeCommandState, and volumeCommandError.
 *                          - Protected against stale transaction callback overwrites without adding visible public device attributes or arbitrary timer suppressions.
...
def timeStamp() { return "2026/09/22 12:15 PM" }
...
void parseDeviceData(Map devData) {
    ...
    // TRANSIENT SERVER TRANSACTION PROTOCOL (Zero public attribute pollution)
    if (devData.volumeCommandId != null) {
        ...
        } else if (cmdState == "success" || cmdState == "pending") {
            state.lastVolumeCommandId = cmdId
        }
    }
