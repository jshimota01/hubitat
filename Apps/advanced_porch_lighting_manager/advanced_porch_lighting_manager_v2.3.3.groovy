...
static String version() { return '2.3.3' }
def timeStamp() { return "2026/09/06 12:05 PM" }
        ...
        section(schedSectionTitle, hideable: true, hidden: true) {
            ...
            if (enableMorningSchedule != false) {
                ...
                paragraph "<div style='background:#F8F9FA; padding:6px 12px; border-left:4px solid #6c757d; margin:6px 0 4px 0; font-size:0.88em; color:#333;'>" +
                          "<b>Currently Selected Twilight Times:</b> &nbsp; Twilight Begin: <span style='color:#2E7D32; font-weight:bold;'>${calculatedPreview.rawMornBegin}</span> &nbsp;|&nbsp; Sunrise: <span style='color:#C62828; font-weight:bold;'>${calculatedPreview.rawSunrise}</span>" +
                          "</div>"

                // Final Adjusted Morning Execution Times
                ...
                input name: "mornOffset", type: "number", title: "Morning On - Lead Time before Twilight Begin (Minutes, Optional)", description: "Default is 30 mins BEFORE Twilight Begin", defaultValue: 30, required: false, submitOnChange: true
                input name: "morningOffTime", type: "time", title: "Morning Off - Specific Time (Optional)", description: "Leave blank to turn OFF automatically 30 mins after Twilight Begin (${calculatedPreview.mornOffDefault})", required: false, submitOnChange: true
            }
            ...
            if (enableEveningSchedule != false) {
                ...
                paragraph "<div style='background:#F8F9FA; padding:6px 12px; border-left:4px solid #6c757d; margin:6px 0 4px 0; font-size:0.88em; color:#333;'>" +
                          "<b>Currently Selected Twilight Times:</b> &nbsp; Twilight End: <span style='color:#2E7D32; font-weight:bold;'>${calculatedPreview.rawEveEnd}</span>" +
                          "</div>"

                // Final Adjusted Evening Execution Times
                paragraph "<div style='background:#F1F3F4; padding:6px 12px; border-left:4px solid #1A73E8; margin:0 0 8px 0; font-size:0.88em; color:#202124;'>" +
                ...
                input name: "eveOffset", type: "number", title: "Evening On - Lead Time before Twilight End (Minutes, Optional)", description: "Default turns ON 30 mins BEFORE Evening Twilight End", defaultValue: 30, required: false, submitOnChange: true
...
private Map calculateSchedulePreview() {
    ...
    if (twilightTimes.begin) {
        ...
        if (mornTwilightDate) {
            ...
            int leadMins = getNormalizedOffsetMins(mornOffset, 30) + envShiftMins
    ...
    if (twilightTimes.end) {
        ...
        if (eveTwilightDate) {
            ...
            int leadMins = getNormalizedOffsetMins(eveOffset, 30) + envShiftMins
...
private int getNormalizedOffsetMins(def settingVal, int defaultFallback = 30) {
    if (settingVal == null) return defaultFallback
    try {
...
private int getEnvironmentalOffsetShift() {
    ...
    if (enableCloudAdjustment && owmDevice) {
        ...
        int cloudShift = Math.round((cloudPct / 100.0) * maxShift) as int
        extraShiftMins += cloudShift
        logTrace "getEnvironmentalOffsetShift() -> Cloud Cover ${cloudPct}% generated +${cloudShift} mins shift."
    }
    ...
    if (enableLuxAdjustment && owmDevice) {
        ...
        if (currentLux <= targetLux) {
            int lShift = (luxShiftMins != null) ? luxShiftMins.toInteger() : 10
            extraShiftMins += lShift
            logTrace "getEnvironmentalOffsetShift() -> Lux ${currentLux} lx <= ${targetLux} lx generated +${lShift} mins shift."
        }
    }

    logTrace "getEnvironmentalOffsetShift() -> Total environmental shift applied: +${extraShiftMins} mins."
    return extraShiftMins
}
...
def buildAndScheduleEvents(evt = null) {
    ...
    if (enableMorningSchedule != false && twilightTimes.begin) {
        Date mornTwilightDate = parseDateString(twilightTimes.begin)
        if (mornTwilightDate) {
            // Default Morning ON: 30 minutes BEFORE Twilight Begin
            int leadMins = getNormalizedOffsetMins(mornOffset, 30) + envShiftMins
            Date mornRunTime = new Date(mornTwilightDate.time - (leadMins * 60 * 1000))
            ...
            // Morning OFF: Explicit time OR Default 30 minutes AFTER Twilight Begin
            if (morningOffTime) {
                Date mornOffDate = parseIsoTimeToTodayDate(morningOffTime)
                if (mornOffDate) {
                ...
                if (defaultMornOffDate.after(now)) {
                    ...
                    logInfo "Scheduled Default Morning OFF for: ${state.schedMornOffDisp} (+30m after Twilight Begin)"
                    if (upcomingEventDesc == "None scheduled") upcomingEventDesc = "Morning OFF at ${state.schedMornOffDisp}"
                }
            }
        }
    ...
    if (enableEveningSchedule != false && twilightTimes.end) {
        Date eveTwilightDate = parseDateString(twilightTimes.end)
        if (eveTwilightDate) {
            // Default Evening ON: 30 minutes BEFORE Twilight End (lights turn ON before twilight finishes)
            int leadMins = getNormalizedOffsetMins(eveOffset, 30) + envShiftMins
            Date eveRunTime = new Date(eveTwilightDate.time - (leadMins * 60 * 1000))
            ...
            if (eveRunTime.after(now)) {
                ...
                logInfo "Scheduled Evening ON for: ${state.schedEveOnDisp} (${leadMins} mins lead time before twilight end)"
                if (upcomingEventDesc == "None scheduled") {
