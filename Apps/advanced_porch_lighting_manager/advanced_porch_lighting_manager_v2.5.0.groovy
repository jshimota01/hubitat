...
def timeStamp() { return "2026/09/06 02:10 PM" }
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section(schedSectionTitle, hideable: true, hidden: true) {
            ...
            if (enableMorningSchedule != false) {
                ...
                input name: "morningOffTime", type: "time", title: "Morning Off - Specific Time (Optional Override)", description: "Leave blank to turn OFF automatically 30 mins after Sunrise Occurs (${calculatedPreview.mornOffDefault})", required: false, submitOnChange: true
            }
...
private Map calculateSchedulePreview() {
    ...
    if (sunriseStr) {
        Date sDate = parseDateString(sunriseStr)
        if (sDate) {
            Date defaultOffDate = new Date(sDate.time + (30 * 60 * 1000))
            ...
            if (morningOffTime) {
                ...
                List<String> mOffFactors = ["Sunrise (+30m)"]
...
def buildAndScheduleEvents(evt = null) {
    ...
    if (enableMorningSchedule != false) {
        ...
        if (morningOffTime) {
            ...
            if (sDate) {
                int extMins = envShifts.luxShift
                Date defaultMornOffDate = new Date(sDate.time + ((30 + extMins) * 60 * 1000))
                ...
                if (defaultMornOffDate.after(now)) {
                    ...
                    logInfo "Scheduled Default Morning OFF for: ${state.schedMornOffDisp} (+30m default + ${extMins}m lux extension post-sunrise)"
                    if (upcomingEventDesc == "None scheduled") upcomingEventDesc = "Morning OFF at ${state.schedMornOffDisp}"
                }
            }
        }
