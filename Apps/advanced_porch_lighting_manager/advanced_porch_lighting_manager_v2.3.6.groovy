...
static String version() { return '2.3.6' }
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section(schedSectionTitle, hideable: true, hidden: true) {
            ...
            if (enableMorningSchedule != false) {
                ...
                paragraph "<div style='background:#F8F9FA; padding:6px 12px; border-left:4px solid #6c757d; margin:6px 0 4px 0; font-size:0.88em; color:#333;'>" +
                          "<b>Currently Selected Twilight Times:</b> &nbsp; Twilight Begins: <span style='color:#2E7D32; font-weight:bold;'>${calculatedPreview.rawMornBegin}</span> &nbsp;|&nbsp; Sunrise: <span style='color:#C62828; font-weight:bold;'>${calculatedPreview.rawSunrise}</span>" +
                          "</div>"

                // Final Adjusted Morning Execution Times + Inline Factors
                paragraph "<div style='background:#F1F3F4; padding:6px 12px; border-left:4px solid #1A73E8; margin:0 0 8px 0; font-size:0.88em; color:#202124;'>" +
                          "<b>Calculated Morning Times:</b> &nbsp; ON: <span style='color:#2E7D32; font-weight:bold;'>${calculatedPreview.mornOn}</span> &nbsp;|&nbsp; OFF: <span style='color:#C62828; font-weight:bold;'>${calculatedPreview.mornOff}</span>" +
                          " &nbsp;&nbsp;<span style='font-size:0.85em; color:#555;'><b>[Factors:</b> ${calculatedPreview.mornFactors}<b>]</b></span>" +
                          "</div>"

                input name: "mornOffset", type: "number", title: "Morning On - Lead Time before Twilight Begins (Minutes, Optional)", description: "Default is 30 mins BEFORE Twilight Begins", defaultValue: 30, required: false, submitOnChange: true
                input name: "morningOffTime", type: "time", title: "Morning Off - Specific Time (Optional)", description: "Leave blank to turn OFF automatically 30 mins after Twilight Begins (${calculatedPreview.mornOffDefault})", required: false, submitOnChange: true
            }
            ...
            if (enableEveningSchedule != false) {
                ...
                paragraph "<div style='background:#F8F9FA; padding:6px 12px; border-left:4px solid #6c757d; margin:6px 0 4px 0; font-size:0.88em; color:#333;'>" +
                          "<b>Currently Selected Twilight Times:</b> &nbsp; Twilight Ends: <span style='color:#2E7D32; font-weight:bold;'>${calculatedPreview.rawEveEnd}</span>" +
                          "</div>"

                // Final Adjusted Evening Execution Times + Inline Factors
                paragraph "<div style='background:#F1F3F4; padding:6px 12px; border-left:4px solid #1A73E8; margin:0 0 8px 0; font-size:0.88em; color:#202124;'>" +
                          "<b>Calculated Evening Times:</b> &nbsp; ON: <span style='color:#2E7D32; font-weight:bold;'>${calculatedPreview.eveOn}</span> &nbsp;|&nbsp; OFF: <span style='color:#C62828; font-weight:bold;'>${calculatedPreview.eveOff}</span>" +
                          " &nbsp;&nbsp;<span style='font-size:0.85em; color:#555;'><b>[Factors:</b> ${calculatedPreview.eveFactors}<b>]</b></span>" +
                          "</div>"

                input name: "eveOffset", type: "number", title: "Evening On - Lead Time before Twilight Ends (Minutes, Optional)", description: "Default turns ON 30 mins BEFORE Evening Twilight Ends", defaultValue: 30, required: false, submitOnChange: true
                input name: "eveningCutoffTime", type: "time", title: "Evening Off - Specific Time (Optional)", description: "Default turns OFF at 12:00 AM", defaultValue: "00:00", required: false, submitOnChange: true
            }
        }
        ...
        section("<b>3. Optional Schedule Adjustments</b>", hideable: true, hidden: true) {
            ...
            input name: "twilightType", type: "enum", title: "Twilight Parser Calculation Reference", 
                  options: ["used": "Used (Default)", "civil": "Civil", "nautical": "Nautical", "astronomical": "Astronomical"], 
                  defaultValue: "used", required: true, submitOnChange: true

            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0;'/>"

            input name: "enableCloudAdjustment", type: "bool", title: "Enable Cloud Cover Adjustment?", description: "Extends Morning On time and starts Evening On earlier on heavy overcast days based on OWM Cloud PCT", defaultValue: false, submitOnChange: true
            if (enableCloudAdjustment) {
                input name: "maxCloudShiftMins", type: "number", title: "Max Cloud Adjustment Offset (Minutes)", description: "Max minutes to extend ON time at 100% cloud cover", defaultValue: 15, required: true, submitOnChange: true
            }

            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0;'/>"

            input name: "enableLuxAdjustment", type: "bool", title: "Enable Lux Impact Adjustment?", description: "Adjusts ON lead time based on OWM calculated illuminance", defaultValue: false, submitOnChange: true
            if (enableLuxAdjustment) {
                input name: "luxThreshold", type: "number", title: "Lux Impact Threshold (lx)", description: "If Lux falls below this value during twilight window, advance turn ON time", defaultValue: 50, required: true, submitOnChange: true
                input name: "luxShiftMins", type: "number", title: "Max Lux Adjustment Offset (Minutes)", description: "Minutes to advance ON time when Lux threshold is triggered", defaultValue: 10, required: true, submitOnChange: true
            }
        }
...
private Map calculateSchedulePreview() {
    Map preview = [
        rawMornBegin: "--", rawSunrise: "--", rawEveEnd: "--",
        mornOn: "--", mornOff: "--", mornOffDefault: "--", mornFactors: "None",
        eveOn: "--", eveOff: "--", eveFactors: "None"
    ]
    ...
    if (twilightTimes.begin) {
        ...
        if (mornTwilightDate) {
            ...
            int totalMornShift = leadMins + envShifts.totalShift
            Date mornRunTime = new Date(mornTwilightDate.time - (totalMornShift * 60 * 1000))
            preview.mornOn = mornRunTime.format("h:mm a", location.timeZone)

            // Construct Morning Factors String
            List<String> mFactors = []
            if (leadMins > 0) mFactors << "Lead Time (-${leadMins}m)"
            if (envShifts.cloudShift > 0) mFactors << "Cloud Cover (-${envShifts.cloudShift}m)"
            if (envShifts.luxShift > 0) mFactors << "Lux Impact (-${envShifts.luxShift}m)"
            preview.mornFactors = mFactors ? mFactors.join(" | ") : "Standard Twilight"

            Date defaultOffDate = new Date(mornTwilightDate.time + (30 * 60 * 1000))
            ...
            if (morningOffTime) {
                ...
                preview.mornOff = "${preview.mornOffDefault} (Default)"
            }
        }
    }
    ...
    if (twilightTimes.end) {
        ...
        if (eveTwilightDate) {
            preview.rawEveEnd = eveTwilightDate.format("h:mm a", location.timeZone)

            int leadMins = getNormalizedOffsetMins(settings.eveOffset, 30)
            int totalEveShift = leadMins + envShifts.totalShift
            Date eveRunTime = new Date(eveTwilightDate.time - (totalEveShift * 60 * 1000))
            preview.eveOn = eveRunTime.format("h:mm a", location.timeZone)

            // Construct Evening Factors String
            List<String> eFactors = []
            if (leadMins > 0) eFactors << "Lead Time (-${leadMins}m)"
            if (envShifts.cloudShift > 0) eFactors << "Cloud Cover (-${envShifts.cloudShift}m)"
            if (envShifts.luxShift > 0) eFactors << "Lux Impact (-${envShifts.luxShift}m)"
            preview.eveFactors = eFactors ? eFactors.join(" | ") : "Standard Twilight"
...
private Map getDetailedEnvironmentalShifts() {
    ...
    return [cloudShift: cShift, luxShift: lShift, totalShift: (cShift + lShift)]
}
...
String getStatusBoardHtml() {
    ...
    String twilightRef = (twilightType ?: "used").capitalize()
    if (twilightRef == "Used") twilightRef = "Used (Default)"
    ...
    <div style='background-color:#1e1e24; color:#ffffff; padding:12px; border-radius:8px; font-family:sans-serif;'>
        ...
        <div style='display:flex; justify-content:space-between; align-items:center; margin-bottom:10px; font-size:0.85em; color:#ddd; background:#121216; padding:6px 8px; border-radius:4px;'>
            ...
            <span><b>Twilight Ref:</b> <span style='color:#AB47BC; font-weight:bold;'>${twilightRef}</span></span>
        </div>
            ...
            <tbody>
                <tr style='font-size:0.88em; border-top:1px solid #444;'>
                    <td style='padding:10px 6px; border-right:1px solid #444; vertical-align:middle;'>
                        ...
                        <div><b>OFF:</b> <span style='color:${enableMorningSchedule != false ? "#E57373" : "#888"}; font-weight:bold;'>${mornOffDisp}</span></div>
                    </td>
                    <td style='padding:10px 6px; vertical-align:middle;'>
                        ...
                        <div><b>OFF:</b> <span style='color:${enableEveningSchedule != false ? "#E57373" : "#888"}; font-weight:bold;'>${eveOffDisp}</span></div>
                    </td>
                </tr>
            </tbody>
        </table>
...
/* Helper to calculate environmental adjustments from OWM Cloud PCT and Lux */
private int getEnvironmentalOffsetShift() {
    Map shifts = getDetailedEnvironmentalShifts()
...
def buildAndScheduleEvents(evt = null) {
    ...
    Map twilightTimes = getTwilightTimestamps()
    int envShiftMins = getEnvironmentalOffsetShift()

    // 1. MORNING SCHEDULE CONSTRUCTION
    if (enableMorningSchedule != false && twilightTimes.begin) {
        Date mornTwilightDate = parseDateString(twilightTimes.begin)
        if (mornTwilightDate) {
            // Default Morning ON: 30 minutes BEFORE Twilight Begins
            int leadMins = getNormalizedOffsetMins(settings.mornOffset, 30) + envShiftMins
            ...
            if (mornRunTime.after(now)) {
                ...
                logInfo "Scheduled Morning ON for: ${state.schedMornOnDisp} (${leadMins} mins lead time before twilight)"
                if (upcomingEventDesc == "None scheduled") {
                    upcomingEventDesc = "Morning ON at ${state.schedMornOnDisp}"
                }
            }

            // Morning OFF: Explicit time OR Default 30 minutes AFTER Twilight Begins
            if (morningOffTime) {
                Date mornOffDate = parseIsoTimeToTodayDate(morningOffTime)
                if (mornOffDate) {
                ...
                Date defaultMornOffDate = new Date(mornTwilightDate.time + (30 * 60 * 1000))
                ...
                if (defaultMornOffDate.after(now)) {
                    ...
                    logInfo "Scheduled Default Morning OFF for: ${state.schedMornOffDisp} (+30m after Twilight Begins)"
                    if (upcomingEventDesc == "None scheduled") upcomingEventDesc = "Morning OFF at ${state.schedMornOffDisp}"
                }
            }
        }
    ...
    if (enableEveningSchedule != false && twilightTimes.end) {
        Date eveTwilightDate = parseDateString(twilightTimes.end)
        if (eveTwilightDate) {
            // Default Evening ON: 30 minutes BEFORE Twilight Ends (lights turn ON before twilight finishes)
            int leadMins = getNormalizedOffsetMins(settings.eveOffset, 30) + envShiftMins
            ...
            if (eveRunTime.after(now)) {
                ...
                logInfo "Scheduled Evening ON for: ${state.schedEveOnDisp} (${leadMins} mins lead time before twilight ends)"
                if (upcomingEventDesc == "None scheduled") {
                    upcomingEventDesc = "Evening ON at ${state.schedEveOnDisp}"
                }
            }

            // Evening OFF: Explicit time or default 12:00 AM
            String offTimeIso = eveningCutoffTime ?: "00:00"
            Date eveOffDate = parseIsoTimeToTodayDate(offTimeIso)
            if (eveOffDate) {
