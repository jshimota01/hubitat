...
static String version() { return '3.3.2' }
def timeStamp() { return "2026/09/07 10:25 AM" }
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>3. Environmental & Schedule Adjustments</b>", hideable: true, hidden: true) {
            ...
            paragraph "<div style='background:#F8F9FA; padding:8px 12px; border-left:4px solid #FFB74D; margin-top:-6px; margin-bottom:8px; font-size:0.85em; color:#333;'>" +
                      ...
                      "• <b>Pre-Poll Mechanism:</b> Lux is evaluated dynamically before target trigger times by repolling OWM illuminance. For Evening ON, pre-poll occurs 5 minutes before the prospective Lux ON time (Sunset - Lead - Cloud - Lux - 5m). For Morning OFF, pre-poll occurs 5 minutes before standard Morning OFF (Sunrise + Lead - 5m). If live illuminance is at or below threshold, Evening ON is advanced and Morning OFF is extended." +
                      "</div>"

            if (enableLuxAdjustment) {
                input name: "luxThreshold", type: "number", title: "Lux Impact Threshold (lx)", description: "If Lux falls below this value during pre-poll, adjust Evening ON earlier and extend Morning OFF window", defaultValue: 50, required: true, submitOnChange: true
...
private Map getEvaluatedScheduleMap() {
    ...
    int boundedLuxMins = Math.min(Math.max(getPositiveOffsetMins(settings.luxShiftMins, 10), 0), 30)
