...
def timeStamp() { return "2026/09/05 11:35 PM" }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>Main App Configuration</b>") {
            ...
            input name: "notifier", type: "capability.notificationDevice", title: "Select Notification Device (Optional)", multiple: true, required: false
        }
        ...
        if (controlMethod == "lux") {
            ...
            section("<b>Solar Altitude Angle Settings</b>") {
                input name: "baseOnAngle", type: "decimal", title: "Base Sun Elevation Angle for Light ON (Degrees below horizon, e.g., -3.5)", defaultValue: -3.5, required: true, submitOnChange: true
                input name: "enableCloudOffset", type: "bool", title: "Adjust angle earlier on heavy overcast days?", defaultValue: true, submitOnChange: true
                if (enableCloudOffset) {
                    input name: "maxCloudShift", type: "decimal", title: "Max Angle Offset for 100% Cloud Cover (Degrees)", defaultValue: 2.5, required: true, submitOnChange: true
                }
            }
...
String getStatusBoardHtml() {
    ...
    if (controlMethod == "angle") {
        BigDecimal targetAngle = (baseOnAngle != null) ? baseOnAngle.toBigDecimal() : -3.5
        if (enableCloudOffset && currentClouds > 0) {
            BigDecimal shift = (maxCloudShift != null) ? maxCloudShift.toBigDecimal() : 2.5
            targetAngle = targetAngle + ((currentClouds / 100.0) * shift)
        }
        targetAngleStr = "${targetAngle.setScale(1, java.math.RoundingMode.HALF_UP)}°"
    }
    ...
    <div style='background-color:#1e1e24; color:#ffffff; padding:12px; border-radius:8px; font-family:sans-serif;'>
        ...
        <table style='width:100%; color:#ddd; font-size:0.9em; border-collapse:collapse;'>
            ...
            <tr style='border-bottom:1px solid #333;'>
                ...
                <td style='padding:4px;'>🎯 <b>Target Angle:</b> ${targetAngleStr}</td>
            </tr>
            <tr>
    ...
    switch (controlMethod) {
        ...
        case "angle":
            ...
            if (enableCloudOffset && cloudPct > 0) {
                BigDecimal shift = (maxCloudShift != null) ? maxCloudShift.toBigDecimal() : 2.5
                targetAngle = targetAngle + ((cloudPct / 100.0) * shift)
            }
            shouldBeOn = (sunAngle <= targetAngle)
            logDebug "Angle Check: Sun=${sunAngle}° | Target=${targetAngle}° (Base: ${baseOnAngle}°, Clouds: ${cloudPct}%) | ShouldBeOn=${shouldBeOn}"
            break
