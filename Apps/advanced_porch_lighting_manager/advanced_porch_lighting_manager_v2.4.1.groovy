...
def timeStamp() { return "2026/09/06 01:35 PM" }
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section(schedSectionTitle, hideable: true, hidden: true) {
            ...
            if (enableMorningSchedule != false) {
                ...
                input name: "mornOnTime", type: "time", title: "Morning On - Specific Time (Optional Override)", description: "If set, overrides all twilight lead times and environmental shifts for Morning ON", required: false, submitOnChange: true
                ...
                input name: "morningOffTime", type: "time", title: "Morning Off - Specific Time (Optional Override)", description: "Leave blank to turn OFF automatically 30 mins after Twilight Begins (${calculatedPreview.mornOffDefault})", required: false, submitOnChange: true
            }
            ...
            if (enableEveningSchedule != false) {
                ...
                input name: "eveOnTime", type: "time", title: "Evening On - Specific Time (Optional Override)", description: "If set, overrides all twilight lead times and environmental shifts for Evening ON", required: false, submitOnChange: true
                input name: "eveOffset", type: "number", title: "Evening On - Lead Time before Twilight Ends (Minutes, Optional)", description: "Default turns ON 30 mins BEFORE Evening Twilight Ends (Bypassed if Specific On Time is set)", defaultValue: 30, required: false, submitOnChange: true
        ...
        section("<b>3. Optional Schedule Adjustments</b>", hideable: true, hidden: true) {
            ...
            input name: "enableLuxAdjustment", type: "bool", title: "Enable Lux Impact Adjustment?", description: "Advances turn ON time and extends Morning OFF time when OWM illuminance falls below threshold", defaultValue: false, submitOnChange: true
            if (enableLuxAdjustment) {
                input name: "luxThreshold", type: "number", title: "Lux Impact Threshold (lx)", description: "If Lux falls below this value, extend lighting coverage window", defaultValue: 50, required: true, submitOnChange: true
                input name: "luxShiftMins", type: "number", title: "Max Lux Adjustment Offset (Minutes)", description: "Minutes to advance ON time and extend Morning OFF time when Lux threshold is triggered", defaultValue: 10, required: true, submitOnChange: true
            }
        }
...
private Map calculateSchedulePreview() {
    Map preview = [
        ...
        mornOn: "--", mornOnFactors: "None", mornOff: "--", mornOffDefault: "--", mornOffFactors: "Default (+30m)",
    ...
    if (twilightTimes.begin) {
        ...
        if (mornTwilightDate) {
            ...
            if (mornOnTime) {
                ...
                int totalMornOnShift = leadMins + envShifts.cloudShift + envShifts.luxShift
                ...
                if (envShifts.luxShift > 0) mOnFactors << "Lux (-${envShifts.luxShift}m)"
                preview.mornOnFactors = mOnFactors ? mOnFactors.join(" + ") : "Twilight Begins"
            }

            // MORNING OFF EVALUATION (Low lux extends Morning OFF later into the morning)
            ...
            if (morningOffTime) {
                ...
                List<String> mOffFactors = ["Default (+30m)"]
                if (extMins > 0) mOffFactors << "Lux Extension (+${extMins}m)"
    ...
    if (twilightTimes.end) {
        ...
        if (eveTwilightDate) {
            ...
            if (eveOnTime) {
                ...
                preview.eveOnFactors = eOnFactors ? eOnFactors.join(" + ") : "Twilight Ends"
            }
...
def buildAndScheduleEvents(evt = null) {
    ...
    if (enableMorningSchedule != false) {
        ...
        if (mornOnTime) {
            ...
            Date mornTwilightDate = parseDateString(twilightTimes.begin)
            if (mornTwilightDate) {
                int leadMins = getNormalizedOffsetMins(settings.mornOffset, 30) + envShifts.cloudShift + envShifts.luxShift
        ...
        if (morningOffTime) {
            ...
            if (mornTwilightDate) {
                int extMins = envShifts.luxShift
                Date defaultMornOffDate = new Date(mornTwilightDate.time + ((30 + extMins) * 60 * 1000))
                ...
                if (defaultMornOffDate.after(now)) {
                    ...
                    logInfo "Scheduled Default Morning OFF for: ${state.schedMornOffDisp} (+30m default + ${extMins}m lux extension post-twilight)"
                    if (upcomingEventDesc == "None scheduled") upcomingEventDesc = "Morning OFF at ${state.schedMornOffDisp}"
                }
            }
        }
    ...
    if (enableEveningSchedule != false) {
        ...
        } else if (twilightTimes.end) {
            Date eveTwilightDate = parseDateString(twilightTimes.end)
            if (eveTwilightDate) {
                int leadMins = getNormalizedOffsetMins(settings.eveOffset, 30) + envShifts.cloudShift + envShifts.luxShift
                ...
                if (eveRunTime.after(now)) {
                    ...
                    logInfo "Scheduled Twilight Evening ON for: ${state.schedEveOnDisp} (${leadMins} mins lead time before twilight ends)"
                    if (upcomingEventDesc == "None scheduled") upcomingEventDesc = "Evening ON at ${state.schedEveOnDisp}"
                }
            }
        }
