...
static String version() { return '2.9.1' }
def timeStamp() { return "2026/09/07 09:00 AM" }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>1. Device Connections</b>", hideable: true, hidden: true) {
            input name: "switches", type: "capability.switch", title: "Select Light/Switch(es)", multiple: true, required: true
        ...
        section("<b>3. Optional Schedule Adjustments</b>", hideable: true, hidden: true) {
            ...
            input name: "enableCloudAdjustment", type: "bool", title: "Enable Cloud Cover Adjustment?", description: "Advances ON times earlier on heavy overcast days based on OWM Cloud PCT. (Scales proportionally based on cloud percentage; e.g. 50% cloud cover applies 50% of your maximum shift offset)", defaultValue: false, submitOnChange: true
            ...
            input name: "enableLuxAdjustment", type: "bool", title: "Enable Lux Impact Adjustment?", description: "Extends Morning OFF time when OWM illuminance falls below threshold. (Binary extension: when current illuminance drops to or below the set lx threshold, the full extension minutes are appended to Morning OFF)", defaultValue: false, submitOnChange: true
...
private Date resolveEveningScheduleTime(def timeInput, Boolean forNextFutureExecution = true) {
    Date todayDate = buildClockDate(timeInput, 0)
    if (!todayDate) return null
...
private Boolean isInsideScheduleWindow(Date now, Date onDate, Date offDate) {
    ...
    // Standard Window (e.g. 6:00 AM to 7:30 AM or 7:00 PM to 11:30 PM)
    if (offTime > onTime) {
        return (nowTime >= onTime && nowTime < offTime)
    }
    
    // Cross-Midnight Window (e.g. 7:00 PM to 12:00 AM / 1:00 AM)
    return (nowTime >= onTime || nowTime < offTime)
}
...
private Map getEvaluatedScheduleMap() {
    ...
    if (sunsetStr) {
        ...
        if (sSetDate) {
            if (eveOnTime) {
                sched.eveOnDate = resolveEveningScheduleTime(eveOnTime, true)
    ...
    // Bulletproof Evening Cutoff Calculation (Anchors Evening OFF after Evening ON)
    String offTimeIso = eveningCutoffTime ?: "00:00"
    Date unanchoredOff = resolveEveningScheduleTime(offTimeIso, true)

    if (sched.eveOnDate && unanchoredOff) {
        Calendar onCal = Calendar.getInstance(location.timeZone)
        onCal.time = sched.eveOnDate

        Calendar offCal = Calendar.getInstance(location.timeZone)
        offCal.time = unanchoredOff

        // If off time hour/minute is earlier or equal to on time hour/minute, anchor off time to the day after ON time
        if (offCal.get(Calendar.HOUR_OF_DAY) < onCal.get(Calendar.HOUR_OF_DAY) || 
           (offCal.get(Calendar.HOUR_OF_DAY) == onCal.get(Calendar.HOUR_OF_DAY) && offCal.get(Calendar.MINUTE) <= onCal.get(Calendar.MINUTE))) {
            ...
            anchoredCal.time = sched.eveOnDate
            ...
            sched.eveOffDate = anchoredCal.time
        } else {
            sched.eveOffDate = unanchoredOff
        }
    } else {
        sched.eveOffDate = unanchoredOff
    }
...
/* Hub Reboot Recovery & Schedule-Aware Reconciliation */
...
private void reconcileStateOnBoot() {
    ...
    // Morning Today Clock Window Construction
    if (enableMorningSchedule != false && twilightTimes.begin) {
        Date mornTwilightRaw = parseDateString(twilightTimes.begin)
        Date mornOnToday = null
        Date mornOffToday = null

        if (mornTwilightRaw) {
            if (mornOnTime) {
                mornOnToday = resolveMorningScheduleTime(mornOnTime, false)
            } else {
                Date mornTwilightToday = buildClockDate(mornTwilightRaw, 0)
                int leadMins = getNormalizedOffsetMins(settings.mornOffset, 30)
                int totalMornOnShift = leadMins + envShifts.cloudShift
                mornOnToday = new Date(mornTwilightToday.time - (totalMornOnShift * 60 * 1000))
            }
        }
        ...
        if (sunriseStr) {
            ...
            if (sDateRaw) {
                if (morningOffTime) {
                    mornOffToday = resolveMorningScheduleTime(morningOffTime, false)
                } else {
                    Date sDateToday = buildClockDate(sDateRaw, 0)
                    int mornOffLeadMins = getNormalizedOffsetMins(settings.mornOffOffset, 30)
                    int extMins = envShifts.luxShift
                    mornOffToday = new Date(sDateToday.time + ((mornOffLeadMins + extMins) * 60 * 1000))
                }
            }
        }

        insideMorningWindow = isInsideScheduleWindow(now, mornOnToday, mornOffToday)
    }

    // Evening Today Clock Window Construction
    if (enableEveningSchedule != false) {
        Date eveOnToday = null
        Date eveOffToday = null

        if (eveOnTime) {
            eveOnToday = resolveEveningScheduleTime(eveOnTime, false)
        } else {
            String sunsetStr = twilightDevice?.currentValue("localSunset")
            if (sunsetStr) {
                ...
                if (sSetRaw) {
                    ...
                    int leadMins = getNormalizedOffsetMins(settings.eveOffset, 30)
                    int totalEveShift = leadMins + envShifts.cloudShift
                    eveOnToday = new Date(sSetToday.time - (totalEveShift * 60 * 1000))
                }
            }
        }

        String offTimeIso = eveningCutoffTime ?: "00:00"
        Date unanchoredOffToday = resolveEveningScheduleTime(offTimeIso, false)

        if (eveOnToday && unanchoredOffToday) {
            Calendar onCal = Calendar.getInstance(location.timeZone)
            onCal.time = eveOnToday

            Calendar offCal = Calendar.getInstance(location.timeZone)
            offCal.time = unanchoredOffToday

            if (offCal.get(Calendar.HOUR_OF_DAY) < onCal.get(Calendar.HOUR_OF_DAY) || 
               (offCal.get(Calendar.HOUR_OF_DAY) == onCal.get(Calendar.HOUR_OF_DAY) && offCal.get(Calendar.MINUTE) <= onCal.get(Calendar.MINUTE))) {
                
                Calendar anchoredCal = Calendar.getInstance(location.timeZone)
                anchoredCal.time = eveOnToday
                anchoredCal.add(Calendar.DAY_OF_YEAR, 1)
                anchoredCal.set(Calendar.HOUR_OF_DAY, offCal.get(Calendar.HOUR_OF_DAY))
                anchoredCal.set(Calendar.MINUTE, offCal.get(Calendar.MINUTE))
                anchoredCal.set(Calendar.SECOND, 0)
                anchoredCal.set(Calendar.MILLISECOND, 0)
                eveOffToday = anchoredCal.time
            } else {
                eveOffToday = unanchoredOffToday
            }
        } else {
            eveOffToday = unanchoredOffToday
        }

        insideEveningWindow = isInsideScheduleWindow(now, eveOnToday, eveOffToday)
    }
