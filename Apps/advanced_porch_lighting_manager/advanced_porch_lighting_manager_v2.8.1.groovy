...
static String version() { return '2.8.1' }
def timeStamp() { return "2026/09/07 08:30 AM" }
...
private void reconcileStateOnBoot() {
    ...
    if (enableMorningSchedule != false && twilightTimes.begin) {
        ...
        if (mornTwilightDate) {
            if (mornOnTime) {
                mornOnToday = getTodayClockTime(mornOnTime)
            } else {
                int leadMins = getNormalizedOffsetMins(settings.mornOffset, 30)
                int totalMornOnShift = leadMins + envShifts.cloudShift
        ...
        if (sunriseStr) {
            ...
            if (sDate) {
                if (morningOffTime) {
                    mornOffToday = getTodayClockTime(morningOffTime)
                } else {
                    int mornOffLeadMins = getNormalizedOffsetMins(settings.mornOffOffset, 30)
                    int extMins = envShifts.luxShift
        ...
        if (mornOnToday && mornOffToday && now.after(mornOnToday) && now.before(mornOffToday)) {
    ...
    if (enableEveningSchedule != false) {
        ...
        if (eveOnTime) {
            eveOnToday = getTodayClockTime(eveOnTime)
        } else {
            String sunsetStr = twilightDevice?.currentValue("localSunset")
            if (sunsetStr) {
                Date sSetDate = parseDateString(sunsetStr)
        ...
        String offTimeIso = eveningCutoffTime ?: "00:00"
        eveOffToday = getTodayClockTime(offTimeIso)

        if (eveOnToday && eveOffToday) {
            // Handle cross-midnight window (e.g. 7:00 PM to 12:00 AM / 1:00 AM)
            if (eveOffToday.before(eveOnToday)) {
                if (now.after(eveOnToday) || now.before(eveOffToday)) {
                    insideEveningWindow = true
                }
            } else if (now.after(eveOnToday) && now.before(eveOffToday)) {
                insideEveningWindow = true
            }
        }
    }
...
/* Today-Only Clock Time Parser (Evaluates bounds for today without rolling dates forward) */
private Date getTodayClockTime(String isoTime) {
    ...
    try {
        ...
        logError "Failed to parse today's clock time '${isoTime}': ${e.message}"
        return null
    }
}

/* Universal Next-Clock-Time Parser (Handles Midnight & Past-Time Rollover for Scheduling) */
