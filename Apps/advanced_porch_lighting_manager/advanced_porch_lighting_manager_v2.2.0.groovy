 ...
 *  
 *  Changelog:
 *	v2.2.0    09/06/26    jshimota    GUI & Logic overhaul: Added Section 3 'Optional Schedule Adjustments' with Twilight Parser attribute selectors (Used/Civil/Nautical/Astronomical), Cloud Cover extension, and Lux impact logic. Rephrased Schedule labels (Morning On -, Evening On -, etc.), split into Morning/Evening subsections, made lead times optional, updated Evening Off default to 12:00 AM, and moved App Preferences to Section 4.
...
def timeStamp() { return "2026/09/06 10:45 AM" }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>2. Schedule Configuration</b>", hideable: true, hidden: true) {
            paragraph "<div style='background:#E8F0FE; padding:4px 8px; border-radius:4px; font-weight:bold; color:#1A73E8;'>Morning</div>"
            input name: "mornOffset", type: "number", title: "Morning On - Lead Time before Twilight Begin (Minutes, Optional)", description: "Minutes BEFORE Twilight Begin (e.g. 15 turns lights ON 15 mins before twilight)", defaultValue: 15, required: false, submitOnChange: true
            input name: "morningOffTime", type: "time", title: "Morning Off - Time (Optional)", description: "Leave blank to turn OFF automatically at Sunrise", required: false, submitOnChange: true

            paragraph "<div style='background:#E8F0FE; padding:4px 8px; border-radius:4px; font-weight:bold; color:#1A73E8; margin-top:8px;'>Evening</div>"
            input name: "eveOffset", type: "number", title: "Evening On - Lead Time before Twilight End (Minutes, Optional)", description: "Minutes BEFORE Twilight End (e.g. 15 turns lights ON 15 mins before twilight)", defaultValue: 15, required: false, submitOnChange: true
            input name: "eveningCutoffTime", type: "time", title: "Evening Off - Time (Optional)", defaultValue: "00:00", required: false, submitOnChange: true

            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0;'/>"
            input name: "turnOffOnSleepMode", type: "bool", title: "Turn OFF lights immediately when Hub Mode changes to 'Sleeping' or 'Night'?", description: "Overrides scheduled cutoff if triggered earlier", defaultValue: true, submitOnChange: true
        }
...
String getStatusBoardHtml() {
    ...
    String eveOffDisp  = state.schedEveOffDisp  ?: (eveningCutoffTime ? formatIsoTimeStr(eveningCutoffTime) : "12:00 AM")
    ...
    <div style='background-color:#1e1e24; color:#ffffff; padding:12px; border-radius:8px; font-family:sans-serif;'>
        ...
        <div style='display:flex; justify-content:space-between; margin-bottom:10px; font-size:0.9em; color:#ddd;'>
            <span><b>Lighting State:</b> <span style='color:#00BCD4; font-weight:bold;'>${lightingState}</span></span>
            <span><b>Hub Mode:</b> <span style='color:#FFB74D; font-weight:bold;'>${currentMode}</span></span>
        </div>
...
def handleModeChange(evt) {
    ...
    if (currentMode in ["sleeping", "sleep", "night"]) {
...
private int getNormalizedOffsetMins(def settingVal) {
    ...
    try {
        ...
        logWarn "getNormalizedOffsetMins() -> Failed to parse offset '${settingVal}', defaulting to 0."
        return 0
    }
}
...
def buildAndScheduleEvents(evt = null) {
    ...
    if (twilightTimes.begin) {
        ...
        if (mornTwilightDate) {
            int leadMins = getNormalizedOffsetMins(mornOffset) + envShiftMins
    ...
    if (twilightTimes.end) {
        ...
        if (eveTwilightDate) {
            int leadMins = getNormalizedOffsetMins(eveOffset) + envShiftMins
            ...
            if (eveRunTime.after(now)) {
                ...
                logInfo "Scheduled Evening ON for: ${state.schedEveOnDisp} (${leadMins} mins lead time before twilight)"
                if (upcomingEventDesc == "None scheduled") {
    ...
    // 4. Evening Cutoff Turn-OFF Event Construction
    if (eveningCutoffTime) {
        Date eveOffDate = parseIsoTimeToTodayDate(eveningCutoffTime)
        logTrace "buildAndScheduleEvents() -> Evening Off Time parsed: ${eveOffDate}"
        if (eveOffDate) {
            ...
            if (eveOffDate.after(now)) {
                ...
                logInfo "Scheduled Evening OFF for: ${state.schedEveOffDisp}"
