...
static String version() { return '2.9.0' }
def timeStamp() { return "2026/09/07 08:45 AM" }
...
private Boolean isInsideScheduleWindow(Date now, Date onDate, Date offDate) {
    if (!onDate || !offDate) return false
    
    // Cross-midnight window check (e.g., 7:00 PM to 12:00 AM / 1:00 AM)
    if (offDate.before(onDate) || offDate.equals(onDate)) {
        return now.after(onDate) || now.before(offDate)
    }
    return now.after(onDate) && now.before(offDate)
}
...
private Map getEvaluatedScheduleMap() {
    ...
    sched.eveOffDate = resolveEveningScheduleTime(offTimeIso, true)
    ...
    if (enableMorningSchedule != false && twilightTimes.begin) {
        ...
        if (mornTwilightDate) {
            if (mornOnTime) {
                ...
                int leadMins = getNormalizedOffsetMins(settings.mornOffset, 30)
                int totalMornOnShift = leadMins + envShifts.cloudShift
                mornOnToday = new Date(mornTwilightDate.time - (totalMornOnShift * 60 * 1000))
            }
        }
        ...
        if (sunriseStr) {
            ...
            if (sDate) {
                if (morningOffTime) {
                    ...
                    int mornOffLeadMins = getNormalizedOffsetMins(settings.mornOffOffset, 30)
                    int extMins = envShifts.luxShift
                    mornOffToday = new Date(sDate.time + ((mornOffLeadMins + extMins) * 60 * 1000))
                }
            }
        }
        ...
        if (eveOnTime) {
            ...
            if (sunsetStr) {
                ...
                if (sSetDate) {
                    int leadMins = getNormalizedOffsetMins(settings.eveOffset, 30)
                    int totalEveShift = leadMins + envShifts.cloudShift
                    eveOnToday = new Date(sSetDate.time - (totalEveShift * 60 * 1000))
                }
            }
        }

        String offTimeIso = eveningCutoffTime ?: "00:00"
        eveOffToday = resolveEveningScheduleTime(offTimeIso, false)
