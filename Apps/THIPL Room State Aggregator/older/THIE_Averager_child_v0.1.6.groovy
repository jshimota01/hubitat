...
def mainPage() {
    dynamicPage(name: "mainPage", title: "THIE Room Averager Configuration", install: true, uninstall: true) {
        ...
        if (activeRoomName) {
            ...
            if (!app.label || app.label == "THIE Room Averager Child" || app.label.startsWith("ATHIE Omni Sensor for ")) {
...
private void createOrUpdateChildDevice() {
    ...
    if (targetRoom) {
        ...
        if (!childDevice) {
            ...
            logInfo "Updating existing Virtual Omni THIE Child Device label to: ${expectedDeviceLabel}"
        ...
        if (childDevice) {
            ...
            logError "Failed to locate or create child device. Please ensure the driver 'Virtual Omni THIE Sensors' is installed in Hubitat's 'Drivers Code' section."
        }
    }
}
