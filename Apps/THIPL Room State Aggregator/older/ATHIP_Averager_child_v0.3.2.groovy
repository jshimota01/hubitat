/*
 * ATHIP Room Averager Child
 * Averages selected THIP sensors for a specific room and outputs to a Virtual ATHIP Omni Sensors device.
...
definition(
    name: "ATHIP Room Averager Child",
    namespace: "hubitat",
    author: "James Shimota",
    description: "Averages selected THIP sensors for a specific room and outputs to a Virtual ATHIP Omni Sensors device.",
    category: "Convenience",
    parent: "hubitat:ATHIP Room Averager Parent",
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "ATHIP Room Averager Configuration", install: true, uninstall: true) {
        ...
        if (activeRoomName) {
            String defaultAppLabel = "ATHIP Omni Sensor for ${activeRoomName}"
            String defaultDeviceName = "${activeRoomName} ATHIP Omni Sensor Device"
            String defaultDeviceLabel = "${activeRoomName} ATHIP Omni"

            if (!app.label || app.label == "ATHIP Room Averager Child" || app.label.startsWith("ATHIP Omni Sensor for ")) {
...
private void logMessage(String level, String msg) {
    if (settings["log${level.capitalize()}Enable"] == true) {
        log."${level}" "${app.label ?: 'ATHIP Room Averager Child'}${level == 'warn' ? ' WARNING' : level == 'error' ? ' ERROR' : ''}: ${msg}"
    }
}
...
private void createOrUpdateChildDevice() {
    ...
    if (targetRoom) {
        String expectedDeviceName = "${targetRoom} ATHIP Omni Sensor Device"
        String expectedDeviceLabel = "${targetRoom} ATHIP Omni"
        
        // Use app.id as the unique child DNI
        def childDevice = getChildDevice("${app.id}")

        if (!childDevice) {
            logInfo "Creating Virtual ATHIP Omni Sensors Child Device (${expectedDeviceLabel})..."
            try {
                childDevice = addChildDevice(
                    "hubitat", 
                    "Virtual ATHIP Omni Sensors", 
                    "${app.id}", 
                    [name: expectedDeviceName, label: expectedDeviceLabel, room: targetRoom]
                )
            } catch (Exception e) {
                logWarn "Could not create child device under 'hubitat' namespace: ${e.message}. Testing fallback..."
            }
        } else {
            logInfo "Updating existing Virtual ATHIP Omni Sensors Child Device label to: ${expectedDeviceLabel}"
        ...
        if (childDevice) {
            try {
                ...
                if (roomId != null) {
                    Long devId = childDevice.id.toString().toLong()
                    location.setDeviceRoom(devId, roomId)
                    logInfo "Successfully assigned child device (" + childDevice.label + ") to room '" + targetRoom + "' (ID: " + roomId + ")"
                }
            } catch (Exception e) {
                logWarn "Could not assign room ID to child device: ${e.message}"
            }
        } else {
            logError "Failed to locate or create child device. Please ensure the driver 'Virtual ATHIP Omni Sensors' is installed in Hubitat's 'Drivers Code' section."
        }
    }
}
...
private calculateAndSetPower() {
    ...
    if (validValues) {
        def avg = validValues.sum() / validValues.size()
        logDebug "Calculated Power Average: ${avg}"
        target.setPower(avg)
    }
}