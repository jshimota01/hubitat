 ...
 * Aggregates selected THIPL sensors for a specific room and outputs to a Virtual THIPL Room State Aggregator device.
    ...
    dynamicPage(name: "mainPage", title: "THIPL Room State Aggregator Configuration", install: true, uninstall: true) {
        ...
        if (activeRoomName) {
            ...
            section("<b>Step 4: Select Source Room Sensors to Aggregate</b>") {
				...
				input name: "lightSensors", type: "capability.switch", title: "Select Room Lights / Switches:", multiple: true, required: false
				input name: "aggregationType", type: "enum", title: "Power & Light Calculation Mode:", options: ["avg": "Average (Room Level)", "sum": "Sum Total (Floor/House Level)"], defaultValue: "avg", required: true
            }
...
private void createOrUpdateChildDevice() {
    ...
    if (targetRoom) {
        ...
        if (!childDevice) {
            ...
            try {
                childDevice = addChildDevice(
                    "hubitat", 
                    "Virtual THIPL Room State Aggregator", 
                    "${app.id}", 
        ...
        if (childDevice) {
            ...
            logError "Failed to locate or create child device. Please ensure the driver 'Virtual THIPL Room State Aggregator' is installed in Hubitat's 'Drivers Code' section."
        }
    }
}
...
private void calculateAndSetPower() {
    ...
    if (validValues) {
        ...
        logDebug "Calculated Power (${settings.aggregationType ?: 'avg'}): ${result}"
...
private void calculateAndSetLights() {
    ...
    int countOn = lightSensors.count { it.currentValue("switch") == "on" }
