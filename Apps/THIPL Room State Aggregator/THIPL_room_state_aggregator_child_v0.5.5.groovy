 ...
 *      2026-08-30    Gemini        0.5.2       Updated parent definition reference to THIPL Room State Aggregator.
 *      2026-09-04    jshimota      0.5.3       Added runIn() 1-second overwrite debouncing on event handlers and target device pre-command equality checks to eliminate duplicate event execution loops.
 *      2026-09-05    Gemini        0.5.4       Explicitly converted childDevice.id to Long in location.setDeviceRoom() call to resolve method signature parameter mismatch.
 *      2026-09-20    jshimota      0.5.5       Method Signature Parameter Fix for setDeviceRoom:
 *                                              - Updated location.setDeviceRoom() target call to pass childDevice.id as String (childDevice.id.toString()) to match native Hubitat setDeviceRoom(String, Long) signature.
 *
 */
...
def timeStamp() { return "2026/09/20 11:30 AM" }
...
definition(
    ...
    parent: "jshimota:THIPL Room State Aggregator",
...
private void createOrUpdateChildDevice() {
    ...
    if (targetRoom) {
        ...
        if (!childDevice) {
            ...
            if (roomId != null) {
                try {
                    location.setDeviceRoom(childDevice.id.toString(), roomId)
...
private void calculateAndSetTemp() {
    ...
    if (target.currentValue("tempDeviceCount") != totalDevices) {
    ...
    if (weightedAvg != null) {
        BigDecimal curVal = target.currentValue("temperature") as BigDecimal
        if (curVal == null || (curVal - weightedAvg).abs() >= 0.05) {
...
private void calculateAndSetHumidity() {
    ...
    if (target.currentValue("humidityDeviceCount") != totalDevices) {
    ...
    if (weightedAvg != null) {
        BigDecimal curVal = target.currentValue("humidity") as BigDecimal
        if (curVal == null || (curVal - weightedAvg).abs() >= 0.05) {
...
private void calculateAndSetLux() {
    ...
    if (target.currentValue("illuminanceDeviceCount") != totalDevices) {
    ...
    if (weightedAvg != null) {
        BigDecimal curVal = target.currentValue("illuminance") as BigDecimal
        if (curVal == null || (curVal - weightedAvg).abs() >= 0.5) {
...
private void calculateAndSetPower() {
    ...
    if (target.currentValue("powerDeviceCount") != totalDevices) {
    ...
    if (settings.aggregationType == "sum") {
        ...
        if (validValues && validValues.size() > 0) {
            ...
            BigDecimal curVal = target.currentValue("power") as BigDecimal
            if (curVal == null || curVal != totalPower) {
        ...
        if (weightedAvg != null) {
            BigDecimal curVal = target.currentValue("power") as BigDecimal
            if (curVal == null || (curVal - weightedAvg).abs() >= 0.1) {
...
private void calculateAndSetLights() {
    ...
    if (target.currentValue("lightDeviceCount") != totalConfiguredLights) {
    ...
    // Only update target driver if active count has changed
    int curOn = target.currentValue("lightsOnCount") != null ? (target.currentValue("lightsOnCount") as Integer) : -1
    if (curOn != countOn) {
        logDebug "Calculated Active Light Count (${countOn}/${totalConfiguredLights} rolled up): ${countOn}"
        target.setLightsOnCount(countOn)
    } else {
        logTrace "Active light count (${countOn}) unchanged on ${target.displayName}. Skipping driver command."
    }
}