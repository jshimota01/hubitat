 ...
 *  v1.3.9    08/28/26    jshimota    Added version and timestamp signature logInfo markers on lifecycle methods
 *  v1.3.8    08/28/26    jshimota    Offloaded tile generation to driver to eliminate event-bus state race conditions
 *  v1.3.7    08/28/26    jshimota    Updated tile title to 'Garage Door Status'
 *  v1.3.6    08/28/26    jshimota    Added manual 'Reset / Repair State' button with active relay safety guard
 *  v1.3.5    08/28/26    jshimota    Added comprehensive trace/debug logging
 *  v1.3.4    08/28/26    jshimota    Reinstated garageControlHandler alias
 *  v1.3.3    08/28/26    jshimota    Replaced getSettings() call with native currentValue() attribute reads
 *  v1.3.2    08/28/26    jshimota    Subscribed strictly to relayTrigger
 *  v1.3.1    08/28/26    jshimota    Added subscription to controller driverVersion updates
 *  v1.3.0    08/28/26    jshimota    Cleaned up redundant HTML tags
 *  v1.2.9    08/28/26    jshimota    Integrated driver typography preferences
 *  v1.2.8    08/28/26    jshimota    Made notificationTile responsive
 *  v1.2.7    08/28/26    jshimota    Added tileFormat preference toggle
 *  v1.2.6    08/28/26    jshimota    Enhanced notificationTile HTML styling
 *  v1.2.5    08/28/26    jshimota    Added automated dashboard notificationTile generation
 *  v1.2.4    08/28/26    jshimota    Added colorized live state badge header
 *  v1.2.3    08/28/26    jshimota    Added HTML status table displaying live states
 *  v1.2.2    08/28/26    jshimota    Added live status summary block
 *  v1.2.1    08/28/26    jshimota    Removed redundant switch-off and reversal timers
 *  v1.2.0    08/28/26    jshimota    Refactored app to pipe contact sensor events into Virtual Driver
 *  v1.1.1    06/21/26    jshimota    Fixed timeout sensor logic typo
...
def timeStamp() { return "2026/08/28 03:52 PM" }
    ...
    dynamicPage(name: "mainPage") {
        ...
        """) {
            if (garageControl) {
                ...
                String shadowChildName = "${garageControl.displayName} Alexa Lock"
                String shadowLockVal = garageControl.currentValue("shadowLockState") ?: "LOCKED"

                if (shadowLockVal == "ACTIVE") shadowLockVal = "LOCKED"

                try {
                    def allHubDevices = getChildDevices()
                    def shadowChild = allHubDevices?.find { it.deviceNetworkId == childNetworkId }
                    if (shadowChild) {
                        shadowChildName = shadowChild.displayName ?: shadowChild.label ?: shadowChildName
                        shadowLockVal = shadowChild.currentValue("lock")?.toUpperCase() ?: shadowLockVal
                    }
                } catch (Exception e) {
                    logDebug "Child device lookup safeguard: ${e.message}"
                }

                String shadowLockColor = shadowLockVal == "LOCKED" ? "#2e7d32" : (shadowLockVal == "UNLOCKED" ? "#c62828" : "#757575")

                String driverVer = garageControl.currentValue("driverVersion") ?: "v1.5.5"
                ...
                <table style='width:100%; border-collapse: collapse; border: 1px solid #ddd; font-family: sans-serif; font-size: 13px;'>
                    ...
                    <tr>
                        ...
                        <td style='padding: 8px; border: 1px solid #ddd;'>${shadowChildName}</td>
