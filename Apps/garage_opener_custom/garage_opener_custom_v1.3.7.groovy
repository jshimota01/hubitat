...
def timeStamp() { return "2026/08/28 01:35 PM" }
    ...
    dynamicPage(name: "mainPage") {
        ...
        """) {
            ...
            if (garageControl) {
                ...
                String driverVer = garageControl.currentValue("driverVersion") ?: "v1.3.6"
...
private void repairAndResetState() {
    ...
    // Force tile re-render and log success message
    ...
    refreshNotificationTile("State reset/repaired manually")
}
...
def setupSubscriptions() {
    ...
    subscribe(garageControl, "door", doorStateTileHandler)
...
// Update Tile Only on Door State Changes (Does NOT trigger relay)
def doorStateTileHandler(evt) {
    logDebug "Virtual Garage Controller door state event: ${evt.value}"
    refreshNotificationTile("Door state changed to ${evt.value}")
}

// Backward Compatibility Handler Alias to protect against orphaned subscription pointers
def garageControlHandler(evt) {
    logDebug "garageControlHandler legacy alias called with event: ${evt.name} -> ${evt.value}"
    doorStateTileHandler(evt)
}

// Driver Preferences Updated Event Listener
def driverUpdatedHandler(evt) {
    logDebug "Driver settings update event detected (${evt.value}). Re-rendering notification tile..."
...
private void refreshNotificationTile(String reason = "") {
    ...
    if (formatType == "plain") {
        tileOutput = "${titleStr} | ${doorState} | Closed:${closedState.toUpperCase()} Open:${openState.toUpperCase()} | ${reason} (${timeStr})"
    } else {
        // Read style attributes safely using native currentValue calls (Normalized around 1.0em = 13px base)
        String tSize = sanitizeFontSize(garageControl.currentValue("titleFontSize"), "1.0em")
        String sSize = sanitizeFontSize(garageControl.currentValue("statusFontSize"), "0.85em")
        String cSize = sanitizeFontSize(garageControl.currentValue("sensorFontSize"), "0.77em")
        String uSize = sanitizeFontSize(garageControl.currentValue("updatedFontSize"), "0.70em")

        String tWeight = (garageControl.currentValue("boldTitle") != "false") ? "font-weight:bold;" : "font-weight:normal;"
        String sWeight = (garageControl.currentValue("boldStatus") != "false") ? "font-weight:bold;" : "font-weight:normal;"
        String cWeight = (garageControl.currentValue("boldSensors") != "false") ? "font-weight:bold;" : "font-weight:normal;"
        ...
        // Buttonized Sensor Badges Logic
        ...
        // Cleaned HTML String with Stacked Sensor Badges and Dynamic Font Sizing
