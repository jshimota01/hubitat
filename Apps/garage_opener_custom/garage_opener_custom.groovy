 ...
 * v1.1.1  6-21-2026      JAS - Fixed timeout sensor logic typo, corrected info logging behaviors
 */
...
preferences {
    page(name: "mainPage", title: "Garage Opener", install: true, uninstall: true) {
        ...
        section("<h2>Options</h2>") {
            ...
            input name: "dbgEnable", type: "bool", title: "Enable Debug Logging?", defaultValue: false, required: true
        }
    }
}
...
def handleTimeout() {
    ...
    if (doorStatus == 'opening') {
        ...
        if (closedContact) { // Fixed: changed from openContact to closedContact
...
def garageClosedContactHandler(evt) {    
    if (evt.value == 'closed') {
        ...
        // Fixed: Set to 'opening' instead of forcing it to 'open' immediately
        garageControl.sendEvent(name: "door", value: "opening", descriptionText: "${closedContact.label} detected that ${garageControl.label} is no longer fully closed")
    }
}

def disableDebugLogging() {
    log.info "Auto-disabling debug logging."
    app.updateSetting("dbgEnable", [value: "false", type: "bool"])
}

def logInfo(msg) {
    log.info msg // Fixed: Always allow info logging for clean app history tracking
}

def logDebug(msg) {
    if (dbgEnable) log.debug msg
}