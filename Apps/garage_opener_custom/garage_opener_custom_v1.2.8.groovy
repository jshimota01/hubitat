...
def timeStamp() { return "2026/08/28 11:25 AM" }
...
private void refreshNotificationTile(String reason = "") {
    ...
    if (formatType == "plain") {
        ...
        String closedBadge = closedState == "closed" ? "<span style='color:#4caf50;'>CLOSED</span>" : "<span style='color:#888;'>OPEN</span>"
        String openBadge = openState == "closed" ? "<span style='color:#f44336;'>CLOSED</span>" : "<span style='color:#888;'>OPEN</span>"

        tileOutput = """<div style='height:100%; width:100%; box-sizing:border-box; padding:4px 6px; font-family:sans-serif; font-size:11px; background:#181818; color:#fff; border-radius:6px; overflow:hidden; display:flex; flex-direction:column; justify-content:space-between;'>
            <div style='display:flex; justify-content:space-between; align-items:center; gap:4px;'>
                <span style='font-weight:bold; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:65%;' title='${nameStr}'>${nameStr}</span>
                <span style='background:${stateBgColor}; color:#fff; padding:1px 6px; border-radius:8px; font-weight:bold; font-size:9px; letter-spacing:0.5px;'>${doorState}</span>
            </div>
            <div style='display:flex; justify-content:space-between; font-size:10px; color:#ccc; margin:2px 0;'>
                <span>C: ${closedBadge}</span>
                <span>O: ${openBadge}</span>
            </div>
            <div style='font-size:9px; color:#4fc3f7; border-top:1px solid #333; padding-top:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;'>
