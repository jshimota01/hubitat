 ...
 *  
 *  Changelog:
 *  v1.7.0    08/30/26    jshimota    Applied App Master Template v1.2.0 (styled banner, badging, collapsible prefs, settings hash + code version re-init)
 *  v1.6.1    08/29/26    jshimota    Updated contact sensor GUI labels to display explanatory position context
 *  v1.6.0    08/28/26    jshimota    Reverted app codebase to stable v1.4.0 baseline versioning
 *  v1.5.4    08/28/26    jshimota    Updated main page table to display exact child device name and explicit LOCKED/UNLOCKED status
 *  v1.5.3    08/28/26    jshimota    Aligned version references with Driver v1.5.4
 *  v1.5.2    08/28/26    jshimota    Fixed IllegalArgumentException on getChildDevice call inside app UI mainPage table
 ...
 *  v1.4.2    08/28/26    jshimota    Added dedicated Bridge App Version row to main GUI status table under Driver Version
 *  v1.4.1    08/28/26    jshimota    Updated app live status table with Alexa Confirmation state attribute row
 *  v1.4.0    08/28/26    jshimota    Converted version marker to a single-shot trace demarcation logged strictly when version changes
 *  v1.3.9    08/28/26    jshimota    Added version and timestamp signature logInfo markers on lifecycle methods (installed/updated/initialize)
...
def timeStamp() { return "2026/08/30 11:15 AM" }
...
definition(
    ...
    iconX2Url: "",
    iconX3Url: "",
    importUrl: "https://raw.githubusercontent.com/jshimota01/hubitat/main/Apps/garage_opener_custom/Garage_Opener_Custom.groovy"
)
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section() {
            paragraph "<div style='background-color:#1A252F; color:#FFFFFF; padding:12px; border-radius:6px; text-align:center; margin-bottom:10px;'>" +
                      "<h2 style='color:#FFFFFF; margin:0; font-size:20px; font-weight:600;'>Garage Opener (Custom)</h2>" +
        ...
        section("<b>Live Controller Overview</b>") {
            if (garageControl) {
                String rawClosedSensor = garageControl.currentValue("closedContact") ?: (closedContact ? closedContact.currentValue("contact") : "Not Configured")
                String rawOpenSensor = garageControl.currentValue("openContact") ?: (openContact ? openContact.currentValue("contact") : "Not Configured")
                
                String closedSensorState = formatClosedContactState(rawClosedSensor)
                String openSensorState = formatOpenContactState(rawOpenSensor)
                
                String relayState = garageSwitch ? garageSwitch.currentValue("switch") : "Not Configured"
                String driverVer = garageControl.currentValue("driverVersion") ?: "v1.7.0"
                ...
                paragraph "<div style='background-color:#FADBD8; border-left:4px solid #CB4335; padding:10px; border-radius:4px; font-size:12px;'>" +
                          "⚠️ <b>Virtual Controller Not Assigned:</b> Please select your Virtual Simple Garage Door Controller device in the Controls section below.</div>"
            }
        }

        // State Recovery / Repair Utilities
        section("<b>State Maintenance & Manual Recovery</b>") {
        ...
        section("<b>App Preferences & Logging Options</b>", hideable: true, hidden: true) {
            ...
            input name: "showStatusInLabel", type: "bool", title: "Show Active Door Status in App Label?", defaultValue: true
...
// Helpers for Explanatory Contact Sensor GUI Display Labels
private String formatClosedContactState(String rawVal) {
    switch (rawVal?.toLowerCase()) {
        case "closed": return "closed (means door is fully down)"
        case "open":   return "open (means door is not down)"
        default:       return rawVal
    }
}

private String formatOpenContactState(String rawVal) {
    switch (rawVal?.toLowerCase()) {
        case "open":   return "open (means door is not up)"
        case "closed": return "closed (means door is fully up)"
        default:       return rawVal
    }
}
...
private void updateAppLabel() {
    ...
    String baseLabel = "Garage Opener (Custom)"
...
void updated() {
    ...
    if (settingsChanged || codeVersionChanged) {
        logInfo "Settings or code version modification detected. Re-establishing subscriptions and sync..."
...
void uninstalled() {
    logInfo "Uninstalling Garage Opener (Custom) app..."
    unsubscribe()
    unschedule()
}
...
def setupSubscriptions() {
    ...
    if (garageControl) {
        ...
        subscribe(garageControl, "door", doorStateHandler)
    }
...
def doorStateHandler(evt) {
    logDebug "Virtual Controller door state changed to: ${evt.value}"
    updateAppLabel()
}

// Explicit Relay Trigger Handler
def relayTriggerHandler(evt) {
    ...
    if (evt.value == "pulse") {
        if (garageSwitch) {
            ...
            logError "relayTrigger received, but physical garage switch is not configured!"
        }
    }
}
...
def garageOpenContactHandler(evt) {    
    ...
    if (garageControl && garageControl.hasCommand("setOpenContact")) {
...
def garageClosedContactHandler(evt) {    
    ...
    if (garageControl && garageControl.hasCommand("setClosedContact")) {
