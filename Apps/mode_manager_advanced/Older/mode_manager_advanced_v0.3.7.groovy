 ...
 *      2026-08-08    jshimota      0.3.7       Added alexaSleepSwitch for Alexa Routine integration (forces Awake switch OFF without triggering manual override state)
 *
 */
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "Mode Manager Advanced Configuration (v${version()})", install: true, uninstall: true) {
        ...
        section("Sleep / Awake State Switches (Reverse Mirrored)") {
            ...
            input name: "alexaSleepSwitch", type: "capability.switch", title: "Alexa Sleeping Virtual Switch (Alexa Routines Sync)", required: false
        }
...
def initialize() {
    ...
    // Reverse-Mirrored Awake & Sleep Subscriptions
    ...
    if (alexaSleepSwitch) subscribe(alexaSleepSwitch, "switch", alexaSleepSwitchHandler)
...
def schedulePeriodTime(String timeIso) {
    if (timeIso) {
        try {
            ...
            logTrace "Scheduled daily period trigger at ${timeDate} via schedule()"
        } catch (Exception e) {
...
def appButtonHandler(btn) {
    if (btn == "btnTrigger") {
        logInfo "Mode recheck triggered manually via App UI button. Clearing manual override."
...
def updateSwitchHandler(evt) {
    logTrace "Mode recheck triggered via external update switch '${evt.device}' turning ON."
    state.manualOverrideMode = null
    state.manualOverridePeriodKey = null
    state.modeReason = null
    evaluateAndSetMode()
    ...
    if (updateTriggerSwitch && updateTriggerSwitch.currentValue("switch") != "off") {
        logTrace "Resetting trigger switch back to OFF"
...
def vSwitchHandler(evt) {
    ...
    if (state.isSyncingVSwitches == true) {
        logTrace "Ignoring virtual switch event for '${evt.device}' generated by internal state synchronization."
        return
    }
    ...
    if (targetPeriod?.mode) {
        ...
        if (vSwitchSleeping && evt.deviceId.toString() != vSwitchSleeping.id.toString() && awakeSwitch.currentValue("switch") == "off") {
            logDebug "Daytime mode switch forced manually while sleeping. Forcing Awake switch ON."
            awakeSwitch.on()
        }
...
// Presence Event Handler (Master Sensor driving Home Switch)
def presenceHandler(evt) {
    logTrace "Master presence change detected: '${evt.device}' is now '${evt.value}'"
    if (evt.value == "present") {
        if (homeSwitch.currentValue("switch") != "on") {
            logDebug "Master presence is present. Turning Home switch ON."
            homeSwitch.on()
        }
    } else {
        if (homeSwitch.currentValue("switch") != "off") {
            logDebug "Master presence is not present. Turning Home switch OFF."
            homeSwitch.off()
        }
    }
}
...
// Away Switch Inverse Mirror Handler (Handles external presence toggles like Alexa/Dashboards)
def awaySwitchHandler(evt) {
    logDebug "Away switch changed to ${evt.value}"
    
    if (evt.value == "on") {
        if (homeSwitch && homeSwitch.currentValue("switch") != "off") {
            logDebug "Away switch turned ON externally. Forcing Home switch OFF."
            homeSwitch.off()
        }
    } else if (evt.value == "off") {
        if (homeSwitch && homeSwitch.currentValue("switch") != "on") {
            logDebug "Away switch turned OFF externally. Forcing Home switch ON."
            homeSwitch.on()
        }
    }
}

// Awake Primary Reverse-Mirroring Handler (Authoritative Sleep/Awake Input)
...
// Sleep Switch Inverse Mirror Handler (Handles external sleep toggles like Alexa/Dashboards)
def sleepSwitchHandler(evt) {
    logDebug "Sleep switch changed to ${evt.value}"
    
    if (evt.value == "on") {
        if (awakeSwitch && awakeSwitch.currentValue("switch") != "off") {
            logDebug "Sleep switch turned ON externally. Forcing Awake switch OFF."
            awakeSwitch.off()
        }
    } else if (evt.value == "off") {
        if (awakeSwitch && awakeSwitch.currentValue("switch") != "on") {
            logDebug "Sleep switch turned OFF externally. Forcing Awake switch ON."
            awakeSwitch.on()
        }
    }
}

// Alexa Sleeping Switch Handler (Triggers Awake Switch OFF without triggering a manual override)
def alexaSleepSwitchHandler(evt) {
    if (state.isSyncingVSwitches == true) return
    
    logDebug "Alexa Sleep Switch changed to ${evt.value}"
    if (evt.value == "on") {
        if (awakeSwitch && awakeSwitch.currentValue("switch") != "off") {
            logInfo "Alexa Sleep Switch turned ON. Forcing Awake switch OFF (Normal state change, not manual override)."
            awakeSwitch.off()
        }
    } else if (evt.value == "off") {
        if (awakeSwitch && awakeSwitch.currentValue("switch") != "on") {
            logInfo "Alexa Sleep Switch turned OFF. Forcing Awake switch ON."
            awakeSwitch.on()
        }
    }
}
...
def periodHandler() {
    logTrace "Scheduled time period boundary hit. Clearing manual override."
    state.manualOverrideMode = null
    state.manualOverridePeriodKey = null
    state.modeReason = null
    evaluateAndSetMode()
...
// Core State Engine (Priority: Manual Override > Away > Sleeping > Scheduled Time Period)
def evaluateAndSetMode() {
    ...
    if (!isHome) {
        ...
        state.manualOverrideMode = null
        state.manualOverridePeriodKey = null
        state.modeReason = "Presence Away"
        ...
        if (manageHSM) {
            if (state.lastHsmState != "armAway") {
                ...
                logTrace "HSM is already armed away. Skipping duplicate HSM command."
            }
        }

        updateVirtualModeSwitches(null) // Turn off period indicator switches when away
        updateAppLabel("Away", "Presence Away")
        return
    } else {
        ...
        if (manageHSM) {
            if (state.lastHsmState != "disarm") {
                ...
                logTrace "HSM is already disarmed. Skipping duplicate HSM command."
            }
        }
    }
    ...
    if (isSleeping) {
        ...
        changeMode(sleepMode?.toString(), "sleeping", "Sleeping")
        return
    }
...
def updateVirtualModeSwitches(String activePeriodKey) {
    // Set flag to inform vSwitchHandler and alexaSleepSwitchHandler to ignore self-generated events
    ...
    try {
            ...
            if (vSwitch) {
                if (activePeriodKey != null && periodKey == activePeriodKey) {
                    if (vSwitch.currentValue("switch") != "on") {
                        logTrace "Turning ON Virtual Period Switch for '${periodKey}' (${vSwitch})"
                        vSwitch.on()
                    }
                } else {
                    if (vSwitch.currentValue("switch") != "off") {
                        logTrace "Turning OFF non-active Virtual Period Switch for '${periodKey}' (${vSwitch})"
                        vSwitch.off()
                    }
                }
            }
        ...
        // Mirror Alexa Sleeping switch state based on active sleeping state
        if (alexaSleepSwitch) {
            Boolean isSleeping = (activePeriodKey == "sleeping") || (awakeSwitch?.currentValue("switch") == "off")
            if (isSleeping && alexaSleepSwitch.currentValue("switch") != "on") {
                logTrace "Syncing Alexa Sleeping Virtual Switch -> ON"
                alexaSleepSwitch.on()
            } else if (!isSleeping && alexaSleepSwitch.currentValue("switch") != "off") {
                logTrace "Syncing Alexa Sleeping Virtual Switch -> OFF"
                alexaSleepSwitch.off()
            }
        }
    } finally {
...
def updateAppLabel(String currentMode, String reason = null) {
    ...
    String activeReason = reason ?: state.modeReason ?: ""
    
    // Clean up reason text for clean HTML badge display
    String shortReason = ""
    if (activeReason.contains("Manual Override")) {
        shortReason = " (Override)"
    } else if (activeReason.contains("Presence Away")) {
        shortReason = " (Away)"
    } else if (activeReason.equalsIgnoreCase("Sleeping")) {
        shortReason = " (Sleep)"
    }
    ...
    if (app.label != formattedLabel) {
        logTrace "Updating App Label to: ${formattedLabel}"
