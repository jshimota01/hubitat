...
private void seedLoggingState() {
    state.lastLogInfoEnable = (settings.logInfoEnable == true)
    state.lastLogDebugEnable = (settings.logDebugEnable == true)
    state.lastLogTraceEnable = (settings.logTraceEnable == true)
}

/* Detect and log warning if logging preferences were modified */
private void checkLoggingChanges() {
    Boolean currentInfo  = (settings.logInfoEnable == true)
    Boolean currentDebug = (settings.logDebugEnable == true)
    Boolean currentTrace = (settings.logTraceEnable == true)

    Boolean lastInfo  = (state.lastLogInfoEnable == null)  ? currentInfo  : state.lastLogInfoEnable
    Boolean lastDebug = (state.lastLogDebugEnable == null) ? currentDebug : state.lastLogDebugEnable
    Boolean lastTrace = (state.lastLogTraceEnable == null) ? currentTrace : state.lastLogTraceEnable

    List<String> changes = []

    if (currentInfo != lastInfo)   changes.add("Info Logging changed from ${lastInfo} to ${currentInfo}")
    if (currentDebug != lastDebug) changes.add("Debug Logging changed from ${lastDebug} to ${currentDebug}")
    if (currentTrace != lastTrace) changes.add("Trace Logging changed from ${lastTrace} to ${currentTrace}")

    if (changes) {
        logWarn "Logging configuration changed: ${changes.join(', ')}"
    }
...
void disableDebugLogging() {
    if (settings.logDebugEnable == true) {
