...
/* Detect and log individual WARN messages per altered logging switch */
private void checkLoggingChanges() {
    ...
    if (currentInfo != lastInfo) {
        logWarn "Logging configuration changed: Info Logging changed from ${lastInfo} to ${currentInfo}"
    }
    if (currentDebug != lastDebug) {
        logWarn "Logging configuration changed: Debug Logging changed from ${lastDebug} to ${currentDebug}"
    }
    if (currentTrace != lastTrace) {
        logWarn "Logging configuration changed: Trace Logging changed from ${lastTrace} to ${currentTrace}"
    }
