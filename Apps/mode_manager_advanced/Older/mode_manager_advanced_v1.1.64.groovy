 ...
 *                        - Demoted raw JSON snapshot payload diff trace lines inside updated() to logDebug to minimize log noise during normal trace execution.
 ...
 * v1.1.59 (2026-08-25) - Sleep Intent Calculation & Synchronization Fix:
 *                        - Fixed race condition in calculateDecision() where incoming Alexa sleep transitions evaluated stale physical awake switch states.
 *                        - Enforced authoritative isSleeping intent mapping to prevent syncOutputs from falsely turning awake switches back ON.
 * v1.1.58 (2026-08-25) - Variable & Setting Naming Refactor:
 *                        - Renamed alexaModeSwitch setting, handler, and references to alexaPresenceSwitch to explicitly denote Home/Away presence functionality.
 * v1.1.57 (2026-08-25) - Alexa Integration & State Pipeline Refactor:
 *                        - Decoupled alexaPresenceSwitch (Home/Away presence) from alexaAwakeSwitch (Awake/Sleeping state).
 *                        - Fixed output synchronization bug where Home presence triggers forced alexaPresenceSwitch state during sleep transitions.
 * v1.1.56 (2026-08-25) - State Persistence & Schedule Period Recovery Patch:
 *                        - Added active period key recovery block inside initialize() to store state.activePeriodKey across version upgrades and app updates.
 *                        - Added debounce guard inside alexaAwakeSwitchHandler to prevent state corruption during rapid Alexa routine switch toggles.
 * v1.1.55 (2026-08-24) - Complete Multi-System Logic Patch & Time Engine Refactor:
 *                        - Fixed Mobile Push plain-text %prevReason% fallback variable substitution typo.
 *                        - Rebuilt timeToMinutes() with native 12-Hour AM/PM parsing for full sandbox stability.
 *                        - Added automated duplicate schedule start time detection & warning logging in startPeriodSchedules().
 *                        - Added UI explanatory banners for circular midnight wrap-around and suspendScheduler interactions.
...
static String version() { return '1.1.64' }
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 8: Event Trigger Dispatch Matrix & Message Templates</b>", hideable: true, hidden: true) {
            ...
            switch(selCause) {
                ...
                case "RecheckNoChange": selectedTemplate = settings.templateRecheckNoChange ?: "State recheck completed: Still in %mode% (%reason%) at %time%"; break
            ...
            paragraph "<div style='background-color:#F4F6F7; border-left:4px solid #7F8C8D; padding:6px 10px; border-radius:4px; font-weight:bold; font-size:13px; color:#2C3E50;'>Passive State Alignment (Unchanged State)</div>" +
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-top:2px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>RecheckNoChange (Periodic state evaluation where state/reason are unchanged)</code></div>", width: 6
            ...
            input name: "templateRecheckNoChange", type: "text", title: "<b>Message Text Template*</b>", defaultValue: "State recheck completed: Still in %mode% (%reason%) at %time%", required: true, width: 12
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:4px 0 12px 0;'/>"
...
private Boolean applyDecision(Map decision) {
    ...
    if (modeChanged && reasonChanged) {
        ...
        logTrace "State evaluation completed: Location Mode remains '${newMode}' and Reason remains '${newReason}' | (${decision.source})"
    }
    ...
    if (reasonVal != "Reboot" && !modeChanged && reasonChanged) {
        ...
        switch (reasonVal) {
            ...
            case "Reboot":
                ...
                break
            case "RecheckNoChange":
                template = settings.templateRecheckNoChange ?: "State recheck completed: Still in %mode% (%reason%) at %time%"
                break
            default:         
...
def alexaPresenceSwitchHandler(evt) {
    ...
    logTrace "Alexa Presence Switch changed to '${evt.value}' (Voice Marker Active: ${isVoiceSource} | Reason Authority: '${evalReason}')"
