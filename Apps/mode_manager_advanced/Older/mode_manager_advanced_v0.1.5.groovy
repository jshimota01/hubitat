...
def vSwitchHandler(evt) {
    ...
    if (targetMode) {
        logInfo "Mode recheck triggered by virtual switch '${evt.device}'. Forcing target mode: '${targetMode}'"
...
def presenceHandler(evt) {
    logInfo "Mode recheck triggered by master presence change: '${evt.device}' is now '${evt.value}'"
