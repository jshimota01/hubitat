...
static String version() { return '0.4.17' }
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>STEP 4: Manual Overrides & Triggers</b>") {
            ...
            paragraph "<div style='color:#7F8C8D; font-size:11px; margin-top:-8px; margin-bottom:10px;'>" +
                      "<i><b>Note:</b> Trigger switches should ideally have an auto-off (momentary/auto-revert) setting enabled in their driver so they naturally return to OFF after firing.</i></div>"
...
def updateSwitchHandler(evt) {
    ...
    /* Reset triggering switch back to OFF so it acts like a momentary push-button */
    if (evt.device && evt.device.currentValue("switch") != "off") {
        logTrace "Resetting trigger switch '${evt.device.displayName}' back to OFF"
        sendVSwitchCommand(evt.device, "off")
    }
}

/* External Trigger Button Handler */
def updateButtonHandler(evt) {
    logTrace "Mode recheck triggered via external update button '${evt.device.displayName}' pushed (Button #${evt.value})."
    state.manualOverrideMode = null
    state.manualOverridePeriodKey = null
    state.modeReason = null
    state.modeSource = null
