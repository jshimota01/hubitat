 ...
 *                        - Fixed accordion state initialization to ensure all GUI sections open collapsed by default.
 *                        - Preserved Hubitat native dynamic section toggling while navigating inside mainPage().
 * v1.1.21 (2026-08-22) - Color Palette Synchronization:
 *                        - Aligned getModeColor() and getReasonColor() palettes directly with Virtual Status Tile Driver v1.4.3.
 *                        - Updated Sleeping mode notification color to #00FFFF for improved visibility.
 * v1.1.20 (2026-08-22) - GUI Expansion & Audio Suppression Fixes:
 *                        - Fixed page re-render bug where section expansion state was reset on UI button presses.
 *                        - Ensured btnTrigger fires reliably on the first press.
 *                        - Suppressed audio/speech notifications when btnTrigger is pressed and Mode/Reason remain unchanged.
 * v1.1.19 (2026-08-22) - UI Button Resync Logging & Audio Integrity:
 *                        - Added explicit logInfo message when 'btnTrigger' is pressed but mode and reason remain unchanged.
 *                        - Retained full audio and tile notification dispatch pipeline when state changes occur.
 * v1.1.18 (2026-08-22) - UI Button Event Isolation Guard:
 *                        - Excluded transient UI button keys (btnTrigger, btnForceTestEvaluation, btn*) from captureSettingsSnapshot().
 *                        - Prevents manual UI button presses from triggering false-positive 'Settings modification' rechecks in updated().
 * v1.1.17 (2026-08-22) - Snapshot Key Isolation Guard:
 *                        - Excluded system/metadata keys ('label') from dynamic captureSettingsSnapshot() iteration.
 *                        - Prevents app label/version string updates from causing false-positive preference rechecks during version upgrades.
 * v1.1.16 (2026-08-22) - Dynamic Settings Snapshot Refactor:
 *                        - Replaced static watchKeys list in captureSettingsSnapshot() with dynamic settings map iteration.
 *                        - Guarantees 100% coverage of all GUI input changes (modes, templates, toggles) without manual list maintenance.
 * v1.1.15 (2026-08-22) - Settings Snapshot Key Coverage Fix:
 *                        - Added missing notification template keys (templateScheduled, templateVoice, etc.) to captureSettingsSnapshot() watch list.
 *                        - Ensures text/template modifications trigger proper setting change detection and logging in updated().
 * v1.1.14 (2026-08-22) - Alexa Voice Marker Window Optimization:
 *                        - Tightened alexaVoiceMarkerSwitchHandler() TTL window from 5,000ms down to 2,500ms based on empirical telemetry (<300ms execution latency).
 * v1.1.13 (2026-08-22) - Structural Optimization & Retry Safety Guard:
 *                        - Added bounded retry count (max 3) to verifyHsmSync() to prevent infinite loop recursion when HSM arming is blocked.
 *                        - Refactored timeToMinutes() to use native string/integer math, bypassing heavy Calendar.getInstance() calls.
 *                        - Optimized getActiveTimePeriodInfo() evaluation math for microsecond execution during event cascades.
 *                        - Added early device availability guard at top of dispatchTileNotification() to skip string formatting when no notification targets exist.
 * v1.1.12 (2026-08-22) - Memory & Diagnostic Instrumentation Refactor:
 *                        - Optimized consumeExpectedAck() token string parsing using indexOf()/substring() to avoid array allocation overhead.
 *                        - Enhanced verifyHsmSync() trace log output to include calculated dispatch transit elapsed duration.
 *                        - Updated app definition icon URLs to lightweight clean metadata attributes.
 * v1.1.11 (2026-08-22) - Authoritative Input Echo Suppression Guard:
 *                        - Integrated consumeExpectedAck() into awakeSwitchHandler(), homeSwitchHandler(), and awaySwitchHandler().
 *                        - Prevents syncOutputs() switch mirroring from self-triggering Override pipeline transitions.
 */
...
static String version() { return '1.1.24' }
        ...
        section("<b>SECTION 6: Status Tiles & Notification Alerts</b>", hideable: true, hidden: true) {
            ...
            if (getSettingBool("enableNotifications", true)) {
                paragraph "<div style='background-color:#F8F9FA; border-left:4px solid #8E44AD; padding:10px; border-radius:4px; font-size:12px; margin-bottom:12px;'>" +
                          ...
                          "You can customize your push and speech notification strings using the dynamic placeholders below:<br/><br/>" +
                          ...
                          "    <td style='padding:5px;'>Active Evaluation Reason</td>" +
...
def verifyHsmSync(Map data) {
    ...
    int retryCount = (data?.retryCount as Integer) ?: 0
    int maxRetries = 3
    
    // Obsolete HSM Retry Guard: Exit cleanly if a newer transaction changed the pending target
...
private void processStatePipeline(Map request) {
    ...
    try {
        ...
        // Pipeline Guard 1: Verify decision applied successfully before proceeding to outputs
        ...
        // Pipeline Guard 2: Only sync outputs if state actually changed OR explicit sync was requested
...
private Boolean applyDecision(Map decision) {
    ...
    // Store immutable transaction metadata directly on decision map
    decision.modeChanged = modeChanged
    decision.reasonChanged = reasonChanged
    decision.stateChanged = (modeChanged || reasonChanged)
...
private boolean consumeExpectedAck(def evt) {
    ...
    if (tokenData != null) {
        ...
        long expiresMs = delimIdx > 0 ? tokenData.substring(delimIdx + 1).toLong() : 0L
        long currentMs = now()
...
private int getMinutesFromSetting(String timeIso, int defaultMinutes) {
    ...
    return timeToMinutes(timeIso)
}

private int timeToMinutes(def timeInput) {
