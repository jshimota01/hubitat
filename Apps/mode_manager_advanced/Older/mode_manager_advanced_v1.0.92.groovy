 ...
 * v1.0.91 (2026-08-18) - Snapshot Watch Expansion, Truthful HSM Badge & Transient State Safety:
 *                        - Added vSwitch* and updateTrigger* keys to captureSettingsSnapshot() to force re-initialization on preference change.
 *                        - Unconditionally reset state.isSettingsSaveEvent at top of dispatchTileNotification() to prevent stale speech notifications.
 *                        - Calculated ground-truth HSM target in Section 4 GUI to display true Synchronized vs Desynchronized status.
 *                        - Cleaned up redundant evalReason ternary in alexaModeSwitchHandler().
 *                        - Clarified Schedule Block GUI banner when overridden by Sleeping overlay.
 * v1.0.90 (2026-08-18) - Pipeline Loopback Decoupling, Transient State Safety & Source Debounce:
 *                        - Removed broad state.activeTransaction short-circuit from isInternalTransaction(); relies strictly on atomicState.pendingOutputSyncs.
 *                        - Safe-guarded state.isSettingsSaveEvent lifecycle in dispatchTileNotification() by clearing flag immediately on entry.
 *                        - Implemented per-source debouncing (state.lastTriggerTimes) with automatic pruning (cleanTriggerTimesMap).
 *                        - Preserved complete change log history (v1.0.1 - v1.0.85) and all logTrace/logDebug diagnostic calls.
 * see changelog.txt for v1.0.1 - v1.0.89
 * v1.0.1  (2026-05-20) - Initial Architectural Concept:
 *                        - Initial release of Mode Manager Advanced framework for Hubitat Elevation.
 *
 */

static String version() { return '1.0.92' }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 8: System Startup & Boot Guard</b>", hideable: true, hidden: isCollapsed) {
            ...
            input name: "bootSyncDelay", type: "number", title: "<b>Hub Boot Synchronization Delay (Seconds)</b>", defaultValue: 120, required: true, range: "5..300"
            ...
            String statusBadge = isPending ? 
                "<span style='color:#D35400; font-weight:bold;'>Boot Delay Active (Waiting for Hub Stabilization...)</span>" : 
                "<span style='color:#27AE60; font-weight:bold;'>Idle / Synchronized</span>"

            paragraph "<div style='background-color:#F8F9FA; border-left:4px solid #2980B9; padding:8px; border-radius:4px; font-size:12px; margin-top:6px;'>" +
...
private String captureSettingsSnapshot() {
    List<String> watchKeys = [
        ...
        "updateTriggerSwitch", "updateTriggerButton", "bootSyncDelay",
...
def delayedStartupHandler() {
    ...
    logInfo "Boot stabilization window elapsed at ${formattedSyncTime}. Executing initial system-wide ground-truth alignment..."
    logTrace "CALL PATH: delayedStartupHandler() -> dispatching processStatePipeline(Reason: Reboot)"
    
    processStatePipeline([reason: "Reboot", source: "Hub System Startup (Post-Boot Delay)"])
}
