 ...
 *  Change History:
 *
 *  v1.0.31 (2026-08-14) - Complete Event Isolation & Scheduler Architecture Overhaul:
 *                         - Decoupled CRON scheduler management from applyDecision() into a state-transition model.
 *                         - Enforced strict String coercion for raw device IDs in syncSwitch() and isInternalTransaction().
...
static String version() { return '1.0.31' }
...
def stopPeriodSchedules() {
    logTrace "Unscheduling all daily time period triggers..."
...
def sleepSwitchHandler(evt) {
    if (isInternalTransaction(evt)) return
    logInfo "Sleep switch '${evt.device.displayName}' toggled externally to '${evt.value}'."
    
    /* Mirror manual toggle directly to primary authoritative input (Awake Switch) */
    String targetAwakeState = (evt.value == "on") ? "off" : "on"
    if (awakeSwitch && awakeSwitch.currentValue("switch") != targetAwakeState) {
        logTrace "Mirroring Sleep Switch change -> setting Awake Switch '${awakeSwitch.displayName}' to ${targetAwakeState.toUpperCase()}"
