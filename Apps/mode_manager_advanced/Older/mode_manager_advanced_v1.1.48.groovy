 ...
 * Change History:
 *
 * v1.1.48 (2026-08-24) - Hubitat Sandbox Compatibility Fix:
 *                        - Removed forbidden java.util.concurrent.atomic.AtomicLong import.
 *                        - Implemented thread-safe synchronized static helper method (getNextTxId) for sub-millisecond TxId generation.
 * v1.1.47 (2026-08-24) - Intent-Based Voice Routing & Attribution Refactor:
 *                        - Separated command authority (Rank) from trigger attribution (Source) for voice commands.
 *                        - Validated voice marker events now execute with Override authority (Rank 4) to win arbitration over Presence (Rank 3).
 *                        - Attached transient isVoiceAttributed flag to preserve 'Voice' as the authoritative state.modeReason and Section 8 notification trigger.
 * v1.1.46 (2026-08-24) - Sub-Millisecond TxId Uniqueness Refactor:
 *                        - Added AtomicLong txCounter to append an atomic incrementing sequence to all txId strings (tx_<now>_<seq>).
 *                        - Eliminated same-millisecond txId collision risk across rapid async execution threads.
 * v1.1.45 (2026-08-24) - GUI De-Cluttering & Dynamic Template Preview Refactor:
 *                        - Refactored Section 3 virtual switch labels to "Period Indicator / Override Switch" with explanatory context notes.
 *                        - Added clean matrix column header row (EVENT TRIGGER | PUSH | PUSH MOBILE | AUDIO) in Section 8.
 *                        - Removed redundant per-row toggle switch titles ("Push?", "Push Mobile?", "Audio/Speech?") across all 8 trigger cards.
 *                        - Upgraded Live Template Preview card with an interactive trigger cause selector dropdown.
 * v1.1.44 (2026-08-24) - In-Flight Transaction & Ack Register Lifecycle Protection:
 *                        - Protected pending Ack tokens (pendingAckTokens) and in-flight HSM state (pendingHsmTarget) from being wiped during initialize() 
 * 						  - \execution on preference edits.
 *                        - Isolated token purging and HSM state clears strictly to true lifecycle resets (installed, uninstalled, hub startup).
 * v1.1.43 (2026-08-24) - Settings Save Event State-Leak Bug Fix:
 *                        - Removed persistent state.isSettingsSaveEvent variable to prevent stale flag leakage into future events.
 *                        - Converted settings save notifications into transient pipeline parameters (isSaveEvent) in decision map.
 *                        - Forced syncOutputs execution on settings save to guarantee immediate flag consumption without lingering in state.
...
private static String getNextTxId() {
    synchronized(txCounterLock) {
        txCounter++
        return "tx_${now()}_${txCounter}"
    }
}

static String version() { return '1.1.48' }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 8: Event Trigger Dispatch Matrix & Message Templates</b>", hideable: true, hidden: true) {
            ...
            // --- MATRIX HEADER ROW ---
            paragraph "<div style='display:flex; background-color:#2C3E50; color:#FFFFFF; padding:6px 10px; font-weight:bold; border-radius:4px; font-size:12px; margin-bottom:8px;'>" +
                      "  <div style='flex: 6;'>EVENT TRIGGER CAUSE</div>" +
                      "  <div style='flex: 2; text-align:center;'>PUSH</div>" +
                      "  <div style='flex: 2; text-align:center;'>PUSH MOBILE</div>" +
                      "  <div style='flex: 2; text-align:center;'>AUDIO / SPEECH</div>" +
                      "</div>"
