 ...
 *                        - Removed 'hidden: true' parameter from STEP 4 section definition to prevent the Simulation Control Panel from recollapsing on submitOnChange dropdown selections.
 ...
 *                        - Removed vSwitchSleeping from STEP 3 and periodSwitchList mapping table.
 *                        - Added self-defense guard in vSwitchHandler to ignore sleep/awake indicator events.
 *                        - Clarified separation between Behavioral Sleep State (Step 2) and Chronological Schedules (Step 3).
...
static String version() { return '1.0.38' }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        /* Step 2: Sleep Architecture (Behavioral State) */
        ...
        /* Step 4: Overrides and Diagnostics (Hideable allowed, but hidden: true removed to prevent auto-collapsing on submitOnChange) */
        section("<b>STEP 4: Manual Overrides and Diagnostics</b>", hideable: true) {
            paragraph "<div style='background-color:#EBF5FB; border-left:4px solid #3498DB; padding:8px; border-radius:4px; font-size:12px; margin-bottom:10px;'>" +
...
def initialize() {
    ...
    /* Subscribing to Hubitat System Startup Event */
...
/* Hub Reboot Handler: Self-Heals and Dispatches Reboot Alert */
...
private void dispatchTileNotification(String modeVal, String reasonVal) {
    ...
    if (state.lastNotifiedMode == modeVal && reasonVal != "Reboot") {
...
def vSwitchHandler(evt) {
    ...
    if ((sleepSwitch && "${sleepSwitch.id}".toString() == deviceId) || (awakeSwitch && "${awakeSwitch.id}".toString() == deviceId)) {
        logTrace "vSwitchHandler: Ignored event from '${evt.device?.displayName}' because it is assigned to Step 2 Sleep Architecture."
        return
    }
