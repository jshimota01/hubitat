...
static String version() { return '0.4.11' }
...
def vSwitchHandler(evt) {
    ...
    if (targetPeriod?.mode) {
        /* Guard against overwriting an active Voice Commanded Sleep transition */
        if (targetPeriod.key == "sleeping" && state.modeSource == "VOICE" && location.mode == targetPeriod.mode) {
            logTrace "Switch '${evt.device.displayName}' turned ON while already in voice-commanded Sleeping mode. Preserving VOICE mode source."
            return
        }
...
def changeMode(String newMode, String activePeriodKey = null, String reason = null) {
    ...
    if (reason) {
        if (state.modeReason != reason) {
            logInfo "Mode Reason updated: '${state.modeReason ?: 'None'}' -> '${reason}'"
