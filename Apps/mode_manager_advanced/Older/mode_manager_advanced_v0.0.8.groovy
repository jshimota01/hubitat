...
def vSwitchHandler(evt) {
    ...
    Map<Object, String> vSwitchToModeMap = [:]
    if (vSwitchSleeping)     vSwitchToModeMap[vSwitchSleeping.id]     = sleepMode
    if (vSwitchWeeHours)     vSwitchToModeMap[vSwitchWeeHours.id]     = weeHoursMode
    if (vSwitchEarlyMorning) vSwitchToModeMap[vSwitchEarlyMorning.id] = earlyMorningMode
    if (vSwitchMorning)      vSwitchToModeMap[vSwitchMorning.id]      = morningMode
    if (vSwitchDay)          vSwitchToModeMap[vSwitchDay.id]          = dayMode
    if (vSwitchEvening)      vSwitchToModeMap[vSwitchEvening.id]      = eveningMode
    if (vSwitchLateEvening)  vSwitchToModeMap[vSwitchLateEvening.id]  = lateEveningMode

    String targetMode = vSwitchToModeMap[evt.deviceId]
    
    if (targetMode) {
        logDebug "Manual switch override selected mode: '${targetMode}'"
        
        // If turning on a daytime mode switch while sleep switch is ON, toggle awake switch ON to sync state
        if (evt.deviceId != vSwitchSleeping?.id && sleepSwitch?.currentValue("switch") == "on") {
