 ...
 * Change History:
 *
 * v1.1.41 (2026-08-24) - Architectural Bug Fixes & Thread Safety Refactor:
 *                        - Timezone-safe CRON generator in toCronExpression() using explicit timeZone formatting.
 *                        - Migrated isTriggerDebounced() to a thread-safe ConcurrentHashMap (lastTriggerTimes) to prevent atomicState map race conditions.
 *                        - Hardened timeToMinutes() ISO string parser using toDateTime() to prevent NumberFormatException.
 *                        - Updated Section 7 & 8 titles and added a Live Notification Template Preview card in Section 8.
 *                        - Preserved existing voice marker consumption behavior.
 * v1.1.40 (2026-08-24) - UI De-Clutter & Section Architectural Split:
 *                        - Split former Section 7 into Section 7 (Notification Output Controls & Matrix) and Section 8 (Notification Message Templates & Placeholders).
 *                        - Renumbered Diagnostics to Section 9 and Startup Guard to Section 10.
 *                        - Reworded master notification toggle to "Enable All Notification Outputs" for clear ON/OFF semantics.
 *                        - Renamed sub-header to "Enabled Notification Types (Master Triggers)".
 *                        - Cleaned up preference DOM structure by removing raw HTML div wrappers around Hubitat preference input elements.
 * v1.1.39 (2026-08-24) - Behavioral Sleep State Correctness Fix:
 *                        - Discarded mode-name string comparison (targetMode == configuredSleepMode) in calculateDecision().
 *                        - Preserved direct Awake Switch behavioral state (isSleeping) throughout the decision pipeline.
 *                        - Prevented false sleep overlays, improper audio muting, and unintended HSM Night arming when daytime modes match the sleep mode name.
 * v1.1.38 (2026-08-24) - Notification Matrix Expansion & Placeholder Table Card Styling:
 *                        - Expanded Section 7 trigger rows to support 3 distinct switches per trigger: Push, Push Mobile, and Audio/Speech.
 *                        - Updated dispatchTileNotification engine to track push<Trigger>, pushMobile<Trigger>, and audio<Trigger>.
 *                        - Refactored the Notification Template Placeholders table with a narrower centered container card and subtle box-shadow styling.
 * v1.1.37 (2026-08-24) - UI Collapse & Section State Retention Fix:
 *                        - Fixed section collapse bug triggered when toggling switches in Section 7.
 *                        - Standardized section rendering to prevent full page redraws from collapsing hideable sections.
 *                        - Ensured notification toggles preserve open section state upon user interaction.
 * v1.1.36 (2026-08-24) - UI Visual Hierarchy Refactor:
 *                        - Indented the 3 master notification switches (Push, Push Mobile, Audio) using a styled container box.
 *                        - Clarified structural hierarchy under the primary 'Disable/Enable All Notification Outputs' master toggle.
 * v1.1.35 (2026-08-24) - Dual Push & Master Output Toggles Refactor:
 *                        - Split Push inputs into standard Push Devices and Push (Mobile) Devices with independent text formatters.
 *                        - Added 3 master output switches for Push, Push Mobile, and Audio types.
 *                        - Updated section header text to 'Notification Preferences (Push/Mobile/Audio)'.
 *                        - Updated master toggle label to 'Disable/Enable All Notification Outputs'.
 * v1.1.34 (2026-08-24) - Trigger Switch Refactor:
 *                        - Replaced dropdown/radio controls with Hubitat native boolean toggle switches (type: "bool").
 *                        - Maintained side-by-side grid layout for Push and Audio toggles on top row next to trigger title.
 *                        - Updated notification dispatch handler to evaluate boolean switch inputs directly.
 * v1.1.33 (2026-08-24) - UI Grid & Version Alignment Refactor:
 *                        - Updated Section 6 subheader to "Notification Target Device(s) Selection".
 *                        - Restored native Hubitat radio button controls (style: "radio") for discrete On/Off selection.
 *                        - Moved Push and Audio radio selectors up to align horizontally with the trigger title row (Width 3 each).
 *                        - Expanded message template text input fields to full width on their own row below (Width 12).
 * v1.1.32 (2026-08-24) - Section Architecture Refactor:
 *                        - Split former Section 6 into Section 6 (Status Tile(s) and Output Devices) and Section 7 (Notification Matrix & Alerts Engine).
 *                        - Relocated notification preferences to top of Section 7 and Placeholders table above Matrix card grid.
 *                        - Renumbered Diagnostics (Section 8) and Startup Guard (Section 9).
 * v1.1.31 (2026-08-24) - Trigger Cause Help Integration:
 *                        - Integrated trigger cause (%reason%) reference notes inline into each corresponding row card in Section 7.
 *                        - Restructured matrix cards into cohesive informational components per event cause.
 * v1.1.30 (2026-08-24) - Section Control Layout Facelift:
 ...
 */
// [KEEP-EXACT] see changelog.txt for v1.0.1 - v1.1.29

import java.util.concurrent.ConcurrentHashMap
import groovy.transform.Field

// Thread-safe heap registers
...
static String version() { return '1.1.41' }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 7: Event Notification Dispatch Matrix</b>", hideable: true, hidden: true) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
                      "<b>Notification Alerts Engine:</b> Configure master outputs and discrete trigger event toggles for Push, Mobile, and Audio dispatch.</div>"

            // --- SECTION PREFERENCES & GLOBAL CONTROLS ---
            paragraph "<span style='color:#2C3E50; font-weight:bold; font-size:13px;'>Notification Preferences</span>"
            ...
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:12px 0 8px 0;'/>" +
                      "<span style='color:#2C3E50; font-weight:bold; font-size:13px;'>Event Trigger Dispatch Matrix</span>"
            ...
            paragraph "<div style='background-color:#F4F6F7; border-left:4px solid #2980B9; padding:6px 10px; margin-bottom:2px; border-radius:4px; font-weight:bold; font-size:13px; color:#2C3E50;'>Scheduled Transitions</div>" +
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>Scheduled</code> (Chronological time period boundaries)</div>", width: 6
            input name: "pushScheduled", type: "bool", title: "<b>Push?</b>", defaultValue: false, width: 2
            input name: "pushMobileScheduled", type: "bool", title: "<b>Push Mobile?</b>", defaultValue: false, width: 2
            input name: "audioScheduled", type: "bool", title: "<b>Audio/Speech?</b>", defaultValue: false, width: 2
            ...
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      ...
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>Voice</code> (Alexa Voice Routine verified via Voice Marker)</div>", width: 6
            input name: "pushVoice", type: "bool", title: "<b>Push?</b>", defaultValue: false, width: 2
            input name: "pushMobileVoice", type: "bool", title: "<b>Push Mobile?</b>", defaultValue: false, width: 2
            input name: "audioVoice", type: "bool", title: "<b>Audio/Speech?</b>", defaultValue: false, width: 2
            ...
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      ...
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>Presence</code> (Master Presence Sensor or Home/Away toggles)</div>", width: 6
            input name: "pushPresence", type: "bool", title: "<b>Push?</b>", defaultValue: false, width: 2
            input name: "pushMobilePresence", type: "bool", title: "<b>Push Mobile?</b>", defaultValue: false, width: 2
            input name: "audioPresence", type: "bool", title: "<b>Audio/Speech?</b>", defaultValue: false, width: 2
            ...
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      ...
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>Override</code> (Dashboard switch toggle or Awake switch override)</div>", width: 6
            input name: "pushOverride", type: "bool", title: "<b>Push?</b>", defaultValue: false, width: 2
            input name: "pushMobileOverride", type: "bool", title: "<b>Push Mobile?</b>", defaultValue: false, width: 2
            input name: "audioOverride", type: "bool", title: "<b>Audio/Speech?</b>", defaultValue: false, width: 2
            ...
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      ...
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>Reboot</code> (Hub startup/reboot system alignment check)</div>", width: 6
            input name: "pushReboot", type: "bool", title: "<b>Push?</b>", defaultValue: false, width: 2
            input name: "pushMobileReboot", type: "bool", title: "<b>Push Mobile?</b>", defaultValue: false, width: 2
            input name: "audioReboot", type: "bool", title: "<b>Audio/Speech?</b>", defaultValue: false, width: 2
            ...
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      ...
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>RecheckNoChange</code> (Periodic recheck where mode/reason are unchanged)</div>", width: 6
            input name: "pushRecheckNoChange", type: "bool", title: "<b>Push?</b>", defaultValue: false, width: 2
            input name: "pushMobileRecheckNoChange", type: "bool", title: "<b>Push Mobile?</b>", defaultValue: false, width: 2
            input name: "audioRecheckNoChange", type: "bool", title: "<b>Audio/Speech?</b>", defaultValue: false, width: 2
            ...
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      ...
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> Dynamic evaluation cause updated while Mode remains identical</div>", width: 6
            input name: "pushReasonOnly", type: "bool", title: "<b>Push?</b>", defaultValue: false, width: 2
            input name: "pushMobileReasonOnly", type: "bool", title: "<b>Push Mobile?</b>", defaultValue: false, width: 2
            input name: "audioReasonOnly", type: "bool", title: "<b>Audio/Speech?</b>", defaultValue: false, width: 2
            ...
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      ...
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> Location Mode changed under an ongoing trigger cause</div>", width: 6
            input name: "pushModeOnly", type: "bool", title: "<b>Push?</b>", defaultValue: false, width: 2
            input name: "pushMobileModeOnly", type: "bool", title: "<b>Push Mobile?</b>", defaultValue: false, width: 2
            input name: "audioModeOnly", type: "bool", title: "<b>Audio/Speech?</b>", defaultValue: false, width: 2
        }

        section("<b>SECTION 8: Message Templates & Dynamic Placeholders</b>", hideable: true, hidden: true) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
                      "<b>Message Format Strings:</b> Define message output text strings per event trigger cause.</div>"
            ...
            String sampleReason = state.modeReason ?: "Scheduled"
            String sampleTpl = settings.templateScheduled ?: "Mode scheduled transition: %mode% (from %prevMode% at %time%)"
            String previewStr = sampleTpl
            ...
            paragraph "<div style='background-color:#F4F6F7; border-left:4px solid #16A085; padding:8px 12px; border-radius:4px; font-size:12px; margin-bottom:12px;'>" +
                      "🔍 <b>Live Preview (Scheduled Example):</b> <i>\"${previewStr}\"</i></div>"

            input name: "templateScheduled", type: "text", title: "<b>Scheduled Message*</b>", defaultValue: "Mode scheduled transition: %mode% (from %prevMode% at %time%)", required: true, width: 12
            input name: "templateVoice", type: "text", title: "<b>Voice Trigger Message*</b>", defaultValue: "Voice command changed mode to %mode% (Reason: %reason% at %time%)", required: true, width: 12
            input name: "templatePresence", type: "text", title: "<b>Presence Change Message*</b>", defaultValue: "Presence update changed mode to %mode% (was %prevMode% at %time%)", required: true, width: 12
            input name: "templateOverride", type: "text", title: "<b>Manual Override Message*</b>", defaultValue: "Manual Override activated: Mode set to %mode% (Reason: %reason% at %time%)", required: true, width: 12
            input name: "templateReboot", type: "text", title: "<b>Hub Reboot Message*</b>", defaultValue: "System restarted following hub boot. Mode synchronized to %mode% (Reason: %reason% at %time%)", required: true, width: 12
            input name: "templateRecheckNoChange", type: "text", title: "<b>Recheck Message*</b>", defaultValue: "Mode recheck completed: Still in %mode% (%reason%) at %time%", required: true, width: 12
            input name: "templateReasonOnly", type: "text", title: "<b>Reason Only Message</b>", defaultValue: "Evaluation Reason updated to %reason% (Mode remains %mode% at %time%)", required: false, width: 12
            input name: "templateModeOnly", type: "text", title: "<b>Mode Only Message</b>", defaultValue: "Mode changed to %mode% (Reason remains %reason% at %time%)", required: false, width: 12
        }
...
private Boolean isTriggerDebounced(String sourceKey, long windowMs = 2000L) {
    ...
    // Housekeeping: remove expired trigger timestamps (> 10s old)
...
private void dispatchTileNotification(Map decision, String overrideReasonVal = null, Boolean isSleepingState = false) {
    ...
    Boolean allowStandardPush = masterPushAllowed && (triggerPushEnabled || isButtonTrigger)
    Boolean allowMobilePush = masterPushMobileAllowed && (triggerPushMobileEnabled || isButtonTrigger)
    Boolean allowAudio = masterAudioAllowed && (triggerAudioEnabled || isButtonTrigger)

    if (!allowStandardPush && !allowMobilePush && !allowAudio && !isButtonTrigger) {
    ...
    if (notificationDeviceMobile && allowMobilePush) {
        ...
        String formattedPrevReasonM = useHtmlPushMobile ? "<span style='color:${getReasonColor(rawPrevReason)}; font-weight:bold;'>${rawPrevReason}</span>" : rawPrevReason

        String pushMsgMobile = template
