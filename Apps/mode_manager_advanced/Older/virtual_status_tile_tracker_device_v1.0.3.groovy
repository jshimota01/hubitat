 ...
 *                        - Added internal device event subscriptions to activeMode, activeReason, and lastTransitionTime.
 *                        - Updated custom setters to explicitly invoke updateTileDisplay() on every state change.
 *                        - Added parse() handler to catch attribute updates pushed via standard device capabilities.
