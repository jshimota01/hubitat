 ...
 *  Change History:
 *
 *  v1.0.23 (2026-08-13) - Pipeline Sync Isolation & HSM De-duplication:
 *                         - Fixed Voice mode state retention when secondary sleep/awake switches fire back-to-back events.
 *                         - Prevented virtual period switches from triggering secondary rechecks when updated internally by syncOutputs().
 *                         - Locked state.lastHsmState prior to sendLocationEvent to eliminate duplicate HSM log chatter.
 *  v1.0.22 (2026-08-13) - Step Consolidation: Merged Step 6 into Step 4, updated simulation options.
 *  v1.0.21 (2026-08-13) - Logic Audit: Immediate label updates on updated(), sanitized badge formatting.
    ...
    if (requestedReason == "Override") {
        ...
        if (!isHome) {
            ...
            if (isSleeping) {
                targetMode = sleepMode?.toString() ?: "Sleeping"
                activePeriodKey = "sleeping"
                /* Retain active Voice/Override reason if sleep switch was toggling as part of a voice command */
                if (state.modeReason == "Voice" || state.modeReason == "Override") {
                    requestedReason = state.modeReason
                }
            } else {
                Map activePeriod = getActiveTimePeriodInfo()
                targetMode = activePeriod?.mode ?: location.mode
