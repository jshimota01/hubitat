...
static String version() { return '1.1.02' }
...
def hsmStatusHandler(evt) {
    ...
    logDebug "HSM Event Received -> Device/Source: '${evt.displayName}' | Value: '${newHsmStatus}' | IsStateChange: ${evt.isStateChange()}"
...
private void updateStatusTileDevice(String modeVal, String reasonVal) {
    ...
    logTrace "updateStatusTileDevice -> Updating '${statusTileDevice.displayName}' with Mode: '${modeVal}', Reason: '${reasonVal}', Time: '${timeStr}'"

    try {
        if (statusTileDevice.hasCommand("setStatus")) {
            logTrace "Calling setStatus('${modeVal}', '${reasonVal}', '${timeStr}') on tile device..."
...
private void syncSwitch(def device, String targetState) {
    if (device != null && device.hasCommand(targetState)) {
        if (device.currentValue("switch") != targetState) {
            ...
            logTrace "Syncing Output Device '${device.displayName}' [ID: ${devId} | Key: ${compositeKey}] -> ${targetState.toUpperCase()}"
            device."${targetState}"()
        }
    }
}
