...
static String version() { return '0.9.10' }
...
private boolean isReservationValid(String mapType, String devId, String value) {
    ...
    /* Consume reservation match regardless of map type if ID exists */
    map.remove(devId)
    if (mapType == "VSwitch") state.pendingVSwitchSync = map
    else state.pendingMirrorSync = map
    return true
...
def vSwitchHandler(evt) {
    ...
    if (isReservationValid("VSwitch", deviceId, evt.value) || isReservationValid("Mirror", deviceId, evt.value)) {
