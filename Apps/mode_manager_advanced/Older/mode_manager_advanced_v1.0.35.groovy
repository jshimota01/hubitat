 ...
 *                         - Added change-detection guard (state.lastNotifiedMode) in dispatchTileNotification().
 *                         - Prevents redundant notifications when opening app and clicking Done or rechecking schedule without mode changes.
 *  v1.0.34 (2026-08-14) - Dedicated Message Templates & UI Refactor:
 *                         - Defaulted master notification toggle (enableNotifications) to true.
 *                         - Added dedicated message templates with sensible defaults for each of the 4 trigger types (Normal, Voice, Presence, Override).
 *                         - Updated dispatchTileNotification() to resolve reason-specific templates dynamically.
...
static String version() { return '1.0.35' }
...
/* Notification Engine with Change-Detection Watch, Reason Filtering, Dynamic Templates, and Speech Output */
private void dispatchTileNotification(String modeVal, String reasonVal) {
    if (!getSettingBool("enableNotifications", true)) return

    /* 1. Change-Detection Notification Watch Guard */
    if (state.lastNotifiedMode == modeVal) {
        logTrace "Notification suppressed: Mode '${modeVal}' has not changed since last notification dispatch."
        return
    }

    /* 2. Resolve Reason-Specific Trigger & Template */
    ...
    /* 3. Build Dynamic Variable Message */
    ...
    /* Store state values for subsequent checks */
    ...
    /* 5. Send Speech Notifications (checking Quiet Hours) */
    if (speechDevice) {
