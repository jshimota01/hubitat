...
static String version() { return '1.1.29' }
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 6: Status Tiles & Notification Alerts</b>", hideable: true, hidden: true) {
            ...
            if (getSettingBool("enableNotifications", true)) {
                paragraph "<div style='background-color:#F8F9FA; border-left:4px solid #8E44AD; padding:10px; border-radius:4px; font-size:12px; margin-bottom:12px;'>" +
...
private void dispatchTileNotification(Map decision, String overrideReasonVal = null, Boolean isSleepingState = false) {
    ...
    // Determine default enablement flag based on the active evaluation reason
    Boolean categoryEnabled = false
    switch (reasonVal) {
        case "Scheduled":       categoryEnabled = getSettingBool("notifyOnScheduled", false); break
        case "Voice":           categoryEnabled = getSettingBool("notifyOnVoice", true); break
        case "Presence":        categoryEnabled = getSettingBool("notifyOnPresence", true); break
        case "Override":        categoryEnabled = getSettingBool("notifyOnOverride", true); break
        case "Reboot":          categoryEnabled = getSettingBool("notifyOnReboot", true); break
        case "RecheckNoChange": categoryEnabled = getSettingBool("notifyOnRecheckNoChange", false); break
        default:                categoryEnabled = true; break
    }

    // BRANCH 1: REASON CHANGED ONLY (Mode Unchanged)
    if (reasonVal != "Reboot" && !modeChanged && reasonChanged) {
        // Notify if category is enabled OR if transitioning off an Override
        shouldNotify = categoryEnabled || getSettingBool("notifyOnOverride", true)
    ...
    if (!shouldNotify && !isButtonTrigger) {
        logTrace "Notification suppressed: Reason '${reasonVal}' (ModeChanged: ${modeChanged}, ReasonChanged: ${reasonChanged}) disabled in preferences."
        return
    }
    ...
    if (notificationDevice && shouldNotify) {
    ...
    if (speechDevice) {
        ...
        } else if (shouldNotify || modeChanged || reasonChanged) {
