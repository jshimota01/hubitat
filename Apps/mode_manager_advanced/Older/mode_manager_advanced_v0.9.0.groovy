 ...
 *      2026-08-12    jshimota      0.8.8       Added Testing & Diagnostics section driven by Home, Awake, and Period controls; added Alexa switch sync and Presence device warning notice
 *      2026-08-12    jshimota      0.8.9       Restored dynamic app.label in logging header with immediate badge sync; protected period boundary transitions from false Override hijacking
...
def initialize() {
    ...
    state.isPeriodTransitioning = false
...
def periodHandler() {
    ...
    /* Set transient flag to block period vSwitches from triggering Override */
    state.isPeriodTransitioning = true
    processReasonTransition("Normal")
    runIn(2, clearPeriodTransitionFlag)
}

void clearPeriodTransitionFlag() {
    state.isPeriodTransitioning = false
}
...
def executeForcedTestEvaluation() {
    ...
    /* Block vSwitchHandler from intercepting forced virtual indicator updates as Overrides */
    state.isPeriodTransitioning = true

    /* 1. Set explicit reason in state and lock dynamic label */
    state.modeReason = targetReason
    updateAppLabel(null, targetReason)

    /* 2. Force Home / Away switches and sync Alexa Mode Switch */
    String homeCmd = isHomeTarget ? "on" : "off"
    syncHomeState(homeCmd)

    /* 3. Force Awake / Sleep switches and sync Alexa Awake Switch */
    ...
    /* 4. Determine Target Location Mode based on forced state parameters */
    ...
    /* 6. Sync Virtual Indicator Switches and App Label */
    updateVirtualModeSwitches(periodKey)
    updateAppLabel(targetMode, targetReason)

    /* Release transition shield after switch events settle */
    runIn(2, clearPeriodTransitionFlag)
}
...
def vSwitchHandler(evt) {
    ...
    if (state.isPeriodTransitioning) {
        logTrace "Ignoring period VSwitch '${evt.device.displayName}' event generated during active scheduled period transition."
        return
    }
