...
static String version() { return '1.0.73' }
    ...
    if (targetMode == "Away") {
        ...
        if (getSettingBool("manageHSM", true)) {
            ...
            if (currentHsm != expectedHsmStatus) {
                logInfo "HSM desynchronization detected. Current: '${currentHsm ?: 'unknown'}' | Target: '${expectedHsmStatus}'"
                logDebug "Executing HSM state correction command -> armAway (Reason: ${reason})"
        ...
        if (getSettingBool("manageHSM", true)) {
            ...
            if (currentHsm != expectedHsmStatus) {
                logInfo "HSM desynchronization detected. Current: '${currentHsm ?: 'unknown'}' | Target: '${expectedHsmStatus}'"
                logDebug "Executing HSM state correction command -> ${hsmCmd} (Reason: ${reason})"
                state.lastHsmState = expectedHsmStatus
