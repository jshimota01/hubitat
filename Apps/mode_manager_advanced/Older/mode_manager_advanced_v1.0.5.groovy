 ...
 *                        - Added explicit trace/debug logs across installed(), updated(), and initialize() for sub/unsub and CRON actions.
 *                        - Updated updated() to report trigger source as 'App Preferences Updated' instead of 'Initialization'.
 *  v1.0.4 (2026-08-13) - UI Refinement:
 *                        - Removed standalone section boolean toggle switches.
 *                        - Converted section headers into native clickable elements with dynamic carets ('▶' collapsed / '▼' expanded).
 *  v1.0.3 (2026-08-13) - UI Enhancement:
 *                        - Dynamic collapsible section caret titles.
 *  v1.0.2 (2026-08-13) - Bug Fixes & Code Audit:
 *                        - Replaced scheduled runIn(1, "clearActiveTransaction") with synchronous try-finally transaction release.
 *                        - Added null/capability checks in syncSwitch() to prevent NullPointerExceptions on unconfigured optional switches.
 *                        - Fixed Step 6 Test Harness forced reason override inside calculateDecision().
...
def initialize() {
    ...
    if (state.modeReason == "Override" && settings.autoReturnNormal == false) {
        logTrace "System is in Override with autoReturnNormal=OFF. CRON schedules kept disabled."
        stopPeriodSchedules()
    } else {
...
private Map calculateDecision(Map request) {
    ...
    if (requestedReason == "Override" || requestedReason == "Test Mode") {
