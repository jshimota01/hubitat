...
def periodHandler() {
    ...
    logInfo "Period boundary hit. Resuming Normal schedule mode."
    processReasonTransition("Normal")
}
...
def processReasonTransition(String reason, Map params = [:]) {
    ...
    switch (reason) {
        case "Override":
            /* Manage period schedules based on autoReturnNormal user setting */
            ...
            /* Home/Away and Alexa Home/Away change to and stay in Home state */
            syncHomeState("on")
            
            /* Sleep/Awake and Alexa Sleep/Awake change to Awake and stay in awake state */
            syncAwakeState("on")
            
            /* Change location mode if specified by target period */
            String targetMode = params.targetMode
            String periodKey = params.periodKey
            if (targetMode) {
                changeMode(targetMode, periodKey, reason)
            } else {
                updateAppLabel(location.mode, reason)
            }
            break
        ...
        case "Owntracks":
            ...
            if (presenceValue == "not present") {
                ...
                /* Home/Away and Alexa Home/Away change to away */
                syncHomeState("off")

                /* Set Location Mode to Away */
                changeMode("Away", null, reason)
            } else {
                /* Mode became Home -> Start scheduling */
                reschedulePeriods()
                
                /* Home/Away and Alexa Home/Away change to Home */
                syncHomeState("on")
        ...
        case "Normal - Rechecked":
            ...
            /* Alexa Home/Away set to Home, Home/Away set to Home */
            syncHomeState("on")
        ...
        case "Normal":
            ...
            /* Check and change if needed Alexa Home/Away and Home/Away set to Home */
            syncHomeState("on")
            
            /* Check and change if needed Alexa Awake/Sleep and Awake/Sleep set to Awake */
            syncAwakeState("on")
...
def vSwitchHandler(evt) {
    ...
    /* Consume pending internal VSwitch reservation if present */
...
def alexaAwakeSwitchHandler(evt) {
    ...
    String targetMode = (evt.value == "off") ? sleepMode?.toString() : (getActiveTimePeriodInfo()?.mode ?: location.mode)
    processReasonTransition("Voice", [targetMode: targetMode])
}
