...
static String version() { return '1.1.28' }
...
def verifyHsmSync(Map data) {
    ...
    if (state.pendingHsmTarget != null && state.pendingHsmTarget != expected) {
        logDebug "verifyHsmSync: Superseded timer ignored. Pending target moved to '${state.pendingHsmTarget}' (was '${expected}')."
        return
    }
...
private boolean consumeVoiceMarker() {
    ...
    if (expires > 0L && currentMs <= expires) {
        logInfo "Alexa Voice Source Marker verified successfully (${expires - currentMs}ms remaining in routine burst window)."
        return true
    } else if (expires > 0L) {
...
def alexaAwakeSwitchHandler(evt) {
    ...
    logInfo "Alexa Awake Switch changed to '${evt.value}' (Voice Marker Active: ${isVoiceSource} | Attributed Reason: '${evalReason}')"
