...
static String version() { return '1.0.49' }
...
private String captureSettingsSnapshot() {
    List<String> watchKeys = [
        ...
        "alexaModeSwitch", "alexaAwakeSwitch"
    ]
...
private void syncOutputs(Map decision) {
    ...
    if (modeChanged || reason == "Reboot") {
        dispatchTileNotification(targetMode, reason, isSleeping)
    } else if (decision.isRecheck == true) {
        dispatchTileNotification(targetMode, "RecheckNoChange", isSleeping)
    } else {
        dispatchTileNotification(targetMode, reason, isSleeping)
    }
}
...
private void dispatchTileNotification(String modeVal, String reasonVal, Boolean isSleepingState = false) {
