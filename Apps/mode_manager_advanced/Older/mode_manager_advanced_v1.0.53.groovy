 ...
 *                        - Implemented captureSettingsSnapshot() to generate a hash of critical configuration settings.
 *                        - Refined updated() to compare current settings against previous snapshot.
 *                        - Closed via Done without settings changes now bypasses recheckSchedule() and suppresses ghost notifications.
 * v1.0.47 (2026-08-15) - Behavioral Audio Suppression & State Clean-Up Sweep:
 *                        - Decoupled dispatchTileNotification audio suppression from target mode strings, using explicit behavioral pipeline flags.
 *                        - Added TTL cleanup pass in syncSwitch() to clear expired suppression markers and prevent state map bloat.
 *                        - Ensured Simulation Engine forceReleaseLock cleanly overrides active holdOverride settings across all test scenarios.
 * v1.0.46 (2026-08-15) - GUI Architecture Description Update:
 *                        - Updated Section 4 description banner to clarify the 2D decision pipeline (Authority Triggers vs. Behavioral Overlays).
 * v1.0.45 (2026-08-15) - Behavioral Sleep & Mode Disambiguation Fix:
 *                        - Removed targetMode string matching (targetMode == sleepMode) from syncOutputs() to decouple behavioral sleep from target mode names.
 *                        - Added decision.isSleeping explicit pipeline property to drive output switch synchronization and HSM night-arming states.
...
static String version() { return '1.0.53' }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 4: Manual Overrides and Diagnostics</b>", hideable: true, hidden: isCollapsed) {
            ...
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
                      "<b>Status Device Output:</b> Select a virtual device (Omnipurpose/Variable/Tile Device) to receive direct ground-truth attributes (<code>activeMode</code>, <code>activeReason</code>, <code>lastTransitionTime</code>, and <code>tile</code>) on every evaluation.</div>"
        ...
        section("<b>SECTION 5: Notification(s) & Alert Preferences</b>", hideable: true, hidden: isCollapsed) {
            ...
            if (getSettingBool("enableNotifications", true)) {
                ...
                paragraph "<div style='color:#7F8C8D; font-size:11px; margin-top:-6px; margin-bottom:10px;'>" +
                          ...
                          "• <code>%mode%</code> = New Mode &nbsp;|&nbsp; <code>%reason%</code> = Reason &nbsp;|&nbsp; <code>%prevMode%</code> = Previous Mode &nbsp;|&nbsp; <code>%prevReason%</code> = Previous Reason &nbsp;|&nbsp; <code>%time%</code> = Time</div>"
...
private void updateStatusTileDevice(String modeVal, String reasonVal) {
    ...
    String formattedTile = (settings.tileFormat == "html") ?
        "<div style='font-size:12px; font-weight:bold; color:#27AE60;'>Mode: ${modeVal}<br/><span style='font-weight:normal; color:#555;'>Reason: ${reasonVal} (${timeStr})</span></div>" :
        "Mode: ${modeVal} | Reason: ${reasonVal} | Time: ${timeStr}"

    try {
        if (statusTileDevice.hasAttribute("activeMode")) statusTileDevice.sendEvent(name: "activeMode", value: modeVal)
        if (statusTileDevice.hasAttribute("activeReason")) statusTileDevice.sendEvent(name: "activeReason", value: reasonVal)
        if (statusTileDevice.hasAttribute("lastTransitionTime")) statusTileDevice.sendEvent(name: "lastTransitionTime", value: timeStr)
        
        if (statusTileDevice.hasAttribute("tile")) {
            statusTileDevice.sendEvent(name: "tile", value: formattedTile)
        } else if (statusTileDevice.hasAttribute("html")) {
            statusTileDevice.sendEvent(name: "html", value: formattedTile)
        } else if (statusTileDevice.hasAttribute("text")) {
            statusTileDevice.sendEvent(name: "text", value: formattedTile)
        } else if (statusTileDevice.hasCommand("setText")) {
            statusTileDevice.setText(formattedTile)
        }
        
        logTrace "Ground-Truth Status Tile Device '${statusTileDevice.displayName}' updated -> Mode: '${modeVal}' | Reason: '${reasonVal}'"
    } catch (Exception e) {
...
private void syncSwitch(def device, String targetState) {
    if (device != null && device.hasCommand(targetState)) {
        if (device.currentValue("switch") != targetState) {
            ...
            logTrace "Syncing Output Device '${device.displayName}' [ID: ${devId} | Class: ${devId?.class}] -> ${targetState.toUpperCase()}"
            ...
            logTrace "REGISTER MARKER: devId='${devId}' (Class: ${devId?.class}) | Target='${targetState}' | ActiveMapKeys=${pendingMap.keySet()}"
            device."${targetState}"()
        }
    }
}
...
private void dispatchTileNotification(String modeVal, String reasonVal, Boolean isSleepingState = false, Boolean modeChanged = true, Boolean reasonChanged = true) {
    ...
    if (speechDevice) {
        Boolean isSleeping = isSleepingState || (state.modeReason == "Sleeping")
        if (isSleeping && getSettingBool("suppressAudioWhenSleeping", true)) {
            logTrace "Audio notification suppressed because system is in Sleeping state."
        } else {
...
private boolean isInternalTransaction(def evt = null) {
    ...
    if (evt != null) {
        ...
        if (rawDevId != null) {
            ...
            logTrace "INSPECT EVENT ID: rawDevId=${rawDevId} (Class: ${rawDevId?.class}) -> String devId='${devId}' (Class: ${devId?.class}) | Value='${evtVal}' (Class: ${evtVal?.class})"

            Map pendingMap = (state.pendingOutputSyncs != null) ? new HashMap(state.pendingOutputSyncs) : [:]

            logTrace "PENDING MAP KEYS: ${pendingMap.keySet()} | Key Classes: ${pendingMap.keySet().collect { it?.class }}"

            if (pendingMap.containsKey(devId)) {
                Map marker = pendingMap[devId] as Map
                long currentMs = now()

                logTrace "FOUND MARKER FOR KEY '${devId}': MarkerValue='${marker?.value}' | MarkerExpires=${marker?.expires} | CurrentTime=${currentMs}"
                ...
                if (marker && marker.value == evtVal && currentMs <= (marker.expires as long)) {
                    ...
                    logTrace "MARKER MATCH FAILED: ValueMatch=${marker?.value == evtVal} (Marker: '${marker?.value}' vs Evt: '${evtVal}') | TimeMatch=${currentMs <= (marker?.expires as long)}"
                }
            } else {
