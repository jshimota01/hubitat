 ...
 *  Change History:
 *
 *  v1.0.20 (2026-08-13) - UI Label Refinement:
 *                         - Renamed Step 4 section header to 'STEP 4: Manual Overrides and Diagnostics'.
 *  v1.0.19 (2026-08-13) - UI Label Refinement:
 *                         - Renamed Step 5 section header to 'STEP 5: Notification(s)'.
 *  v1.0.18 (2026-08-13) - UI Restructuring & App Preferences Section:
 *                         - Added unnumbered 'App Preferences' section at the bottom of the GUI.
 *                         - Added toggles for App Label badge customization (showVersionInLabel, showModeInLabel, showReasonInLabel).
 *                         - Moved Dashboard Tile output formatting toggle (tileFormat) to App Preferences.
 *                         - Moved Info, Debug, and Detailed Trace logging toggles to App Preferences.
 *  v1.0.17 (2026-08-13) - HSM Logging Refinement:
 *                         - Added clear logInfo output for HSM state changes ('HSM changed: From -> To').
 *                         - Restored detailed logDebug logging for HSM execution tracing.
 *  v1.0.16 (2026-08-13) - HSM Tracking & UI Expansion:
 *                         - Added informational paragraph under Step 1 detailing HSM state handling.
 *  v1.0.15 (2026-08-13) - HSM Integration & Label Refinement:
 *                         - Updated Step 1 label to 'Control Hubitat Safety Monitor (HSM) based on selected Master Presence Sensor?'.
 *                         - Enhanced HSM integration logic to natively manage armNight during Sleeping / Night modes alongside armAway and disarm states.
 ...
 *                         - Removed legacy 'Test Mode' reason choice.
...
private void updateAppLabel(String currentMode = null, String reason = null) {
    ...
    if (showMode || showReason) {
        formattedLabel += " - ["
        if (showMode) {
            formattedLabel += "<span style='color:green; font-weight:bold;'>${displayMode}</span>"
        }
        if (showMode && showReason) {
            formattedLabel += " "
        }
        if (showReason) {
            formattedLabel += "(${currentReason})"
        }
        formattedLabel += "]"
    }
