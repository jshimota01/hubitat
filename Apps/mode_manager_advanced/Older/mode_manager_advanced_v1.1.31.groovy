 ...
 * Change History:
 *
 * v1.1.31 (2026-08-24) - Trigger Cause Help Integration:
 *                        - Integrated trigger cause (%reason%) reference notes inline into each corresponding row card in Section 6.
 *                        - Restructured matrix cards into cohesive informational components per event cause.
 * v1.1.30 (2026-08-24) - Section 6 Control Layout Facelift:
 *                        - Restructured notification configuration into unified row cards per trigger cause.
 *                        - Replaced toggle switches with radio controls for discrete Push and Audio selection per trigger cause.
 *                        - Expanded matrix to include Reason Changed Only and Mode Changed Only templates.
 *                        - Relocated output device selections under status tile configuration and moved master toggles to section footer.
 * v1.1.29 (2026-08-24) - Trace Logging Verbosity Adjustments:
 *                        - Demoted Alexa Mode, Alexa Awake, and Alexa Voice Marker event logging from INFO to TRACE.
 *                        - Demoted routine HSM event status updates, sync state evaluations, and confirmation checks from INFO/DEBUG to TRACE.
 *                        - Retained WARNING logging for critical HSM desynchronization timeouts and verification failures.
 * v1.1.28 (2026-08-23) - Notification Gating Logic & Reason-Only Branch Fix:
 *                        - Refactored dispatchTileNotification() to evaluate Mode-Only and Reason-Only branches against category settings.
 *                        - Fixed edge case where transitioning from Override to Scheduled without a Mode change was suppressed if Reason-Only template was blank or notifyOnScheduled was false.
 *                        - Ensures transitions releasing an Override properly dispatch notifications when notifyOnOverride or category settings are active.
 * v1.1.27 (2026-08-23) - UI Trigger Cause Table Card:
 *                        - Exposed Supported Trigger Causes / Evaluation Reasons table in Section 6 UI (%reason% reference card).
 *                        - Explicitly documented Scheduled, Presence, Voice, Override, and Reboot trigger causes for user template authoring.
 * v1.1.26 (2026-08-23) - Tile Metadata Integration & Extended Diagnostic Telemetry:
 *                        - Updated updateStatusTileDevice() to pass extended decision metadata (prevMode, prevReason, periodKey, isSleeping, txId, source) to Tile Driver v1.5.0.
 *                        - Added graceful fallback in updateStatusTileDevice() to support legacy 3- and 4-argument setStatus() tile command signatures.
 * v1.1.25 (2026-08-23) - Safe Type Coercion, Period Sorting & Multi-Event Voice Window:
 *                        - Added periodChanged and previousPeriodAtStart tracking to decision Map for intra-mode period boundary transitions.
 *                        - Added dynamic start-time sorting to getActiveTimePeriodInfo() to guarantee accuracy regardless of UI entry order.
 *                        - Hardened parseLongSafely() across consumeVoiceMarker(), verifyHsmSync(), and syncHsmState().
 *                        - Updated consumeVoiceMarker() to inspect non-destructive TTL window so multi-device Alexa routine bursts retain Voice attribution.
 *                        - Updated timeToMinutes() to return fallback default instead of silently defaulting to midnight (0).
 *                        - Guarded state.activeTransaction reset to ensure transactions only clear their own ID.
 * v1.1.24 (2026-08-23) - Obsolete HSM Retry Guard & Pure Transaction Notification Attributes:
 *                        - Fixed race condition in verifyHsmSync by validating expected target against state.pendingHsmTarget to prevent stale retries.
 *                        - Removed state.previousMode and state.previousReason mutations in dispatchTileNotification to guarantee 100% transaction isolation.
 * v1.1.23 (2026-08-23) - Pipeline Integrity, Voice Marker Alignment & UI Reference Card:
 *                        - Added Notification Variable Reference Table to Section 6 UI (restored %prevMode% and %prevReason%).
 *                        - Refactored applyDecision() to return Boolean status; processStatePipeline() short-circuits on false.
 *                        - Gated syncOutputs() behind decision.stateChanged or explicit force flags (isButtonTrigger/isStartupSync).
 *                        - Populated immutable transaction flags (modeChanged, reasonChanged, stateChanged) on decision Map.
 *                        - Enforced Path A Voice Marker validation across alexaModeSwitchHandler() and alexaAwakeSwitchHandler() (falls back to Override if marker absent).
 * v1.1.22 (2026-08-22) - Section Expansion & Collapse Memory Fix
 * v1.1.21 (2026-08-22) - Color Palette Synchronization
 * v1.1.20 (2026-08-22) - GUI Expansion & Audio Suppression Fixes
 * v1.1.19 (2026-08-22) - UI Button Resync Logging & Audio Integrity
 * v1.1.18 (2026-08-22) - UI Button Event Isolation Guard
 * v1.1.17 (2026-08-22) - Snapshot Key Isolation Guard
 * v1.1.16 (2026-08-22) - Dynamic Settings Snapshot Refactor
 * v1.1.15 (2026-08-22) - Settings Snapshot Key Coverage Fix
 * v1.1.14 (2026-08-22) - Alexa Voice Marker Window Optimization
 * v1.1.13 (2026-08-22) - Structural Optimization & Retry Safety Guard
 * v1.1.12 (2026-08-22) - Memory & Diagnostic Instrumentation Refactor
 * v1.1.11 (2026-08-22) - Authoritative Input Echo Suppression Guard
 */
// [KEEP-EXACT] see changelog.txt for v1.0.1 - v1.1.10

import java.util.concurrent.ConcurrentHashMap
import groovy.transform.Field

// Thread-safe heap register for expected acknowledgments on bidirectional devices
@Field static ConcurrentHashMap<String, String> pendingAckTokens = new ConcurrentHashMap<>()

static String version() { return '1.1.31' }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 6: Status Tiles & Notification Alerts</b>", hideable: true, hidden: true) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
            ...
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:12px 0 8px 0;'/>" +
                      "<span style='color:#2C3E50; font-weight:bold; font-size:13px;'>Notification Output Devices & Format</span>"
            ...
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:12px 0 8px 0;'/>" +
                      "<span style='color:#2C3E50; font-weight:bold; font-size:13px;'>Push & Audio Notifications Matrix</span>"
            
            /* DISCRETE NOTIFICATION ROUTING MATRIX CONTROL TABLE HEADER */
            paragraph "<table style='width:100%; border-collapse:collapse; font-family:sans-serif; font-size:12px; margin-top:8px; margin-bottom:8px;'>" +
                      "  <tr style='background-color:#2C3E50; color:#FFFFFF; text-align:left;'>" +
                      "    <th style='padding:8px; width:50%; font-size:13px;'>Trigger Cause / Event Type</th>" +
                      "    <th style='padding:8px; width:25%; text-align:center; font-size:13px;'>Push Notification</th>" +
                      "    <th style='padding:8px; width:25%; text-align:center; font-size:13px;'>Audio / Speech</th>" +
                      "  </tr>" +
                      "</table>"

            // --- ROW 1: SCHEDULED ---
            paragraph "<div style='background-color:#F4F6F7; border-left:4px solid #2980B9; padding:6px 10px; margin-bottom:2px; border-radius:4px; font-weight:bold; font-size:13px; color:#2C3E50;'>Scheduled Transitions</div>" +
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>Scheduled</code> (Chronological time period CRON boundaries or routine rechecks)</div>"
            input name: "templateScheduled", type: "text", title: "<b>Scheduled Message*</b>", defaultValue: "Mode scheduled transition: %mode% (from %prevMode% at %time%)", required: true, width: 6
            input name: "pushScheduled", type: "enum", title: "<b>Push Notification</b>", options: ["true": "On", "false": "Off"], defaultValue: "false", style: "radio", width: 3
            input name: "audioScheduled", type: "enum", title: "<b>Audio / Speech</b>", options: ["true": "On", "false": "Off"], defaultValue: "false", style: "radio", width: 3

            // --- ROW 2: VOICE ---
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      "<div style='background-color:#F4F6F7; border-left:4px solid #E67E22; padding:6px 10px; margin-bottom:2px; border-radius:4px; font-weight:bold; font-size:13px; color:#2C3E50;'>Voice / Alexa Triggers</div>" +
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>Voice</code> (Alexa Voice Routine execution verified via Voice Marker pulse)</div>"
            input name: "templateVoice", type: "text", title: "<b>Voice Trigger Message*</b>", defaultValue: "Voice command changed mode to %mode% (Reason: %reason% at %time%)", required: true, width: 6
            input name: "pushVoice", type: "enum", title: "<b>Push Notification</b>", options: ["true": "On", "false": "Off"], defaultValue: "true", style: "radio", width: 3
            input name: "audioVoice", type: "enum", title: "<b>Audio / Speech</b>", options: ["true": "On", "false": "Off"], defaultValue: "true", style: "radio", width: 3

            // --- ROW 3: PRESENCE ---
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      "<div style='background-color:#F4F6F7; border-left:4px solid #27AE60; padding:6px 10px; margin-bottom:2px; border-radius:4px; font-weight:bold; font-size:13px; color:#2C3E50;'>Presence Changes</div>" +
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>Presence</code> (Master Presence Sensor or Home/Away state switch toggles)</div>"
            input name: "templatePresence", type: "text", title: "<b>Presence Change Message*</b>", defaultValue: "Presence update changed mode to %mode% (was %prevMode% at %time%)", required: true, width: 6
            input name: "pushPresence", type: "enum", title: "<b>Push Notification</b>", options: ["true": "On", "false": "Off"], defaultValue: "true", style: "radio", width: 3
            input name: "audioPresence", type: "enum", title: "<b>Audio / Speech</b>", options: ["true": "On", "false": "Off"], defaultValue: "true", style: "radio", width: 3

            // --- ROW 4: OVERRIDE ---
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      "<div style='background-color:#F4F6F7; border-left:4px solid #C0392B; padding:6px 10px; margin-bottom:2px; border-radius:4px; font-weight:bold; font-size:13px; color:#2C3E50;'>Manual Override Changes</div>" +
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>Override</code> (Manual Dashboard virtual switch toggle or Awake switch override)</div>"
            input name: "templateOverride", type: "text", title: "<b>Manual Override Message*</b>", defaultValue: "Manual Override activated: Mode set to %mode% (Reason: %reason% at %time%)", required: true, width: 6
            input name: "pushOverride", type: "enum", title: "<b>Push Notification</b>", options: ["true": "On", "false": "Off"], defaultValue: "true", style: "radio", width: 3
            input name: "audioOverride", type: "enum", title: "<b>Audio / Speech</b>", options: ["true": "On", "false": "Off"], defaultValue: "true", style: "radio", width: 3

            // --- ROW 5: REBOOT ---
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      "<div style='background-color:#F4F6F7; border-left:4px solid #8E44AD; padding:6px 10px; margin-bottom:2px; border-radius:4px; font-weight:bold; font-size:13px; color:#2C3E50;'>Hub Reboot / Startup Synchronization</div>" +
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>Reboot</code> (Hub startup/reboot system alignment check)</div>"
            input name: "templateReboot", type: "text", title: "<b>Hub Reboot Message*</b>", defaultValue: "System restarted following hub boot. Mode synchronized to %mode% (Reason: %reason% at %time%)", required: true, width: 6
            input name: "pushReboot", type: "enum", title: "<b>Push Notification</b>", options: ["true": "On", "false": "Off"], defaultValue: "true", style: "radio", width: 3
            input name: "audioReboot", type: "enum", title: "<b>Audio / Speech</b>", options: ["true": "On", "false": "Off"], defaultValue: "false", style: "radio", width: 3

            // --- ROW 6: RECHECK NO CHANGE ---
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      "<div style='background-color:#F4F6F7; border-left:4px solid #7F8C8D; padding:6px 10px; margin-bottom:2px; border-radius:4px; font-weight:bold; font-size:13px; color:#2C3E50;'>Passive Recheck (Unchanged State)</div>" +
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>RecheckNoChange</code> (Periodic status check where mode and evaluation reason remain unchanged)</div>"
            input name: "templateRecheckNoChange", type: "text", title: "<b>Recheck Message*</b>", defaultValue: "Mode recheck completed: Still in %mode% (%reason%) at %time%", required: true, width: 6
            input name: "pushRecheckNoChange", type: "enum", title: "<b>Push Notification</b>", options: ["true": "On", "false": "Off"], defaultValue: "false", style: "radio", width: 3
            input name: "audioRecheckNoChange", type: "enum", title: "<b>Audio / Speech</b>", options: ["true": "On", "false": "Off"], defaultValue: "false", style: "radio", width: 3

            // --- ROW 7: REASON CHANGED ONLY ---
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      "<div style='background-color:#F4F6F7; border-left:4px solid #D35400; padding:6px 10px; margin-bottom:2px; border-radius:4px; font-weight:bold; font-size:13px; color:#2C3E50;'>Evaluation Reason Changed Only (Mode Unchanged)</div>" +
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> Dynamic evaluation cause updated (e.g. Scheduled -> Override) while Mode remains identical</div>"
            input name: "templateReasonOnly", type: "text", title: "<b>Reason Only Message</b>", defaultValue: "Evaluation Reason updated to %reason% (Mode remains %mode% at %time%)", required: false, width: 6
            input name: "pushReasonOnly", type: "enum", title: "<b>Push Notification</b>", options: ["true": "On", "false": "Off"], defaultValue: "true", style: "radio", width: 3
            input name: "audioReasonOnly", type: "enum", title: "<b>Audio / Speech</b>", options: ["true": "On", "false": "Off"], defaultValue: "false", style: "radio", width: 3

            // --- ROW 8: MODE CHANGED ONLY ---
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 6px 0;'/>" +
                      "<div style='background-color:#F4F6F7; border-left:4px solid #16A085; padding:6px 10px; margin-bottom:2px; border-radius:4px; font-weight:bold; font-size:13px; color:#2C3E50;'>Mode Changed Only (Reason Unchanged)</div>" +
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-bottom:4px;'>ℹ️ <b>Trigger Cause (%reason%):</b> Location Mode changed (e.g. Day -> Evening) under an ongoing trigger cause (e.g. Scheduled)</div>"
            input name: "templateModeOnly", type: "text", title: "<b>Mode Only Message</b>", defaultValue: "Mode changed to %mode% (Reason remains %reason% at %time%)", required: false, width: 6
            input name: "pushModeOnly", type: "enum", title: "<b>Push Notification</b>", options: ["true": "On", "false": "Off"], defaultValue: "true", style: "radio", width: 3
            input name: "audioModeOnly", type: "enum", title: "<b>Audio / Speech</b>", options: ["true": "On", "false": "Off"], defaultValue: "true", style: "radio", width: 3

            // --- SECTION PREFERENCES & GLOBAL CONTROLS ---
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:12px 0 8px 0;'/>" +
                      "<span style='color:#2C3E50; font-weight:bold; font-size:13px;'>Notification Preferences & Global Mute</span>"

            input name: "enableNotifications", type: "bool", title: "<b>Enable Notifications System-Wide?</b>", defaultValue: true, submitOnChange: true
            ...
            paragraph "<div style='background-color:#F8F9FA; border-left:4px solid #8E44AD; padding:10px; border-radius:4px; font-size:12px; margin-top:10px; margin-bottom:12px;'>" +
                      "<b>Notification Template Placeholders</b><br/>" +
                      "Customize push and speech notification strings using dynamic placeholders below:<br/><br/>" +
                      "<table style='width:100%; border-collapse:collapse; font-size:12px; font-family:sans-serif; margin-bottom:10px;'>" +
        ...
        section("<b>SECTION 7: System Diagnostics, Overrides & Simulation Panel</b>", hideable: true, hidden: true) {
        ...
        section("<b>SECTION 8: System Startup & Boot Guard</b>", hideable: true, hidden: true) {
...
private Boolean isTriggerDebounced(String sourceKey, long windowMs = 2000L) {
    ...
    long lastTime = parseLongSafely(times[sourceKey], 0L)
...
private void cleanTriggerTimesMap(Map timesMap, long currentMs) {
    if (timesMap) {
        timesMap.entrySet().removeIf { entry -> (currentMs - parseLongSafely(entry.value, 0L)) > 10000L }
    }
}
...
private void recheckSchedule(String triggerSource, Boolean isButtonTrigger = false) {
    logInfo "Mode recheck requested by trigger source: '${triggerSource}'"
    processStatePipeline([reason: "Scheduled", source: triggerSource, isRecheck: true, isButtonTrigger: isButtonTrigger])
}
...
private void processStatePipeline(Map request) {
    ...
    try {
        ...
        Boolean forceSync = (decision.isButtonTrigger || decision.isStartupSync)
...
private void dispatchTileNotification(Map decision, String overrideReasonVal = null, Boolean isSleepingState = false) {
    if (!notificationDevice && !speechDevice) return
    ...
    // Suppress purely static rechecks where NOTHING (Mode OR Reason) changed
    if (reasonVal != "RecheckNoChange" && reasonVal != "Reboot" && state.lastNotifiedMode == modeVal && !reasonChanged && !isButtonTrigger) {
        logTrace "Notification suppressed: Mode '${modeVal}' and Reason '${reasonVal}' completely unchanged."
        return
    }

    // Determine discrete Push and Audio enablement flags by trigger cause
    Boolean pushEnabled = false
    Boolean audioEnabled = false

    // BRANCH 1: REASON CHANGED ONLY (Mode Unchanged)
    if (reasonVal != "Reboot" && !modeChanged && reasonChanged) {
        pushEnabled  = (settings.pushReasonOnly != null) ? settings.pushReasonOnly.toBoolean() : true
        audioEnabled = (settings.audioReasonOnly != null) ? settings.audioReasonOnly.toBoolean() : false
    } 
    // BRANCH 2: MODE CHANGED ONLY (Reason Unchanged)
    else if (reasonVal != "Reboot" && modeChanged && !reasonChanged) {
        pushEnabled  = (settings.pushModeOnly != null) ? settings.pushModeOnly.toBoolean() : true
        audioEnabled = (settings.audioModeOnly != null) ? settings.audioModeOnly.toBoolean() : true
    } 
    // BRANCH 3: BOTH MODE AND REASON CHANGED (OR STANDARD RECHECK / REBOOT)
    else {
        switch (reasonVal) {
            case "Scheduled":
                pushEnabled  = (settings.pushScheduled != null) ? settings.pushScheduled.toBoolean() : false
                audioEnabled = (settings.audioScheduled != null) ? settings.audioScheduled.toBoolean() : false
                break
            case "Voice":
                pushEnabled  = (settings.pushVoice != null) ? settings.pushVoice.toBoolean() : true
                audioEnabled = (settings.audioVoice != null) ? settings.audioVoice.toBoolean() : true
                break
            case "Presence":
                pushEnabled  = (settings.pushPresence != null) ? settings.pushPresence.toBoolean() : true
                audioEnabled = (settings.audioPresence != null) ? settings.audioPresence.toBoolean() : true
                break
            case "Override":
                pushEnabled  = (settings.pushOverride != null) ? settings.pushOverride.toBoolean() : true
                audioEnabled = (settings.audioOverride != null) ? settings.audioOverride.toBoolean() : true
                break
            case "Reboot":
                pushEnabled  = (settings.pushReboot != null) ? settings.pushReboot.toBoolean() : true
                audioEnabled = (settings.audioReboot != null) ? settings.audioReboot.toBoolean() : false
                break
            case "RecheckNoChange":
                pushEnabled  = (settings.pushRecheckNoChange != null) ? settings.pushRecheckNoChange.toBoolean() : false
                audioEnabled = (settings.audioRecheckNoChange != null) ? settings.audioRecheckNoChange.toBoolean() : false
                break
            default:
                pushEnabled  = true
                audioEnabled = true
    ...
    if (!pushEnabled && !audioEnabled && !isButtonTrigger) {
        logTrace "Notification suppressed: Push and Audio both disabled for '${reasonVal}'."
        return
    }
    ...
    if (notificationDevice && (pushEnabled || isButtonTrigger)) {
    ...
    if (speechDevice && (audioEnabled || isButtonTrigger)) {
...
def alexaModeSwitchHandler(evt) {
    ...
    logTrace "Alexa Mode Switch changed to '${evt.value}' (Voice Marker Active: ${isVoiceSource} | Reason: '${evalReason}')"
...
def alexaAwakeSwitchHandler(evt) {
    ...
    logTrace "Alexa Awake Switch changed to '${evt.value}' (Voice Marker Active: ${isVoiceSource} | Attributed Reason: '${evalReason}')"
...
private int getMinutesFromSetting(String timeIso, int defaultMinutes) {
    ...
    return timeToMinutes(timeIso, defaultMinutes)
}
...
private int timeToMinutes(def timeInput, int defaultVal = 0) {
    ...
    if (timeInput instanceof Date) {
        String formattedTime = timeInput.format("HH:mm", location.timeZone ?: TimeZone.getDefault())
        int colonIdx = formattedTime.indexOf(":")
        if (colonIdx > 0) {
            return (formattedTime.substring(0, colonIdx).toInteger() * 60) + formattedTime.substring(colonIdx + 1).toInteger()
        }
        return defaultVal
    }
    ...
    if (tIdx >= 0) {
        timeStr = timeStr.substring(tIdx + 1)
    }
    
    int colonIdx = timeStr.indexOf(":")
    if (colonIdx > 0) {
        try {
            int hrs = timeStr.substring(0, colonIdx).replaceAll("[^0-9]", "").toInteger()
            int mins = timeStr.substring(colonIdx + 1, Math.min(colonIdx + 3, timeStr.length())).replaceAll("[^0-9]", "").toInteger()
            return (hrs * 60) + mins
        } catch (Exception e) {
            return defaultVal
        }
