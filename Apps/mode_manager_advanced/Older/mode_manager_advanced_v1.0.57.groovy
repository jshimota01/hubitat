 ...
 * Change History:
 *
 * v1.0.57 (2026-08-16) - Full Trace & Debug Infrastructure Restoration:
 *                        - Restored complete granular trace and debug logging across all pipeline methods, handlers, and switch isolations.
 *                        - Re-established transaction boundary logging (#tx_) and Class/Type safety inspection output.
 *                        - Fixed logging preference snapshotting to prevent unwanted debug/trace toggle resets on updated().
 * v1.0.56 (2026-08-16) - Period Switch Loop Suppression & Logging Toggle Fix:
 *                        - Added virtual period indicator switches to pendingOutputSyncs markers during syncOutputs().
 *                        - Prevents app-driven virtual switch ON events from triggering vSwitchHandler() as a manual Override.
 *                        - Fixed default fallback values in checkLoggingChanges() to preserve debug/trace settings across updated() calls.
 * v1.0.55 (2026-08-16) - App Label Preferences Logic Fix.
 ...
 * v1.0.49 (2026-08-16) - Diagnostics & Data Type Inspection Logging.
...
static String version() { return '1.0.57' }
