...
def initialize() {
    ...
    logTrace "Reset execution state flags: state.lastHsmState=null, state.isSyncingVSwitches=false"
    ...
    if (alexaSleepSwitch) {
        logTrace "Subscribing to Alexa Sleep Switch: ${alexaSleepSwitch.displayName}"
        subscribe(alexaSleepSwitch, "switch", alexaSleepSwitchHandler)
    }
...
/* Schedule period boundary trigger */
...
def appButtonHandler(btn) {
    ...
    if (btn == "btnTrigger") {
        ...
        logTrace "Cleared override state flags: state.manualOverrideMode=null, state.manualOverridePeriodKey=null, state.modeReason=null"
        evaluateAndSetMode()
    } else {
...
def updateSwitchHandler(evt) {
    ...
    state.manualOverridePeriodKey = null
    state.modeReason = null
    logTrace "Cleared manual override flags prior to triggering state recheck."
    evaluateAndSetMode()
...
def alexaSleepSwitchHandler(evt) {
    if (state.isSyncingVSwitches == true) {
        logTrace "Ignoring Alexa Sleep Switch event generated during internal state synchronization."
        return
    }
    
    logDebug "Alexa Sleep Switch '${evt.device.displayName}' changed to value: ${evt.value}"
    if (evt.value == "on") {
        if (awakeSwitch && awakeSwitch.currentValue("switch") != "off") {
            logInfo "Alexa Sleep Switch turned ON. Forcing Awake switch '${awakeSwitch.displayName}' OFF (Normal state change, not manual override)."
            awakeSwitch.off()
        } else {
        ...
        if (awakeSwitch && awakeSwitch.currentValue("switch") != "on") {
            logInfo "Alexa Sleep Switch turned OFF. Forcing Awake switch '${awakeSwitch.displayName}' ON."
            awakeSwitch.on()
        } else {
...
def evaluateAndSetMode() {
    ...
    if (isSleeping) {
        logDebug "State Priority 3 Matched: Home & Sleeping. Target Mode: '${sleepMode}'"
...
/* Mode Changer and Virtual Switch Synchronization */
...
def updateVirtualModeSwitches(String activePeriodKey) {
    ...
    try {
        ...
        /* Mirror Alexa Sleeping virtual switch state based on active sleep state */
        if (alexaSleepSwitch) {
            Boolean isSleeping = (activePeriodKey == "sleeping") || (awakeSwitch?.currentValue("switch") == "off")
            logTrace "Checking Alexa Sleeping Virtual Switch sync state. isSleeping calculated as: ${isSleeping}"
            
            if (isSleeping && alexaSleepSwitch.currentValue("switch") != "on") {
                logTrace "Syncing Alexa Sleeping Virtual Switch '${alexaSleepSwitch.displayName}' -> ON"
                alexaSleepSwitch.on()
            } else if (!isSleeping && alexaSleepSwitch.currentValue("switch") != "off") {
                logTrace "Syncing Alexa Sleeping Virtual Switch '${alexaSleepSwitch.displayName}' -> OFF"
                alexaSleepSwitch.off()
            } else {
                logTrace "Alexa Sleeping Virtual Switch '${alexaSleepSwitch.displayName}' already synchronized."
            }
        }
    } finally {
...
def updateAppLabel(String currentMode, String reason = null) {
    ...
    logTrace "updateAppLabel formatting app status badge: Mode='${displayMode}', Reason='${activeReason}'"
