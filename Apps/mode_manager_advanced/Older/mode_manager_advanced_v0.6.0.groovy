...
private boolean isReservationValid(Map inputMap, String devId, String value) {
    if (inputMap == null || !inputMap.containsKey(devId)) return false
    
    Map map = new HashMap(inputMap)
    ...
    if (reservation?.expires && currentMs > (reservation.expires as Long)) {
        ...
        map.remove(devId)
        if (inputMap == state.pendingVSwitchSync) state.pendingVSwitchSync = map
        if (inputMap == state.pendingMirrorSync) state.pendingMirrorSync = map
        return false
    }
