 ...
 *                         - Overhauled STEP 5 GUI with master toggle, push vs speech device selectors, and quiet-hours mute options.
 *                         - Implemented event reason filtering (Normal, Voice, Presence, Override) to eliminate notification spam.
 *                         - Added dynamic template variable support (%mode%, %reason%, %prevMode%, %time%).
 *                         - Added speech device support (.speak()) alongside standard tile/push notifications (.deviceNotification()).
 ...
 *                         - Added comprehensive trace logging across all call paths entering processStatePipeline() and restartPeriodSchedules().
 *                         - Decoupled CRON scheduler management from applyDecision() into a state-transition model.
 *                         - Enforced strict String coercion for raw device IDs in syncSwitch(), isInternalTransaction(), and vSwitchHandler().
 *                         - Implemented in-memory HashMap cloning for pendingOutputSyncs to prevent marker overwrites.
 *                         - Demoted Sleeping Switch to a passive inverse mirror; Awake Switch made primary authority.
 *                         - Updated stopPeriodSchedules() with blanket unschedule() to prevent Quartz job stacking.
 *                         - Added live location.hsmStatus checks in syncOutputs() to defend against duplicate HSM commands.
 *                         - Applied safe navigation operators and NPE defenses across all device event handlers.
...
static String version() { return '1.0.33' }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>STEP 2: Sleep Architecture & State Switches</b>", hideable: true, hidden: true) {
            ...
            paragraph "<div style='background-color:#EBF5FB; border-left:4px solid #3498DB; padding:8px; border-radius:4px; font-size:12px; margin-bottom:10px;'>" +
                      "ℹ️ <b>Note on Sleep Modes:</b> When <b>Sleeping</b> (Awake Switch = OFF), the app overrides the time schedule and enforces the target mode selected below. When <b>Awake</b> (Awake Switch = ON), the sleep overlay releases and your system automatically resumes the Step 3 Time Period schedule.</div>"
            ...
            input name: "vSwitchSleeping", type: "capability.switch", title: "Virtual Indicator Switch for Sleeping Mode", required: false
        ...
        section("<b>STEP 5: Notification(s) & Alert Preferences</b>", hideable: true, hidden: true) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
            ...
            input name: "enableNotifications", type: "bool", title: "<b>Enable Notifications?</b>", defaultValue: false, submitOnChange: true

            if (getSettingBool("enableNotifications", false)) {
                ...
                paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:12px 0 8px 0;'/>" +
                          "<span style='color:#2C3E50; font-weight:bold; font-size:13px;'>2. Event Triggers (What to notify on)</span>"
                paragraph "<div style='color:#7F8C8D; font-size:11px; margin-top:-6px; margin-bottom:8px;'>Select which event reasons should fire notifications:</div>"

                input name: "notifyOnNormal", type: "bool", title: "Notify on <b>Normal</b> Schedule Transitions? <i>(e.g., 07:30 Morning CRON)</i>", defaultValue: false
                input name: "notifyOnVoice", type: "bool", title: "Notify on <b>Voice / Alexa</b> Triggers?", defaultValue: true
                input name: "notifyOnPresence", type: "bool", title: "Notify on <b>Presence</b> Changes? <i>(e.g., Home / Away transitions)</i>", defaultValue: true
                input name: "notifyOnOverride", type: "bool", title: "Notify on Manual <b>Override</b> Changes? <i>(Virtual switches, UI buttons)</i>", defaultValue: true

                paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:12px 0 8px 0;'/>" +
                          "<span style='color:#2C3E50; font-weight:bold; font-size:13px;'>3. Message Formatting & Templates</span>"

                input name: "customMsgTemplate", type: "text", title: "<b>Custom Notification Message Template</b>", 
                      defaultValue: "Location Mode changed to %mode% (Reason: %reason%)", required: true
                
                paragraph "<div style='color:#7F8C8D; font-size:11px; margin-top:-6px; margin-bottom:10px;'>" +
                          "<b>Available Variables:</b><br/>" +
                          "• <code>%mode%</code> = New Location Mode (e.g., Morning)<br/>" +
                          "• <code>%reason%</code> = Evaluation Reason (e.g., Voice)<br/>" +
                          "• <code>%prevMode%</code> = Previous Location Mode (e.g., Sleeping)<br/>" +
                          "• <code>%time%</code> = Time of change (e.g., 09:38 AM)</div>"
...
def initialize() {
    ...
    if (sleepSwitch) {
        /* sleepSwitch is maintained purely as an inverse mirror output by syncOutputs().
           Subscribe to handle direct dashboard toggles by mirroring to awakeSwitch. */
...
def stopPeriodSchedules() {
    ...
    /* Blanket unschedule completely purges all Quartz jobs for this app instance */
    unschedule()
}
...
private void applyDecision(Map decision) {
    ...
    /* Scheduler Management: Transition-based rather than evaluation-based */
    if (suspendOnOverride) {
        // Entering Override: Stop schedules if coming from a non-Override state
        ...
        // Exiting Override: Restart schedules when releasing back to normal modes
...
private void syncOutputs(Map decision) {
    ...
    /* Dashboard Tile & Speech Notification Sync */
...
private void syncSwitch(def device, String targetState) {
    if (device != null && device.hasCommand(targetState)) {
        if (device.currentValue("switch") != targetState) {
            ...
            /* Fetch existing pending map and clone to prevent reference corruption */
            Map pendingMap = (state.pendingOutputSyncs != null) ? new HashMap(state.pendingOutputSyncs) : [:]
            
            /* Append expected output marker (valid for 3000ms) */
...
/* Notification Engine with Reason Filtering, Dynamic Templates, and Speech Output */
private void dispatchTileNotification(String modeVal, String reasonVal) {
    if (!getSettingBool("enableNotifications", false)) return

    /* 1. Reason Filtering Check */
    Boolean shouldNotify = false
    switch (reasonVal) {
        case "Normal":   shouldNotify = getSettingBool("notifyOnNormal", false); break
        case "Voice":    shouldNotify = getSettingBool("notifyOnVoice", true); break
        case "Presence": shouldNotify = getSettingBool("notifyOnPresence", true); break
        case "Override": shouldNotify = getSettingBool("notifyOnOverride", true); break
        default:         shouldNotify = true; break
    }
    ...
    /* 2. Build Dynamic Variable Message */
    String template = settings.customMsgTemplate ?: "Location Mode changed to %mode% (Reason: %reason%)"
    ...
    String formattedMsg = template
        ...
        .replace("%reason%", reasonVal ?: "Normal")
        .replace("%prevMode%", prevMode)
        .replace("%time%", timeStr)

    /* Store current mode as previousMode for future notifications */
    ...
    /* 4. Send Speech Notifications (checking Quiet Hours) */
    if (speechDevice) {
        ...
        if (isSleeping && getSettingBool("suppressAudioWhenSleeping", true)) {
            ...
            /* Speech devices use clean plain text without HTML tags */
...
def sleepSwitchHandler(evt) {
    ...
    /* Mirror manual toggle directly to primary authoritative input (Awake Switch) */
