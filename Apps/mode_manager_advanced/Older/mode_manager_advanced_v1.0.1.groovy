 ...
 *  Change History:
 *
 *  v1.0.1 (2026-08-13) - Full Architectural Refactor:
 *                        - Implemented single linear decision pipeline (Calculate -> Apply -> Sync -> Present).
 *                        - Replaced TTL reservation maps and guards with state.activeTransaction output isolation.
 *                        - Decoupled CRON scheduler from state engine; scheduling now managed strictly as infrastructure.
 *                        - Simplified production reasons to Normal, Presence, Voice, and Override.
 *                        - Converted "Normal - Rechecked" into an operation with explicit trigger source info logging.
 *                        - Preserved full 6-Step UI layout, HSM integrations, notification tiles, and Step 6 Test Harness.
 *
 */
        ...
        section("<b>STEP 6: Testing & Diagnostics Engine</b>") {
            ...
            if (settings.expandStep6) {
                ...
                input name: "testReason", type: "enum", title: "<b>Forced State Reason</b>", defaultValue: "Test Mode", required: false, submitOnChange: true,
                      options: ["Test Mode": "Test Mode", "Normal": "Normal", "Presence": "Presence", "Voice": "Voice", "Override": "Override"]
...
def initialize() {
    ...
    recheckSchedule("Initialization")
}
...
private void handlePeriodBoundary(String periodKey) {
    ...
    if (state.modeReason == "Override" && settings.autoReturnNormal == false) {
        logInfo "Period boundary '${periodKey}' hit but autoReturnNormal is disabled. Staying in Override."
        return
    }
...
private void processStatePipeline(Map request) {
    ...
    runIn(1, "clearActiveTransaction")
}
...
private void applyDecision(Map decision) {
    ...
    if (newReason == "Override" && settings.autoReturnNormal == false) {
        stopPeriodSchedules()
...
private void syncSwitch(def device, String targetState) {
    if (device && device.currentValue("switch") != targetState) {
...
private void updateAppLabel(String currentMode = null, String reason = null) {
    ...
    switch (currentReason) {
        ...
        case "Presence":
            formattedLabel = "${baseLabel} - [<span style='color:green; font-weight:bold;'>${displayMode} (Presence)</span>]"
            break
...
def appButtonHandler(btn) {
    if (btn == "btnTrigger") {
        ...
        recheckSchedule("Manual UI Button ('Evaluate & Set Mode Now')")
