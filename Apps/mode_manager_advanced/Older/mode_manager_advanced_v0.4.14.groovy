 ...
 * 		2026-08-09	  jshimota      0.4.14		Cleaned up user interface
 *
 */

static String version() { return '0.4.14' }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "Mode Manager Advanced Configuration (v${version()})", install: true, uninstall: true) {
        ...
        section("Step 1: Presence Architecture & State Switches") {
        ...
        section("Step 3: Time Period & Target Mode Schedule") {
            paragraph "<b>Schedule Architecture:</b> Dynamic chronological time periods evaluated sequentially throughout the day when Home and Awake."
        ...
        section("Step 5: Logging & System Diagnostics") {
            paragraph "<b>Logging Architecture:</b> Info logs communicate state changes. Debug logs automatically disable after 30 minutes."
...
def updateSwitchHandler(evt) {
    ...
    if (updateTriggerSwitch && updateTriggerSwitch.currentValue("switch") != "off") {
        ...
        sendVSwitchCommand(updateTriggerSwitch, "off")
    }
}
