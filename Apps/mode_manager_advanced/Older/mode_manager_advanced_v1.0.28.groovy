 ...
 *                         - Refactored executeForcedTestEvaluation() to inject simulatedHome, simulatedAwake, and simulatedPeriodKey directly into request map.
 *                         - Updated calculateDecision() to pass simulated inputs through the exact same logic paths as live events, making simulations 100% accurate.
...
static String version() { return '1.0.28' }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>STEP 4: Manual Overrides and Diagnostics</b>", hideable: true, hidden: true) {
            ...
            input name: "holdOverride", type: "bool", title: "<b>Lock Override against lower-priority events?</b><br/><span style='font-size:11px; color:#555;'>Turn ON to prevent incoming Voice or Normal event rechecks from unseating an active Manual Override.</span>", defaultValue: true
...
private Map calculateDecision(Map request) {
    ...
    /* Preemption & Lock Checks */
    if (currentActiveReason == "Override" && getSettingBool("holdOverride", true)) {
