...
/* Alexa Awake Switch Handler (Mirrors state and enforces Voice reason) */
def alexaAwakeSwitchHandler(evt) {
    ...
    if (evt.value == "off") {
        // Synchronize all sleep/awake mirror switches to OFF under reservation guard
        syncAwakeState("off")
        // Route through Voice reason transition targeting sleepMode
        processReasonTransition("Voice", [targetMode: sleepMode?.toString()])
    } else if (evt.value == "on") {
        // Synchronize all sleep/awake mirror switches to ON under reservation guard
        syncAwakeState("on")
        // Determine current scheduled time period mode when waking up
