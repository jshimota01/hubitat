 ...
 *                        - Aligned discrete Push and Audio controls side-by-side with message text templates.
 *                        - Updated dispatchTileNotification() to evaluate push and audio enablement discretely per cause.
...
static String version() { return '1.1.30' }
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 6: Status Tiles & Notification Alerts</b>", hideable: true, hidden: true) {
            ...
            if (getSettingBool("enableNotifications", true)) {
                ...
                paragraph "<table style='width:100%; border-collapse:collapse; font-family:sans-serif; font-size:12px; margin-top:8px; margin-bottom:12px;'>" +
                ...
                // --- CARD 1: SCHEDULED ---
                paragraph "<div style='background-color:#F4F6F7; border-left:4px solid #2980B9; padding:6px 10px; margin-bottom:2px; border-radius:4px 4px 0 0; font-weight:bold; font-size:13px; color:#2C3E50;'>Scheduled Transitions</div>"
                input name: "templateScheduled", type: "text", title: "<b>Scheduled Message*</b>", defaultValue: "Mode scheduled transition: %mode% (from %prevMode% at %time%)", required: true, width: 8
                input name: "pushScheduled", type: "bool", title: "<b>Push</b>", defaultValue: false, width: 2
                input name: "audioScheduled", type: "bool", title: "<b>Audio</b>", defaultValue: false, width: 2

                // --- CARD 2: VOICE ---
                paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:10px 0 6px 0;'/>" +
                          "<div style='background-color:#F4F6F7; border-left:4px solid #E67E22; padding:6px 10px; margin-bottom:2px; border-radius:4px 4px 0 0; font-weight:bold; font-size:13px; color:#2C3E50;'>Voice / Alexa Triggers</div>"
                input name: "templateVoice", type: "text", title: "<b>Voice Trigger Message*</b>", defaultValue: "Voice command changed mode to %mode% (Reason: %reason% at %time%)", required: true, width: 8
                input name: "pushVoice", type: "bool", title: "<b>Push</b>", defaultValue: true, width: 2
                input name: "audioVoice", type: "bool", title: "<b>Audio</b>", defaultValue: true, width: 2

                // --- CARD 3: PRESENCE ---
                paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:10px 0 6px 0;'/>" +
                          "<div style='background-color:#F4F6F7; border-left:4px solid #27AE60; padding:6px 10px; margin-bottom:2px; border-radius:4px 4px 0 0; font-weight:bold; font-size:13px; color:#2C3E50;'>Presence Changes</div>"
                input name: "templatePresence", type: "text", title: "<b>Presence Change Message*</b>", defaultValue: "Presence update changed mode to %mode% (was %prevMode% at %time%)", required: true, width: 8
                input name: "pushPresence", type: "bool", title: "<b>Push</b>", defaultValue: true, width: 2
                input name: "audioPresence", type: "bool", title: "<b>Audio</b>", defaultValue: true, width: 2

                // --- CARD 4: OVERRIDE ---
                paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:10px 0 6px 0;'/>" +
                          "<div style='background-color:#F4F6F7; border-left:4px solid #C0392B; padding:6px 10px; margin-bottom:2px; border-radius:4px 4px 0 0; font-weight:bold; font-size:13px; color:#2C3E50;'>Manual Override Changes</div>"
                input name: "templateOverride", type: "text", title: "<b>Manual Override Message*</b>", defaultValue: "Manual Override activated: Mode set to %mode% (Reason: %reason% at %time%)", required: true, width: 8
                input name: "pushOverride", type: "bool", title: "<b>Push</b>", defaultValue: true, width: 2
                input name: "audioOverride", type: "bool", title: "<b>Audio</b>", defaultValue: true, width: 2

                // --- CARD 5: REBOOT ---
                paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:10px 0 6px 0;'/>" +
                          "<div style='background-color:#F4F6F7; border-left:4px solid #8E44AD; padding:6px 10px; margin-bottom:2px; border-radius:4px 4px 0 0; font-weight:bold; font-size:13px; color:#2C3E50;'>Hub Reboot / Startup Synchronization</div>"
                input name: "templateReboot", type: "text", title: "<b>Hub Reboot Message*</b>", defaultValue: "System restarted following hub boot. Mode synchronized to %mode% (Reason: %reason% at %time%)", required: true, width: 8
                input name: "pushReboot", type: "bool", title: "<b>Push</b>", defaultValue: true, width: 2
                input name: "audioReboot", type: "bool", title: "<b>Audio</b>", defaultValue: false, width: 2

                // --- CARD 6: RECHECK NO CHANGE ---
                paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:10px 0 6px 0;'/>" +
                          "<div style='background-color:#F4F6F7; border-left:4px solid #7F8C8D; padding:6px 10px; margin-bottom:2px; border-radius:4px 4px 0 0; font-weight:bold; font-size:13px; color:#2C3E50;'>Passive Recheck (Unchanged State)</div>"
                input name: "templateRecheckNoChange", type: "text", title: "<b>Recheck Message*</b>", defaultValue: "Mode recheck completed: Still in %mode% (%reason%) at %time%", required: true, width: 8
                input name: "pushRecheckNoChange", type: "bool", title: "<b>Push</b>", defaultValue: false, width: 2
                input name: "audioRecheckNoChange", type: "bool", title: "<b>Audio</b>", defaultValue: false, width: 2

                // --- AUXILIARY SINGLE-DIMENSION BRANCH TEMPLATES ---
                paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:12px 0 8px 0;'/>" +
                          "<span style='color:#2C3E50; font-weight:bold; font-size:13px;'>Single-Dimension Transition Override Templates (Optional)</span>"
                ...
                paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:12px 0 8px 0;'/>" +
                          "<span style='color:#2C3E50; font-weight:bold; font-size:13px;'>Notification Output Devices & Speech Control</span>"
                ...
                paragraph "<div style='background-color:#F8F9FA; border-left:4px solid #8E44AD; padding:10px; border-radius:4px; font-size:12px; margin-top:10px; margin-bottom:12px;'>" +
                          ...
                          "  </tr>" +
                          "</table>" +
                          "<b>Supported Trigger Causes (%reason%)</b><br/>" +
                          "<table style='width:100%; border-collapse:collapse; font-size:11px; font-family:sans-serif; margin-top:4px;'>" +
                          "  <tr style='background-color:#EAEDED; text-align:left; border-bottom:1px solid #BDC3C7;'>" +
                          "    <th style='padding:4px;'>Trigger Value</th>" +
                          "    <th style='padding:4px;'>Source / Event Origin</th>" +
                          "  </tr>" +
                          "  <tr style='border-bottom:1px solid #E0E0E0;'>" +
                          "    <td style='padding:4px;'><code>Scheduled</code></td>" +
                          "    <td style='padding:4px;'>Chronological time period CRON boundaries or routine rechecks</td>" +
                          "  </tr>" +
                          "  <tr style='border-bottom:1px solid #E0E0E0; background-color:#FAFAFA;'>" +
                          "    <td style='padding:4px;'><code>Presence</code></td>" +
                          "    <td style='padding:4px;'>Master Presence Sensor or Home/Away state switch toggles</td>" +
                          "  </tr>" +
                          "  <tr style='border-bottom:1px solid #E0E0E0;'>" +
                          "    <td style='padding:4px;'><code>Voice</code></td>" +
                          "    <td style='padding:4px;'>Alexa Voice Routine execution (verified via Voice Marker pulse)</td>" +
                          "  </tr>" +
                          "  <tr style='border-bottom:1px solid #E0E0E0; background-color:#FAFAFA;'>" +
                          "    <td style='padding:4px;'><code>Override</code></td>" +
                          "    <td style='padding:4px;'>Manual Dashboard virtual switch toggle or Awake switch override</td>" +
                          "  </tr>" +
                          "  <tr style='border-bottom:1px solid #E0E0E0;'>" +
                          "    <td style='padding:4px;'><code>Reboot</code></td>" +
                          "    <td style='padding:4px;'>Hub startup/reboot system alignment check</td>" +
                          "  </tr>" +
                          "</table></div>"
            }
        }
...
private void dispatchTileNotification(Map decision, String overrideReasonVal = null, Boolean isSleepingState = false) {
    ...
    switch (reasonVal) {
        case "Scheduled":
            pushEnabled  = getSettingBool("pushScheduled", false)
            audioEnabled = getSettingBool("audioScheduled", false)
            break
        case "Voice":
            pushEnabled  = getSettingBool("pushVoice", true)
            audioEnabled = getSettingBool("audioVoice", true)
            break
        case "Presence":
            pushEnabled  = getSettingBool("pushPresence", true)
            audioEnabled = getSettingBool("audioPresence", true)
            break
        case "Override":
            pushEnabled  = getSettingBool("pushOverride", true)
            audioEnabled = getSettingBool("audioOverride", true)
            break
        case "Reboot":
            pushEnabled  = getSettingBool("pushReboot", true)
            audioEnabled = getSettingBool("audioReboot", false)
            break
        case "RecheckNoChange":
            pushEnabled  = getSettingBool("pushRecheckNoChange", false)
            audioEnabled = getSettingBool("audioRecheckNoChange", false)
            break
        default:
            pushEnabled  = true
            audioEnabled = true
    ...
    if (reasonVal != "Reboot" && !modeChanged && reasonChanged) {
        if (settings.templateReasonOnly && settings.templateReasonOnly.trim() != "") {
            template = settings.templateReasonOnly
        } else {
            template = "Evaluation Reason updated to %reason% (Mode remains %mode% at %time%)"
        }
    } 
    // BRANCH 2: MODE CHANGED ONLY (Reason Unchanged)
    else if (reasonVal != "Reboot" && modeChanged && !reasonChanged) {
        if (settings.templateModeOnly && settings.templateModeOnly.trim() != "") {
            template = settings.templateModeOnly
        } else {
            template = "Mode changed to %mode% (Reason remains %reason% at %time%)"
        }
    } 
