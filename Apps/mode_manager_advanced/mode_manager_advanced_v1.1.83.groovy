 ...
 *
 *  Changelog:
 *  v1.1.83 (2026-09-25) - HSM State Machine Hardening, Warning Log Formatting & Variable Scope Cleanup:
 *                        - Resolved duplicate @Field variable scope redeclaring error in compilation stack.
 *                        - Resolved stale transaction state leak in syncHsmState() where old state.pendingHsmTarget persisted across new mode transitions.
 *                        - Added explicit pending target reset when evaluating new/differing HSM targets to eliminate false timeout logs.
 *                        - Refactored warning log text from confusing "desynchronization timeout" to clear, accurate HSM verification phrasing.
 *  v1.1.82 (2026-09-21) - Comprehensive Ground Truth Switch Restoration Fix:
 *                        - Overhauled restorePersistedGroundTruth() to explicitly restore ALL monitored output switches (Home, Away, Awake, Sleeping, Alexa Presence, Alexa Awake, and all 6 Period Switches) directly from state.lastKnownGroundTruth.
 *                        - Confirmed event suppression protection across all handlers while state.isInitializing == true during boot restore sequence.
 ...
 **/
// [KEEP-EXACT] see changelog.txt for v1.0.1 - v1.1.79
...
static String version() { return '1.1.83' }
def timeStamp() { return "2026/09/25 08:40 AM" }
...
private void restorePersistedGroundTruth() {
    ...
    logInfo "EXPLICIT GROUND TRUTH RESTORE -> Mode: '${restoredMode}' | Control Source: '${restoredControlSource}' | Period: '${restoredPeriodKey}' (Saved: ${groundTruth.timestamp ?: 'N/A'})"
    ...
    // Explicitly restore Core Control Switches from persisted snapshot values
    if (groundTruth.homeSwitch && groundTruth.homeSwitch != "none") syncSwitch(homeSwitch, groundTruth.homeSwitch)
    if (groundTruth.awaySwitch && groundTruth.awaySwitch != "none") syncSwitch(awaySwitch, groundTruth.awaySwitch)
    if (groundTruth.awakeSwitch && groundTruth.awakeSwitch != "none") syncSwitch(awakeSwitch, groundTruth.awakeSwitch)
    if (groundTruth.sleepSwitch && groundTruth.sleepSwitch != "none") syncSwitch(sleepSwitch, groundTruth.sleepSwitch)

    // Explicitly restore Alexa Ecosystem Switches
    if (groundTruth.alexaPresenceSwitch && groundTruth.alexaPresenceSwitch != "none") syncSwitch(alexaPresenceSwitch, groundTruth.alexaPresenceSwitch)
    if (groundTruth.alexaAwakeSwitch && groundTruth.alexaAwakeSwitch != "none") syncSwitch(alexaAwakeSwitch, groundTruth.alexaAwakeSwitch)

    // Explicitly restore Schedule Period Switches
    Map periodSwitchStates = groundTruth.periodSwitches ?: [:]
    if (periodSwitchStates.weeHours && periodSwitchStates.weeHours != "none") syncSwitch(vSwitchWeeHours, periodSwitchStates.weeHours)
    if (periodSwitchStates.earlyMorning && periodSwitchStates.earlyMorning != "none") syncSwitch(vSwitchEarlyMorning, periodSwitchStates.earlyMorning)
    if (periodSwitchStates.morning && periodSwitchStates.morning != "none") syncSwitch(vSwitchMorning, periodSwitchStates.morning)
    if (periodSwitchStates.day && periodSwitchStates.day != "none") syncSwitch(vSwitchDay, periodSwitchStates.day)
    if (periodSwitchStates.evening && periodSwitchStates.evening != "none") syncSwitch(vSwitchEvening, periodSwitchStates.evening)
    if (periodSwitchStates.lateEvening && periodSwitchStates.lateEvening != "none") syncSwitch(vSwitchLateEvening, periodSwitchStates.lateEvening)
...
private void syncHsmState(String expectedHsmStatus, String hsmCmd, String controlSource, int retryCount = 0) {
    ...
    // 1. MATCH CHECK: If already at target, clear any pending state and exit
    if (currentHsm == expectedHsmStatus) {
        if (state.pendingHsmTarget != null) {
            logTrace "HSM synchronized: Currently '${currentHsm}'."
            logTrace "Clearing pending target '${state.pendingHsmTarget}' as location HSM matches expected status."
            state.pendingHsmTarget = null
            state.pendingHsmDispatchedTime = null
            state.pendingHsmDispatchedMs = null
            state.pendingHsmExpires = 0L
    ...
    // 2. IN-FLIGHT GUARD: If identical command was dispatched within 5s, suppress duplicate
    ...
    // 3. TRANSACTION TRANSITION: If targeting a NEW state, clear old pending target
    if (state.pendingHsmTarget != null && !isMatchingPending) {
        logTrace "New HSM target requested ('${expectedHsmStatus}'). Overriding previous pending target '${state.pendingHsmTarget}'."
        state.pendingHsmTarget = null
        state.pendingHsmExpires = 0L
    }

    // 4. TIMEOUT / DISPATCH LOGGING
    if (isMatchingPending && !isWindowActive) {
        logWarn "HSM sync timeout! Target state '${expectedHsmStatus}' was not reached within 5s window. Current status: '${currentHsm}'. Re-issuing command: '${hsmCmd}'"
    } else {
    ...
    // 5. COMMIT NEW TRANSACTION & DISPATCH
