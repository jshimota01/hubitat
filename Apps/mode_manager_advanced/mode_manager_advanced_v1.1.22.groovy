...
static String version() { return '1.1.22' }
...
private String captureSettingsSnapshot() {
    // Dynamically snapshot all user-configured setting keys, explicitly excluding metadata ('label') and transient UI button keys
    Map snapshot = [:]
...
private void syncOutputs(Map decision) {
    ...
    if (isStartupSync) {
        dispatchTileNotification(targetMode, "Reboot", isSleeping, modeChanged, reasonChanged, decision.isButtonTrigger)
    } else if (modeChanged || reasonChanged) {
        dispatchTileNotification(targetMode, reason, isSleeping, modeChanged, reasonChanged, decision.isButtonTrigger)
    } else if (decision.isRecheck == true) {
        dispatchTileNotification(targetMode, "RecheckNoChange", isSleeping, false, false, decision.isButtonTrigger)
    } else {
        dispatchTileNotification(targetMode, reason, isSleeping, false, false, decision.isButtonTrigger)
    }
...
private void registerExpectedAck(String deviceId, String expectedValue) {
    if (!deviceId) return
    long expirationMs = now() + 2000L // 2.0s tight lifespan strictly for housekeeping
...
private void dispatchTileNotification(String modeVal, String reasonVal, Boolean isSleepingState = false, Boolean modeChanged = true, Boolean reasonChanged = true, Boolean isButtonTrigger = false) {
...
def vSwitchHandler(evt) {
    // 1. Check & consume exact token match if app generated this sync
    if (consumeExpectedAck(evt)) return
    
    // 2. Filter non-ON transitions
