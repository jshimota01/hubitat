...
static String version() { return '1.0.75' }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        /* Section 2: Sleep Architecture (Behavioral State Overlay) */
        ...
        /* Section 3: Chronological Time Period Schedule */
        ...
        /* Section 4: Hubitat Safety Monitor (HSM) Integration */
        ...
        /* Section 6: Overrides and Diagnostics */
        section("<b>SECTION 6: Manual Overrides and Diagnostics</b>", hideable: true, hidden: isCollapsed) {
            paragraph "<div style='background-color:#EBF5FB; border-left:4px solid #3498DB; padding:8px; border-radius:4px; font-size:12px; margin-bottom:10px;'>" +
        ...
        /* Section 7: Notification(s) */
        section("<b>SECTION 7: Notification(s) & Alert Preferences</b>", hideable: true, hidden: isCollapsed) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
...
def initialize() {
    ...
    /* Section 2 Sleep Switch is a strict passive output indicator (Option A).
       No event subscription is established to prevent two-way loopbacks. */
...
private void syncHsmState(String expectedHsmStatus, String hsmCmd, String reason) {
    ...
    // 1. Target already matches reported location state
    ...
    // 2. Check for an active, unexpired pending command
    ...
    // 3. Handle expired pending commands vs. standard new transitions
    ...
    // 4. Record state markers before sending location event
    ...
    state.pendingHsmExpires = currentMs + 5000 // 5-second window for Hubitat event bus propagation
...
def sleepSwitchHandler(evt) {
    // Deprecated in v1.0.73 under Option A architecture (Sleeping Switch is strict passive output)
...
def executeForcedTestEvaluation() {
    ...
    processStatePipeline([
        ...
        source: "Section 6 Simulation Control Panel",
