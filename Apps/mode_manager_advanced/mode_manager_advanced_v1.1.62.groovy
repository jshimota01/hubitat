...
static String version() { return '1.1.62' }
...
def updated() {
    ...
    if (settingsChanged && previousSnapshot != null) {
        logTrace "SNAPSHOT DIFF DETECTED!"
        logTrace "Previous Snapshot Data: ${state.lastSettingsJson ?: 'None (First run or upgraded from prior version)'}"
        logTrace "Current Snapshot Data:  ${state.currentSettingsJson}"
    }
