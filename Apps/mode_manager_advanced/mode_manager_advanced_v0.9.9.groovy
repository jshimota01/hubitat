 ...
 *
 *  Change History: See changelog.txt stored on Git
 *
 */

static String version() { return '0.9.9' }
...
private boolean isReservationValid(String mapType, String devId, String value) {
    ...
    if (reservation?.command == value || reservation != null) {
        map.remove(devId)
        if (mapType == "VSwitch") state.pendingVSwitchSync = map
        else state.pendingMirrorSync = map
        return true
...
def vSwitchHandler(evt) {
    ...
    if (targetPeriod?.mode) {
        /* Prevent Virtual Indicator activations from converting 'Voice' into 'Override' */
