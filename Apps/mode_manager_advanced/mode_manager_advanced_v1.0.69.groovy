 ...
 * see changelog.txt for 1.0.1 - 1.0.66
 *
 */

static String version() { return '1.0.69' }
...
def alexaAwakeSwitchHandler(evt) {
    ...
    log.warn "ALEXA EVENT: type=${evt?.type} | source=${evt?.source} | isPhysical=${evt?.isPhysical()} | isDigital=${evt?.isDigital()} | name=${evt?.name} | value=${evt?.value} | description=${evt?.description}"
