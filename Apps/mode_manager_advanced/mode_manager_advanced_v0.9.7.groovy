...
static String version() { return '0.9.7' }
...
def schedulePeriodTime(String timeIso) {
    if (timeIso) {
        ...
        if (cronExpr) {
            try {
                schedule(cronExpr, periodHandler)
                logTrace "Successfully scheduled daily recurring CRON period trigger '${cronExpr}' for setting '${timeIso}'"
            } catch (Exception e) {
                logWarn "Unable to schedule CRON period trigger '${cronExpr}': ${e.message}"
            }
        }
    } else {
