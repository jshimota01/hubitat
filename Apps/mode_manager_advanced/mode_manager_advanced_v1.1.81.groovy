/**
 * Mode Manager Advanced App
 * Platform: Hubitat Elevation
 *
 * Notes: Advanced Hubitat Mode Manager driven by master presence, reverse-mirrored presence and sleep/awake switches, dynamic time periods, virtual mode indicators, Alexa Mode sync, HSM control, and Dashboard Notification Tile outputs.
 *
...
/**
 *  Purpose:
 *  Advanced Hubitat Mode Manager driven by master presence, reverse-mirrored presence and sleep/awake switches, 
 *  dynamic time periods, virtual mode indicators, Alexa Mode sync, HSM control, and Dashboard Notification Tile outputs.
 *
 *  Changelog:
 *  v1.1.81 (2026-09-21) - Expanded Monitored Switches Ground Truth Snapshot & Section 9 UI:
 *                        - Overhauled commitGroundTruthSnapshot() to capture live states of ALL core, Alexa, and schedule period switches.
 *                        - Expanded restorePersistedGroundTruth() to restore period virtual switches from persisted state.
 *                        - Redesigned Section 9 Ground Truth Snapshot card to display full breakdown of all monitored switch states.
 *  v1.1.80 (2026-09-21) - Section 9 Ground Truth Snapshot & Restore UI:
 *                        - Added Section 9 live status card displaying state.lastKnownGroundTruth snapshot metadata (Mode, Control Source, Period, Home/Awake switches, Timestamp).
 *                        - Added btnRestoreGroundTruth UI button allowing manual execution of restorePersistedGroundTruth().
 *  v1.1.79 (2026-09-21) - Deterministic Persisted Ground Truth & Boot Guard Architecture:
 *                        - Implemented state.lastKnownGroundTruth snapshot commit pipeline.
 *                        - Enforced snapshot commit strictly AFTER successful syncOutputs() execution.
 *                        - Implemented state.isInitializing boot guard to suppress transient mesh/virtual switch reads during hub startup.
 *                        - Decoupled state restoration from operational time-boundary rechecks following reboot.
 *  v1.1.78 (2026-09-13) - Section 4 GUI Solar Night & Day UI Update:
 *                        - Added live dynamic Solar Night Window status badge displaying hub's calculated Sunset (+60m) and Sunrise (-30m) times.
 *                        - Clarified textual rules explaining how Night vs. Day solar windows govern Armed Night vs. Disarmed states during sleep.
 *  v1.1.77 (2026-09-13) - Integrated Solar Window Guard for HSM Arm Night:
 *                        - Added isNightWindow() helper computing [Sunset + 60m] to [Sunrise - 30m] using Hubitat native location sunrise/sunset times.
 *                        - Enforced armedNight strictly when Home, Sleeping, AND within the solar night window. Naps during daytime retain Disarmed status.
 *  v1.1.76 (2026-09-13) - HSM Arm Night Decoupled from Wee Hours:
 *                        - Refactored syncOutputs(), Section 4 GUI, and HSM evaluation logic to enforce armedNight strictly when Sleeping.
 *                        - Un-sleept night periods (Wee Hours) now maintain Disarmed status so opening doors late at night does not trigger intrusion alerts.
 *                        - Standardized header comment block layout.
 *  v1.1.75 (2026-09-13) - Resolved Echo Loop and Presence Override Race:
 *                        - Added consumeExpectedAck(evt) to alexaPresenceSwitchHandler() to suppress back-fed switch updates from internal syncOutputs.
 *                        - Fixed hardcoded "Override" control source in alexaPresenceSwitchHandler() to attribute correctly to Presence or Voice.
 *                        - Retained background time period tracking while Away to prevent "unknown" period transition log entries upon returning home.
 *  v1.1.74 (2026-09-11) - Hardened Subscription Lifecycle Management:
 *                        - Added unconditional unsubscribe() at top of updated() to prevent orphaned subscriptions on deselecting/clearing trigger switches/buttons.
 *                        - Hardened initialize() collection handling for multi-device inputs (trigger switches, trigger buttons, period switches).
 *  v1.1.73 (2026-08-30) - Applied App Master Template v1.2.1:
 *                        - Integrated single-shot version demarcation trace logging helper checkAndLogVersionDemarcation().
 *                        - Added state.lastInitializedVersion != version() code-version upgrade check in updated().
 *                        - Standardized app label badging and collapsible preferences UI structure.
 *                        - Standardized centralized logging engine and auto-debug disabler routine.
 *  v1.1.72 (2026-08-27) - Pure Control Source & HSM Messaging Engine Alignment:
 *                        - Added dedicated HSM Control Source GUI controls, message templates, and notification dispatch routing in Section 8.
 *                        - Added logInfo entry inside syncHsmState() when dispatching HSM state transitions.
 *                        - Completely purged legacy 'Reason' terminology from message template placeholders (%controlSource% replacing %reason%).
 *                        - Updated updateStatusTileDevice discrete fallback setters to target setControlSource().
 *  v1.1.71 (2026-08-27) - Pipeline Architecture Refactor (Evaluation Trigger vs. Control Source):
 *                        - Explicitly decoupled transient Evaluation Triggers (e.g. Schedule CRON, Recheck Switch, Startup, Manual Button) from persistent Control Sources (Voice, Presence, Override, Scheduled, Reboot).
 *                        - Evaluation Triggers are handled strictly as in-flight transaction metadata passed through processStatePipeline() without polluting persistent state.
 *                        - Overhauled log wrappers, transaction headers, and tile output metadata to report Evaluation Trigger and Control Source as distinct dimensions.
 ...
 *  v1.1.65 (2026-08-26) - Converted HTML push notification wrappers from block <div> to inline <span> tags to prevent unwanted line breaks.
 *  v1.1.64 (2026-08-26) - Demoted raw JSON snapshot payload diff trace lines inside updated() to logDebug to minimize log noise.
 *  v1.1.63 (2026-08-26) - Refactored all log strings, preference templates, UI labels, and helper methods from 'Mode Recheck' to 'State Recheck' / 'State Evaluation'.
 *  v1.1.62 (2026-08-26) - Added alphabetical sorting to settings.keySet() and List values inside captureSettingsSnapshot() to ensure canonical JSON hashing.
 *  v1.1.61 (2026-08-25) - Fixed transient stale desync warning inside syncHsmState by explicitly zeroing out state.pendingHsmExpires when pending targets are cleared.
 *  v1.1.60 (2026-08-25) - Overhauled calculateDecision() to prioritize incoming Alexa awake/sleep intent over stale physical switch reads.
 **/
...
static String version() { return '1.1.81' }
def timeStamp() { return "2026/09/21 02:10 PM" }
...
definition(
    ...
    iconX2Url: "",
    iconX3Url: "",
    importUrl: "https://raw.githubusercontent.com/jshimota01/hubitat/main/Apps/mode_manager_advanced/mode_manager_advanced.groovy"
)
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 4: Hubitat Safety Monitor (HSM) Integration</b>", hideable: true, hidden: true) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
                      "<b>HSM Safety Automation:</b> Hubitat Safety Monitor is driven by Mode Manager's combined state engine (Presence + Sleep Overlay + Solar Night/Day Window).</div>"
            ...
            if (getSettingBool("manageHSM", true)) {
                ...
                Boolean isSleeping = (awakeSwitch ? (awakeSwitch.currentValue("switch") == "off") : false)
                Boolean isNight = isNightWindow()

                String expectedHsm
                if (!isHome) {
                    expectedHsm = "armedAway"
                } else {
                    expectedHsm = (isSleeping && isNight) ? "armedNight" : "disarmed"
                }
                ...
                Date sunsetTime = location.sunset
                Date sunriseTime = location.sunrise
                String nightStartStr = sunsetTime ? new Date(sunsetTime.getTime() + (60 * 60 * 1000L)).format("hh:mm a", location.timeZone ?: TimeZone.getDefault()) : "Sunset +60m"
                String nightEndStr = sunriseTime ? new Date(sunriseTime.getTime() - (30 * 60 * 1000L)).format("hh:mm a", location.timeZone ?: TimeZone.getDefault()) : "Sunrise -30m"
                
                String solarWindowBadge = isNight ? 
                    "<span style='color:#8E44AD; font-weight:bold;'>NIGHT WINDOW ACTIVE</span> (${nightStartStr} to ${nightEndStr})" : 
                    "<span style='color:#D35400; font-weight:bold;'>DAYTIME WINDOW ACTIVE</span> (${nightEndStr} to ${nightStartStr})"
                ...
                paragraph "<div style='background-color:#F8F9FA; border-left:4px solid #2980B9; padding:8px; border-radius:4px; font-size:12px; margin-bottom:10px;'>" +
                          ...
                          "<b>Solar Window:</b> ${solarWindowBadge}<br/><br/>" +
                          "• <b>Arm Away:</b> Enforced when Master Presence is <b>Away</b>.<br/>" +
                          "• <b>Arm Night:</b> Enforced strictly when Home, <b>Sleeping</b> (Awake Switch = OFF), AND within the <b>Solar Night Window</b> (<i>Sunset + 60m [${nightStartStr}] to Sunrise - 30m [${nightEndStr}]</i>).<br/>" +
                          "• <b>Disarm:</b> Enforced when Home and <b>Awake</b> across all time periods, or when sleeping during the <b>Daytime Window</b> (e.g., afternoon naps).</div>"
            }
        }
        ...
        section("<b>SECTION 8: Event Trigger Dispatch Matrix & Message Templates</b>", hideable: true, hidden: true) {
            ...
            paragraph "<div style='width:85%; margin:4px auto 16px auto; background-color:#FFFFFF; border:1px solid #E0E0E0; border-left:5px solid #8E44AD; border-radius:6px; box-shadow:0 4px 8px rgba(0,0,0,0.1); padding:12px 16px; font-size:12px; font-family:sans-serif;'>" +
                      ...
                      "    </tr>" +
                      "    <tr style='border-bottom:1px solid #E0E0E0;'>" +
                      "      <td style='padding:5px;'><code>%hsmStatus%</code></td>" +
                      "      <td style='padding:5px;'>Hubitat Safety Monitor Status</td>" +
                      "      <td style='padding:5px;'><code>Disarmed</code></td>" +
                      "    </tr>" +
                      "    <tr style='border-bottom:1px solid #E0E0E0; background-color:#FAFAFA;'>" +
                      "      <td style='padding:5px;'><code>%trigger%</code></td>" +
                      "      <td style='padding:5px;'>Evaluation Trigger (Transient Pipeline Cause)</td>" +
                      "      <td style='padding:5px;'><code>Schedule CRON (morning)</code></td>" +
                      "    </tr>" +
                      "    <tr style='border-bottom:1px solid #E0E0E0;'>" +
            ...
            String sampleHsm = location.hsmStatus ?: "disarmed"
            ...
            switch(selCause) {
                ...
                case "HSM":         selectedTemplate = settings.templateHsm ?: "HSM security status updated to %hsmStatus% (Mode: %mode% at %time%)"; break
            ...
            String previewStr = selectedTemplate
                ...
                .replace("%hsmStatus%", sampleHsm)
                .replace("%trigger%", "Schedule CRON (morning)")
            ...
            paragraph "<div style='background-color:#F4F6F7; border-left:4px solid #E74C3C; padding:6px 10px; border-radius:4px; font-weight:bold; font-size:13px; color:#2C3E50;'>Hubitat Safety Monitor (HSM) Changes</div>" +
                      "<div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-top:2px;'>ℹ️ <b>Control Source (%controlSource%):</b> <code>HSM Security Arming State Transitions</code></div>", width: 6
            input name: "pushHsm", type: "bool", title: "", defaultValue: false, width: 2
            input name: "pushMobileHsm", type: "bool", title: "", defaultValue: false, width: 2
            input name: "audioHsm", type: "bool", title: "", defaultValue: false, width: 2
            input name: "templateHsm", type: "text", title: "<b>Message Text Template*</b>", defaultValue: "HSM security status updated to %hsmStatus% (Mode: %mode% at %time%)", required: true, width: 12
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:4px 0 12px 0;'/>"
        ...
        section("<b>SECTION 9: System Diagnostics, Overrides & Persisted Ground Truth</b>", hideable: true, hidden: true) {
            
            /* Monitored Switches Ground Truth Live Snapshot Display Card */
            Map gt = state.lastKnownGroundTruth ?: [:]
            if (!gt.isEmpty()) {
                String gtHomeStr  = (gt.homeSwitch == "on")  ? "<span style='color:#27AE60; font-weight:bold;'>ON</span>"  : "<span style='color:#C0392B; font-weight:bold;'>OFF</span>"
                String gtAwayStr  = (gt.awaySwitch == "on")  ? "<span style='color:#27AE60; font-weight:bold;'>ON</span>"  : "<span style='color:#C0392B; font-weight:bold;'>OFF</span>"
                String gtAwakeStr = (gt.awakeSwitch == "on") ? "<span style='color:#27AE60; font-weight:bold;'>ON</span>"  : "<span style='color:#8E44AD; font-weight:bold;'>OFF</span>"
                String gtSleepStr = (gt.sleepSwitch == "on") ? "<span style='color:#8E44AD; font-weight:bold;'>ON</span>"  : "<span style='color:#27AE60; font-weight:bold;'>OFF</span>"
                
                String gtAlexaPresStr  = gt.alexaPresenceSwitch  ? ((gt.alexaPresenceSwitch == "on")  ? "<span style='color:#27AE60;'>ON</span>" : "<span style='color:#C0392B;'>OFF</span>") : "N/A"
                String gtAlexaAwakeStr = gt.alexaAwakeSwitch     ? ((gt.alexaAwakeSwitch == "on")     ? "<span style='color:#27AE60;'>ON</span>" : "<span style='color:#8E44AD;'>OFF</span>") : "N/A"
                
                Map periodStates = gt.periodSwitches ?: [:]
                String weeStr   = periodStates.weeHours     ? ((periodStates.weeHours == "on")     ? "<b style='color:#2980B9;'>ON</b>" : "off") : "N/A"
                String earlyStr = periodStates.earlyMorning ? ((periodStates.earlyMorning == "on") ? "<b style='color:#2980B9;'>ON</b>" : "off") : "N/A"
                String mornStr  = periodStates.morning      ? ((periodStates.morning == "on")      ? "<b style='color:#2980B9;'>ON</b>" : "off") : "N/A"
                String dayStr   = periodStates.day          ? ((periodStates.day == "on")          ? "<b style='color:#2980B9;'>ON</b>" : "off") : "N/A"
                String eveStr   = periodStates.evening      ? ((periodStates.evening == "on")      ? "<b style='color:#2980B9;'>ON</b>" : "off") : "N/A"
                String lateStr  = periodStates.lateEvening  ? ((periodStates.lateEvening == "on")  ? "<b style='color:#2980B9;'>ON</b>" : "off") : "N/A"

                paragraph "<div style='background-color:#F8F9FA; border-left:4px solid #8E44AD; padding:10px; border-radius:4px; font-size:12px; margin-bottom:10px;'>" +
                          "<b style='font-size:13px; color:#2C3E50;'>Saved Ground-Truth Snapshot State</b> <i>(Last Committed: ${gt.timestamp ?: 'N/A'})</i><br/><br/>" +
                          "• <b>System Baseline:</b> Target Mode = <span style='color:#2980B9; font-weight:bold;'>${gt.mode ?: 'Unknown'}</span> &nbsp;|&nbsp; " +
                          "Control Source = <i>${gt.controlSource ?: 'Unknown'}</i> &nbsp;|&nbsp; " +
                          "Schedule Block = <code>${gt.periodKey ?: 'none'}</code><br/><br/>" +
                          "• <b>Core Control Switches:</b> Home = ${gtHomeStr} | Away = ${gtAwayStr} | Awake = ${gtAwakeStr} | Sleeping = ${gtSleepStr}<br/>" +
                          "• <b>Alexa Sync Switches:</b> Presence Switch = ${gtAlexaPresStr} | Awake Switch = ${gtAlexaAwakeStr}<br/>" +
                          "• <b>Schedule Period Switches:</b> WeeHours = ${weeStr} | EarlyMorning = ${earlyStr} | Morning = ${mornStr} | Day = ${dayStr} | Evening = ${eveStr} | LateEvening = ${lateStr}</div>"
            } else {
                paragraph "<div style='background-color:#FEF9E7; border-left:4px solid #F39C12; padding:8px; border-radius:4px; font-size:12px; margin-bottom:10px;'>" +
                          "ℹ️ <b>Persisted Ground Truth:</b> No saved state snapshot found. Snapshot will automatically record upon the first authoritative state transition.</div>"
            }

            input name: "btnRestoreGroundTruth", type: "button", title: "Restore Persisted Ground Truth State"
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:12px 0;'/>"
...
/* =========================================================================================
   APPLICATION LIFECYCLE & BOOT GUARD RECOVERY PIPELINE
   ========================================================================================= */
...
def uninstalled() {
    ...
    logInfo "Uninstalling Mode Manager Advanced app..."
    unsubscribe()
    stopPeriodSchedules()
    unschedule()
    purgePendingAckTokens()
...
def updated() {
    ...
    // Explicitly purge all subscriptions unconditionally to prevent orphaned listeners
    unsubscribe()
    ...
    if (settingsChanged || codeVersionChanged) {
        logInfo "Settings modification or code version upgrade detected. Re-establishing subscriptions and schedules..."
        ...
        logInfo "App closed via Done without setting changes. Re-subscribing active configuration..."
        initialize(false)
        updatePresentation([targetMode: location.mode, controlSource: state.controlSource ?: "Scheduled"])
    }
...
private Boolean isNightWindow() {
    try {
        ...
        if (!sunsetTime || !sunriseTime) {
            logTrace "isNightWindow: Location sunrise/sunset uninitialized. Falling back to true."
            return true
        }

        long nightStartMs = sunsetTime.getTime() + (60 * 60 * 1000L) // Sunset + 60 mins
        long nightEndMs = sunriseTime.getTime() - (30 * 60 * 1000L)   // Sunrise - 30 mins
        long nowMs = nowTime.getTime()

        Boolean inNightWindow = (nowMs >= nightStartMs || nowMs < nightEndMs)
        logTrace "Solar Night Window Evaluation -> Current: ${nowTime.format('HH:mm')} | Night Start (Sunset+60m): ${new Date(nightStartMs).format('HH:mm')} | Night End (Sunrise-30m): ${new Date(nightEndMs).format('HH:mm')} | Result: ${inNightWindow}"
        return inNightWindow
    } catch (Exception e) {
        logWarn "isNightWindow calculation failed: ${e.message}"
        return true
    }
}
...
private void initialize(Boolean isInstall = false) {
    ...
    if (updateTriggerSwitch) {
        (updateTriggerSwitch instanceof List ? updateTriggerSwitch : [updateTriggerSwitch]).each { dev ->
            if (dev) subscribe(dev, "switch.on", updateSwitchHandler)
        }
    }
    if (updateTriggerButton) {
        (updateTriggerButton instanceof List ? updateTriggerButton : [updateTriggerButton]).each { dev ->
            if (dev) subscribe(dev, "pushed", updateButtonHandler)
        }
    }
...
def hubStartupHandler(evt = null) {
    ...
    state.isInitializing = true // SET BOOT GUARD: Suppress external device event handling
    ...
    logInfo "HUB STARTUP DETECTED at ${formattedBootTime}. Boot guard active. Holding state evaluation for ${delaySec} seconds..."
...
def delayedStartupHandler() {
    ...
    logInfo "Boot stabilization window elapsed at ${formattedSyncTime}. Step 1: Restoring persisted Ground Truth state..."
    
    // STEP 1: Restore persisted ground truth state before re-evaluating time or devices
    restorePersistedGroundTruth()

    // STEP 2: Release boot guard so alignment pipeline can proceed normally
    state.isInitializing = false
    state.isStartupPending = false

    // STEP 3: Execute system-wide alignment check (evaluates current clock/schedules against restored baseline)
    logInfo "Step 2: Executing system-wide ground-truth schedule alignment..."
    processStatePipeline([
        controlSource: "Reboot", 
        evaluationTrigger: "Hub Startup Alignment", 
        isStartupSync: true, 
        source: "Hub System Startup (Post-Boot Alignment)"
    ])
}

private void restorePersistedGroundTruth() {
    Map groundTruth = state.lastKnownGroundTruth ?: [:]

    if (!groundTruth || groundTruth.isEmpty()) {
        logWarn "No persisted lastKnownGroundTruth snapshot found (First boot/install). Defaulting to clock evaluation."
        return
    }

    String restoredMode = groundTruth.mode ?: location.mode
    String restoredControlSource = groundTruth.controlSource ?: "Scheduled"
    String restoredPeriodKey = groundTruth.periodKey ?: "day"
    Boolean restoredIsHome = (groundTruth.isHome != null) ? groundTruth.isHome : (groundTruth.homeSwitch == "on")
    Boolean restoredIsAwake = (groundTruth.isAwake != null) ? groundTruth.isAwake : (groundTruth.awakeSwitch == "on")

    logInfo "RESTRICTED GROUND TRUTH RESTORE -> Mode: '${restoredMode}' | Control Source: '${restoredControlSource}' | Period: '${restoredPeriodKey}' | Home: ${restoredIsHome} | Awake: ${restoredIsAwake} (Saved: ${groundTruth.timestamp ?: 'N/A'})"

    // Set internal state tracking to match restored ground truth
    state.controlSource = restoredControlSource
    state.activePeriodKey = restoredPeriodKey
    
    if (location.mode != restoredMode) {
        setLocationMode(restoredMode)
    }

    // Force-sync control switches to restored authoritative ground truth
    if (restoredIsHome) {
    ...
    } else if (restoredIsHome) {
        syncSwitch(awakeSwitch, "on")
        syncSwitch(sleepSwitch, "off")
        syncSwitch(alexaAwakeSwitch, "on")
    }

    updateVirtualModeSwitches(restoredPeriodKey)
    updatePresentation([targetMode: restoredMode, controlSource: restoredControlSource])
}
...
def hsmStatusHandler(evt) {
    ...
    updateStatusTileDevice([
        ...
        evaluationTrigger: "HSM Event (${newHsmStatus})",
...
private void syncHsmState(String expectedHsmStatus, String hsmCmd, String controlSource, int retryCount = 0) {
    ...
    if (state.pendingHsmTarget != null && !isWindowActive) {
        ...
        logInfo "HSM status transition initiated -> Target: '${expectedHsmStatus}' | Command: '${hsmCmd}' | Control Source: '${controlSource}'"
...
private void handlePeriodBoundary(String periodKey) {
    ...
    processStatePipeline([
        ...
        evaluationTrigger: "Schedule CRON (${periodKey})", 
        source: "Schedule CRON (${periodKey})", 
...
private void recheckState(String triggerSource, Boolean isButtonTrigger = false, Boolean isSaveEvent = false) {
    ...
    processStatePipeline([
        controlSource: "Scheduled", 
        evaluationTrigger: triggerSource, 
...
private void processStatePipeline(Map request) {
    ...
    String reqTrigger = request.evaluationTrigger ?: request.source ?: "Internal Pipeline"
    ...
    logTrace "=== Mode Manager Adv [Pipeline Trigger: ${reqTrigger} | Control: ${reqControlSource} | ${txId}] Begins ==="
    ...
    try {
        ...
        decision.txId = txId
        decision.evaluationTrigger = reqTrigger
        ...
        if (decision.stateChanged || forceSync) {
            ...
            logTrace "Skipping syncOutputs execution: System state unchanged (${decision.targetMode} | Control: ${decision.controlSource}) on passive recheck/CRON."
        }

        logTrace "COMPLETE TRANSACTION #${txId} -> Mode: '${location.mode}' | Control Source: '${state.controlSource}' | Evaluation Trigger: '${reqTrigger}'"
        logTrace "=== Mode Manager Adv [Pipeline Trigger: ${reqTrigger} | Control: ${reqControlSource} | ${txId}] Ends ==="
    } finally {
...
private Map calculateDecision(Map request) {
    ...
    Map timePeriodInfo = getActiveTimePeriodInfo(request.simulatedPeriodKey)

    if (requestedControlSource == "Override") {
        targetMode = request.targetMode ?: location.mode
        activePeriodKey = request.periodKey ?: timePeriodInfo?.key
    } else if (requestedControlSource == "Voice") {
        targetMode = request.targetMode
        activePeriodKey = request.periodKey ?: (isSleeping ? "sleeping" : timePeriodInfo?.key)
        if (!targetMode) {
            if (isSleeping) {
                targetMode = sleepMode?.toString() ?: "Sleeping"
            } else {
                targetMode = timePeriodInfo?.mode ?: location.mode
            }
        }
    } else if (requestedControlSource == "Presence") {
        if (!isHome) {
            targetMode = "Away"
            activePeriodKey = timePeriodInfo?.key
        } else {
            if (isSleeping) {
                targetMode = sleepMode?.toString() ?: "Sleeping"
...
/* =========================================================================================
   OUTPUT SYNCHRONIZATION WITH COMMIT GATE
   ========================================================================================= */
...
private void syncOutputs(Map decision) {
    ...
    logTrace "Parameters -> TargetMode: '${targetMode}' | PeriodKey: '${periodKey}' | Control Source: '${controlSource}' | Trigger: '${decision.evaluationTrigger}' | IsSleeping: ${isSleeping} | IsStartupSync: ${isStartupSync}"

    // STEP 1: PHYSICAL & VIRTUAL SWITCH SYNCHRONIZATION
    logTrace "syncOutputs [Step 1/4]: Synchronizing Presence, Sleep, and Alexa switches..."
    if (targetMode == "Away") {
        syncSwitch(homeSwitch, "off")
        syncSwitch(awaySwitch, "on")
        syncSwitch(alexaPresenceSwitch, "off")
    ...
    logTrace "syncOutputs [Step 2/4]: Processing status tile update and push notifications..."
    ...
    logTrace "syncOutputs [Step 3/4]: Evaluating HSM state synchronization..."
    if (targetMode == "Away") {
        syncHsmState("armedAway", "armAway", controlSource)
    } else {
        Boolean isNight = isNightWindow()
        Boolean isNightAndSleeping = (isSleeping && isNight)
        
        String expectedHsmStatus = isNightAndSleeping ? "armedNight" : "disarmed"
        String hsmCmd = isNightAndSleeping ? "armNight" : "disarm"

        logTrace "HSM Target Calculation -> Mode: '${targetMode}' | Period: '${periodKey}' | IsSleeping: ${isSleeping} | IsNightWindow: ${isNight} | Calculated Target: '${expectedHsmStatus}' (${hsmCmd})"
        syncHsmState(expectedHsmStatus, hsmCmd, controlSource)
    }

    // STEP 4: AUTHORITATIVE COMMIT GATE (Only commit snapshot AFTER full successful sync)
    logTrace "syncOutputs [Step 4/4]: Committing authoritative Ground Truth snapshot..."
    commitGroundTruthSnapshot(decision)

    logTrace "=== syncOutputs Execution Sequence Completed ==="
}

private void commitGroundTruthSnapshot(Map decision) {
    String timeStampStr = new Date().format("yyyy-MM-dd hh:mm:ss a", location.timeZone ?: TimeZone.getDefault())

    Map periodSwitchStates = [
        weeHours:     vSwitchWeeHours     ? vSwitchWeeHours.currentValue("switch")     : "none",
        earlyMorning: vSwitchEarlyMorning ? vSwitchEarlyMorning.currentValue("switch") : "none",
        morning:      vSwitchMorning      ? vSwitchMorning.currentValue("switch")      : "none",
        day:          vSwitchDay          ? vSwitchDay.currentValue("switch")          : "none",
        evening:      vSwitchEvening      ? vSwitchEvening.currentValue("switch")      : "none",
        lateEvening:  vSwitchLateEvening  ? vSwitchLateEvening.currentValue("switch")  : "none"
    ]

    Map snapshot = [
        mode: decision.targetMode ?: location.mode,
        controlSource: decision.controlSource ?: state.controlSource ?: "Scheduled",
        periodKey: decision.periodKey ?: state.activePeriodKey ?: "none",
        isHome: (homeSwitch ? (homeSwitch.currentValue("switch") == "on") : true),
        isAwake: (awakeSwitch ? (awakeSwitch.currentValue("switch") == "on") : true),
        homeSwitch: homeSwitch ? homeSwitch.currentValue("switch") : "none",
        awaySwitch: awaySwitch ? awaySwitch.currentValue("switch") : "none",
        awakeSwitch: awakeSwitch ? awakeSwitch.currentValue("switch") : "none",
        sleepSwitch: sleepSwitch ? sleepSwitch.currentValue("switch") : "none",
        alexaPresenceSwitch: alexaPresenceSwitch ? alexaPresenceSwitch.currentValue("switch") : "none",
        alexaAwakeSwitch: alexaAwakeSwitch ? alexaAwakeSwitch.currentValue("switch") : "none",
        periodSwitches: periodSwitchStates,
        timestamp: timeStampStr
    ]

    state.lastKnownGroundTruth = snapshot
    logTrace "AUTHORITATIVE GROUND TRUTH COMMITTED -> ${snapshot}"
}
...
private void updateStatusTileDevice(Map decision) {
    ...
    Map extendedMetadata = [
        ...
        evaluationTrigger: decision.evaluationTrigger ?: decision.source ?: "Internal Pipeline",
    ...
    logTrace "updateStatusTileDevice -> Updating '${statusTileDevice.displayName}' | Mode: '${modeVal}' | Control Source: '${controlSourceVal}' | Trigger: '${extendedMetadata.evaluationTrigger}' | HSM: '${formattedHsm}' | Time: '${timeStr}'"
...
private void dispatchTileNotification(Map decision, String overrideSourceVal = null, Boolean isSleepingState = false) {
    ...
    if (controlSourceVal != "Reboot" && !modeChanged && controlSourceChanged) {
        ...
        switch (controlSourceVal) {
            ...
            case "Override":
                ...
                break
            case "HSM":
                triggerPushEnabled       = getSettingBool("pushHsm", false)
                triggerPushMobileEnabled = getSettingBool("pushMobileHsm", false)
                triggerAudioEnabled      = getSettingBool("audioHsm", false)
                break
            case "Reboot":
    ...
    if (controlSourceVal != "Reboot" && !modeChanged && controlSourceChanged) {
        ...
        switch (controlSourceVal) {
            ...
            case "Override": 
                ...
                break
            case "HSM":
                template = settings.templateHsm ?: "HSM security status updated to %hsmStatus% (Mode: %mode% at %time%)"
                break
            case "Reboot":
    ...
    String rawTrigger = decision.evaluationTrigger ?: decision.source ?: "Internal Pipeline"
    String rawHsm = location.hsmStatus ?: "disarmed"
    ...
    if (notificationDevice && allowStandardPush) {
        ...
        String pushMsg = isSaveEvent ? "Mode Manager settings changed and saved." : template
            ...
            .replace("%hsmStatus%", rawHsm)
            .replace("%trigger%", rawTrigger)
...
/* =========================================================================================
   EVENT HANDLERS & SIMULATION ENGINE WITH BOOT GUARD SUPPRESSION
   ========================================================================================= */
...
def appButtonHandler(btn) {
    ...
    } else if (btnName == "btnRestoreGroundTruth") {
        logInfo "Manual UI button pressed: Restoring persisted Ground Truth state snapshot..."
        restorePersistedGroundTruth()
...
def updateSwitchHandler(evt) {
    if (state.isInitializing == true) {
        logTrace "updateSwitchHandler: Event suppressed during boot initialization guard."
        return
    }
...
def updateButtonHandler(evt) {
    if (state.isInitializing == true) {
        logTrace "updateButtonHandler: Event suppressed during boot initialization guard."
        return
    }
...
def presenceHandler(evt) {
    if (state.isInitializing == true) {
        logTrace "presenceHandler: Event suppressed during boot initialization guard."
        return
    }
    logInfo "Master presence sensor '${evt.device?.displayName}' changed to '${evt.value}'"
    processStatePipeline([
        controlSource: "Presence", 
        evaluationTrigger: "Presence Sensor '${evt.device?.displayName}' (${evt.value})", 
        presenceValue: evt.value, 
        source: "Master Presence Sensor"
    ])
}

def homeSwitchHandler(evt) {
    if (state.isInitializing == true) {
        logTrace "homeSwitchHandler: Event suppressed during boot initialization guard."
        return
    }
    ...
    processStatePipeline([
        controlSource: "Presence", 
        evaluationTrigger: "Home Switch '${evt.device?.displayName}' (${evt.value})", 
        presenceValue: presenceVal, 
        source: "Home Switch"
    ])
}
...
def awaySwitchHandler(evt) {
    if (state.isInitializing == true) {
        logTrace "awaySwitchHandler: Event suppressed during boot initialization guard."
        return
    }
    if (consumeExpectedAck(evt)) return
    ...
    processStatePipeline([
        controlSource: "Presence", 
        evaluationTrigger: "Away Switch '${evt.device?.displayName}' (${evt.value})", 
        presenceValue: presenceVal, 
        source: "Away Switch"
    ])
}
...
def alexaPresenceSwitchHandler(evt) {
    if (state.isInitializing == true) {
        logTrace "alexaPresenceSwitchHandler: Event suppressed during boot initialization guard."
        return
    }
    if (consumeExpectedAck(evt)) return
    Boolean isVoiceSource = consumeVoiceMarker()
    String evalSource = isVoiceSource ? "Voice" : "Presence"
    ...
    processStatePipeline([
        ...
        evaluationTrigger: "Alexa Presence Switch '${evt.device?.displayName}' (${evt.value})",
...
def alexaVoiceMarkerSwitchHandler(evt) {
    if (state.isInitializing == true) {
        logTrace "alexaVoiceMarkerSwitchHandler: Event suppressed during boot initialization guard."
        return
    }
    if (evt.value == "on") {
        long currentMs = now()
...
def awakeSwitchHandler(evt) {
    if (state.isInitializing == true) {
        logTrace "awakeSwitchHandler: Event suppressed during boot initialization guard."
        return
    }
    if (consumeExpectedAck(evt)) return
    Boolean isVoiceSource = consumeVoiceMarker()
    Boolean isAwake = (evt.value == "on")
    String evalSource = isVoiceSource ? "Voice" : "Override"
    String srcLabel = isVoiceSource ? 
    ...
    logInfo "Awake switch '${evt.device?.displayName}' changed to '${evt.value}' (Voice Marker Active: ${isVoiceSource} | Attributed Control Source: '${evalSource}')"
    ...
    processStatePipeline([
        controlSource: evalSource,
        evaluationTrigger: "Awake Switch '${evt.device?.displayName}' (${evt.value})",
        isVoiceAttributed: isVoiceSource,
        targetMode: targetMode,
        periodKey: targetKey,
...
def alexaAwakeSwitchHandler(evt) {
    if (state.isInitializing == true) {
        logTrace "alexaAwakeSwitchHandler: Event suppressed during boot initialization guard."
        return
    }
    if (consumeExpectedAck(evt)) return
    ...
    logTrace "Alexa Awake Switch changed to '${evt.value}' (Voice Marker Active: ${isVoiceSource} | Attributed Control Source: '${evalSource}')"
    
    if (!isAwake) {
        processStatePipeline([
            controlSource: evalSource,
            evaluationTrigger: "Alexa Awake Switch '${evt.device?.displayName}' (${evt.value})",
            isVoiceAttributed: isVoiceSource,
            targetMode: sleepMode?.toString() ?: "Sleeping",
            periodKey: "sleeping",
...
def vSwitchHandler(evt) {
    if (state.isInitializing == true) {
        logTrace "vSwitchHandler: Event suppressed during boot initialization guard."
        return
    }
    if (consumeExpectedAck(evt)) return
    ...
    if (targetPeriod?.mode) {
        ...
        processStatePipeline([
            ...
            evaluationTrigger: "Period Switch '${evt.device?.displayName}'",
...
def executeForcedTestEvaluation() {
    ...
    processStatePipeline([
        ...
        evaluationTrigger: "Simulation Panel (Forced Test)",
