...
static String version() { return '1.1.50' }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        // Section 8 dynamic collapse protection & pixel-perfect grid matrix alignment
        section("<b>SECTION 8: Event Trigger Dispatch Matrix & Message Templates</b>", hideable: true, hidden: !keepSection8Open) {
            ...
            // --- PIXEL-PERFECT MATRIX HEADER ROW ---
            paragraph "<table style='width:100%; table-layout:fixed; border-collapse:collapse; background-color:#2C3E50; color:#FFFFFF; border-radius:4px; font-weight:bold; font-size:12px; margin-bottom:10px;'>" +
                      "  <tr>" +
                      "    <th style='width:52%; text-align:left; padding:8px 10px;'>EVENT TRIGGER CAUSE</th>" +
                      "    <th style='width:16%; text-align:center; padding:8px 0;'>PUSH</th>" +
                      "    <th style='width:16%; text-align:center; padding:8px 0;'>PUSH MOBILE</th>" +
                      "    <th style='width:16%; text-align:center; padding:8px 0;'>AUDIO / SPEECH</th>" +
                      "  </tr>" +
                      "</table>"

            // Helper closure to build perfectly aligned matrix rows
            def renderMatrixRow = { String causeKey, String title, String borderColor, String desc, String defaultTemplate, String templateKey ->
                paragraph "<table style='width:100%; table-layout:fixed; border-collapse:collapse; margin-bottom:0px;'>" +
                          "  <tr>" +
                          "    <td style='width:52%; vertical-align:middle; padding:2px 0;'>" +
                          "      <div style='background-color:#F4F6F7; border-left:4px solid ${borderColor}; padding:6px 10px; border-radius:4px; font-weight:bold; font-size:13px; color:#2C3E50;'>${title}</div>" +
                          "      <div style='color:#7F8C8D; font-size:11px; margin-left:10px; margin-top:2px;'>ℹ️ <b>Trigger Cause (%reason%):</b> <code>${desc}</code></div>" +
                          "    </td>" +
                ...
                input name: "push${causeKey}", type: "bool", title: "", defaultValue: false, width: 2
                input name: "pushMobile${causeKey}", type: "bool", title: "", defaultValue: false, width: 2
                input name: "audio${causeKey}", type: "bool", title: "", defaultValue: false, width: 2

                input name: templateKey, type: "text", title: "<b>Message Text Template*</b>", defaultValue: defaultTemplate, required: true, width: 12
                paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0 12px 0;'/>"
            }

            // --- ROW 1: SCHEDULED ---
            renderMatrixRow("Scheduled", "Scheduled Transitions", "#2980B9", "Scheduled (Chronological time period boundaries)", "Mode scheduled transition: %mode% (from %prevMode% at %time%)", "templateScheduled")

            // --- ROW 2: VOICE ---
            renderMatrixRow("Voice", "Voice / Alexa Triggers", "#E67E22", "Voice (Alexa Voice Routine verified via Voice Marker)", "Voice command changed mode to %mode% (Reason: %reason% at %time%)", "templateVoice")

            // --- ROW 3: PRESENCE ---
            renderMatrixRow("Presence", "Presence Changes", "#27AE60", "Presence (Master Presence Sensor or Home/Away toggles)", "Presence update changed mode to %mode% (was %prevMode% at %time%)", "templatePresence")

            // --- ROW 4: OVERRIDE ---
            renderMatrixRow("Override", "Manual Override Changes", "#C0392B", "Override (Dashboard switch toggle or Awake switch override)", "Manual Override activated: Mode set to %mode% (Reason: %reason% at %time%)", "templateOverride")

            // --- ROW 5: REBOOT ---
            renderMatrixRow("Reboot", "Hub Reboot / Startup Synchronization", "#8E44AD", "Reboot (Hub startup/reboot system alignment check)", "System restarted following hub boot. Mode synchronized to %mode% (Reason: %reason% at %time%)", "templateReboot")

            // --- ROW 6: RECHECK NO CHANGE ---
            renderMatrixRow("RecheckNoChange", "Passive Recheck (Unchanged State)", "#7F8C8D", "RecheckNoChange (Periodic recheck where mode/reason are unchanged)", "Mode recheck completed: Still in %mode% (%reason%) at %time%", "templateRecheckNoChange")

            // --- ROW 7: REASON CHANGED ONLY ---
            renderMatrixRow("ReasonOnly", "Evaluation Reason Changed Only (Mode Unchanged)", "#D35400", "ReasonOnly (Dynamic evaluation cause updated while Mode remains identical)", "Evaluation Reason updated to %reason% (Mode remains %mode% at %time%)", "templateReasonOnly")

            // --- ROW 8: MODE CHANGED ONLY ---
            renderMatrixRow("ModeOnly", "Mode Changed Only (Reason Unchanged)", "#16A085", "ModeOnly (Location Mode changed under an ongoing trigger cause)", "Mode changed to %mode% (Reason remains %reason% at %time%)", "templateModeOnly")
        }
