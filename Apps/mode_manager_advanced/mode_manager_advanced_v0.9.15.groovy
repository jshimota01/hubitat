 ...
 *
 *  Change History: (See changelog.txt stored on Git for versions prior to 0.9.0)
 *
 *  v0.9.15 (2026-08-12) - Enforced explicit updateAppLabel() execution after Voice state sync completes to prevent badge label mismatches.
 *                       - Extended notificationDevice dispatches to trigger on reason changes even when location mode is unchanged, and enabled tile notifications during Debug/Trace modes.
 *  v0.9.14 (2026-08-12) - Preserved active Voice state reason inside evaluateAndSetMode() to prevent app label from reverting to Normal.
 *  v0.9.13 (2026-08-12) - Reordered alexaAwakeSwitchHandler execution to set state.modeReason = 'Voice' prior to invoking syncAwakeState().
 *                       - Added Override clear guard on Alexa Voice wake-up to allow unblocking from false Sleeping Override state.
 *  v0.9.12 (2026-08-12) - Refactored isReservationValid() to scan and purge both VSwitch and Mirror maps concurrently for matching device ID.
 *                       - Added Voice reason guard in vSwitchHandler to prevent mode activator app echoes from escalating Voice transitions to Override.
 *  v0.9.11 (2026-08-12) - Standardized isReservationValid() to check and consume both VSwitch and Mirror reservation maps concurrently.
 *                       - Fixed vSwitchHandler reservation clearing bug that allowed virtual indicator switch echo events to trigger Override state.
 *  v0.9.10 (2026-08-12) - Extended reservation lock consumption across both VSwitch and Mirror maps in vSwitchHandler to prevent false Override triggers during state sync.
 *                       - Fixed processReasonTransition("Override") forcing Awake switch ON when target override period is 'sleeping'.
 *  v0.9.9  (2026-08-12) - Increased switch reservation TTL to 5000ms to cover hub execution latency.
 *                       - Added state protection guard preventing virtual indicator switch events from escalating Voice transitions into Override.
 *  v0.9.8  (2026-08-12) - Replaced shared periodHandler callback with 6 dedicated method handlers (periodWeeHoursHandler, periodMorningHandler, etc.) to prevent Hubitat CRON scheduler job overwrites.
 *  v0.9.7  (2026-08-12) - Removed inline ternary fallbacks in evaluateAndSetMode() to enforce explicit Reason authority set by caller.
 *                       - Documented Test Mode as diagnostic-only forced state.
 *                       - Normalized state.lastHsmState cached values to match native location.hsmStatus vocabulary ("armedAway" / "disarmed").
 *  v0.9.6  (2026-08-12) - Added test home/awake toggles and forced state evaluation harness to Step 6 UI.
 *                       - Standardized WARN log format to use uppercase ENABLED / DISABLED text on setting change.
 *  v0.9.5  (2026-08-12) - Implemented timed switch reservation maps (pendingVSwitchSync / pendingMirrorSync) with 3000ms TTL to eliminate mirror loop feedback echo.
 *  v0.9.4  (2026-08-12) - Added Notification Tile integration support with selectable Plain Text or HTML output formatting.
 *  v0.9.3  (2026-08-12) - Refactored initialize() to evaluate existing physical switch states rather than imposing default Normal mode.
 *  v0.9.2  (2026-08-12) - Added unidirectional Alexa Awake Virtual Switch handler for voice routine integration.
 *  v0.9.1  (2026-08-12) - Added dynamic period chronological sequence validator check in preferences page.
 *  v0.9.0  (2026-08-12) - Initial refactor of Mode Manager Advanced with step-by-step preference UI layout and dynamic HTML app label status badge.
 *
 */

static String version() { return '0.9.15' }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        /* App Title Banner & Active Status Card (Always Visible) */
        section() {
        ...
        section("<b>STEP 1: Presence Architecture & State Switches</b>") {
            ...
            input name: "expandStep1", type: "bool", title: "<b>▼ Click to expand/collapse Presence settings</b>", defaultValue: false, submitOnChange: true

            if (settings.expandStep1) {
        ...
        section("<b>STEP 2: Sleep Architecture & State Switches</b>") {
            ...
            input name: "expandStep2", type: "bool", title: "<b>▼ Click to expand/collapse Sleep settings</b>", defaultValue: false, submitOnChange: true

            if (settings.expandStep2) {
        ...
        section("<b>STEP 3: Time Period & Target Mode Schedule</b>") {
            ...
            input name: "expandStep3", type: "bool", title: "<b>▼ Click to expand/collapse Schedule settings</b>", defaultValue: false, submitOnChange: true

            if (settings.expandStep3) {
        ...
        section("<b>STEP 4: Manual Overrides & Triggers</b>") {
            ...
            input name: "expandStep4", type: "bool", title: "<b>▼ Click to expand/collapse Overrides & Triggers settings</b>", defaultValue: false, submitOnChange: true

            if (settings.expandStep4) {
        ...
        section("<b>STEP 5: Logging & System Diagnostics</b>") {
            ...
            input name: "expandStep5", type: "bool", title: "<b>▼ Click to expand/collapse Logging settings</b>", defaultValue: false, submitOnChange: true

            if (settings.expandStep5) {
        ...
        /* Step 6: Testing & Diagnostics */
        section("<b>STEP 6: Testing & Diagnostics Engine</b>") {
            paragraph "<div style='background-color:#FDEDEC; border-left:4px solid #CB4335; padding:8px; border-radius:4px; font-size:12px; margin-bottom:10px;'>" +
                      "<b>Test Control Panel:</b> Force exact state combinations for Home/Away, Awake/Sleeping, Period, and Reason. Executing forced evaluation will update state, command virtual mode switches, and sync Alexa state switches.</div>"

            input name: "expandStep6", type: "bool", title: "<b>▼ Click to expand/collapse Test Control Panel</b>", defaultValue: false, submitOnChange: true

            if (settings.expandStep6) {
                ...
                /* Dynamic Presence Device Warning Banner */
                ...
                input name: "testPeriodKey", type: "enum", title: "<b>Target Time Period / Mode</b>", required: false, submitOnChange: true,
                      options: [
                          "weeHours": "Wee Hours (${weeHoursMode ?: 'Night'})",
                          "earlyMorning": "Early Morning (${earlyMorningMode ?: 'Early Morning'})",
                          "morning": "Morning (${morningMode ?: 'Morning'})",
                          "day": "Day (${dayMode ?: 'Day'})",
                          "evening": "Evening (${eveningMode ?: 'Evening'})",
                          "lateEvening": "Late Evening (${lateEveningMode ?: 'Late Evening'})",
                          "sleeping": "Sleeping (${sleepMode ?: 'Sleeping'})"
                      ]

                input name: "testReason", type: "enum", title: "<b>Forced State Reason</b>", defaultValue: "Test Mode", required: false, submitOnChange: true,
                      options: ["Test Mode": "Test Mode", "Normal": "Normal", "Normal - Rechecked": "Normal - Rechecked", "Owntracks": "Owntracks", "Voice": "Voice", "Override": "Override"]
...
/* Command Virtual Mode Indicator Switch with Timed Reservation (5000ms TTL) */
private void sendVSwitchCommand(def device, String command) {
    ...
    map[devId] = [command: command, expires: now() + 5000]
    state.pendingVSwitchSync = map
    
    logTrace "Queued pending VSwitch reservation '${command}' for '${device.displayName}' (ID: ${devId}, Expires: +5000ms)"
    device."${command}"()
}

/* Command Mirror Switch with Timed Reservation (5000ms TTL) */
private void sendMirrorCommand(def device, String command) {
    ...
    logTrace "Queued pending Mirror reservation '${command}' for '${device.displayName}' (ID: ${devId}, Expires: +5000ms)"
    device."${command}"()
}

/* Helper to validate and consume active reservation maps safely across BOTH VSwitch and Mirror queues */
private boolean isReservationValid(String mapType, String devId, String value) {
    boolean found = false
    long currentMs = now()
    
    /* Purge and inspect VSwitch map */
    if (state.pendingVSwitchSync && state.pendingVSwitchSync.containsKey(devId)) {
        Map map = new HashMap(state.pendingVSwitchSync)
    ...
    /* Purge and inspect Mirror map */
    if (state.pendingMirrorSync && state.pendingMirrorSync.containsKey(devId)) {
        Map map = new HashMap(state.pendingMirrorSync)
        Map reservation = map[devId]
        if (reservation?.expires && currentMs <= (reservation.expires as Long)) {
            found = true
        }
...
/* Start Active Period Boundary Schedules with Unique Method Handlers */
...
/* Convert ISO or Date time setting into daily 7-part CRON string (ss mm HH * * ? *) */
...
/* Schedule Individual Daily Period Boundary Trigger via CRON */
    ...
    if (timeIso && handlerMethod) {
        ...
        if (cronExpr) {
            try {
                ...
                logTrace "Successfully scheduled daily recurring CRON period trigger '${cronExpr}' -> Handler: ${handlerMethod}()"
            } catch (Exception e) {
                logWarn "Unable to schedule CRON period trigger '${cronExpr}' for '${handlerMethod}': ${e.message}"
            }
        }
    }
}

/* Individual Period Boundary Callback Router Handlers */
...
/* Centralized Cron Period Boundary Processing Engine */
private void handlePeriodBoundary(String periodKey) {
    logTrace "Scheduled time period boundary hit for key: '${periodKey}'"
    if (state.modeReason == "Override" && settings.autoReturnNormal == false) {
        logInfo "Period boundary '${periodKey}' hit but autoReturnNormal is disabled (Stay in Override). Preserving Override mode."
        return
    }
    
    logInfo "Period boundary '${periodKey}' hit. Reverting reason to Normal schedule mode."
...
def processReasonTransition(String reason, Map params = [:]) {
    ...
    /* Update App Label early so logging statements display the target transition state */
    updateAppLabel(params.targetMode ?: location.mode, reason)
    ...
    switch (reason) {
        case "Override":
            ...
            /* If Override target is Sleeping, ensure Awake state matches Sleep */
        ...
        case "Voice":
            ...
            /* Re-assert Voice badge AFTER all child switch events finish executing */
            state.modeReason = "Voice"
            updateAppLabel(targetMode ?: location.mode, "Voice")
            
            /* Dispatch notification tile update for Voice transition */
            dispatchTileNotification(targetMode ?: location.mode, "Voice", true)
            break
        ...
        case "Test Mode":
            /* Note: 'Test Mode' is a diagnostic forced state managed by executeForcedTestEvaluation()
               and is not evaluated as a production transition reason. */
            logTrace "Test Mode engaged."
            break
...
/* Forced Test Harness & Diagnostic Evaluator */
def executeForcedTestEvaluation() {
    ...
    String targetReason = settings.testReason ?: "Test Mode"

    logInfo "Executing Forced Test Evaluation -> Home: ${isHomeTarget} | Awake: ${isAwakeTarget} | Period: ${periodKey} | Reason: ${targetReason}"

    /* 1. Sync CRON Period Schedules based on forced reason */
    if (targetReason == "Override" && settings.autoReturnNormal == false) {
        logTrace "Test Harness: Disabling period CRON schedules for forced Override state."
        stopPeriodSchedules()
    } else {
        logTrace "Test Harness: Re-arming active period CRON schedules for forced '${targetReason}' state."
        startPeriodSchedules()
    }

    /* 2. Determine Target Location Mode based on forced state parameters */
    ...
    if (!isHomeTarget) {
        ...
        Map<String, String> periodToModeMap = [
            ...
            "sleeping": sleepMode?.toString()
        ]
        targetMode = periodToModeMap[periodKey] ?: location.mode
    }

    /* 3. Force Home / Away switches and sync Alexa Mode Switch */
    String homeCmd = isHomeTarget ? "on" : "off"
    syncHomeState(homeCmd)

    /* 4. Force Awake / Sleep switches and sync Alexa Awake Switch */
    String awakeCmd = isAwakeTarget ? "on" : "off"
    syncAwakeState(awakeCmd)

    if (alexaAwakeSwitch) {
        String alexaAwakeState = isAwakeTarget ? "on" : "off"
        if (alexaAwakeSwitch.currentValue("switch") != alexaAwakeState) {
            sendVSwitchCommand(alexaAwakeSwitch, alexaAwakeState)
        }
    }

    /* 5. Update Location Mode */
    if (targetMode && location.mode != targetMode) {
        logInfo "Test Harness: Changing location mode from '${location.mode}' to '${targetMode}'"
        setLocationMode(targetMode)
    }

    /* 6. Sync Virtual Indicator Switches (Uses timed switch reservations) */
    updateVirtualModeSwitches(periodKey)

    /* 7. Lock in explicit forced reason and refresh dynamic App Label AFTER all switch syncs finish */
    state.modeReason = targetReason
    updateAppLabel(targetMode, targetReason)
}
...
def vSwitchHandler(evt) {
    ...
    if (state.modeReason == "Voice") {
        logInfo "Period virtual switch '${evt.device.displayName}' event suppressed during active Voice transition."
        return
    }
...
def alexaAwakeSwitchHandler(evt) {
    ...
    /* Lock in Voice authority IMMEDIATELY so child event triggers recognize active Voice state */
    state.modeReason = "Voice"
...
def evaluateAndSetMode() {
    ...
    /* Determine priority active reason (preserve Voice, Owntracks, or Override if set) */
    String currentReason = state.modeReason
    if (!currentReason || currentReason == "Initialization / Idle") {
        currentReason = "Normal"
    }

    /* 1. If in Override, do not evaluate normal flow */
    if (currentReason == "Override") {
    ...
    if (!isHome) {
        ...
        changeMode("Away", null, currentReason)
        return
    } else {
        if (getSettingBool("manageHSM", true) && state.lastHsmState != "disarmed") {
            logDebug "Disarming Hubitat Safety Monitor (HSM: disarm)"
    ...
    if (isSleeping) {
        ...
        changeMode(sleepMode?.toString(), "sleeping", currentReason)
        return
    }
    ...
    if (periodInfo?.mode) {
        ...
        changeMode(periodInfo.mode, periodInfo.key, currentReason)
    } else {
...
/* Centralized Notification Device Dispatch Routine */
private void dispatchTileNotification(String modeVal, String reasonVal, boolean forceDispatch = false) {
    if (!notificationDevice) return

    Boolean isVerboseLogging = getSettingBool("logDebugEnable", true) || getSettingBool("logTraceEnable", false)

    if (forceDispatch || isVerboseLogging) {
        String msg = ""
        if (settings.tileFormat == "html") {
            msg = "<div style='font-size:13px; font-weight:bold; color:#27AE60;'>Mode is now ${modeVal}</div><div style='font-size:11px; color:#7F8C8D; font-style:italic;'>Reason: ${reasonVal}</div>"
        } else {
            msg = "Mode is now ${modeVal} (Reason: ${reasonVal})"
        }

        logTrace "Dispatching Notification Tile output: '${msg}'"
...
def changeMode(String newMode, String activePeriodKey = null, String reason = null) {
    ...
    String oldMode = location.mode
    Boolean modeChanged = (oldMode != newMode)

    if (modeChanged) {
        logInfo "Changing Hubitat Location Mode from '${oldMode}' to '${newMode}' | Reason: ${state.modeReason}"
        setLocationMode(newMode)
        dispatchTileNotification(newMode, state.modeReason, true)
    } else {
        dispatchTileNotification(newMode, state.modeReason, false)
    }
    ...
    /* Emit completion log event if location mode was altered */
    if (modeChanged) {
        logInfo "Hubitat Location Mode changed from '${oldMode}' to '${newMode}' | Reason: ${state.modeReason}"
    }
}
...
def updateAppLabel(String currentMode = null, String reason = null) {
    ...
    switch (currentReason) {
        ...
        case "Test Mode":
            formattedLabel = "${baseLabel} - [<span style='color:orange; font-weight:bold;'>${displayMode} (Test Mode)</span>]"
            break
