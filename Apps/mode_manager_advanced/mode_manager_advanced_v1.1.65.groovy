 ...
 *                        - Converted HTML push notification wrappers from block <div> to inline <span> tags to prevent unwanted line breaks before/after timestamps on notification tiles.
...
static String version() { return '1.1.65' }
...
private void dispatchTileNotification(Map decision, String overrideReasonVal = null, Boolean isSleepingState = false) {
    ...
    if (notificationDevice && allowStandardPush) {
        ...
        if (useHtmlPush) pushMsg = "<span style='white-space: nowrap; font-size:1.0em;'>${pushMsg}</span>"
    ...
    if (notificationDeviceMobile && allowMobilePush) {
        ...
        if (useHtmlPushMobile) pushMsgMobile = "<span style='white-space: nowrap; font-size:1.0em;'>${pushMsgMobile}</span>"
