/**
 *  Multiperiod Environment Scheduler
 *  Schedules 6 heating/cooling setpoint windows with mode overrides, live device tracking, dynamic fan circulation, and AQI Air Quality filtering.
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "Multiperiod Environment Scheduler Setup v${version()}", install: true, uninstall: true) {
...
def modeChangeHandler(evt) {
    logInfo "Hub Mode changed to: ${evt.value}. Re-evaluating schedule."
    evaluateSchedule()
}
...
def evaluateSchedule() {
    if (!appEnabled) {
        logInfo "Multiperiod Environment is currently DISABLED via main switch."
...
def aqiHandler(evt) {
    logDebug "Air Quality Sensor Event (${evt.device.displayName}): ${evt.value}"
    evaluateAirQuality()
}
...
private void sendIfChanged(Map args) {
    ...
    // Handles state evaluation within SmartApp context or device context safely
...
private void logMessage(String level, String msg) {
    if (settings["log${level.capitalize()}Enable"] == true) {
        log."${level}" "${app.label ?: 'Multiperiod Environment Scheduler'}${level == 'warn' ? ' WARNING' : level == 'error' ? ' ERROR' : ''}: ${msg}"
    }
}
