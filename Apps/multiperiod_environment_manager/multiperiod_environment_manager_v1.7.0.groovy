...
def timeStamp() { return "2026/09/02 02:00 PM" }
...
private int getBoundaryMinutes(String prefix, String defaultHour, String defaultAmPm) {
    String hStr = settings["${prefix}Hour"]?.toString() ?: defaultHour
    String mStr = settings["${prefix}Min"]?.toString() ?: "00"
    String ap = settings["${prefix}AmPm"]?.toString() ?: defaultAmPm
    
    int hrs = safeToInt(hStr, safeToInt(defaultHour, 6))
    int mins = safeToInt(mStr, 0)
...
private Map validateConfiguration() {
    ...
    int mMin = getBoundaryMinutes("mStart", "06", "AM")
    int dMin = getBoundaryMinutes("dStart", "09", "AM")
    int eMin = getBoundaryMinutes("eStart", "05", "PM")
    int nMin = getBoundaryMinutes("nStart", "10", "PM")

    if (!(mMin < dMin && dMin < eMin && eMin < nMin)) {
        warnings.add("Schedule period boundary times do not follow strict chronological order (Morning < Day < Evening < Night). Evaluation sequence may produce unexpected window matching.")
    }
