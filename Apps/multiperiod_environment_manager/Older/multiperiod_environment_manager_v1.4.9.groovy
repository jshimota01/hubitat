...
static String version() { return '1.4.9' }
def timeStamp() { return "2026/09/02 10:15 AM" }
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 1: House HVAC Controller (Primary)</b>", hideable: true, hidden: true) {
            ...
            paragraph "<div style='color:#2980B9; font-size:11px; margin-top:-6px; margin-bottom:8px;'>" +
                      "<b>When disabled:</b> Scheduled target setpoints are suspended. The House HVAC unit holds its existing setpoints unless overridden by Away/Sleeping mode switches.</div>"

            paragraph "<span style='color:#2C3E50; font-weight:bold; font-size:13px;'>Setpoints for Periods</span>"
            
            // Interactive HTML Spinner Table for House Setpoints
            String inputStyle = "width:65px; padding:3px; font-size:13px; text-align:center; border:1px solid #ccc; border-radius:4px;"
            
            StringBuilder d1Table = new StringBuilder()
            d1Table.append("<table style='width:100%; border-collapse:collapse; font-size:13px; margin:8px 0; text-align:left;'>")
            d1Table.append("<tr style='border-bottom:2px solid #2C3E50; background-color:#F2F4F4;'>")
            d1Table.append("<th style='padding:6px;'>Period</th>")
            d1Table.append("<th style='padding:6px;'>Cooling (°F) <i>(if temp exceeds)</i></th>")
            d1Table.append("<th style='padding:6px;'>Heating (°F) <i>(if temp falls below)</i></th>")
            d1Table.append("</tr>")

            List d1Periods = [
                [name: "Morning", coolKey: "d1MorningCool", heatKey: "d1MorningHeat", defaultCool: 72, defaultHeat: 68],
                [name: "Day",     coolKey: "d1DayCool",     heatKey: "d1DayHeat",     defaultCool: 75, defaultHeat: 66],
                [name: "Evening", coolKey: "d1EveningCool", heatKey: "d1EveningHeat", defaultCool: 73, defaultHeat: 69],
                [name: "Night",   coolKey: "d1NightCool",   heatKey: "d1NightHeat",   defaultCool: 75, defaultHeat: 64]
            ]

            d1Periods.each { p ->
                def coolVal = settings[p.coolKey] != null ? settings[p.coolKey] : (state[p.coolKey] != null ? state[p.coolKey] : p.defaultCool)
                def heatVal = settings[p.heatKey] != null ? settings[p.heatKey] : (state[p.heatKey] != null ? state[p.heatKey] : p.defaultHeat)
                
                d1Table.append("<tr style='border-bottom:1px solid #E0E0E0;'>")
                d1Table.append("<td style='padding:6px;'><b>${p.name}</b></td>")
                d1Table.append("<td style='padding:6px;'><input type='number' name='${p.coolKey}' value='${coolVal}' min='50' max='90' style='${inputStyle}' /></td>")
                d1Table.append("<td style='padding:6px;'><input type='number' name='${p.heatKey}' value='${heatVal}' min='50' max='90' style='${inputStyle}' /></td>")
                d1Table.append("</tr>")
            }
            d1Table.append("</table>")
            paragraph d1Table.toString()
        }
        ...
        section("<b>SECTION 2: Office HVAC Controller (Secondary / Floor Heater)</b>", hideable: true, hidden: true) {
            ...
            paragraph "<div style='color:#2980B9; font-size:11px; margin-top:-6px; margin-bottom:8px;'>" +
                      "<b>When disabled:</b> Scheduled target setpoints are suspended. The Office HVAC unit holds its existing setpoints unless overridden by Away/Sleeping modes or Safety Forced-OFF triggers.</div>"

            paragraph "<span style='color:#2C3E50; font-weight:bold; font-size:13px;'>Setpoints for Periods</span>"

            // Interactive HTML Spinner Table for Office Setpoints
            String inputStyle = "width:65px; padding:3px; font-size:13px; text-align:center; border:1px solid #ccc; border-radius:4px;"

            StringBuilder d2Table = new StringBuilder()
            d2Table.append("<table style='width:100%; border-collapse:collapse; font-size:13px; margin:8px 0; text-align:left;'>")
            d2Table.append("<tr style='border-bottom:2px solid #2C3E50; background-color:#F2F4F4;'>")
            d2Table.append("<th style='padding:6px;'>Period</th>")
            d2Table.append("<th style='padding:6px;'>Cooling (°F) <i>(if temp exceeds)</i></th>")
            d2Table.append("<th style='padding:6px;'>Heating (°F) <i>(if temp falls below)</i></th>")
            d2Table.append("</tr>")

            List d2Periods = [
                [name: "Morning", coolKey: "d2MorningCool", heatKey: "d2MorningHeat", defaultCool: 72, defaultHeat: 68],
                [name: "Day",     coolKey: "d2DayCool",     heatKey: "d2DayHeat",     defaultCool: 75, defaultHeat: 66],
                [name: "Evening", coolKey: "d2EveningCool", heatKey: "d2EveningHeat", defaultCool: 73, defaultHeat: 69],
                [name: "Night",   coolKey: "d2NightCool",   heatKey: "d2NightHeat",   defaultCool: 78, defaultHeat: 62]
            ]
            ...
            d2Periods.each { p ->
                ...
                d2Table.append("<tr style='border-bottom:1px solid #E0E0E0;'>")
                d2Table.append("<td style='padding:6px;'><b>${p.name}</b></td>")
                d2Table.append("<td style='padding:6px;'><input type='number' name='${p.coolKey}' value='${coolVal}' min='50' max='90' style='${inputStyle}' /></td>")
                d2Table.append("<td style='padding:6px;'><input type='number' name='${p.heatKey}' value='${heatVal}' min='50' max='90' style='${inputStyle}' /></td>")
                d2Table.append("</tr>")
            }
            d2Table.append("</table>")
            paragraph d2Table.toString()
        }
        ...
        section("<b>SECTION 4: System Overrides & Safety Controls</b>", hideable: true, hidden: true) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
                      "Configure setpoints or force devices <b>OFF</b> when Away or Sleeping switches are active.</div>"

            // --- AWAY OVERRIDE GROUP ---
            paragraph "<div style='background-color:#F2F4F4; padding:6px; font-weight:bold; border-left:4px solid #2980B9; margin-bottom:8px;'>1. Away Mode Overrides</div>"
            ...
            input name: "awayDev1Enabled", type: "bool", title: "<b>Enable House HVAC Away Override</b>", defaultValue: true, submitOnChange: false
            input name: "awayDev1Heat", type: "number", title: "House Away Heat Setpoint (°F)", required: true, defaultValue: 62, width: 6
            input name: "awayDev1Cool", type: "number", title: "House Away Cool Setpoint (°F)", required: true, defaultValue: 78, width: 6
            
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:6px 0;'/>"
            
            input name: "awayDev2Enabled", type: "bool", title: "<b>Enable Office HVAC Away Override</b>", defaultValue: true, submitOnChange: false
            input name: "awayDev2ForceOff", type: "bool", title: "<b>Force Office HVAC OFF (Safety Control)</b>", defaultValue: true, submitOnChange: false
            input name: "awayDev2Heat", type: "number", title: "Office Away Heat Setpoint (°F)", required: true, defaultValue: 60, width: 6
            input name: "awayDev2Cool", type: "number", title: "Office Away Cool Setpoint (°F)", required: true, defaultValue: 80, width: 6

            // --- SLEEPING OVERRIDE GROUP ---
            paragraph "<div style='background-color:#F2F4F4; padding:6px; font-weight:bold; border-left:4px solid #8E44AD; margin-top:12px; margin-bottom:8px;'>2. Sleeping Mode Overrides</div>"
            ...
            input name: "sleepDev1Heat", type: "number", title: "House Sleeping Heat Setpoint (°F)", required: true, defaultValue: 65, width: 6
            input name: "sleepDev1Cool", type: "number", title: "House Sleeping Cool Setpoint (°F)", required: true, defaultValue: 74, width: 6
            ...
            input name: "sleepDev2ForceOff", type: "bool", title: "<b>Force Office HVAC OFF (Safety Control)</b>", defaultValue: true, submitOnChange: false
            input name: "sleepDev2Heat", type: "number", title: "Office Sleeping Heat Setpoint (°F)", required: true, defaultValue: 62, width: 6
            input name: "sleepDev2Cool", type: "number", title: "Office Sleeping Cool Setpoint (°F)", required: true, defaultValue: 76, width: 6
        }
        ...
        section("<b>SECTION 5: Automatic Fan Circulation Control</b>", hideable: true, hidden: true) {
            ...
            input name: "fanOnMinutes", type: "number", title: "Standard Fan ON Duration (1-240 Minutes)", required: true, defaultValue: 10, width: 6
            input name: "fanOffMinutes", type: "number", title: "Standard Fan OFF / AUTO Duration (1-240 Minutes)", required: true, defaultValue: 50, width: 6
            
            paragraph "<div style='background-color:#F2F4F4; padding:6px; font-weight:bold; border-left:4px solid #8E44AD; margin-top:10px; margin-bottom:6px;'>Mode Sleeping Circulation Schedule</div>"
            input name: "fanSleepOnMinutes", type: "number", title: "Sleeping Fan ON Duration (1-240 Minutes)", required: true, defaultValue: 5, width: 6
            input name: "fanSleepOffMinutes", type: "number", title: "Sleeping Fan OFF / AUTO Duration (1-240 Minutes)", required: true, defaultValue: 115, width: 6
        }
...
private void syncMatrixInputsToState() {
    List keys = [
        "d1MorningCool", "d1MorningHeat", "d1DayCool", "d1DayHeat", "d1EveningCool", "d1EveningHeat", "d1NightCool", "d1NightHeat",
        "d2MorningCool", "d2MorningHeat", "d2DayCool", "d2DayHeat", "d2EveningCool", "d2EveningHeat", "d2NightCool", "d2NightHeat"
    ]
    
    keys.each { key ->
        if (settings[key] != null) {
            state[key] = safeToDecimal(settings[key])
        }
    }
}
...
private Map getCalculatedScheduleData() {
    ...
    if (isAway) {
        ...
        switch(activePeriod) {
            case "Morning":
                d1TargetHeat = safeToDecimal(settings.d1MorningHeat ?: state.d1MorningHeat, 68.0)
                d1TargetCool = safeToDecimal(settings.d1MorningCool ?: state.d1MorningCool, 72.0)
                d2TargetHeat = safeToDecimal(settings.d2MorningHeat ?: state.d2MorningHeat, 68.0)
                d2TargetCool = safeToDecimal(settings.d2MorningCool ?: state.d2MorningCool, 72.0)
                break
            case "Day":
                d1TargetHeat = safeToDecimal(settings.d1DayHeat ?: state.d1DayHeat, 66.0)
                d1TargetCool = safeToDecimal(settings.d1DayCool ?: state.d1DayCool, 75.0)
                d2TargetHeat = safeToDecimal(settings.d2DayHeat ?: state.d2DayHeat, 66.0)
                d2TargetCool = safeToDecimal(settings.d2DayCool ?: state.d2DayCool, 75.0)
                break
            case "Evening":
                d1TargetHeat = safeToDecimal(settings.d1EveningHeat ?: state.d1EveningHeat, 69.0)
                d1TargetCool = safeToDecimal(settings.d1EveningCool ?: state.d1EveningCool, 73.0)
                d2TargetHeat = safeToDecimal(settings.d2EveningHeat ?: state.d2EveningHeat, 69.0)
                d2TargetCool = safeToDecimal(settings.d2EveningCool ?: state.d2EveningCool, 73.0)
                break
            case "Night":
                d1TargetHeat = safeToDecimal(settings.d1NightHeat ?: state.d1NightHeat, 64.0)
                d1TargetCool = safeToDecimal(settings.d1NightCool ?: state.d1NightCool, 75.0)
                d2TargetHeat = safeToDecimal(settings.d2NightHeat ?: state.d2NightHeat, 62.0)
                d2TargetCool = safeToDecimal(settings.d2NightCool ?: state.d2NightCool, 78.0)
                break
        }
    }
...
def manageFanCirculation(Boolean isInitial = false, String targetPhase = null) {
    ...
    Integer safeOn = isSleeping ? safeFanDuration(fanSleepOnMinutes, 5) : safeFanDuration(fanOnMinutes, 10)
    Integer safeOff = isSleeping ? safeFanDuration(fanSleepOffMinutes, 115) : safeFanDuration(fanOffMinutes, 50)
...
private Map validateConfiguration() {
    ...
    if (fanCirculateEnabled) {
        Integer onMin = safeToInt(fanOnMinutes, 10)
        Integer offMin = safeToInt(fanOffMinutes, 50)
        Integer sleepOnMin = safeToInt(fanSleepOnMinutes, 5)
        Integer sleepOffMin = safeToInt(fanSleepOffMinutes, 115)
