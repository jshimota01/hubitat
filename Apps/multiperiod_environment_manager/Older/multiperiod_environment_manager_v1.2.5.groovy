...
def timeStamp() { return "2026/08/31 10:15 PM" }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        // FIX: Force all sections to default to collapsed (hidden: true) on page load
        ...
        section("<b>SECTION 7: System Operational Status Summary</b>", hideable: true, hidden: isCollapsed) {
            paragraph "<div style='background-color:#F8F9FA; border-left:4px solid #2980B9; padding:10px; border-radius:4px; font-size:12px;'>" +
                      ...
                      "<b>Auto-Changeover Mode Engine:</b> ${autoChangeoverEnabled ? 'Active (Hysteresis Buffer: ' + (tempDeadband ?: 2) + '°)' : 'Disabled'}<br/>" +
    ...
    if (getSettingBool("autoChangeoverEnabled", true) && currentTemp != null) {
        ...
        try {
            if (currentMode == "cool" && currentTemp <= targetHeat) {
                logInfo "Auto-Changeover: Temp (${currentTemp}°) dropped <= Heat Setpoint (${targetHeat}°). Changing mode from COOL to HEAT."
                ...
                logInfo "Auto-Changeover: Temp (${currentTemp}°) rose >= Cool Setpoint (${targetCool}°). Changing mode from HEAT to COOL."
                state.modeChangePending = true
                runIn(10, "clearModeChangeGuard")
                targetThermostat.cool()
            } else if (currentMode == "off" || currentMode == "auto") {
                if (currentTemp <= (targetHeat - deadband)) {
                    logInfo "Auto-Changeover: Temp (${currentTemp}°) breached Heat Deadband (${targetHeat - deadband}°). Setting mode to HEAT."
                    state.modeChangePending = true
                    runIn(10, "clearModeChangeGuard")
                    targetThermostat.heat()
                } else if (currentTemp >= (targetCool + deadband)) {
                    logInfo "Auto-Changeover: Temp (${currentTemp}°) breached Cool Deadband (${targetCool + deadband}°). Setting mode to COOL."
                    state.modeChangePending = true
                    runIn(10, "clearModeChangeGuard")
                    targetThermostat.cool()
                }
            } else {
                logDebug "Auto-Changeover: Temp (${currentTemp}°) is within setpoints and hysteresis buffer. Holding current mode '${currentMode}'."
            }
        } catch (Exception e) {
