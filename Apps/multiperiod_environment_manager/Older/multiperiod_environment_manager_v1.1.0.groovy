 ...
 *
 *  Instructions:
 *  1. Select primary physical thermostat device.
 *  2. Configure switch overrides, time-based setpoint schedules, and auto-changeover settings.
...
// [KEEP-EXACT] See possible changelog.txt for past changelog history for pre-0.10.0 versions.

static String version() { return '1.1.0' }
def timeStamp() { return "2026/08/31 04:38 PM" }
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 1: Control & Target Thermostat Device</b>", hideable: true, hidden: isCollapsed) {
            ...
            if (autoChangeoverEnabled) {
                input name: "tempDeadband", type: "number", title: "Setpoint Deadband Buffer (°F)", description: "Degree offset to prevent rapid short-cycling", defaultValue: 2, required: true, width: 6
            }
        }
        ...
        section("<b>SECTION 7: System Operational Status Summary</b>", hideable: true, hidden: isCollapsed) {
            paragraph "<div style='background-color:#F8F9FA; border-left:4px solid #2980B9; padding:10px; border-radius:4px; font-size:12px;'>" +
                      ...
                      "<b>Thermostat Telemetry:</b> Temp: ${tStates.temp ?: '--'}° | Heat: ${devHeat}° | Cool: ${devCool}° | Mode: ${devThermostatMode} | Fan: ${devFanMode} | Operating State: ${devOpState}<br/>" +
                      ...
                      "<b>Auto-Changeover Mode Engine:</b> ${autoChangeoverEnabled ? 'Active (Deadband: ' + (tempDeadband ?: 2) + '°)' : 'Disabled'}<br/>" +
...
private void initialize(Boolean isInstall = false) {
    ...
    // Subscribe to Target Thermostat Ambient Temperature updates for auto-changeover
    if (targetThermostat) {
...
def executeEvaluationPass(String cause = "Manual / Scheduled", Boolean forceSetpointApply = false) {
    ...
    // STEP 2: DISPATCH THERMOSTAT SETPOINTS & AUTO-CHANGEOVER (USES SINGLE-PASS SNAPSHOT)
...
/**
 * Dispatches heating and cooling setpoints and evaluates ambient room temperature to automatically 
 * select physical HEAT or COOL system mode[cite: 1, 2].
 */
private void applySetpointsAndEvaluateMode(BigDecimal targetHeat, BigDecimal targetCool, Map thermostat = null, Boolean forceSetpointApply = false) {
    ...
    // 1. Dispatch Setpoints if forced or changed
    try {
    ...
    // 2. Evaluate Automatic System Mode Changeover
    if (getSettingBool("autoChangeoverEnabled", true) && currentTemp != null) {
        BigDecimal deadband = safeToDecimal(tempDeadband, 2.0)
        
        try {
            // Heating Call: Temp drops below or equal to target heat setpoint
            if (currentTemp <= targetHeat && currentMode != "heat") {
                logInfo "Auto-Changeover: Ambient temp (${currentTemp}°) <= Heat Setpoint (${targetHeat}°). Setting thermostat mode to HEAT."
                targetThermostat.heat()
            // Cooling Call: Temp rises above or equal to target cool setpoint
            } else if (currentTemp >= targetCool && currentMode != "cool") {
                logInfo "Auto-Changeover: Ambient temp (${currentTemp}°) >= Cool Setpoint (${targetCool}°). Setting thermostat mode to COOL."
                targetThermostat.cool()
            // Deadband Idle Check: Turn off heating/cooling if ambient temp settles comfortably within deadband buffer
            } else if (currentMode == "heat" && currentTemp >= (targetHeat + deadband)) {
                logInfo "Auto-Changeover: Ambient temp (${currentTemp}°) reached Heat Target + Deadband Buffer (${targetHeat + deadband}°). Leaving mode in HEAT for thermostat internal hysteresis."
            } else if (currentMode == "cool" && currentTemp <= (targetCool - deadband)) {
                logInfo "Auto-Changeover: Ambient temp (${currentTemp}°) reached Cool Target - Deadband Buffer (${targetCool - deadband}°). Leaving mode in COOL for thermostat internal hysteresis."
            } else {
                logDebug "Auto-Changeover: Ambient temp (${currentTemp}°) within deadband (${targetHeat}° - ${targetCool}°). Maintaining mode '${currentMode}'."
            }
        } catch (Exception e) {
...
def manageFanCirculation(Boolean isInitial = false, String targetPhase = null) {
    ...
    try {
        if (executeOnPhase) {
            ...
            Boolean issuedAutoCommand = false
            if (opState != "heating" && opState != "cooling") {
                if (thermostat.fan != "auto") {
                    ...
                    targetThermostat.fanAuto()
                    issuedAutoCommand = true
                }
            } else {
                logDebug "Fan Circulation Loop: HVAC actively ${opState}. Leaving fan mode undisturbed."
            }
            
            if (issuedAutoCommand || thermostat.fan == "auto") {
                sendIfChangedStateValue("fanOwned", false)
            } else {
                logDebug "Fan Circulation Loop: Retaining fan ownership state while fan remains active due to HVAC ${opState}."
            }
