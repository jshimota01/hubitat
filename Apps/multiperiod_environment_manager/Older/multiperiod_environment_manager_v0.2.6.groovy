 ...
 *      ----          ------        -------     ----                                                ---
 *      2026-06-01    jshimota      0.0.1       Initial release[cite: 1, 4]
 *      2026-08-01    Gemini        0.0.4       Added Air Filter AQI control[cite: 1, 4]
 *      2026-08-05    jshimota      0.0.5       Implemented unified logging, auto-debug disable, and sendIfChanged routine[cite: 1, 4]
 *      2026-08-05    jshimota      0.0.6       Renamed app to Multiperiod Environment Manager[cite: 1, 4]
 *      2026-08-05    Gemini        0.0.7       Bug check fix: quoted subscribe/schedule handlers, fixed boundary condition in isTimeBetween[cite: 1, 4]
 *      2026-08-05    jshimota      0.0.8       Added child tile device support and HTML memTile attribute output[cite: 1, 4]
 *      2026-08-05    Gemini        0.0.9       Replaced Mode overrides with Switch devices, split Air Filter into separate section[cite: 1, 4]
 *      2026-08-05    Gemini        0.1.0       Made Air Filter independently toggleable[cite: 1, 4]
 *      2026-08-05    Gemini        0.1.1       Updated AQI sensor pick list to Hubitat capability.airQuality[cite: 1, 4]
 *      2026-08-05    jshimota      0.1.2       Execute immediate AQI evaluation and average upon pressing Done (installed/updated)[cite: 1, 4]
 *      2026-08-05    jshimota      0.1.3       Renamed section to Room Air Filter Control, default ON, added numerical AQI threshold table[cite: 1, 4]
 *      2026-08-05    jshimota      0.1.4       Added AQI thresholds and color table to Air Quality Monitoring section UI[cite: 1, 4]
 *      2026-08-05    jshimota      0.1.5       Immediate Air Filter switch status evaluation on app save/initialize[cite: 1, 4]
 *      2026-08-05    Gemini        0.1.6       Fixed AQI evaluation null guard on initialize, forced direct switch state refresh[cite: 1, 4]
 *      2026-08-05    Gemini        0.1.8       Made AQI range thresholds fully configurable via app settings[cite: 1, 4]
 *      2026-08-05    Gemini        0.1.9       Execute dynamic AQI evaluation directly inside mainPage UI render pass[cite: 1, 4]
 *      2026-08-05    Gemini        0.2.0       Fixed UnknownDeviceTypeException by switching child device type to 'Virtual Variable'[cite: 1, 4]
 *      2026-08-05    jshimota      0.2.1       Fixed ensureChildDevice syntax error and map bracket structure[cite: 4]
 *      2026-08-05    Gemini        0.2.2       Updated ensureChildDevice to target custom 'MEM Dashboard Tile' driver[cite: 4]
 *      2026-08-05    Gemini        0.2.3       Completed comprehensive 17-point verification audit and code pass[cite: 4]
 *      2026-08-05    Gemini        0.2.4       Refactored transient state variables into local variables to reduce DB writes[cite: 4]
 *      2026-08-05    Gemini        0.2.5       Added memTile HTML string comparison guard to suppress redundant tile events[cite: 4]
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "Multiperiod Environment Manager Setup v${version()}", install: true, uninstall: true) {
        ...
        section("<b>Current Status</b>") {
            ...
            paragraph "--------------------------------------------------"
            if (aqiEnabled) {
                paragraph "<b>AQI Sensor 1 (${aqiSensor1?.displayName ?: 'None'}):</b> ${aqiMetrics.v1 ?: '--'}"
                paragraph "<b>AQI Sensor 2 (${aqiSensor2?.displayName ?: 'None'}):</b> ${aqiMetrics.v2 ?: '--'}"
                paragraph "<b>Average AQI:</b> ${aqiMetrics.avgAqi ?: '--'} (${aqiMetrics.aqiStatus ?: 'Not Evaluated'})"
            } else {
...
def evaluateSchedule() {
    ...
    if (!appEnabled) {
        ...
        sendIfChanged([name: "activeSet", value: sched.activePeriod])
        updateMemTile()
        return
    }
    ...
    // Update essential persistent tracking via sendIfChanged
    sendIfChanged([name: "activeSet", value: sched.activePeriod])
    ...
    if (sched.targetHeat != null && sched.targetCool != null && targetThermostat) {
        applySetpoints(sched.targetHeat, sched.targetCool)
    } else {
...
def evaluateAirQuality() {
    ...
    return [v1: v1, v2: v2, avgAqi: avgAqi, aqiStatus: aqiStatus]
}
...
private void updateMemTile(Integer currentAvgAqi = null) {
    ...
    String avgAqi = currentAvgAqi != null ? "${currentAvgAqi}" : "N/A"
    ...
    // Deduplicated event dispatching via sendIfChanged
