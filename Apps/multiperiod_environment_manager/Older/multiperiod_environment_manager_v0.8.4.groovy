 ...
 */
static String version() { return '0.8.4' }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "Multiperiod Environment Manager Setup v${version()}", install: true, uninstall: true) {
        ...
        section("<b>Air Quality Monitoring & RGB Indicator</b>") {
            ...
            Integer tGoodMax = limits.good
            Integer tModMax = limits.mod
            Integer tSensMax = limits.sens
            Integer tUnhealthMax = limits.unhealthy
            Integer tVeryUnhealthMax = limits.veryUnhealthy
            ...
            colorTableSb.append("<table style='width:100%; border-collapse:collapse; font-size:12px; margin-bottom:10px; text-align:left;'>")
                       ...
                       .append("</tr>")
                       .append("<tr style='border-bottom:1px solid #eee;'>")
                       .append("<td style='padding:4px;'>").append(tSensMax + 1).append(" – ").append(tUnhealthMax).append("</td><td style='padding:4px; color:red;'><b>Unhealthy</b></td><td style='padding:4px;'>🔴 Red</td><td style='padding:4px;'>Active danger. Everyone may experience adverse effects.</td>")
                       .append("</tr>")
                       .append("<tr style='border-bottom:1px solid #eee;'>")
                       .append("<td style='padding:4px;'>").append(tUnhealthMax + 1).append(" – ").append(tVeryUnhealthMax).append("</td><td style='padding:4px; color:purple;'><b>Very Unhealthy</b></td><td style='padding:4px;'>🟣 Purple</td><td style='padding:4px;'>Health alert. High risk of respiratory irritation.</td>")
                       .append("</tr>")
                       .append("<tr>")
        ...
        section("<b>Room Air Filter Control</b>") {
            paragraph "Independently turns on a Room Air Filter device based on calculated AQI thresholds."
            ...
            aqiTableSb.append("<table style='width:100%; border-collapse:collapse; font-size:12px; margin-bottom:10px; text-align:left;'>")
                     .append("<tr style='background-color:#f2f2f2; border-bottom:1px solid #ccc;'>")
                     .append("<th style='padding:4px;'>AQI Range</th><th style='padding:4px;'>Level of Concern</th><th style='padding:4px;'>Indicator Light</th><th style='padding:4px;'>Room Air Filter State</th>")
                     .append("</tr>")
                     .append("<tr style='border-bottom:1px solid #eee;'>")
                     ...
                     .append("</tr>")
                     .append("<tr style='border-bottom:1px solid #eee;'>")
                     .append("<td style='padding:4px;'>").append(tSensMax + 1).append(" – ").append(tUnhealthMax).append("</td><td style='padding:4px; color:red;'><b>Unhealthy</b></td><td style='padding:4px;'>🔴 Red</td><td style='padding:4px; color:green;'><b>ON</b></td>")
                     .append("</tr>")
                     .append("<tr style='border-bottom:1px solid #eee;'>")
                     .append("<td style='padding:4px;'>").append(tUnhealthMax + 1).append(" – ").append(tVeryUnhealthMax).append("</td><td style='padding:4px; color:purple;'><b>Very Unhealthy</b></td><td style='padding:4px;'>🟣 Purple</td><td style='padding:4px; color:green;'><b>ON</b></td>")
                     .append("</tr>")
                     .append("<tr>")
...
def updated() {
    ...
    if (aqiEnabled && !validateAqiThresholds()) {
        logError "AQI Threshold inputs are invalid! Values must be strictly ascending (0 < Good < Mod < Sens < Unhealthy < VeryUnhealthy). Applying default boundaries."
    }
...
def initialize() {
    ...
    if (fanCirculateEnabled) {
        ...
        if (state.fanControlOwned == true) {
            ...
            sendIfChangedStateValue("fanControlOwned", false)
        }
 ...
 * Passes [overwrite: false] to allow multiple unique schedules to target evaluateSchedule.
 */
 ...
 * Tracks explicit fan control ownership in state.fanControlOwned.
 * @param isInitial Boolean indicating if this is an explicit initialization pass.
 * @param targetPhase Optional String phase target ("ON" or "OFF"). Authoritative if provided.
 */
def manageFanCirculation(Boolean isInitial = false, String targetPhase = null) {
    ...
    // Determine target phase authoritatively if supplied, else infer from state
    ...
    try {
        if (executeOnPhase) {
            ...
            sendIfChangedStateValue("fanControlOwned", true)
 ...
 * Authoritatively passes data.nextPhase to manageFanCirculation.
...
/**
 * Validates that user-configured AQI threshold settings follow strictly ascending numerical order.
 * @return Boolean true if valid (0 < Good < Moderate < Sensitive < Unhealthy < VeryUnhealthy).
 */
...
/**
 * Returns validated AQI tier limits, falling back safely to standard EPA defaults if settings are invalid.
 * @return Map containing good, mod, sens, unhealthy, and veryUnhealthy boundaries.
 */
...
/**
 * Updates the dashboard tile with the calculated AQI value and optional filter state override. Action Routine.
 * @param avgAqi Integer average AQI value.
 * @param filterStateOverride Optional String filter state override ("ON"/"OFF").
 */
private void updateDashboard(Integer avgAqi, String filterStateOverride = null) {
    updateTile(avgAqi, filterStateOverride)
}
...
def evaluateAirQuality() {
    ...
    String expectedFilterState = updateAirFilter(statusMap.filterOn, avgAqi)
    updateDashboard(avgAqi, expectedFilterState)
 ...
 * @return String resulting expected filter state ("ON" or "OFF").
 */
private String updateAirFilter(Boolean filterOn, Integer avgAqi) {
    if (!airFilterEnabled || !airFilterSwitch) return "OFF"
    String currentFilterState = safeGetDeviceAttribute(airFilterSwitch, "switch")
    String targetState = filterOn ? "ON" : "OFF"
    try {
 ...
 * @param filterStateOverride Optional String filter status override ("ON"/"OFF").
 * @return String HTML snippet representing current MEM status.
 */
private String buildTileHtml(Integer currentAvgAqi = null, String filterStateOverride = null) {
    ...
    String filterStatus = "N/A"
    if (airFilterEnabled && airFilterSwitch) {
        filterStatus = filterStateOverride ?: (safeGetDeviceAttribute(airFilterSwitch, "switch")?.toUpperCase() ?: "OFF")
    }
...
private void updateTile(Integer currentAvgAqi = null, String filterStateOverride = null) {
    ...
    String tileHtml = buildTileHtml(currentAvgAqi, filterStateOverride)
...
/**
 * Safely converts an input setting or object to an Integer, returning defaultVal if null, empty, or unparseable.
 * @param val Setting value or object.
 * @param defaultVal Fallback integer value.
 * @return Integer converted value or defaultVal.
 */
