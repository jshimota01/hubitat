 ...
 * Notes: Schedules heating/cooling setpoint windows with switch overrides, dual HVAC device management, 
 *        fire safety forced-off modes, automatic fan circulation, 6-tier EPA AQI Air Quality monitoring 
 *        with health action strings, independent Air Filter control, native automatic heat/cool mode changeover, 
 *        and dashboard tile output.
...
/**
 *  Purpose:
 *  Schedules independent heating/cooling setpoint windows for up to 2 HVAC devices, with switch overrides, 
 *  safety forced-off triggers for space heaters, live device tracking, automatic fan circulation, 
 *  6-tier EPA AQI filtering with health alerts, native heat/cool auto-changeover, and dashboard tile output.
 *
 *  Changelog:
 *	v1.3.0	  09/01/26	  jshimota	  Expanded to dual HVAC device management. Isolated per-device scheduling, hysteresis, and system mode changeovers. Added forced-off safety overrides for floor heaters. Restructured UI headers and collapse properties.
 *	v1.2.6	  08/31/26	  jshimota	  Standardized log unit string formatting across setpoint updates and auto-changeover evaluations to ' [Value] °[Scale]'.
 *	v1.2.5	  08/31/26	  jshimota	  Fixed mainPage() section collapse engine to default all GUI sections to collapsed on initial page load.
 *	v1.2.4	  08/31/26	  jshimota	  Synchronized GUI terminology for 'Temperature Hysteresis Buffer' and expanded Fan Control Source operational guidance text.
 *	v1.2.3	  08/31/26	  jshimota	  Fixed previous thermostat target cleanup resolution bug, enforced operatingState guard in manageFanCirculation(), and restructured auto-changeover logic for true hysteresis deadband evaluation.
 *	v1.2.2	  08/31/26	  jshimota	  Implemented previous thermostat target cleanup handoff on device changes, added hardware capability validation, updated GUI terminology to 'Automatic Fan Circulation Control', and added 24-hour schedule gap validation checks.
 *	v1.2.1	  08/31/26	  jshimota	  Implemented active deadband hysteresis evaluation in auto-changeover engine, added 10-sec mode change race guard, and added GUI schedule overlap validation warnings.
...
def timeStamp() { return "2026/09/01 09:44 AM" }

definition(
    name: "Multiperiod Environment Manager",
    namespace: "jshimota",
    author: "James Shimota",
    description: "Schedules heating/cooling setpoint windows for up to 2 HVAC devices with switch overrides, safety forced-off modes, live device tracking, automatic fan circulation, EPA AQI monitoring, and dashboard tile output.",
    category: "Convenience, HVAC, Thermostat, Fan",
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section() {
            ...
            String dev1Badge = dev1AppEnabled ? "<span style='color:#27AE60; font-weight:bold;'>DEV 1 ENABLED</span>" : "<span style='color:#C0392B; font-weight:bold;'>DEV 1 DISABLED</span>"
            String dev2Badge = dev2AppEnabled ? "<span style='color:#27AE60; font-weight:bold;'>DEV 2 ENABLED</span>" : "<span style='color:#C0392B; font-weight:bold;'>DEV 2 DISABLED</span>"
            
            paragraph "<div style='background-color:#F8F9FA; border-left:4px solid #27AE60; padding:10px; border-radius:4px; font-size:13px;'>" +
                      "<b>Automations:</b> ${dev1Badge} | ${dev2Badge} &nbsp;|&nbsp; " +
                      "<b>Active Period:</b> <span style='color:#2980B9; font-weight:bold;'>${activePeriodStr}</span><br/>" +
                      "<b>Dev 1 Live:</b> Heat: <b>${dev1Heat} °F</b> | Cool: <b>${dev1Cool} °F</b> (${dev1Mode}) &nbsp;|&nbsp; " +
                      "<b>Dev 2 Live:</b> Heat: <b>${dev2Heat} °F</b> | Cool: <b>${dev2Cool} °F</b> (${dev2Mode})</div>"
        }

        /* ---------------------------------------------------------------------------------
           CATEGORY A: HVAC DEVICES & SCHEDULES
           --------------------------------------------------------------------------------- */
        ...
        /* Section 1: HVAC Device 1 Configuration */
        section("<b>SECTION 1: Managed HVAC Device 1 (Primary HVAC)</b>", hideable: true, hidden: true) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
                      "Configure settings, automated mode changeover, and time schedules for Primary HVAC Device 1.</div>"

            input name: "dev1AppEnabled", type: "bool", title: "<b>Enable Automated Setpoint Scheduling (Device 1)</b>", defaultValue: true, submitOnChange: true
            input name: "targetThermostat", type: "capability.thermostat", title: "<b>Select Primary Thermostat (Device 1)</b>", required: true, multiple: false, submitOnChange: true
            
            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0;'/>"
            
            input name: "dev1AutoChangeoverEnabled", type: "bool", title: "<b>Enable Automated System Mode Changeover (Device 1)</b>", defaultValue: true, submitOnChange: true
            if (dev1AutoChangeoverEnabled) {
                input name: "dev1TempDeadband", type: "number", title: "Device 1 Temperature Hysteresis Buffer (°F)", description: "Offset required beyond setpoint before triggering mode change", defaultValue: 2, required: true, width: 6
            }

            paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0;'/>"
            paragraph "<span style='color:#2C3E50; font-weight:bold; font-size:13px;'>Device 1 Schedule Profiles</span>"

            input name: "d1MorningHeat", type: "number", title: "Morning Heat (°F)", required: true, defaultValue: 68, width: 3
            input name: "d1MorningCool", type: "number", title: "Morning Cool (°F)", required: true, defaultValue: 72, width: 3
            input name: "d1DayHeat", type: "number", title: "Day Heat (°F)", required: true, defaultValue: 66, width: 3
            input name: "d1DayCool", type: "number", title: "Day Cool (°F)", required: true, defaultValue: 75, width: 3
            
            input name: "d1EveningHeat", type: "number", title: "Evening Heat (°F)", required: true, defaultValue: 69, width: 3
            input name: "d1EveningCool", type: "number", title: "Evening Cool (°F)", required: true, defaultValue: 73, width: 3
            input name: "d1NightHeat", type: "number", title: "Night Heat (°F)", required: true, defaultValue: 64, width: 3
            input name: "d1NightCool", type: "number", title: "Night Cool (°F)", required: true, defaultValue: 75, width: 3
        }

        /* Section 2: HVAC Device 2 Configuration */
        section("<b>SECTION 2: Managed HVAC Device 2 (Secondary / Floor Heater)</b>", hideable: true, hidden: true) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
                      "Configure settings, automated mode changeover, and time schedules for Secondary Device 2.</div>"

            input name: "dev2AppEnabled", type: "bool", title: "<b>Enable Automated Setpoint Scheduling (Device 2)</b>", defaultValue: false, submitOnChange: true
            if (dev2AppEnabled) {
                input name: "targetThermostat2", type: "capability.thermostat", title: "<b>Select Secondary Thermostat/Heater (Device 2)</b>", required: true, multiple: false, submitOnChange: true
                
                paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0;'/>"
                
                input name: "dev2AutoChangeoverEnabled", type: "bool", title: "<b>Enable Automated System Mode Changeover (Device 2)</b>", defaultValue: false, submitOnChange: true
                if (dev2AutoChangeoverEnabled) {
                    input name: "dev2TempDeadband", type: "number", title: "Device 2 Temperature Hysteresis Buffer (°F)", description: "Offset required beyond setpoint before triggering mode change", defaultValue: 2, required: true, width: 6
                }

                paragraph "<hr style='border:0; border-top:1px solid #E0E0E0; margin:8px 0;'/>"
                paragraph "<span style='color:#2C3E50; font-weight:bold; font-size:13px;'>Device 2 Schedule Profiles</span>"

                input name: "d2MorningHeat", type: "number", title: "Morning Heat (°F)", required: true, defaultValue: 68, width: 3
                input name: "d2MorningCool", type: "number", title: "Morning Cool (°F)", required: true, defaultValue: 72, width: 3
                input name: "d2DayHeat", type: "number", title: "Day Heat (°F)", required: true, defaultValue: 66, width: 3
                input name: "d2DayCool", type: "number", title: "Day Cool (°F)", required: true, defaultValue: 75, width: 3
                
                input name: "d2EveningHeat", type: "number", title: "Evening Heat (°F)", required: true, defaultValue: 69, width: 3
                input name: "d2EveningCool", type: "number", title: "Evening Cool (°F)", required: true, defaultValue: 73, width: 3
                input name: "d2NightHeat", type: "number", title: "Night Heat (°F)", required: true, defaultValue: 62, width: 3
                input name: "d2NightCool", type: "number", title: "Night Cool (°F)", required: true, defaultValue: 78, width: 3
            }
        }
        ...
        section("<b>SECTION 3: Time Schedule Window Boundaries</b>", hideable: true, hidden: true) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
                      "Set global start and end times for daily schedule periods. <i>(Applies to both Device 1 and Device 2).</i></div>"
        ...
        section("<b>SECTION 4: System Overrides & Safety Fire Hazard Controls</b>", hideable: true, hidden: true) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
            ...
            if (awaySwitch) {
                paragraph "<span style='color:#2C3E50; font-weight:bold;'>Device 1 Away Configuration</span>"
                input name: "awayDev1Heat", type: "number", title: "Dev 1 Away Heat Setpoint", required: true, defaultValue: 62, width: 6
                input name: "awayDev1Cool", type: "number", title: "Dev 1 Away Cool Setpoint", required: true, defaultValue: 78, width: 6
                
                paragraph "<span style='color:#C0392B; font-weight:bold;'>Device 2 Away Safety Configuration</span>"
                input name: "awayDev2Action", type: "enum", title: "Device 2 Away Mode Action", options: ["setpoint": "Use Away Setpoints", "off": "FORCE OFF (Fire Hazard Safety)"], defaultValue: "off", submitOnChange: true
                if (awayDev2Action == "setpoint") {
                    input name: "awayDev2Heat", type: "number", title: "Dev 2 Away Heat Setpoint", required: true, defaultValue: 60, width: 6
                    input name: "awayDev2Cool", type: "number", title: "Dev 2 Away Cool Setpoint", required: true, defaultValue: 80, width: 6
                }
            }
            ...
            if (sleepSwitch) {
                paragraph "<span style='color:#2C3E50; font-weight:bold;'>Device 1 Sleeping Configuration</span>"
                input name: "sleepDev1Heat", type: "number", title: "Dev 1 Sleeping Heat Setpoint", required: true, defaultValue: 65, width: 6
                input name: "sleepDev1Cool", type: "number", title: "Dev 1 Sleeping Cool Setpoint", required: true, defaultValue: 74, width: 6
                
                paragraph "<span style='color:#C0392B; font-weight:bold;'>Device 2 Sleeping Safety Configuration</span>"
                input name: "sleepDev2Action", type: "enum", title: "Device 2 Sleeping Mode Action", options: ["setpoint": "Use Sleeping Setpoints", "off": "FORCE OFF (Fire Hazard Safety)"], defaultValue: "off", submitOnChange: true
                if (sleepDev2Action == "setpoint") {
                    input name: "sleepDev2Heat", type: "number", title: "Dev 2 Sleeping Heat Setpoint", required: true, defaultValue: 62, width: 6
                    input name: "sleepDev2Cool", type: "number", title: "Dev 2 Sleeping Cool Setpoint", required: true, defaultValue: 76, width: 6
                }
            }
        }
        ...
        section("<b>SECTION 5: Automatic Fan Circulation Control</b>", hideable: true, hidden: true) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
                      "When enabled, MEM manages fan circulation on Device 1. Set the thermostat's Fan Control Source to <b>external</b> so the driver does not compete.</div>"

            input name: "fanCirculateEnabled", type: "bool", title: "<b>Enable Fan Circulation Loop (Device 1)</b>", defaultValue: false, submitOnChange: true
        ...
        section("<b>SECTION 8: System Operational Status Summary</b>", hideable: true, hidden: true) {
            paragraph "<div style='background-color:#F8F9FA; border-left:4px solid #2980B9; padding:10px; border-radius:4px; font-size:12px;'>" +
                      "<b>Device 1 (${targetThermostat ? targetThermostat.displayName : 'None'}):</b> Temp: ${t1States.temp ?: '--'} °F | Heat: ${dev1Heat} °F | Cool: ${dev1Cool} °F | Mode: ${dev1Mode}<br/>" +
                      "<b>Device 2 (${targetThermostat2 ? targetThermostat2.displayName : 'None'}):</b> Temp: ${t2States.temp ?: '--'} °F | Heat: ${dev2Heat} °F | Cool: ${dev2Cool} °F | Mode: ${dev2Mode}<br/>" +
                      "<hr style='border:0; border-top:1px solid #E0E0E0; margin:6px 0;'/>" +
                      "<b>Active Window:</b> ${scheduleData.activePeriod ?: 'Not Evaluated'}<br/>" +
                      "<b>Dev 1 Target Setpoints:</b> Heat: ${scheduleData.d1TargetHeat ?: '--'} °F | Cool: ${scheduleData.d1TargetCool ?: '--'} °F<br/>" +
                      "<b>Dev 2 Target Setpoints:</b> " + (scheduleData.d2ForceOff ? "<span style='color:red; font-weight:bold;'>FORCED OFF</span>" : "Heat: ${scheduleData.d2TargetHeat ?: '--'} °F | Cool: ${scheduleData.d2TargetCool ?: '--'} °F") + "<br/>" +
                      "<hr style='border:0; border-top:1px solid #E0E0E0; margin:6px 0;'/>" +
...
private void initialize(Boolean isInstall = false) {
    ...
    if (fanCirculateEnabled) {
        ...
        if (state.fanOwned == true) {
            logInfo "Fan Circulation disabled. Restoring Device 1 fan to AUTO."
...
def executeEvaluationPass(String cause = "Manual / Scheduled", Boolean forceSetpointApply = false) {
    ...
    // Process Device 1
    if (dev1AppEnabled && targetThermostat && sched.d1TargetHeat != null && sched.d1TargetCool != null) {
    ...
    // Process Device 2
    if (dev2AppEnabled && targetThermostat2) {
        if (sched.d2ForceOff) {
            logInfo "Device 2 Safety Trigger Active (${sched.activePeriod}). Forcing Device 2 OFF."
            try { targetThermostat2.off() } catch (Exception e) { logError "Failed to execute OFF on Device 2: ${e.message}" }
...
private Map getCalculatedScheduleData() {
    ...
    if (isAway) {
        ...
        if (isTimeInWindow(mStart, mEnd, currentMin)) {
            ...
            d1TargetHeat = safeToDecimal(d1MorningHeat, 68.0)
            d1TargetCool = safeToDecimal(d1MorningCool, 72.0)
            d2TargetHeat = safeToDecimal(d2MorningHeat, 68.0)
            d2TargetCool = safeToDecimal(d2MorningCool, 72.0)
        } else if (isTimeInWindow(dStart, dEnd, currentMin)) {
            activePeriod = "Day"
            d1TargetHeat = safeToDecimal(d1DayHeat, 66.0)
            d1TargetCool = safeToDecimal(d1DayCool, 75.0)
            d2TargetHeat = safeToDecimal(d2DayHeat, 66.0)
            d2TargetCool = safeToDecimal(d2DayCool, 75.0)
        } else if (isTimeInWindow(eStart, eEnd, currentMin)) {
            activePeriod = "Evening"
            d1TargetHeat = safeToDecimal(d1EveningHeat, 69.0)
            d1TargetCool = safeToDecimal(d1EveningCool, 73.0)
            d2TargetHeat = safeToDecimal(d2EveningHeat, 69.0)
            d2TargetCool = safeToDecimal(d2EveningCool, 73.0)
        } else if (isTimeInWindow(nStart, nEnd, currentMin)) {
            activePeriod = "Night"
            d1TargetHeat = safeToDecimal(d1NightHeat, 64.0)
            d1TargetCool = safeToDecimal(d1NightCool, 75.0)
            d2TargetHeat = safeToDecimal(d2NightHeat, 62.0)
            d2TargetCool = safeToDecimal(d2NightCool, 78.0)
        } else {
...
private void applySetpointsAndEvaluateMode(Object dev, BigDecimal targetHeat, BigDecimal targetCool, Map thermostatSnapshot = null, Boolean autoChangeover = true, Object hysteresisBuffer = 2, Boolean isDevice2 = false, Boolean forceSetpointApply = false) {
    ...
    if (autoChangeover && currentTemp != null) {
        ...
        String guardKey = isDevice2 ? "dev2ModeChangePending" : "dev1ModeChangePending"
        ...
        try {
            if (currentMode == "cool" && currentTemp <= targetHeat) {
                ...
                state[guardKey] = true
                runIn(10, isDevice2 ? "clearDev2ModeGuard" : "clearDev1ModeGuard")
                dev.heat()
...
private String buildTileHtml(Map ctx = null) {
    ...
    tileSb.append("<b>Dev 1 Temp:</b> ").append(t1Temp).append(" | <b>Dev 2 Mode:</b> ").append(t2Mode).append("<br/>")
...
private Map validateConfiguration() {
    ...
    if (dev1AppEnabled && !targetThermostat) errors.add("Device 1 Thermostat is required when Device 1 scheduling is enabled.")
    if (dev2AppEnabled && !targetThermostat2) errors.add("Device 2 Thermostat is required when Device 2 scheduling is enabled.")
