...
def mainPage() {
    dynamicPage(name: "mainPage", title: "Multiperiod Environment Manager Setup v${version()}", install: true, uninstall: true) {
        ...
        section("<b>Air Quality Monitoring & RGB Indicator</b>") {
            ...
            Integer tGoodMax = safeToInt(settings?.aqiGoodMax, 50)
            Integer tModMax = safeToInt(settings?.aqiModMax, 100)
            Integer tSensMax = safeToInt(settings?.aqiSensMax, 150)
            Integer tUnhealthMax = safeToInt(settings?.aqiUnhealthyMax, 200)
            Integer tVeryUnhealthMax = safeToInt(settings?.aqiVeryUnhealthyMax, 300)
...
private Map determineStatus(Object avgAqiVal) {
    ...
    Integer gMax = safeToInt(settings?.aqiGoodMax, 50)
    Integer mMax = safeToInt(settings?.aqiModMax, 100)
    Integer sMax = safeToInt(settings?.aqiSensMax, 150)
    Integer uMax = safeToInt(settings?.aqiUnhealthyMax, 200)
    Integer vuMax = safeToInt(settings?.aqiVeryUnhealthyMax, 300)
