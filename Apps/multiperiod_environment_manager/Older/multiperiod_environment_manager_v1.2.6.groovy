 ...
 *        automatic fan circulation, 6-tier EPA AQI Air Quality monitoring with health action strings, 
 ...
 *  automatic fan circulation, 6-tier EPA AQI filtering with health alerts, native automatic heat/cool 
...
def timeStamp() { return "2026/08/31 10:20 PM" }

definition(
    name: "Multiperiod Environment Manager",
    namespace: "jshimota",
    author: "James Shimota",
    description: "Schedules 6 heating/cooling setpoint windows with switch overrides, live device tracking, automatic fan circulation, 6-tier EPA AQI filtering with health alerts, native heat/cool auto-changeover, and dashboard tile output.",
    category: "Convenience, HVAC, Thermostat, Fan",
    ...
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section() {
            ...
            paragraph "<div style='background-color:#F8F9FA; border-left:4px solid #27AE60; padding:10px; border-radius:4px; font-size:13px;'>" +
                      ...
                      "<b>Live Setpoints:</b> Heat: <b>${devHeat} °F</b> | Cool: <b>${devCool} °F</b></div>"
        }
        ...
        /* Section 1: Managed Thermostat Device */
        section("<b>SECTION 1: Control & Managed Thermostat Device</b>", hideable: true, hidden: isCollapsed) {
            ...
            input name: "targetThermostat", type: "capability.thermostat", title: "<b>Select Managed Thermostat Device</b>", required: true, multiple: false, submitOnChange: true
            ...
            if (autoChangeoverEnabled) {
                input name: "tempDeadband", type: "number", title: "Temperature Hysteresis Buffer (°F)", description: "Degree offset required beyond opposing setpoints before triggering system mode change", defaultValue: 2, required: true, width: 6
            }
        }
        ...
        /* Section 4: Automatic Fan Circulation Control */
        section("<b>SECTION 4: Automatic Fan Circulation Control</b>", hideable: true, hidden: isCollapsed) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
                      "When enabled, MEM controls the thermostat fan. Set the thermostat's Fan Control Source to <b>external</b> so the driver's own circulation engine does not compete with MEM.</div>"
        ...
        section("<b>SECTION 7: System Operational Status Summary</b>", hideable: true, hidden: isCollapsed) {
            paragraph "<div style='background-color:#F8F9FA; border-left:4px solid #2980B9; padding:10px; border-radius:4px; font-size:12px;'>" +
                      ...
                      "<b>Thermostat Telemetry:</b> Temp: ${tStates.temp ?: '--'} °F | Heat: ${devHeat} °F | Cool: ${devCool} °F | Mode: ${devThermostatMode} | Fan Mode: ${devFanMode} | Operating State: ${devOpState}<br/>" +
                      ...
                      "<b>Target Schedule Setpoints:</b> Heat: ${scheduleData.targetHeat ?: '--'} °F | Cool: ${scheduleData.targetCool ?: '--'} °F<br/>" +
                      "<b>Auto-Changeover Mode Engine:</b> ${autoChangeoverEnabled ? 'Active (Hysteresis Buffer: ' + (tempDeadband ?: 2) + ' °F)' : 'Disabled'}<br/>" +
...
void updated() {
    ...
    if (state.previousThermostatDni && targetThermostat && state.previousThermostatDni != targetThermostat.deviceNetworkId) {
        logInfo "Thermostat selection changed. Resolving previous target device (${state.previousThermostatDni}) to release ownership..."
        def prevThermostat = targetThermostat.host.getDeviceList()?.find { it.deviceNetworkId == state.previousThermostatDni }
        if (prevThermostat != null && state.fanOwned == true) {
            try {
                logInfo "Releasing fan ownership on previous thermostat '${prevThermostat.displayName}' -> fanAuto()"
                prevThermostat.fanAuto()
            } catch (Exception e) {
                logError "Failed to release ownership on previous thermostat: ${e.message}"
            }
            sendIfChangedStateValue("fanOwned", false)
        }
    }
    if (targetThermostat) {
        state.previousThermostatDni = targetThermostat.deviceNetworkId
    }
...
def temperatureHandler(evt) { executeEvaluationPass("Thermostat Ambient Temperature Updated (${evt?.value} °F)", false) }
...
private void applySetpointsAndEvaluateMode(BigDecimal targetHeat, BigDecimal targetCool, Map thermostat = null, Boolean forceSetpointApply = false) {
    ...
    try {
        if (forceSetpointApply || currentHeatSp != targetHeat) {
            logInfo "Setting ${targetThermostat.displayName} Heating Setpoint to ${targetHeat} °${scale} (Active Period: ${state.activeSet})"
            targetThermostat.setHeatingSetpoint(targetHeat)
        }

        if (forceSetpointApply || currentCoolSp != targetCool) {
            logInfo "Setting ${targetThermostat.displayName} Cooling Setpoint to ${targetCool} °${scale} (Active Period: ${state.activeSet})"
    ...
    if (getSettingBool("autoChangeoverEnabled", true) && currentTemp != null) {
        ...
        try {
            if (currentMode == "cool" && currentTemp <= targetHeat) {
                logInfo "Auto-Changeover: Temp (${currentTemp} °${scale}) dropped <= Heat Setpoint (${targetHeat} °${scale}). Changing mode from COOL to HEAT."
                ...
                logInfo "Auto-Changeover: Temp (${currentTemp} °${scale}) rose >= Cool Setpoint (${targetCool} °${scale}). Changing mode from HEAT to COOL."
                state.modeChangePending = true
                runIn(10, "clearModeChangeGuard")
                targetThermostat.cool()
            } else if (currentMode == "off" || currentMode == "auto") {
                if (currentTemp <= (targetHeat - deadband)) {
                    logInfo "Auto-Changeover: Temp (${currentTemp} °${scale}) breached Heat Deadband (${targetHeat - deadband} °${scale}). Setting mode to HEAT."
                    state.modeChangePending = true
                    runIn(10, "clearModeChangeGuard")
                    targetThermostat.heat()
                } else if (currentTemp >= (targetCool + deadband)) {
                    logInfo "Auto-Changeover: Temp (${currentTemp} °${scale}) breached Cool Deadband (${targetCool + deadband} °${scale}). Setting mode to COOL."
                    state.modeChangePending = true
                    runIn(10, "clearModeChangeGuard")
                    targetThermostat.cool()
                }
            } else {
                logDebug "Auto-Changeover: Temp (${currentTemp} °${scale}) is within setpoints and hysteresis buffer. Holding current mode '${currentMode}'."
            }
        } catch (Exception e) {
    ...
    try {
        if (executeOnPhase) {
            ...
            if (opState != "heating" && opState != "cooling") {
                logInfo "Fan Circulation Loop: Turning fan ON for ${safeOn} minutes (HVAC idle)."
                targetThermostat.fanOn()
                sendIfChangedStateValue("fanOwned", true)
            } else {
                logInfo "Fan Circulation Loop: HVAC actively ${opState}. Skipping explicit fanOn command and ownership claim."
                sendIfChangedStateValue("fanOwned", false)
            }
...
private String buildTileHtml(Map ctx = null) {
    ...
    def appHeat = sched.targetHeat != null ? "${sched.targetHeat} °F" : "--"
    def appCool = sched.targetCool != null ? "${sched.targetCool} °F" : "--"

    Map thermostat = ctx.thermostat ?: [:]
    def realHeat = thermostat.heat != null ? "${thermostat.heat} °F" : "--"
    def realCool = thermostat.cool != null ? "${thermostat.cool} °F" : "--"
    def realTemp = thermostat.temp != null ? "${thermostat.temp} °F" : "--"
...
private Map validateConfiguration() {
    ...
    if (!targetThermostat) {
        errors.add("Managed Thermostat device is required.")
    } else {
        // Hardware Capability Sanity Checks
        List requiredAttrs = ["temperature", "heatingSetpoint", "coolingSetpoint", "thermostatMode"]
        requiredAttrs.each { attr ->
            if (safeGetDeviceAttribute(targetThermostat, attr) == null) {
                warnings.add("Selected thermostat '${targetThermostat.displayName}' does not report attribute '${attr}' yet.")
            }
        }
    }
    ...
    if (hAway >= cAway) errors.add("Away Heat setpoint (${hAway} °F) must be strictly less than Away Cool setpoint (${cAway} °F).")

    BigDecimal hSleep = safeToDecimal(sleepHeat, 65.0)
    BigDecimal cSleep = safeToDecimal(sleepCool, 74.0)
    if (hSleep >= cSleep) errors.add("Sleeping Heat setpoint (${hSleep} °F) must be strictly less than Sleeping Cool setpoint (${cSleep} °F).")
    ...
    periods.each { p ->
        ...
        if (h >= c) errors.add("${p.name} Heat setpoint (${h} °F) must be strictly less than ${p.name} Cool setpoint (${c} °F).")
    ...
    // 24-Hour Schedule Gap Detection Check
    if (mStart != null && mEnd != null && dStart != null && dEnd != null && eStart != null && eEnd != null && nStart != null && nEnd != null) {
        Boolean gapDetected = !(mEnd == dStart && dEnd == eStart && eEnd == nStart && nEnd == mStart)
        if (gapDetected) {
            warnings.add("Time windows do not form a seamless 24-hour schedule. Uncovered periods will fall back to 'Default / Unassigned Window'.")
        }
    }
