...
def timeStamp() { return "2026/08/31 09:55 PM" }
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "", install: true, uninstall: true) {
        ...
        section("<b>SECTION 4: Automatic Fan Circulation Control</b>", hideable: true, hidden: isCollapsed) {
            paragraph "<div style='color:#555; font-size:12px; margin-bottom:8px;'>" +
                      "Cycles the thermostat fan between <b>ON</b> and <b>AUTO</b> to circulate air throughout the home when HVAC is idle. <i>(Recommended: Set target thermostat Fan Control Source command to <b>external</b>)</i>.</div>"
...
void updated() {
    ...
    // FIX BUG #2: Explicitly resolve and release the previous target thermostat device object
...
private void applySetpointsAndEvaluateMode(BigDecimal targetHeat, BigDecimal targetCool, Map thermostat = null, Boolean forceSetpointApply = false) {
    ...
    // FIX BUG #5: True Deadband Hysteresis Order of Operations
    if (getSettingBool("autoChangeoverEnabled", true) && currentTemp != null) {
        ...
        try {
            // Evaluates true hysteresis around boundaries
            if (currentMode == "cool" && currentTemp <= targetHeat) {
                ...
                logDebug "Auto-Changeover: Temp (${currentTemp}°) is within setpoints and deadband hysteresis. Holding current mode '${currentMode}'."
            }
        } catch (Exception e) {
...
def manageFanCirculation(Boolean isInitial = false, String targetPhase = null) {
    ...
    try {
        if (executeOnPhase) {
            ...
            // FIX BUG #4: Operating State Guard - Skip fanOn override if HVAC is active
