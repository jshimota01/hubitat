 ...
 *  Schedules 6 heating/cooling setpoint windows with switch-based Away/Sleeping overrides, live device tracking, dynamic fan circulation, AQI Air Quality filtering, and dynamic dashboard tile output.
...
definition(
    ...
    description: "Schedules 6 heating/cooling setpoint windows with switch overrides, live device tracking, dynamic fan circulation, and AQI Air Quality filtering.",
...
def mainPage() {
    dynamicPage(name: "mainPage", title: "Multiperiod Environment Manager Setup v${version()}", install: true, uninstall: true) {
        ...
        section("<b>Air Quality Sensors</b>") {
            paragraph "Monitors two air quality sensors and averages their airQualityIndex attributes."
        ...
        section("<b>Air Filter Device</b>") {
            paragraph "Controls a dedicated air filter switch based on averaged AQI thresholds."
            if (aqiEnabled) {
                input name: "airFilterSwitch", type: "capability.switch", title: "Select Air Filter Switch/Device", required: false, multiple: false
            } else {
                paragraph "<i>Enable Air Quality Monitoring above to assign an Air Filter device.</i>"
            }
        }
...
private void updateMemTile() {
    ...
    String filterStatus = airFilterSwitch ? (airFilterSwitch.currentValue("switch")?.toUpperCase() ?: "OFF") : "N/A"
