...
def mainPage() {
    dynamicPage(name: "mainPage", title: "Multiperiod Environment Manager Setup v${version()}", install: true, uninstall: true) {
        ...
        section("<b>Air Quality Monitoring & RGB Indicator</b>") {
            paragraph "Monitors two airQuality sensors and averages their airQualityIndex attributes."
        ...
        section("<b>Air Filter Device Control</b>") {
            paragraph "Independently turns on an Air Filter device based on calculated AQI thresholds (Yellow/Red)."
            input name: "airFilterEnabled", type: "bool", title: "Enable Automated Air Filter Control", defaultValue: false, submitOnChange: true
            if (airFilterEnabled) {
                input name: "airFilterSwitch", type: "capability.switch", title: "Select Air Filter Switch/Device", required: true, multiple: false
            }
        }
...
def initialize() {
    ...
    if (aqiEnabled) {
        ...
        // Calculate AQI average immediately upon initialization/saving settings
