...
def timeStamp() { return "2026/09/07 10:15 AM" }
    ...
    if (isOfficeActive) {
        if (sched.d2ForceOff) {
            ...
            if (currentOfficeMode != "off") {
                ...
                logWarn "Office HVAC Safety Trigger Active (${sched.activePeriod}). Office HVAC remains held OFF."
            }
            sendIfChangedStateValue("officeSafetyActive", true)
            ...
            if (state.officeSafetyActive == true) {
                logWarn "Office HVAC Safety Trigger Cleared (${sched.activePeriod}). Resuming normal scheduled operation."
                sendIfChangedStateValue("officeSafetyActive", false)
            }
...
private void applySetpointsAndEvaluateMode(Object dev, BigDecimal targetHeat, BigDecimal targetCool, Map thermostatSnapshot = null, Boolean autoChangeover = true, Object hysteresisBuffer = 2, Boolean isOffice = false, Boolean forceSetpointApply = false) {
    ...
    if (!forceSetpointApply && isCommandPending) {
        ...
        if (commandSent && !forceSetpointApply) {
...
private String buildTileHtml(Map ctx = null) {
    ...
    // Base container set to 12px (1.0em) with line height tight enough to prevent tile scrollbars
    tileSb.append("<div style='font-size:12px; line-height:1.15; text-align:center; padding:1px;'>")
    
    // MEM Period set to 0.833em (~10px) to stop vertical stretching
    tileSb.append("<div style='font-size:0.833em;'><b>MEM Period:</b> ").append(activePeriod).append("</div>")
    
    // House and Office explicitly separated onto two distinct lines at 1.0em (12px)
    tileSb.append("<div style='font-size:1.0em;'><b>House (${t1Temp}):</b> ").append(hTargetStr).append("</div>")
    tileSb.append("<div style='font-size:1.0em;'><b>Office (${t2Temp}):</b> ").append(oTargetStr).append("</div>")
    
    // Remaining lines set to 0.833em (~10px)
    tileSb.append("<div style='font-size:0.833em;'><b>Device Mode:</b> House - ").append(t1Mode).append(" | Office - ").append(t2Mode).append("</div>")
    tileSb.append("<div style='font-size:0.833em;'><b>Air Filter:</b> ").append(airFilterState).append(" | <b>Circulation:</b> ").append(circState).append("</div>")
    tileSb.append("<div style='font-size:0.833em;'><b>AQI:</b> <span style='color:").append(aqiColorCss).append("; font-weight:bold;'>").append(avgAqiStr).append(" ").append(aqiStatusStr).append("</span></div>")
