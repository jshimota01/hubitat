...
static String version() { return '0.7.9' }
def timeStamp() { return "2026/09/25 08:55 AM" }
...
private List<String> processSetpoint(degrees, int attributeId) { 
    ...
    logInfo "Setting ${setpointLabel} to ${degreesInteger} °${getTemperatureScale()}"
    
    updateAttribute(attrName, degreesInteger, getTemperatureScale(), "digital", setpointLabel)
    updateAttribute("thermostatSetpoint", degreesInteger, getTemperatureScale(), "digital", setpointLabel)
    
    if (attrName == "heatingSetpoint" && device.currentValue("coolingSetpoint") == null) {
        updateAttribute("coolingSetpoint", (degreesInteger + 5), getTemperatureScale(), "digital", "Cooling setpoint")
    } else if (attrName == "coolingSetpoint" && device.currentValue("heatingSetpoint") == null) {
        updateAttribute("heatingSetpoint", (degreesInteger - 5), getTemperatureScale(), "digital", "Heating setpoint")
    }
            ...
            switch(clusterInt) {
                case 0x0201: // Thermostat Cluster
                    if (attrInt == 0x0000) {
                        ...
                        updateAttribute("coolingSetpoint", wholeTemp, getTemperatureScale(), "physical", "Cooling setpoint")
                        updateAttribute("thermostatSetpoint", wholeTemp, getTemperatureScale(), "physical", "Cooling setpoint")
                    } else if (attrInt == 0x0012) {
                        BigDecimal temp = getTemperature(descMap.value)
                        int wholeTemp = Math.round(temp).toInteger()
                        updateAttribute("heatingSetpoint", wholeTemp, getTemperatureScale(), "physical", "Heating setpoint")
                        updateAttribute("thermostatSetpoint", wholeTemp, getTemperatureScale(), "physical", "Heating setpoint")
...
private void updateAttribute(final String attribute, final Object value, final String unit = null, final String type = null, final String customLabel = null) {
    ...
    String labelText = customLabel ?: attribute
    final String descriptionText = "${device.displayName} - ${labelText} was set to ${value}${formattedUnit}"
