...
def mainPage() {
    dynamicPage(name: "mainPage", title: "Multiperiod Environment Manager Setup v${version()}", install: true, uninstall: true) {
        ...
        def v1 = aqiSensor1 ? aqiSensor1.currentValue("airQualityIndex") : null
        def v2 = aqiSensor2 ? aqiSensor2.currentValue("airQualityIndex") : null
        def avgAqi = (v1 != null && v2 != null) ? Math.round((v1 + v2) / 2.0) : (v1 ?: v2 ?: "--")
        
        String displayAqiStatus = "--"
        if (avgAqi instanceof Number) {
            Integer goodMax = (aqiGoodMax != null) ? (aqiGoodMax as Integer) : 50
            Integer modMax = (aqiModMax != null) ? (aqiModMax as Integer) : 99
            if (avgAqi <= goodMax) displayAqiStatus = "Green (Good)"
            else if (avgAqi <= modMax) displayAqiStatus = "Yellow (Moderate)"
            else displayAqiStatus = "Red (Unhealthy)"
        }
...
private Map getThermostatStateSnapshot() {
    ...
    return [
        heatingSetpoint: targetThermostat.currentValue("heatingSetpoint"),
        coolingSetpoint: targetThermostat.currentValue("coolingSetpoint"),
        thermostatMode: targetThermostat.currentValue("thermostatMode"),
        thermostatFanMode: targetThermostat.currentValue("thermostatFanMode"),
        thermostatOperatingState: targetThermostat.currentValue("thermostatOperatingState")
    ]
}
...
private void updateMemTile(Integer currentAvgAqi = null) {
    ...
    if (currentAvgAqi != null) {
        ...
        def v2 = aqiSensor2 ? aqiSensor2.currentValue("airQualityIndex") : null
        if (v1 != null || v2 != null) {
            avgAqi = "${Math.round(((v1 ?: v2) + (v2 ?: v1)) / 2.0)}"
        }
    }
