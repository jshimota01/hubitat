...
def timeStamp() { return "2026/09/14 09:45 AM" }
...
private Map getCalculatedScheduleData() {
    ...
    // Dashboard setpoint overrides are intentionally temporary. They remain active
    // for the current normal schedule period and are discarded automatically when
    // the schedule changes or a Presence/Sleeping override becomes active.
    ...
    if (normalSchedulePeriod) {
        ...
        // System safety overrides always outrank dashboard changes.
...
def setpointChangeHandler(evt) {
    ...
    BigDecimal expectedValue = safeToDecimal(state[attributeName == "heatingSetpoint" ? pendingHeatKey : pendingCoolKey], null)

    // MEM's own setpoint command produces the same device event as a dashboard
    // change. Do not turn our scheduled command into a manual override.
    if (state[pendingKey] == true && expectedValue != null && expectedValue == newValue) {
        logDebug "Ignoring internal ${evt.device.displayName} ${attributeName} event (${newValue}°F); MEM command is pending."
        return
    }
    ...
    if (!(sched.activePeriod in ["Presence Override", "Sleeping Override"])) {
        ...
        executeEvaluationPass("Dashboard Setpoint Override (${evt.device.displayName})", false)
    }
}
...
private void applySetpointsAndEvaluateMode(Object dev, BigDecimal targetHeat, BigDecimal targetCool, Map thermostatSnapshot = null, Boolean autoChangeover = true, Object hysteresisBuffer = 2, Boolean isOffice = false, Boolean forceSetpointApply = false) {
    ...
    // Check if we are currently awaiting acknowledgment of this exact setpoint command
    ...
    if (!forceSetpointApply && isCommandPending) {
        ...
        if (commandSent) {
            // Guard both normal and forced MEM commands so their resulting device
            // events cannot be mistaken for dashboard/manual overrides.
    ...
    if (autoChangeover && currentTemp != null) {
        ...
        try {
            // OFFICE ONLY: A manual dashboard heat override is a direct user request.
            // Once that requested heat setpoint is satisfied, release the Office
            // thermostat back to AUTO and let its existing driver evaluation move
            // the operating state to IDLE. The Office child app then turns the
            // floor-heater outlet OFF. House HVAC is intentionally not affected.
            if (isOffice && manualOverrideActive && currentTemp >= targetHeat &&
                (currentMode == "heat" || snapshot.operatingState?.toLowerCase() == "heating")) {
                logInfo "Office Manual Heat Override: Temp (${currentTemp} °${scale}) >= Heat Setpoint (${targetHeat} °${scale}). Returning Office thermostat to AUTO."
                state[guardKey] = true
                runIn(10, "clearDev2ModeGuard")
                dev.auto()
                ...
                // A dashboard/manual setpoint is an explicit user request. Do not
                // make the user wait for MEM's configured auto-changeover deadband.
...
def manageFanCirculation(Boolean isInitial = false, String targetPhase = null) {
    ...
    // --- SLEEPING MODE 1-HOUR ACTIVITY COOLDOWN GUARD ---
    if (isSleeping && executeOnPhase) {
        Long lastRun = state.lastSystemRunTime as Long
        if (lastRun != null) {
            Long elapsedMs = now() - lastRun
            Long requiredDelayMs = 60 * 60 * 1000L // 1 hour cooldown threshold
...
private String buildTileHtml(Map ctx = null) {
    ...
    // House & Office Current Temperatures
    ...
    // Room Air Filter & Circulation States combined
    ...
    // Base tile container without hardcoded px values to allow pure dynamic EM scaling
    tileSb.append("<div style='line-height:1.15; text-align:center; padding:1px;'>")
    
    // MEM Period set to base 0.45em
    tileSb.append("<div style='font-size:0.45em;'><b>MEM Period:</b> ").append(activePeriod).append("</div>")
    
    // House and Office explicitly separated on 2 lines set to 0.55em 
    ...
    // Remaining lines set to base 0.45em
