 ...
 *      dynamic fan circulation, AQI Air Quality monitoring, independent Air Filter control, and dashboard tile output.
 *
...
private Integer timeToMinutes(Object timeInput) {
    ...
    try {
        ...
        if (str.contains("T")) {
            // ISO string from toDateTime / Hubitat time picker
...
private Boolean isTimeInWindow(Integer startMin, Integer endMin, Integer currentMin) {
    ...
    if (startMin <= endMin) {
        ...
        // Overnight period spanning midnight
        return (currentMin >= startMin || currentMin < endMin)
    }
}
