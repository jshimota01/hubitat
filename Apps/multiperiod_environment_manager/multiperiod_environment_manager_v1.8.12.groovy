 ...
 *
 *  Changelog:
 *  v1.8.12   09/25/26    jshimota    Setpoint Log Disambiguation Refactor: Replaced ambiguous 'thermostatSetpoint' / generic setpoint log strings with explicit 'Heating setpoint' and 'Cooling setpoint' descriptors across all command dispatchers and event absorption handlers.
...
static String version() { return '1.8.12' }
def timeStamp() { return "2026/09/25 08:50 AM" }
...
def executeEvaluationPass(String cause = "Manual / Scheduled", Boolean forceSetpointApply = false) {
    ...
    if (state.lastActivePeriod != currentPeriod) {
        ...
        if (sched.d1TargetHeat != null && sched.d1TargetCool != null) {
            transitionSb.append(" | House Target: Heating setpoint ").append(sched.d1TargetHeat).append(" °F, Cooling setpoint ").append(sched.d1TargetCool).append(" °F")
        }
        ...
        if (targetThermostat2 && getSettingBool("dev2AppEnabled", false)) {
            if (sched.d2ForceOff) {
                ...
                transitionSb.append(" | Office Target: Heating setpoint ").append(sched.d2TargetHeat).append(" °F, Cooling setpoint ").append(sched.d2TargetCool).append(" °F")
            }
        }
...
def setpointChangeHandler(evt) {
    ...
    String setpointLabel = (attributeName == "heatingSetpoint") ? "Heating setpoint" : "Cooling setpoint"
    ...
    if (state[pendingKey] == true && expectedValue != null && expectedValue == newValue) {
        logDebug "Absorbed internal ${evt.device.displayName} ${setpointLabel} event (${newValue}°F); MEM command acknowledged."
        return
    }
    ...
    if (!(sched.activePeriod in ["Presence Override", "Sleeping Override"])) {
        ...
        logInfo "Dashboard override detected: ${evt.device.displayName} ${setpointLabel} = ${newValue}°F. Honoring manual setpoint for ${sched.activePeriod}."
...
private void applySetpointsAndEvaluateMode(Object dev, BigDecimal targetHeat, BigDecimal targetCool, Map thermostatSnapshot = null, Boolean autoChangeover = true, Object hysteresisBuffer = 2, Boolean isOffice = false, Boolean forceSetpointApply = false) {
    ...
    if (!forceSetpointApply && isCommandPending) {
        logDebug "Setpoint command currently in-flight for ${dev.displayName} (Heating setpoint: ${targetHeat}° / Cooling setpoint: ${targetCool}°). Suppressing duplicate transmission."
    } else {
    ...
    if (autoChangeover && currentTemp != null) {
        ...
        try {
            ...
            if (isOffice && manualOverrideActive && currentTemp >= targetHeat &&
                ...
                logInfo "Office Manual Heat Override Satisfied: Temp (${currentTemp} °${scale}) >= Heating setpoint (${targetHeat} °${scale}). Clearing override state."
                clearOfficeManualHeatOverride()
            } else if (currentMode == "cool" && currentTemp <= targetHeat) {
                logInfo "Auto-Changeover (${dev.displayName}): Temp (${currentTemp} °${scale}) <= Heating setpoint (${targetHeat} °${scale}). Changing to HEAT."
                state[guardKey] = true
                runIn(10, isOffice ? "clearDev2ModeGuard" : "clearDev1ModeGuard")
                dev.heat()
            } else if (currentMode == "heat" && currentTemp >= targetCool) {
                logInfo "Auto-Changeover (${dev.displayName}): Temp (${currentTemp} °${scale}) >= Cooling setpoint (${targetCool} °${scale}). Changing to COOL."
                state[guardKey] = true
                runIn(10, isOffice ? "clearDev2ModeGuard" : "clearDev1ModeGuard")
                dev.cool()
                ...
                if (currentTemp <= heatTrigger) {
                    logInfo "Auto-Changeover (${dev.displayName}): Temp (${currentTemp} °${scale}) ${manualOverrideActive ? "reached manual Heating setpoint" : "breached Heat Deadband"}. Setting to HEAT."
                    state[guardKey] = true
                    runIn(10, isOffice ? "clearDev2ModeGuard" : "clearDev1ModeGuard")
                    dev.heat()
                } else if (currentTemp >= coolTrigger) {
                    logInfo "Auto-Changeover (${dev.displayName}): Temp (${currentTemp} °${scale}) ${manualOverrideActive ? "reached manual Cooling setpoint" : "breached Cool Deadband"}. Setting to COOL."
                    state[guardKey] = true
                    runIn(10, isOffice ? "clearDev2ModeGuard" : "clearDev1ModeGuard")
                    dev.cool()
                }
