...
def illumHandler(evt) {
	...
	if(vIDevice.supportedCommands.find{it.toString() == "setIlluminance"}) { settings.vIDevice.setIlluminance("${state.avgL.round(1)}"); sendEvent(name: "illuminance", value: state.avgL, unit: "lx", displayed: true)  }
...
def energyHandler(evt) {
    ...
    if(vEDevice.supportedCommands.find{it.toString() == "setEnergy"}) { settings.vEDevice.setEnergy("${state.avgE.round(1)}"); sendEvent(name: "energy", value: state.avgE, unit:"kWh", displayed: true)  }
...
def initialize() {
    ...
    if (state.avgL == null) state.avgL = 68
   	if (state.avgE == null) state.avgE = .1
    subscribeHSelected()
	subscribeTSelected()
	subscribeISelected()
	subscripeESelected()
	...
	dynamicPage(name: "mainPage", uninstall: true, install: true) {
		section(getFormat("title", " ${app.label}")) {
			section{paragraph "<div style='color:#1A77C9'>Calculate a Rolling Average of a set of sensors.</div>"    
				...
				section{paragraph "<div style='color:#1A77C9'>Energy sensors:</div>"   
