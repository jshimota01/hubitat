...
def mainPage() {
    ...
    dynamicPage(name: "mainPage", title: "THIE Room Averager Configuration", install: true, uninstall: true) {
        ...
        // STEP 1: Select Target Room First
        section("<b>Step 1: Select Target Room</b>") {
            if (roomList) {
                ...
                paragraph "<i>No rooms configured in Hubitat Settings -> Rooms. Please create rooms in Hubitat first.</i>"
            }
        }

        // Display remaining configuration options once a room has been chosen
        if (selectedRoomId) {
            def selectedRoomName = roomList[selectedRoomId] ?: "Room"
            
            section("<b>Step 2: Label App Instance</b>") {
                label title: "Enter a name for this room averager instance:", required: true, defaultValue: "${selectedRoomName} THIE Averager"
            }
...
private calculateAndSetTemp() {
    ...
    if (validValues) {
        ...
        logDebug "Calculated Temp Average for Room: ${avg}"
...
private calculateAndSetHumidity() {
    ...
    if (validValues) {
        def avg = validValues.sum() / validValues.size()
        logDebug "Calculated Humidity Average for Room: ${avg}"
...
private calculateAndSetLux() {
    ...
    if (validValues) {
        def avg = validValues.sum() / validValues.size()
        logDebug "Calculated Lux Average for Room: ${avg}"
...
private calculateAndSetEnergy() {
    ...
    if (validValues) {
        def avg = validValues.sum() / validValues.size()
        logDebug "Calculated Energy Average for Room: ${avg}"
