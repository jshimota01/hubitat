/**
 *  AvgTHIE Child (v1.1.7)
 *  Optimized for straight mathematical averaging and totalized energy summing.
 */
...
definition (
    ...
    namespace: "jshimota",
    author: "J Shimota",
    description: "Child: Clean mathematical average of humidity, temperature, illuminance, or power sensors, and totalized sum for energy.",
...
def periodicRefresh() {
    if (debugOutput) log.debug "Executing scheduled 5-minute periodic refresh."
    calculateTemp()
    calculateHumid()
    calculateIllum()
    calculateEnergy()
...
def calculateEnergy() {
    ...
    if (count > 0) {
        // Changed from an average to a flat mathematical sum
        vEDevice.setEnergy(total.round(3))
    }
}
...
// forced refreshes from device

def requestTemperatureRefresh() {
    if (debugOutput) log.debug "Received forced refresh request from virtual temperature device."
    calculateTemp()
}
def requestIlluminanceRefresh() {
    if (debugOutput) log.debug "Received forced refresh request from virtual illuminance device."
    calculateIllum()
}
def requestHumidityRefresh() {
    if (debugOutput) log.debug "Received forced refresh request from virtual humidity device."
    calculateHumid()
}
def requestPowerRefresh() {
    if (debugOutput) log.debug "Received forced refresh request from virtual power device."
    calculatePower()
}
def requestEnergyRefresh() {
    if (debugOutput) log.debug "Received forced refresh request from virtual energy device."
    calculateEnergy()
}
...
def mainPage() {
    ...
    dynamicPage(name: "mainPage", uninstall: true, install: true) {
        section(getFormat("title", " ${app.label}")) {
            paragraph "<div style='color:#1A77C9'>Calculate rolling averages or aggregate totals for your specialized virtual devices.</div>"
        }
        ...
        section("Energy Config (Totalized Sum)") {
            input "vEDevice", "capability.energyMeter", title: "Virtual Energy Device", required: false
            input "energySensors", "capability.energyMeter", title: "Energy Sensors (Values will be summed)", multiple: true, required: false
        }
        ...
        section("Logging Settings") {
			input name: "thisName", type: "text", title: "Child App Name", callback: "updateChildLabel", onChange: "updateChildLabel", required: false
        }
        section {
            display()
        }
