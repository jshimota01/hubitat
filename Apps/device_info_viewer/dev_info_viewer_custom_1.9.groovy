 ...
 *  
 *  Changelog:
 *  v1.9.0    08/30/26    jshimota    Fixed JS delimiter & timestamp filename bugs; added escapeHtml() sanitizer for table cell safety
...
def timeStamp() { return "2026/08/30 10:45 AM" }
...
def viewData() {
    dynamicPage(name: "viewData", title: "", install: false, uninstall: false) {
        ...
        def delimiterChar = getDelimiter() == "\t" ? "\\t" : getDelimiter()
        def targetFilename = getFilename()
        ...
        section("<b>Export Actions</b>") {
            paragraph """
                ...
                <script>
                const activeDelimiter = '${delimiterChar}';
                const activeFilename = '${targetFilename}';
                ...
                function tableToCSV() {
                    ...
                    rows.forEach(row => {
                        ...
                        csv += rowData.join(activeDelimiter) + '\\n';
                    });
        ...
        section("<b>Raw CSV Preview</b>") {
            paragraph "<pre style='background: #f5f5f5; padding: 10px; border-radius: 5px; overflow-x: auto; max-height: 400px;'>${escapeHtml(generateCsv())}</pre>"
        }
    }
}
...
private String escapeHtml(value) {
...
def generateHtmlTable() {
    ...
    columns.each { colName ->
        ...
        def displayName = escapeHtml(getDisplayName(colName))
    ...
    devices.each { deviceId, deviceInfo ->
        def safeName = escapeHtml(deviceInfo.name)
        def safeLabel = escapeHtml(deviceInfo.label)
        def safeId = escapeHtml(deviceInfo.id)

        html += """
                <tr>
                <td><b><a href='/device/edit/${safeId}' target='_blank' style='color: #2196F3; text-decoration: underline;'>${safeName}</a></b></td>
                <td>${safeLabel}</td>
                <td>${safeId}</td>
        """
        columns.each { colName ->
            def rawValue = deviceInfo.values[colName] ?: ""
            def safeValue = escapeHtml(rawValue)
...
def getFilename() {
    def timestamp = getSettingBool("includeTimestamp", true) ? "_${new Date().format('yyyyMMdd_HHmmss')}" : ""
