...
def mainPage() {
    dynamicPage(name: "mainPage", title: "Device Information Viewer", install: true, uninstall: true) {
        section("Device Selection") {
            input "selectedDevices", "capability.actuator", title: "Choose Devices (Actuators)", multiple: true, required: false
            input "selectedSensors", "capability.sensor", title: "Choose Devices (Sensors)", multiple: true, required: false
        }
...
def collectDeviceData() {
    ...
    if (selectedSensors) combinedDevices.addAll(selectedSensors)
    ...
    combinedDevices.each { device ->
        ...
        deviceDataList.add([
            ...
            label: device.label ?: "", // Extracts custom device label configuration
            id: device.id,
            values: deviceValues
        ])
    }
...
def getDefaultSortColumnIndex(columns) {
    ...
    // Shift indices down by 1 because Label now sits at index 1
