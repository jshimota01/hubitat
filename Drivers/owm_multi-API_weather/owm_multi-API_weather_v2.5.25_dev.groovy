...
static String version()    {  return '2.5.25-Dev'  }
...
metadata {
    ...
    preferences {
        ...
        input name: "precipUnit", type: "enum", title: "Display Unit - Precipitation (Rain/Sleet/Snow)", options: ["mmHr": "Millimeters per Hour (mmHr)", "inHr": "Inches per Hour (inHr)", "\"": "Inches (\")", "none": "None (No Unit Suffix)"], description: "Choice of precipitation unit formatting used in tiles, short-term metrics, and logging<br>Default: <b>Inches per Hour (inHr)</b>", defaultValue: "inHr", required: true
...
private void calcMinutelyPrecipitation(List minutelyList) {
    ...
    if (!minutelyList || minutelyList.isEmpty()) {
        ...
        sendIfChanged(name: "precipNextHrCurrentRateText", value: "0.0${spaceStr}${pUnit}")
        sendIfChanged(name: "precipNextHrAccumulation", value: 0.0)
        sendIfChanged(name: "precipNextHrAccumulationText", value: "0.0${spaceStr}${pUnit}")
    ...
    BigDecimal displayTotalAccum = convertPrecip(totalAccumulationMm)
    ...
    if (precipStartMinute == -1) {
        ...
        if (precipStopMinute != -1) {
            summaryText = "Precipitation active now, stopping in ${precipStopMinute} min (peak ${displayPeakRate}${spaceStr}${pUnit})."
        } else {
            summaryText = "Precipitation active now and continuing through the next hour (peak ${displayPeakRate}${spaceStr}${pUnit})."
        }
    } else {
        if (precipStopMinute != -1) {
            summaryText = "Precipitation starting in ${precipStartMinute} min and stopping in ${precipStopMinute} min (peak ${displayPeakRate}${spaceStr}${pUnit})."
        } else {
            summaryText = "Precipitation starting in ${precipStartMinute} min and continuing through the next hour (peak ${displayPeakRate}${spaceStr}${pUnit})."
        }
    }

    String currentRateText = "${displayCurrentRate}${spaceStr}${pUnit}"
    String accumulationText = "${displayTotalAccum}${spaceStr}${pUnit}"
