...
static String version()    {  return '2.5.16-Dev'  }
...
metadata {
    definition(
        ...
        // - Minutely Rain attributes
        attribute "minutelyRainCurrent", "number"
        attribute "minutelyRainCurrentText", "string"
        attribute "minutelyRainNextHourTotal", "number"
        attribute "minutelyRainNextHourTotalText", "string"
        attribute "minutelyRainExpected", "number"
        attribute "minutelyRainSummaryText", "string"
    ...
    preferences {
        ...
        input name: "overrideMinutelyExclude", type: "bool", title: "Base Override - Gather Minutely Rain Data", description: "Turn ON to retrieve minutely precipitation forecast data from OpenWeatherMap during scheduled API calls (Does not increase API call quantity)<br>Default: <b>OFF</b> (Excluded)", defaultValue: false, required: true
...
private void parseOWMData(Map json) {
    ...
    // Digest minutely rainfall payload
...
private void calcMinutelyPrecipitation(List minutelyList) {
    ...
    if (!minutelyList || minutelyList.isEmpty()) {
        logDebug "calcMinutelyPrecipitation: No minutely data available in payload."
        sendIfChanged(name: "minutelyRainCurrent", value: 0.0)
        sendIfChanged(name: "minutelyRainCurrentText", value: "0.0${spaceStr}${pUnit}")
        sendIfChanged(name: "minutelyRainNextHourTotal", value: 0.0)
        sendIfChanged(name: "minutelyRainNextHourTotalText", value: "0.0${spaceStr}${pUnit}")
        sendIfChanged(name: "minutelyRainExpected", value: -1)
        sendIfChanged(name: "minutelyRainSummaryText", value: "Minutely data not provided by API.")
        return
    }
    ...
    int rainStartMinute = -1
    int rainStopMinute = -1
    ...
    minutelyList.eachWithIndex { entry, idx ->
        ...
        if (precipRate > 0) {
            if (rainStartMinute == -1) rainStartMinute = idx
            isRaining = true
        } else if (isRaining && rainStopMinute == -1) {
            rainStopMinute = idx
            isRaining = false
        }
    }
    ...
    boolean currentlyRaining = (currentMinuteRate > 0)
    int minutesUntilRain = currentlyRaining ? 0 : rainStartMinute

    String summaryText = ""
    if (rainStartMinute == -1) {
        summaryText = "No precipitation expected in the next hour."
    } else if (currentlyRaining) {
        if (rainStopMinute != -1) {
            summaryText = "Precipitation stopping in ${rainStopMinute} min (peak ${displayPeakRate}${spaceStr}${pUnit})."
        } else {
            summaryText = "Precipitation continuing for the next hour (peak ${displayPeakRate}${spaceStr}${pUnit})."
        }
    } else {
        summaryText = "Precipitation starting in ${rainStartMinute} min (peak ${displayPeakRate}${spaceStr}${pUnit})."
    }

    String currentRateText = "${displayCurrentRate}${spaceStr}${pUnit}"
    String totalAccumText = "${displayTotalAccum}${spaceStr}${pUnit}"

    sendIfChanged(name: "minutelyRainCurrent", value: displayCurrentRate)
    sendIfChanged(name: "minutelyRainCurrentText", value: currentRateText)
    sendIfChanged(name: "minutelyRainNextHourTotal", value: displayTotalAccum)
    sendIfChanged(name: "minutelyRainNextHourTotalText", value: totalAccumText)
    sendIfChanged(name: "minutelyRainExpected", value: minutesUntilRain)
    sendIfChanged(name: "minutelyRainSummaryText", value: summaryText)

    logDebug "Minutely Parsed -> Expected in: ${minutesUntilRain} min | Current: ${currentRateText} | Total Depth: ${totalAccumText} | ${summaryText}"
}
