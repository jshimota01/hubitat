...
static String version()    {  return '2.5.20-Dev'  }
...
metadata {
    definition(
        ...
        attribute "next60MinPrecipCurrent", "number"
        attribute "next60MinPrecipCurrentText", "string"
        attribute "next60MinPrecipTotal", "number"
        attribute "next60MinPrecipTotalText", "string"
    ...
    preferences {
        ...
        input name: "overrideMinutelyExclude", type: "bool", title: "Base Override - Gather Next 60-Minute Precipitation Data", description: "Turn ON to retrieve 60-minute  precipitation forecast data during scheduled API calls (Precipitation = Rain, Sleet & Snow) (Does not increase API call quantity)<br><i>*Formatted using the Display Decimal Precision & Unit for Precipitation settings below.</i><br>Default: <b>OFF</b> (Excluded)", defaultValue: false, required: true
        
        input name: "precipOverflowHandling", type: "enum", title: "Short-Term Precip - 60+ Min Continuation Output", options: ["60": "60 (Numeric Cap: Cap at 60 min for rule threshold comparisons)", "-2": "-2 (Explicit Overflow: Use -2 when precip continues past 60 min window)", "-1": "-1 (Legacy Sentinel: Use -1 for any unobserved stop time)"], defaultValue: "60", description: "Select what value next60MinPrecipStopMinutes reports when precipitation continues beyond the visible 60-minute forecast window.<br>Default: <b>60</b>", required: true
        ...
        input name: "precisionPrecip", type: "enum", title: "Display Decimal Precision - Precipitation", options: ["0": "0 Places", "1": "1 Place", "2": "2 Places"], description: "Choice of decimal precision for precipitation readings (rain, snow, and next60Min short-term metrics) in logging and tiles<br>Default: <b>2</b><br><i>EG: 1, 1.5, 1.55</i>", defaultValue: "2", required: true
        ...
        input name: "precipUnit", type: "enum", title: "Display Unit - Precipitation (Rain/Snow/Next60Min)", options: ["mmHr": "Millimeters per Hour (mmHr)", "inHr": "Inches per Hour (inHr)", "\"": "Inches (\")", "none": "None (No Unit Suffix)"], description: "Choice of precipitation unit formatting used in tiles, short-term next60Min metrics, and logging<br>Default: <b>Inches per Hour (inHr)</b>", defaultValue: "inHr", required: true
...
private void calcMinutelyPrecipitation(List minutelyList) {
    ...
    if (!minutelyList || minutelyList.isEmpty()) {
        ...
        sendIfChanged(name: "next60MinPrecipCurrent", value: 0.0)
        sendIfChanged(name: "next60MinPrecipCurrentText", value: "0.0${spaceStr}${pUnit}")
        sendIfChanged(name: "next60MinPrecipTotal", value: 0.0)
        sendIfChanged(name: "next60MinPrecipTotalText", value: "0.0${spaceStr}${pUnit}")
    ...
    // Resolve Stop Minutes based on user preference when rain continues past 60 min
    int minutesUntilPrecipStop = precipStopMinute
    if (precipStopMinute == -1 && precipStartMinute != -1) {
        String overflowPref = settings.precipOverflowHandling ?: "60"
        switch (overflowPref) {
            case "60":
                minutesUntilPrecipStop = 60
                break
            case "-2":
                minutesUntilPrecipStop = -2
                break
            case "-1":
            default:
                minutesUntilPrecipStop = -1
                break
        }
    }
    ...
    String totalAccumText = "${displayTotalAccum}${spaceStr}${pUnit}"

    sendIfChanged(name: "next60MinPrecipCurrent", value: displayCurrentRate)
    sendIfChanged(name: "next60MinPrecipCurrentText", value: currentRateText)
    sendIfChanged(name: "next60MinPrecipTotal", value: displayTotalAccum)
    sendIfChanged(name: "next60MinPrecipTotalText", value: totalAccumText)
    ...
    logDebug "Next60Min Short-Term Precip Parsed -> Start in: ${minutesUntilPrecipStart} min | Stop in: ${minutesUntilPrecipStop} min | Current: ${currentRateText} | Total Depth: ${totalAccumText} | ${summaryText}"
}
