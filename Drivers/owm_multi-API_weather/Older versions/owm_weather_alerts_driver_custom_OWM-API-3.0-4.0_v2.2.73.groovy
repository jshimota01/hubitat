...
metadata {
    ...
    preferences {
        ...
        input name: "altMoonPhaseImageLoc", type: "text", title: "Base Override - Moon Phase Image Location", description: "Optional - Moon Phase Image Source Location:<br><i>blank for default Moon Phase Image location</i>", required: false
...
void calcMoonPhaseValues(Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    def phases = [
        [sourceMap: todayData, apiKey: "moon_phase", valAttr: "todayMoonPhase", iconAttr: "todayMoonPhaseIcon", textAttr: "todayMoonPhaseText", emojiAttr: "todayMoonPhaseEmojiIcon"],
        [sourceMap: tomData,   apiKey: "moon_phase", valAttr: "tomMoonPhase",   iconAttr: "tomMoonPhaseIcon",   textAttr: "tomMoonPhaseText",   emojiAttr: "tomMoonPhaseEmojiIcon"],
        [sourceMap: tdaData,   apiKey: "moon_phase", valAttr: "tdaMoonPhase",   iconAttr: "tdaMoonPhaseIcon",   textAttr: "tdaMoonPhaseText",   emojiAttr: "tdaMoonPhaseEmojiIcon"]
    ]
    ...
    phases.each { phase ->
        ...
        if (rawVal != null && rawVal.toString().isNumber()) {
            ...
            // Choose the filename pattern dynamically based on override preference state
            String filename = isPathOverridden ? "moon-phase-icon-${imgNum}.png" : "mp${imgNum}.png"
            ...
            // 2. Map the normalized value directly to the correct emoji index
            ...
            // 3. Determine phase name using small range tolerances (+/- 0.02) around major quarter marks
    ...
    // Process the 'current' attribute based on today's calculated values
    if (todayVal != null) {
...
String generateSvgMask(double phase, String path) {
...
void calcMoonPhaseSVGIcon(Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    targets.each { entry ->
        ...
        if (rawVal != null && rawVal.toString().isNumber()) {
            double val = (rawVal.toDouble() % 1.0 + 1.0) % 1.0
            String svgContent = generateSvgMask(val, path)
