...
private void parseOWMData(Map json) {
    if (!json) {
        logWarn "parseOWMData received an empty payload map." [cite: 27, 28]
        return [cite: 28]
    }
    
    logDebug "Parsing newly received OpenWeatherMap response data structure..." [cite: 28]
    
    // Extract location and configuration details for the alert builder
    String calculatedCityAttr = state.usedCity ?: "Local Area" [cite: 28]
    String iconBasePath = state.iconBasePath ?: "https://tinyurl.com/icnqz/" [cite: 28]
    
    // Execute alerts calculation with live payload data
    calcAlertsState(json, calculatedCityAttr, iconBasePath) [cite: 28]
    ...
    def currentData = json.current ?: [:] [cite: 28]
    if (currentData) {
        logTrace "Current weather data payload extracted successfully." [cite: 28]
        // --- ADDED: Extract and track true sunrise/sunset epochs ---
        if (currentData.sunrise) state.todaySunriseEpoch = currentData.sunrise.toLong() [cite: 29]
        if (currentData.sunset) state.todaySunsetEpoch = currentData.sunset.toLong() [cite: 29]
        
        // Ensure rain and snow default to 0 if missing or nested improperly from OWM
        def rainVal = currentData.rain?.getAt("1h") != null ? currentData.rain["1h"] : 0 [cite: 29, 30]
        def snowVal = currentData.snow?.getAt("1h") != null ? currentData.snow["1h"] : 0 [cite: 30, 31]
        currentData["calculatedRain"] = rainVal [cite: 31]
        currentData["calculatedSnow"] = snowVal [cite: 31]
    }
    
    // 3. Process Daily forecast arrays safely
    def dailyList = json.daily ?: [] [cite: 31]
    
    // Gather Today data (data.0)
    def data0 = dailyList.size() > 0 ? dailyList[0] : [:] [cite: 31, 32]
    def data1 = dailyList.size() > 1 ? dailyList[1] : [:] [cite: 32]
    def data2 = dailyList.size() > 2 ? dailyList[2] : [:] [cite: 32, 33]
    
    // Route all isolated datasets into the custom event dispatcher
    sendOWMData(currentData, data0, data1, data2) [cite: 33]
    
    // --- ADDED: Dynamically rebuild cron schedules based on the new true values ---
    if (state.todaySunriseEpoch && state.todaySunsetEpoch) {
        updateDynamicSchedules(state.todaySunriseEpoch, state.todaySunsetEpoch) [cite: 33]
    }
}
