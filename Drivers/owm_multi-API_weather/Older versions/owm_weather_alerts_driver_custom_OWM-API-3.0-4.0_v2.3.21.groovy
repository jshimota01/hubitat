...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    String moonBodyHtml = "<div style='display:flex;justify-content:center;align-items:center;height:100%;width:100%;text-align:center;position:relative;overflow:hidden;container-type:inline-size;'>" +
                          graphicDisplay +
                          "<div style='position:absolute;bottom:42%;width:90%;font-size:clamp(.6rem,11cqi,1.2rem);font-weight:bold;text-shadow:1px 1px 3px #000;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#fff;'>${moonPhaseText}</div>" +
                          "</div>"
