...
metadata {
    definition(name: "OpenWeatherMap Multi-Version Weather Driver 2.0", namespace: "jshimota", author: "James Shimota") {
		...
		attribute "currentMoonPhaseSVGIcon", "string"
		attribute "currentMoonPhasePNGIcon", "string"
		attribute "currentMoonPhaseEmojiIcon", "string"
		attribute "currentMoonPhaseText", "string"
        ...
        attribute "todayMoonPhaseSVGIcon", "string"
        attribute "tomMoonPhaseSVGIcon", "string"
        attribute "tdaMoonPhaseSVGIcon", "string"
        attribute "todayMoonPhasePNGIcon", "string"
        attribute "tomMoonPhasePNGIcon", "string"
        attribute "tdaMoonPhasePNGIcon", "string"
    ...
    preferences {
        ...
        input name: "altMoonPhaseImageLoc", type: "text", title: "Base Override - Moon Phase Image Location", description: "Optional - Moon Phase Image Source Location:<br><i>blank for default Moon Phase Image location<br>https://raw.githubusercontent.com/thebearmay/hubitat/main/moonPhaseRes/<br>(Supplied by @thebearmay of the Hubitat community who wrote & maintains the original MoonPhase Tile driver)</i>", required: false
        input name: "altWindDirImageLoc", type: "text", title: "Base Override - Wind Direction Image Location", description: "Optional - Wind Direction Image Source Location:<br><i>blank for default Moon Phase Image location<br>https://raw.githubusercontent.com/thebearmay/hubitat/main/moonPhaseRes/<br>(Supplied by @thebearmay of the Hubitat community who wrote & maintains the original MoonPhase Tile driver)</i>", required: false
...
def updated() {
	...
	["currentMoonPhaseText", "todayMoonPhaseText", "tomMoonPhaseText", "tdaMoonPhaseText"].each { sendIfChanged(name: it, value: "--") }
    ...
    // Update moon phase base path dynamically on preferences update
    state.moonPhaseImageLoc = calcMoonPhaseImageLoc(settings.altMoonPhaseImageLoc)
...
def initialize() {
    ...
    // Initialize MoonPhaseImageLoc fallback path safely as a persistent state
    state.moonPhaseImageLoc = calcMoonPhaseImageLoc(settings.altMoonPhaseImageLoc)
...
void scheduledTextValue(List dataList) {
    ...
    if (dataList && dataList.size() >= 9) {
        ...
        // --- RACE CONDITION FIX: Run Moon Phase calculations synchronously before generating tiles ---
        calcMoonPhaseValue(todayData, tomData, tdaData)
        calcMoonPhaseSVGIcon(todayData, tomData, tdaData)
        
        // --- Run tile generation ONLY after the delayed async calculation finishes ---
...
void scheduledMoonPhaseValue(List dataList) {
    logDebug "Unpacking scheduled calcMoonPhaseValue arguments safely..."
    if (dataList && dataList.size() >= 3) {
        calcMoonPhaseValue(
            dataList[0] instanceof Map ? dataList[0] : [:],
            dataList[1] instanceof Map ? dataList[1] : [:],
            dataList[2] instanceof Map ? dataList[2] : [:]
        )
    } else {
        logWarn "Scheduled calcMoonPhaseValue skipped: Argument list was incomplete."
    }
}
...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    String forecastHtml = ""
    if (settings.debugTileEnable == true) {
        String forecastDivider = "<div style='border-top:1px solid rgba(255,255,255,0.1); padding-top:4px; margin-top:4px;'>"
        String forecastSuffixTemplate = "<span style='font-size:0.6em;color:#888;'>Char length: 0000</span></div></div>"
        int forecastLenEstimate = forecastBodyHtml.length() + forecastDivider.length() + forecastSuffixTemplate.length()
        forecastHtml = forecastBodyHtml + forecastDivider + "<span style='font-size:0.6em;color:#888;'>Char length: ${forecastLenEstimate}</span></div></div>"
    } else {
        forecastHtml = forecastBodyHtml + "</div>"
    }
    ...
    String alertDesc = device.currentValue("currentAlertDesc") ?: "No active alerts"
    String lastPollTime = new Date().format("HH:mm", location.timeZone)
    String baseStyle = "display:flex;flex-direction:column;justify-content:space-between;height:100%;padding:8px;box-sizing:border-box;background:rgba(30,30,40,0.65);border-radius:6px;color:#fff;font-family:sans-serif;line-height:1.2;text-align:center"
    String currentAlertTile = ""

    if (alertActive != "No active alerts" && alertActive != "N/A" && alertActive != "") {
        String alertBody = "<div style='${baseStyle}'><div style='font-size:0.85em;font-weight:bold;color:#ff5555;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-bottom:2px;'>⚠️ ${alertActive}</div><div style='font-size:0.8em;font-weight:600;color:#ddd;padding:4px;overflow:hidden;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;'>${alertDesc}</div><div style='display:flex;justify-content:space-between;border-top:1px solid rgba(255,255,255,0.15);padding-top:4px;font-size:0.7em;color:#bbb'><span>👤 ${alertSender.take(12)}</span><span>⏰ ${lastPollTime}</span></div>"
        
        if (settings.debugTileEnable == true) {
            String footerStart = "<div style='font-size:0.6em;color:#888;margin-top:2px;'>"
            int alertLenEstimate = alertBody.length() + footerStart.length() + "999 chars long</div></div>".length()
            currentAlertTile = alertBody + footerStart + "${alertLenEstimate} chars long</div></div>"
        } else {
            currentAlertTile = alertBody + "</div>"
        }
    } else {
        String noAlertBody = "<div style='${baseStyle}'><div style='font-size:0.85em;font-weight:bold;color:#55ff55;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-bottom:2px;'>✅ ${cityName}</br>Clear Weather</div><div style='font-size:0.8em;font-weight:600;color:#aaa;padding:8px 4px;'>No active weather alerts from OpenWeatherMap or their sources</div><div style='display:flex;justify-content:center;border-top:1px solid rgba(255,255,255,0.15);padding-top:4px;font-size:0.7em;color:#bbb'><span>Updated ${lastPollTime}</span></div>"
        
        if (settings.debugTileEnable == true) {
            String footerStart = "<div style='font-size:0.6em;color:#888;margin-top:2px;'>"
            String alertSuffixTemplate = "Char length: 0000</div></div>"
            int alertLenEstimate = noAlertBody.length() + footerStart.length() + alertSuffixTemplate.length()
            currentAlertTile = noAlertBody + footerStart + "Char length: ${alertLenEstimate}</div></div>"
        } else {
            currentAlertTile = noAlertBody + "</div>"
        }
    }
    sendIfChanged(name: "currentAlertTile", value: currentAlertTile)

    // =========================================================================
    // SECTION 4: currentMoonPhaseTile (Dynamic Image / SVG Overlay Style)
    // =========================================================================
    String moonPhaseText = "--"
    String moonSvg = ""
    String moonPngUrl = ""

    def rawMoonPhase = (todayData && todayData.moon_phase != null) ? todayData.moon_phase : device.currentValue("todayMoonPhase")
    
    if (rawMoonPhase != null && rawMoonPhase.toString().isNumber()) {
        double val = (rawMoonPhase.toDouble() % 1.0 + 1.0) % 1.0
        int index = (int) Math.floor((val * 8) + 0.5) % 8
        
        double tolerance = 0.02
        if (val <= tolerance || val >= (1.0 - tolerance)) moonPhaseText = "New Moon"
        else if (val > tolerance && val < (0.25 - tolerance)) moonPhaseText = "Waxing Crescent"
        else if (val >= (0.25 - tolerance) && val <= (0.25 + tolerance)) moonPhaseText = "First Quarter Moon"
        else if (val > (0.25 + tolerance) && val < (0.5 - tolerance)) moonPhaseText = "Waxing Gibbous"
        else if (val >= (0.5 - tolerance) && val <= (0.5 + tolerance)) moonPhaseText = "Full Moon"
        else if (val > (0.5 + tolerance) && val < (0.75 - tolerance)) moonPhaseText = "Waning Gibbous"
        else if (val >= (0.75 - tolerance) && val <= (0.75 + tolerance)) moonPhaseText = "Last Quarter Moon"
        else if (val > (0.75 + tolerance) && val < (1.0 - tolerance)) moonPhaseText = "Waning Crescent"

        boolean isPathOverridden = (settings.altMoonPhaseImageLoc != null && settings.altMoonPhaseImageLoc.trim() != "")
        String filename = isPathOverridden ? "mp${index}.png" : "moon-phase-icon-${index}.png"
        String basePath = state.moonPhaseImageLoc ?: "https://raw.githubusercontent.com/thebearmay/hubitat/main/moonPhaseRes/"
        moonPngUrl = "${basePath}${filename}"
        moonSvg = calcMoonPhaseSvgMask(val, basePath)
    } else {
        moonPhaseText = device.currentValue("currentMoonPhaseText") ?: "Unknown"
        moonSvg = device.currentValue("currentMoonPhaseSVGIcon") ?: ""
        moonPngUrl = device.currentValue("currentMoonPhasePNGIcon") ?: ""
    }

    String graphicDisplay = ""
    if (settings.displayTileMoonPhaseSVGEnable == true && moonSvg != "") {
        // Highly compressed dynamic style block replacements
        graphicDisplay = moonSvg.replace("<svg ", "<svg style='width:100%;height:100%;max-width:100%;max-height:100%;margin:auto;display:block;' ")
    } else if (moonPngUrl != "") {
        graphicDisplay = "<img src='${moonPngUrl}' style='width:100%;height:100%;max-width:100%;max-height:100%;object-fit:contain;margin:auto;display:block;'>"
    } else {
        String moonEmoji = device.currentValue("currentMoonPhaseEmojiIcon") ?: "🌑"
        graphicDisplay = "<div style='font-size:min(12vw,12vh,4.5em);line-height:1;margin:auto;'>${moonEmoji}</div>"
    }

    // Minimized HTML wrap with streamlined styling properties
    String moonBodyHtml = "<div style='display:flex;flex-direction:column;justify-content:center;align-items:center;height:100%;padding:8px;box-sizing:border-box;background:rgba(30,30,40,0.65);border-radius:6px;color:#fff;font-family:sans-serif;position:relative;text-align:center;overflow:hidden;'>" +
                          "<div style='position:relative;display:flex;align-items:center;justify-content:center;z-index:1;width:100%;height:calc(100% - 25px);max-height:calc(100% - 25px);min-height:0;'>" +
                          graphicDisplay +
                          "</div>" +
                          "<div style='position:absolute;width:100%;bottom:10px;font-size:0.85em;font-weight:bold;-webkit-text-stroke:1px #000;text-shadow:2px 2px 2px #000;z-index:2;pointer-events:none;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'>${moonPhaseText}</div>"

    String currentMoonPhaseTile = ""
    if (settings.debugTileEnable == true) {
        String footerStart = "<div style='font-size:0.55em;color:#777;margin-top:2px;z-index:2;'>"
        String suffixTemplate = "Char length: 0000</div></div>"
        int moonLenEstimate = moonBodyHtml.length() + footerStart.length() + suffixTemplate.length()
        currentMoonPhaseTile = moonBodyHtml + footerStart + "Char length: ${moonLenEstimate}</div></div>"
    } else {
        currentMoonPhaseTile = moonBodyHtml + "</div>"
    }
    sendIfChanged(name: "currentMoonPhaseTile", value: currentMoonPhaseTile)
}
...
private void parseOWMData(Map json) {
    ...
    // --- FIX PATH: If initializing, skip current inline tile generation, as it is handled by the 2-second out-of-band schedule.
    if (state.isInitializing == true) {
        ...
        // Pass safely instantiated local values to out-of-band thread (generateTiles will execute inside scheduledTextValue)
        runIn(2, "scheduledTextValue", [data: [freshLux, freshTemp, freshPress, freshWind, freshHum, currentData, data0, data1, data2]])
    } else {
        // Safe direct execution pathway during normal updates
...
private void sendOWMData(Map current, Map today, Map tom, Map tda) {
    ...
    // uses api response data so it works on initialization 
    calcMoonPhaseValue(today, tom, tda)
    // Dynamic SVG Generation triggered right alongside traditional asset processing
    calcMoonPhaseSVGIcon(today, tom, tda)
...
private Map convertWindDirectionState(degrees) {
    if (degrees == null) {
        ...
        return [cardinal: "Unknown", full: "Unknown", iconUrl: "💨 N/A"]
    }
    double deg = 0.0
    try {
    ...
    // Check if we have a valid custom icon location preference set
    String rawPath = settings.altIconLoc ?: ""
    String finalIconUrl = ""

    if (rawPath.trim() != "" && settings.altIconsEnable == true) {
        ...
        finalIconUrl = basePath.contains("openweathermap.org") ? 
            "https://raw.githubusercontent.com/HubitatCommunity/WeatherIcons/master/${filename}" : 
            "${basePath}${filename}"
    } else {
        // Fallback directly to your custom Emoji and Compass string if no location is configured
        finalIconUrl = getWindDirectionEmoji(deg)
    }
...
//**
 * Resolves wind degrees to a 16-point compass heading with an emoji.
 * @param degrees The wind direction in degrees (0 - 360).
 * @return A string containing [Emoji] [Compass Abbreviation] (e.g., "↙️ SSW")
 */
String getWindDirectionEmoji(Object degrees) {
    if (degrees == null) return "💨 N/A"
    
    // Normalize input to float and handle bounds
    float deg = degrees.toFloat()
    deg = (deg % 360 + 360) % 360 

    // 16-point compass lookup array (Emoji + Abbreviation)
    // Arrows point in the direction the wind is blowing (from -> to)
    def compass = [
        [emoji: "⬇️", dir: "N"],   [emoji: "⬇️", dir: "NNE"], [emoji: "↙️", dir: "NE"],  [emoji: "↙️", dir: "ENE"],
        [emoji: "⬅️", dir: "E"],   [emoji: "⬅️", dir: "ESE"], [emoji: "↖️", dir: "SE"],  [emoji: "↖️", dir: "SSE"],
        [emoji: "⬆️", dir: "S"],   [emoji: "⬆️", dir: "SSW"], [emoji: "↗️", dir: "SW"],  [emoji: "↗️", dir: "WSW"],
        [emoji: "➡️", dir: "W"],   [emoji: "➡️", dir: "WNW"], [emoji: "↘️", dir: "NW"],  [emoji: "↘️", dir: "NNW"]
    ]

    // Calculate 16-point index: (degrees + 11.25) / 22.5, wrapped at 16
    int index = Math.round((deg + 11.25) / 22.5) % 16
    def point = compass[index]

    return "${point.emoji} ${point.dir}"
}


private String calcMoonPhaseImageLoc(String altLoc) {
    // Clean and validate user preference override string, fall back to GitHub main path
    String base = altLoc ? altLoc.trim() : "https://raw.githubusercontent.com/thebearmay/hubitat/main/moonPhaseRes/"
    if (!base.endsWith("/")) base += "/"
    logDebug "Calculated Moon Phase Image base path resolved to: ${base}"
    return base
}

void calcMoonPhaseValue(Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    def phases = [
        [sourceMap: todayData, apiKey: "moon_phase", valAttr: "todayMoonPhase", iconAttr: "todayMoonPhaseIcon", pngAttr: "todayMoonPhasePNGIcon", textAttr: "todayMoonPhaseText", emojiAttr: "todayMoonPhaseEmojiIcon"],
        [sourceMap: tomData,   apiKey: "moon_phase", valAttr: "tomMoonPhase",   iconAttr: "tomMoonPhaseIcon",   pngAttr: "tomMoonPhasePNGIcon",   textAttr: "tomMoonPhaseText",   emojiAttr: "tomMoonPhaseEmojiIcon"],
        [sourceMap: tdaData,   apiKey: "moon_phase", valAttr: "tdaMoonPhase",   iconAttr: "tdaMoonPhaseIcon",   pngAttr: "tdaMoonPhasePNGIcon",   textAttr: "tdaMoonPhaseText",   emojiAttr: "tdaMoonPhaseEmojiIcon"]
    ]
    ...
    phases.each { phase ->
        ...
        if (rawVal != null && rawVal.toString().isNumber()) {
            ...
            // Re-routed dynamically to the newly initialized moonPhaseImageLoc state path
            String basePath = state.moonPhaseImageLoc ?: "https://raw.githubusercontent.com/thebearmay/hubitat/main/moonPhaseRes/"
            String fullPath = "${basePath}${filename}"
            
            // Store to both standard legacy icon mapping and the dedicated PNG attributes
            sendIfChanged(name: phase.iconAttr, value: fullPath)
	...
	// Process the 'current' attributes based on today's calculated values
    if (todayVal != null) {
        int currentEmojiIndex = (int) Math.floor((todayVal * 8) + 0.5) % 8
        sendIfChanged(name: "currentMoonPhaseEmojiIcon", value: emojis[currentEmojiIndex])
        
        // --- ADDED: Track and update current text ---
        String currentPhaseName = device.currentValue("todayMoonPhaseText") ?: "Unknown"
        sendIfChanged(name: "currentMoonPhaseText", value: currentPhaseName)
        
        // Output current PNG icon path
        int index = (int) Math.floor((todayVal * 8) + 0.5) % 8
        String filename = isPathOverridden ? "mp${index}.png" : "moon-phase-icon-${index}.png"
        String basePath = state.moonPhaseImageLoc ?: "https://raw.githubusercontent.com/thebearmay/hubitat/main/moonPhaseRes/"
        sendIfChanged(name: "currentMoonPhasePNGIcon", value: "${basePath}${filename}")
    } else {
        // Fallback to active DB values if OWM payload maps were empty
        def fallbackRaw = device.currentValue("todayMoonPhase")
        if (fallbackRaw != null && fallbackRaw.toString().isNumber()) {
            double fallbackVal = (fallbackRaw.toDouble() % 1.0 + 1.0) % 1.0
            int currentEmojiIndex = (int) Math.floor((fallbackVal * 8) + 0.5) % 8
            sendIfChanged(name: "currentMoonPhaseEmojiIcon", value: emojis[currentEmojiIndex])
            ...
            int index = (int) Math.floor((fallbackVal * 8) + 0.5) % 8
            String filename = isPathOverridden ? "mp${index}.png" : "moon-phase-icon-${index}.png"
            String basePath = state.moonPhaseImageLoc ?: "https://raw.githubusercontent.com/thebearmay/hubitat/main/moonPhaseRes/"
            sendIfChanged(name: "currentMoonPhasePNGIcon", value: "${basePath}${filename}")
        }
...
String calcMoonPhaseSvgMask(double phase, String path) {
    if (phase <= 0.02 || phase >= 0.98) {
        return """<svg viewBox="0 0 256 256"><circle cx="128" cy="128" r="120" fill="#121214"/></svg>"""
    }
    double rx1 = 120.0, rx2 = 120.0
    int sf1 = 1, sf2 = 1
    ...
    // Strip out all non-essential formatting spaces and shorten float decimals
    return """<svg viewBox="0 0 256 256"><filter id="b"><feGaussianBlur stdDeviation="6"/></filter><mask id="a"><path d="M128,8A${rx1.round(1)},120 180 0 $sf1 128,248A${rx2.round(1)},120 180 0 $sf2 128,8z" fill="#fff" filter="url(#b)"/></mask><radialGradient id="s"><stop offset="10%" stop-color="#0007"/><stop offset="90%" stop-color="#000d"/></radialGradient><image width="256" height="256" href="${path}lunar_surface.png"/><circle cx="128" cy="128" r="120" mask="url(#a)" fill="url(#s)"/></svg>"""
}

// Custom handler calculating SVGs from target forecast maps and publishing to new attributes
void calcMoonPhaseSVGIcon(Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    logDebug "Evaluating moon phase calculations for SVG output variables..."
    
    // Correctly routes background image references using the state.moonPhaseImageLoc variable
    String path = state.moonPhaseImageLoc ?: "https://raw.githubusercontent.com/thebearmay/hubitat/main/moonPhaseRes/"
    
    def targets = [
        [sourceMap: todayData, apiKey: "moon_phase", valAttr: "todayMoonPhase", targetAttr: "todayMoonPhaseSVGIcon"],
        [sourceMap: todayData, apiKey: "moon_phase", valAttr: "todayMoonPhase", targetAttr: "currentMoonPhaseSVGIcon"], // Map to current
        [sourceMap: tomData,   apiKey: "moon_phase", valAttr: "tomMoonPhase",   targetAttr: "tomMoonPhaseSVGIcon"],
        [sourceMap: tdaData,   apiKey: "moon_phase", valAttr: "tdaMoonPhase",   targetAttr: "tdaMoonPhaseSVGIcon"]
    ]
