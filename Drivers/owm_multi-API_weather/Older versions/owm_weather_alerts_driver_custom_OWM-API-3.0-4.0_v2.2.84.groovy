...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    // Resolve "today" (current) moon phase values in-memory to bypass Hubitat DB commit latency
    ...
    if (rawMoonPhase != null && rawMoonPhase.toString().isNumber()) {
        ...
        // Fallback to database values if no map context exists
    ...
    // Build the graphic visualization block respecting settings.displayTileMoonPhaseSVGEnable
    String graphicDisplay = ""
    if (settings.displayTileMoonPhaseSVGEnable == true && moonSvg != "") {
        // Use responsive dimensions to scale to the parent SVG container
        graphicDisplay = moonSvg.replace("<svg ", "<svg style='width:100%; height:100%; max-width:100%; max-height:100%; margin: auto; display:block;' ")
    } else if (moonPngUrl != "") {
        // Remove fixed widths/heights and use object-fit: contain for dynamic scaling
    ...
    // CSS layer setup with absolute overlays to perfectly center the outlined text directly over the graphic
    // Removed the fixed width:90px and height:90px constraint, letting height:calc(100% - 25px) take over dynamically
    String moonBodyHtml = "<div style='display:flex; flex-direction:column; justify-content:center; align-items:center; height:100%; padding:8px; box-sizing:border-box; background:rgba(30,30,40,0.65); border-radius:6px; color:#fff; font-family:sans-serif; position:relative; text-align:center; overflow:hidden;'>" +
                          ...
                          graphicDisplay +
                          "</div>" +
                          "<div style='position:absolute; width:100%; bottom:10px; font-size:0.85em; font-weight:bold; color:#fff; -webkit-text-stroke:1px #000; text-shadow:2px 2px 2px #000; z-index:2; pointer-events:none; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;'>${moonPhaseText}</div>"

    String currentMoonPhaseTile = ""
    if (settings.debugTileEnable == true) {
        String footerStart = "<div style='font-size:0.55em; color:#777; margin-top:2px; z-index:2;'>"
        int moonLenEstimate = moonBodyHtml.length() + footerStart.length() + "Char length: 0000</div></div>".length()
...
String calcMoonPhaseSvgMask(double phase, String path) {
    ...
    // Coordinates updated to 256x256 native scale
    return """<svg width="256" height="256" viewBox="0 0 256 256">
      <filter id="blur"><feGaussianBlur stdDeviation="8"/></filter>
      <mask id="arc"><path d="M128,8 A$rx1,120 180 0 $sf1 128,248 A$rx2,120 180 0 $sf2 128,8 z" fill="#fff" filter="url(#blur)"/></mask>
      <radialGradient id="shadow"><stop offset="10%" stop-color="#0007"/><stop offset="90%" stop-color="#000d"/></radialGradient>
      <image x="0" y="0" width="256" height="256" href="${path}lunar_surface.png"/>
      <circle cx="128" cy="128" r="120" mask="url(#arc)" fill="url(#shadow)"/>
    </svg>"""
}
