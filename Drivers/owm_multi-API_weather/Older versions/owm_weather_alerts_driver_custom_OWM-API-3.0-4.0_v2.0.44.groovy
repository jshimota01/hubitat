...
private void requestWeatherData(def lat, def lon) {
	...
	if (apiSelection == "2.5") {
		...
		url = "https://api.openweathermap.org/data/4.0/onecall?lat=${lat}&lon=${lon}&units=${owmUnits}&exclude=minutely,hourly&appid=${apiKey}"
	}
...
private def parsePayload(Map json) {
    ...
    if (obTime) {
        ...
        sendIfChanged(name: "lastObservationForecast", value: obDate.format("yyyy-MM-dd HH:mm:ss", location.timeZone))
    }
    ...
    sendIfChanged(name: "conditionIconTodayName", value: iconNameValue)
    ...
    sendIfChanged(name: "sunrise", value: srTime)
    sendIfChanged(name: "sunriseTime", value: srTime)
    sendIfChanged(name: "sunset", value: ssTime)
    ...
    sendIfChanged(name: "moonise", value: moonRiseStr)
