...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    // Helper to append debug length footer consistently across all tiles
    def appendTileDebug = { String bodyHtml ->
        if (settings.debugTileEnable != true) return bodyHtml + "</div>"
        String footerStart = "<hr style='width:100%'><div align='center' style='font-size:.6em;color:#555'>Len:"
        int estimatedLen = bodyHtml.length() + footerStart.length() + "999</div></div>".length()
        return bodyHtml + footerStart + "${estimatedLen}</div></div>"
    }
    ...
    String alertBodyHtml = "<div style='display:flex;flex-direction:column;justify-content:space-between;height:100%;padding:8px;box-sizing:border-box;border-radius:6px;color:#fff;line-height:1.2;text-align:center'>"

    if (alertActive != "No active alerts" && alertActive != "N/A" && alertActive != "") {
        // --- DYNAMIC 1024-CHARACTER BOUNDARY CALCULATION ---
        String headerHtml = "<div style='font-size:.85em;color:#ff5555;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'><b>⚠️ ${alertActive}</b></div><div style='font-size:.5em;color:#ddd;padding:1px 0;overflow:hidden;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;'>"
        String footerHtml = "</div><div style='display:flex;justify-content:space-between;border-top:1px solid rgba(255,255,255,0.15);padding-top:4px;font-size:0.7em;color:#bbb'><span>👤 ${alertSender.take(12)}</span><span>⏰ ${lastPollTime}</span></div>"
        
        // Calculate overhead from outer tags + debug footer
        int debugOverhead = (settings.debugTileEnable == true) ? "<hr style='width:100%'><div align='center' style='font-size:.6em;color:#555'>Len:1024</div></div>".length() : "</div>".length()
        int currentOverhead = alertBodyHtml.length() + headerHtml.length() + footerHtml.length() + debugOverhead

        // Maximum string length budget allocated to alertDescFull
        int maxDescLen = 1024 - currentOverhead
        ...
        alertBodyHtml += "${headerHtml}${truncatedDesc}${footerHtml}"
    } else {
        alertBodyHtml += "<div style='font-size:0.85em;font-weight:bold;color:#55ff55;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-bottom:2px;'>✅ ${cityName}</br>Clear Weather</div><div style='font-size:0.8em;color:#aaa;padding:8px 4px;'>No active weather alerts from OpenWeatherMap or their sources</div><div style='display:flex;justify-content:center;border-top:1px solid rgba(255,255,255,0.15);padding-top:4px;font-size:0.7em;color:#bbb'><span>Updated ${lastPollTime}</span></div>"
    }
    ...
    String moonBodyHtml = "<div style='display:flex;flex-direction:column;justify-content:center;align-items:center;height:100%;padding:8px;box-sizing:border-box;background:#666;border-radius:6px;color:#fff;font-family:sans-serif;text-align:center;overflow:hidden;'>" +
                          ...
                          "</div>"
    sendIfChanged(name: "todayMoonPhaseTile", value: appendTileDebug(moonBodyHtml))
}
