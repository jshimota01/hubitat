...
private BigDecimal calcSunPosition() {
	...
	azimuthDeg = azimuthDeg.toBigDecimal().setScale(2, RoundingMode.HALF_UP)
	altitudeDeg = altitudeDeg.toBigDecimal().setScale(2, RoundingMode.HALF_UP)

    sendIfChanged(name: "azimuth", value: azimuthDeg.setScale(2, BigDecimal.ROUND_HALF_UP).toPlainString(), unit: "°")
    sendIfChanged(name: "altitude", value: altitudeDeg.setScale(2, BigDecimal.ROUND_HALF_UP).toPlainString(), unit: "°")
    return altitudeDeg
}
...
private void calcCurrentIlluminance() {
    ...
    BigDecimal finalValue = convertedValue.setScale(precision, BigDecimal.ROUND_HALF_UP)
    ...
    logDebug "Illuminance Parsed: ${finalValue} ${targetUnit} (Base Lux: ${attenuatedLux.setScale(0, BigDecimal.ROUND_HALF_UP)} lx)"
