...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    def buildForecastCol = { dayLabel, prefix, Map sourceMap ->
        ...
        if (sourceMap && sourceMap.temp) {
            ...
            fPop = device.currentValue("todayPOP") ?: 0
    ...
    // CSS layer setup creating a 3D-feeling card overlay
    String moonBodyHtml = "<div style='display:flex; flex-direction:column; justify-content:space-between; height:100%; padding:8px; box-sizing:border-box; background:rgba(30,30,40,0.65); border-radius:6px; color:#fff; font-family:sans-serif; position:relative; text-align:center; overflow:hidden;'>" +
                          "<div style='font-size:0.85em; font-weight:bold; text-transform:uppercase; letter-spacing:0.05em; color:#bbb; z-index:2;'>Moon Phase</div>" +
...
String calcMoonPhaseSvgMask(double phase, String path) {
    ...
    if (phase <= 0.02 || phase >= 0.98) {
        return """<svg width="256" height="256" viewBox="0 0 140 140" xmlns="http://www.w3.org/2000/svg">
    ...
    // Dynamic graphic generation pulling directly from path setup
	return """<svg width="256" height="256" viewBox="0 0 140 140" xmlns="http://www.w3.org/2000/svg">
