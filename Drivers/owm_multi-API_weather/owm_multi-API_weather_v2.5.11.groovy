...
static String version()    {  return '2.5.11'  }
...
private BigDecimal calcCurrentIlluminance(BigDecimal sunAltitude, def liveClouds = null) {
    ...
    double clouds = Math.max(0.0, Math.min(100.0, rawClouds))
    ...
    if (alt > 0.0) {
        ...
        clearSkyLux = 120000.0 * Math.pow(Math.sin(radAlt), 1.25)
    } else if (alt >= -6.0) {
        // Civil Twilight Exponential Decay (400 lx at 0° down to ~2.5 lx at -6°)
        clearSkyLux = 400.0 * Math.exp(0.85 * alt)
    } else {
    ...
    double cloudTransmittance = 0.20 + (0.80 * Math.pow((100.0 - clouds) / 100.0, 1.5))
    ...
    logDebug "Illuminance Parsed: ${finalValue} ${targetUnit} (Base CIE Lux: ${attenuatedLux.setScale(0, 4)} lx)"
