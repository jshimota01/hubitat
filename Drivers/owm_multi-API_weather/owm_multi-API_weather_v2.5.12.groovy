...
static String version()    {  return '2.5.12'  }
...
private BigDecimal calcCurrentIlluminance(BigDecimal sunAltitude, def liveClouds = null) {
    logDebug "Calculating dynamic current illuminance using CIE Clear/Overcast Sky model..."
    ...
    double alt = sunAltitude.toDouble()
    double rawClouds = cloudPctVal.toDouble()
    
    // Explicit primitive double casting avoids Groovy BigDecimal parameter signature mismatches
    double clouds = Math.max(0.0d, Math.min(100.0d, rawClouds))
    double clearSkyLux = 0.0

    // 1. Solar Elevation Illuminance Model (CIE Standard + Civil Twilight Exponential Decay)
    if (alt > 0.0) {
        // Direct Sun + Clear Sky Skylight (Max ~120,000 lx at zenith)
        double radAlt = Math.toRadians(alt)
        clearSkyLux = 120000.0d * Math.pow(Math.sin(radAlt), 1.25d)
    } else if (alt >= -6.0) {
    ...
    // 2. CIE Non-Linear Cloud Transmittance Attenuation
    ...
    // Safely format primitive double into integer string for log output without triggering setScale on Double
    logDebug "Illuminance Parsed: ${finalValue} ${targetUnit} (Base CIE Lux: ${rawCalculatedLux.setScale(0, java.math.RoundingMode.HALF_UP)} lx)"
