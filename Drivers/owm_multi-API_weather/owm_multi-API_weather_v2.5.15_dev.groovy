...
static String version()    {  return '2.5.15-Dev'  }
...
private void calcMinutelyPrecipitation(List minutelyList) {
    ...
    if (!minutelyList || minutelyList.size() == 0) {
    ...
    minutelyList.eachWithIndex { entry, idx ->
        ...
        if (idx == 0) {
            ...
            if (precipRate > 0) currentlyRaining = true
        }
        ...
        if (precipRate > 0 && rainStartMinute == -1) {
            rainStartMinute = idx
        }
        if (currentlyRaining && precipRate == 0 && rainStopMinute == -1) {
            rainStopMinute = idx
        }
    }
    ...
    BigDecimal displayTotalAccum = totalAccumulationMm
    if (pUnit in ["inHr", "in", "\""]) {
        displayTotalAccum = totalAccumulationMm * 0.0393701
    }
    int precision = (settings.precisionPrecip != null && settings.precisionPrecip != "") ? settings.precisionPrecip.toInteger() : 2
    displayTotalAccum = displayTotalAccum.setScale(precision, java.math.RoundingMode.HALF_UP)

    BigDecimal displayPeakRate = convertPrecip(peakRateMm)

    int minutesUntilRain = rainStartMinute
