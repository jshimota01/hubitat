...
static String version()    {  return '2.5.24-Dev'  }
...
metadata {
    definition(
        ...
        // - Next 60 Minutes Short-Term Precipitation attributes (Rain, Sleet, Snow)
        attribute "next60MinPrecipCurrentRate", "number"
        attribute "next60MinPrecipCurrentRateText", "string"
        attribute "next60MinPrecipHourTotal", "number"
        attribute "next60MinPrecipHourTotalText", "string"
        attribute "next60MinPrecipStartMinutes", "number"
        attribute "next60MinPrecipStopMinutes", "number"
        attribute "next60MinPrecipSummaryText", "string"
...
private void calcMinutelyPrecipitation(List minutelyList) {
    ...
    if (!minutelyList || minutelyList.isEmpty()) {
        ...
        sendIfChanged(name: "next60MinPrecipCurrentRate", value: 0.0)
        sendIfChanged(name: "next60MinPrecipCurrentRateText", value: "0.0${spaceStr}${pUnit}")
        sendIfChanged(name: "next60MinPrecipHourTotal", value: 0.0)
        sendIfChanged(name: "next60MinPrecipHourTotalText", value: "0.0${spaceStr}${pUnit}")
        sendIfChanged(name: "next60MinPrecipStartMinutes", value: -1)
        sendIfChanged(name: "next60MinPrecipStopMinutes", value: -1)
        sendIfChanged(name: "next60MinPrecipSummaryText", value: "Short-term precipitation data not provided by API.")
        return
    }
    ...
    String hourTotalText = "${displayTotalAccum}${spaceStr}${pUnit}"

    sendIfChanged(name: "next60MinPrecipCurrentRate", value: displayCurrentRate)
    sendIfChanged(name: "next60MinPrecipCurrentRateText", value: currentRateText)
    sendIfChanged(name: "next60MinPrecipHourTotal", value: displayTotalAccum)
    sendIfChanged(name: "next60MinPrecipHourTotalText", value: hourTotalText)
    sendIfChanged(name: "next60MinPrecipStartMinutes", value: minutesUntilPrecipStart)
    sendIfChanged(name: "next60MinPrecipStopMinutes", value: minutesUntilPrecipStop)
    sendIfChanged(name: "next60MinPrecipSummaryText", value: summaryText)

    logDebug "Next60Min Short-Term Precip Parsed -> Start in: ${minutesUntilPrecipStart} min | Stop in: ${minutesUntilPrecipStop} min | Current Rate: ${currentRateText} | 1-Hour Depth Total: ${hourTotalText} | ${summaryText}"
}
