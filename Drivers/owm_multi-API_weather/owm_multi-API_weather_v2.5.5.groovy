...
static String version()    {  return '2.5.5'  }
...
String calcMoonPhaseSvgImage(Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    targets.each { entry ->
        def rawVal = (entry.sourceMap && entry.sourceMap[entry.apiKey] != null) ? entry.sourceMap[entry.apiKey] : device.currentValue(entry.targetAttr)
        if (rawVal != null && rawVal.toString().isNumber()) {
            double val = (rawVal.toDouble() % 1.0 + 1.0) % 1.0
