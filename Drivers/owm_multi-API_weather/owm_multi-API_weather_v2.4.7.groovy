...
static String version()    {  return '2.4.7'  }
...
private void parseOWMData(Map json) {
    ...
    if (currentData) {
        ...
        // Extract precipitation safely from '1h' map if present
...
private void sendOWMData(Map current, Map today, Map tom, Map tda) {
    ...
    forecastDays.each { day ->
        ...
        if (data) {
            ...
            // Forecast Rain / Snow extraction
            def fRain = (data.rain instanceof Map) ? (data.rain?.getAt("1h") ?: 0.0) : (data.rain ?: 0.0)
            def fSnow = (data.snow instanceof Map) ? (data.snow?.getAt("1h") ?: 0.0) : (data.snow ?: 0.0)
