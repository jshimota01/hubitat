 ...
 * v1.62    09/14/26    jshimota    Enforced lastActivity refresh on all Health Check hits (isStateChange: false); added ZDO Match Descriptor (0x0006/0x8006) cluster handling.
 * v1.61    09/10/26    jshimota    Corrected Health Check ZCL target to Attribute 0x0000 (ZCL Version), promoted Health Check logs to logInfo, and standardized timeout warnings.
 * v1.60    09/06/26    jshimota    Synchronized powerText ("0.0 Watts"), energyText ("0.0 kWh"), and powerFactor (0.0G) zero formats across installed() and parseOnOffCluster() to match exact handler decimal scaling.
 * v1.59    09/06/26    jshimota    Backed out executeHardwareRefreshSync and delayed refresh logic on switch off to eliminate updated() logging loop; enforced clean deterministic zeroing for power and amperage (0.0G / 0.0 A).
 * v1.58    09/06/26    jshimota    Standardized amperageText zero string to '0.0 A' across installed() and parseOnOffCluster() to match handleRmsCurrentValue decimal formatting and eliminate '0 A' vs '0.0 A' state toggling.
 * v1.57    09/06/26    jshimota    Updated parseOnOffCluster off-state forced zero values to BigDecimal (0.0G) to match decimal scaling precision and eliminate 0 vs 0.0 state toggling.
 * v1.56    09/06/26    jshimota    Implemented hybrid power/amperage zeroing in parseOnOffCluster on switch off, added delayed refresh sync, and fixed clearAllAttributes attribute string mapping.
...
static String version() { return '1.63' }
def timeStamp() { return "2026/09/24 08:30 AM" }
...
metadata {
    definition(
        ...
        importUrl: "https://raw.githubusercontent.com/jshimota01/hubitat/main/Drivers/third_reality_plug_w-power-meter_custom/third_reality_plug_w-power-meter_custom.groovy"
    ) {
        capability "Actuator"
...
void installed() {
    ...
    if (supportsPowerMeter()) {
        ...
        sendEvent(name: "amperage", value: 0.0G, unit: "A")
        sendEvent(name: "amperageText", value: "0.0 A")
        sendEvent(name: "energy", value: 0.0G, unit: "kWh")
        sendEvent(name: "energyText", value: "0.0 kWh")
        sendEvent(name: "frequency", value: 0, unit: "Hz")
        sendEvent(name: "power", value: 0.0G, unit: "W")
        sendEvent(name: "powerText", value: "0.0 Watts")
        sendEvent(name: "voltage", value: 0, unit: "V")
        sendEvent(name: "voltageText", value: "0 V")
        sendEvent(name: "powerFactor", value: 0.0G)
    } else {
...
private void scheduleHealthCheck(String methodToSchedule, int intervalMin) {
    ...
    logDebug "Scheduling Health Check every ${intervalMin} minutes anchored at ${String.format('%02d:%02d', h, m)} daily"
    ...
    switch (descMap.attrInt as Integer) {
        case HEALTH_CHECK_ATTR_ID:
            if (state.healthCheckPending == true) {
                logInfo "Health Check complete (online)"
                state.healthCheckPending = false
                unschedule("deviceCommandTimeout")
            } else {
                logTrace "Health Check response received from cluster 0x0000"
            }
...
void parseOnOffCluster(final Map descMap) {
    ...
    switch (descMap.attrInt as Integer) {
        case POWER_ON_OFF_ID:
            ...
            updateAttribute("switch", isOn ? "on" : "off", null, type)

            if (!isOn && supportsPowerMeter()) {
                logDebug "Switch reported off: Forcing instantaneous power and amperage attributes to zero..."
                updateAttribute("power", 0.0G, "W", type)
                updateAttribute("powerText", "0.0 Watts", null, type, "info", "Instantaneous Power Text")
                updateAttribute("amperage", 0.0G, "A", type)
                updateAttribute("amperageText", "0.0 A", null, type, "info", "Instantaneous Current Text")
                updateAttribute("powerFactor", 0.0G, null, type)
            }
            break
        case POWER_RESTORE_ID:
            ...
            logDebug "Power restore mode is '${options[value]}' (0x${descMap.value})"
            
            if (settings.powerRestore?.toString() != valStr) {
...
void parseZdoClusters(final Map descMap) {
    if (!(descMap.data instanceof List) || ((List)descMap.data).isEmpty()) return
    ...
    if (clusterId >= 0x8000 && descMap.data.size() >= 2) {
        ...
        logTrace "ZDO ${clusterName} received:${descMap.data}"
    }
}
...
void clearAllAttributes() {
    ...
    device.supportedAttributes*.name.each { String attrName -> 
        if (attrName) device.deleteCurrentState(attrName) 
    }
...
@Field static final Map<Integer, String> ZdoClusterEnum = [
    0x0006: "Match Descriptor Request", 0x0013: "Device announce", 
    0x8004: "Simple Descriptor Response", 0x8005: "Active Endpoints Response", 0x8006: "Match Descriptor Response",
