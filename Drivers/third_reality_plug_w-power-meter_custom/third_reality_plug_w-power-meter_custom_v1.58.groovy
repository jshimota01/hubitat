...
static String version() { return '1.58' }
def timeStamp() { return "2026/09/06 11:25 PM" }
...
void parseOnOffCluster(final Map descMap) {
    ...
    switch (descMap.attrInt as Integer) {
        case POWER_ON_OFF_ID:
            ...
            // Force scaled BigDecimal zeros to match handleRmsCurrentValue / handleActivePowerValue precision
            if (!isOn && supportsPowerMeter()) {
                ...
                updateAttribute("powerText", "0 Watts", null, type, "info", "Instantaneous Power Text")
                updateAttribute("amperage", 0.0G, "A", type)
                updateAttribute("amperageText", "0.0 A", null, type, "info", "Instantaneous Current Text")
                updateAttribute("powerFactor", 0, null, type)

                // Read values from physical device after 1 second to ensure state synchronization
                runIn(1, "executeHardwareRefreshSync", [overwrite: true])
            }
            break
...
/**
 * Executes a delayed hardware attribute refresh to maintain hardware state alignment.
 **/
void executeHardwareRefreshSync() {
