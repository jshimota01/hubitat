 ...
 * v1.49    09/01/26    jshimota    Architectural Refinements: Added public scheduled callback wrapper for Health Check, enforced runIn overwrite guard, preserved persistent phase anchors across state clear, eliminated redundant initialize calls during configure(), and renamed PING_ATTR_ID constant.
 * v1.48    09/01/26    jshimota    Standardized all health tracking routine names, log output strings, and scheduler targets to 'Health Check' (eliminating all 'Ping' nomenclature).
 * v1.47    09/01/26    jshimota    Enforced aggressive unschedule() during initialize/updated lifecycle to purge legacy/orphaned scheduled jobs.
 * v1.46    09/01/26    jshimota    Added public ping() delegate method to catch external/scheduled ping calls and resolve MissingMethodException.
 * v1.45    08/31/26    jshimota    Redirected Voltage attribute updates to output as logDebug rather than logInfo to suppress unnecessary log chatter.
 * v1.44    08/31/26    jshimota    Silenced unhandled Electrical Measurement attribute 0x0510 (Overload/Status Flag) to logDebug.
...
static String version() { return '1.50' }
def timeStamp() { return "2026/09/05 08:40 AM" }
