...
static String version() { return '1.24' }
def timeStamp() { return "2026/08/31 02:15 PM" }
...
private void scheduleHealthCheck(String methodToSchedule, int intervalMin) {
    ...
    switch (intervalMin) {
        ...
        case 480:  schedule("0 ${minuteOffset} */8 ? * * *", methodToSchedule); break
        case 720:  schedule("0 ${minuteOffset} */12 ? * * *", methodToSchedule); break
        case 1440: schedule("0 ${minuteOffset} 0 ? * * *", methodToSchedule); break
        default:
            if (intervalMin >= 60) {
