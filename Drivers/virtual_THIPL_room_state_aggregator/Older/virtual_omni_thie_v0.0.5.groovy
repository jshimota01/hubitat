...
def disableCapability(String type) {
    logInfo "Disabling capability stream: ${type} for room"
    switch(type.toLowerCase()) {
        case "temperature":
            sendIfChanged([name: "temperature", value: null])
            sendIfChanged([name: "temperatureText", value: "Disabled"])
            state.tempEnabled = false
            break
        case "humidity":
            sendIfChanged([name: "humidity", value: null])
            sendIfChanged([name: "humidityText", value: "Disabled"])
            state.humidEnabled = false
            break
        case "illuminance":
            sendIfChanged([name: "illuminance", value: null])
            sendIfChanged([name: "illuminanceText", value: "Disabled"])
            state.luxEnabled = false
            break
        case "energy":
            sendIfChanged([name: "energy", value: null])
            sendIfChanged([name: "energyText", value: "Disabled"])
            state.energyEnabled = false
            break
    }
    updateActiveCapabilitiesAttribute()
}

def enableCapability(String type) {
    logInfo "Enabling capability stream: ${type} for room"
    switch(type.toLowerCase()) {
...
private void updateActiveCapabilitiesAttribute() {
    List active = []
    if (state.tempEnabled != false) active.add("Temperature")
    if (state.humidEnabled != false) active.add("Humidity")
    if (state.luxEnabled != false) active.add("Illuminance")
    if (state.energyEnabled != false) active.add("Energy")
