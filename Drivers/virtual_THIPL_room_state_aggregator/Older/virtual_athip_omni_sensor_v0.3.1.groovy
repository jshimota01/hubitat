...
metadata {
    ...
    preferences {
        // Architectural Tier Configuration
        ...
        // Minimum Delta (Deadband) Preferences
...
private boolean sendIfChanged(Map args) {
    ...
    // Check if attribute is numeric to evaluate Delta threshold
    ...
    // Native Hubitat deduplication (drops sendEvent if state matches)
...
// Router for Tier Execution Models (Event-driven vs Debounced vs Scheduled)
private void routeValueUpdate(String capType, val) {
    ...
    switch(tier) {
        case "1": // Room - Direct Execution
            processSingleCapability(capType, val)
            break
            
        case "2": // Floor - Debounced Execution
            int delaySeconds = settings.debounceTime ? settings.debounceTime.toInteger() : 30
            logDebug "Tier 2 (Floor) change detected for ${capType}. Debouncing execution for ${delaySeconds}s."
            runIn(delaySeconds, "processPendingBuffer", [overwrite: true])
            break
            
        case "3": // House - Scheduled Execution Only
            logTrace "Tier 3 (House) buffered ${capType} update (${val}). Waiting for next scheduled poll."
            break
    }
}

// Debounce / Scheduled Execution Target
void processPendingBuffer() {
    if (!state.pendingBuffer) return
    logDebug "Flushing buffered upper-tier updates: ${state.pendingBuffer}"
...
private void processSingleCapability(String capType, val) {
    switch(capType) {
        case "temperature":
            ...
            // Only update text attribute if numeric value met the delta requirement
