...
def installed() {
    ...
    // Set baseline initial state for light counters
    setLightsOnCount(0)
...
private void routeValueUpdate(String capType, val) {
    // Immediate execution bypass for real-time capabilities across all tiers (Lights & Power)
    if (capType == "lightsOnCount" || capType == "power") {
    ...
    // Buffer sensor data for Tier 2 and Tier 3 processing
    if (tier != "1") {
