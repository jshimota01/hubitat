...
metadata {
    definition (name: "Virtual THIPL Room State Aggregator Driver", namespace: "hubitat", author: "James Shimota") {
		...
		capability "Light" // Replaced capability "Switch"
		//capability "Switch" interferes with Alexa detection
...
def installed() {
    ...
    // Set baseline initial state for light counters and device counts
    setLightsOnCount(0)
...
private void routeValueUpdate(String capType, val) {
    // Immediate execution bypass for real-time capabilities across all tiers (Lights, Device Counts, & Power)
