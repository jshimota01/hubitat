 ...
 *  
 *  06-20-2026 	jhimota	Forked to Custom version for bug fixes
 * 
 */

metadata {
	definition (name: "Advanced vThermostat Device Custom", 
		namespace: "jshimota", 
		author: "Nelson Clark",
		...
		attribute "thermostatTemperatureSetpoint", "number"	//google - Fixed: Changed from String to number for voice assistant schema matching
...
def installed() {
	...
	// Initialize default logging level to prevent null-pointer exceptions
...
//************************************************************
def updated() {
	// Send native List rather than stringified raw array markup
	sendEvent(name: "supportedThermostatModes", value: ["heat", "cool", "auto", "off"])
	...
	if (state.currentUnit != hubScale) {
		...
		currentStatesList.each {
			if (it.unit && it.unit != hubScale) {
				// Fixed: Added explicit typecast to Double and added 'def' definition keyword to prevent sandboxed environment crashes
				def newValue = convertToHubTempScale(it.value.toDouble())
...
//************************************************************
// evaluateMode
//     Run every minute to see where we are at with current 
...
def evaluateMode() {
	...
	// If fetched maxInterval from user is higher than 180, set to 180
	...
	// Do we have a threshold set, if not we do nothing 
	if ( !threshold ) {
...
def setHeatingSetpoint(Double value) {
	...
	if (newHeatingSetpoint == device.currentValue("heatingSetpoint")) {
		...
		def setpointDistance = (getTemperatureScale() == "C") ? 3 : 5
		...
		if (newHeatingSetpoint > heatmax || newHeatingSetpoint < heatmin) {
			...
			// Maintain Amazon Alexa / Google Home integration attributes dynamically
			sendEvent(name: "thermostatTemperatureSetpoint", value: newHeatingSetpoint, unit: units)
...
//     Let's set the desired threshold
// Signature(s)
//     setThermostatThreshold(value)
...
def setMinCoolTemp(Double value) {
	...
	sendEvent(name: "minCoolingSetpoint", value: value, unit: units) // Sync assistant boundary
	if (t < value) {
		setCoolingSetpoint(value)
	}
}
...
def setMaxCoolTemp(Double value) {
	...
	sendEvent(name: "maxCoolingSetpoint", value: value, unit: units) // Sync assistant boundary
	if (t > value) {
		setCoolingSetpoint(value)
	}
}
...
def setMinHeatTemp(Double value) {
	...
	sendEvent(name: "minHeatingSetpoint", value: value, unit: units) // Sync assistant boundary
	if (t < value) {
		setHeatingSetpoint(value)
	}
}
...
def setMaxHeatTemp(Double value) {
	...
	sendEvent(name: "maxHeatingSetpoint", value: value, unit: units) // Sync assistant boundary
	if (t > value) {
		setHeatingSetpoint(value)
	}
}
