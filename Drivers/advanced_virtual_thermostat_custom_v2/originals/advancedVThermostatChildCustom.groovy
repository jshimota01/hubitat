...
def updateTemperature() {
    ...
    // Round to 1 decimal place to stop event filtering spam
    def avgTemp = (total / count).toDouble().round(1)
    
    // Only update the virtual thermostat if the temperature actually shifted
...
// New helper method to prevent IllegalArgumentExceptions from bad drivers
def safelyControlSwitches(devices, String targetState) {
    if (!devices) return
    
    devices.each { device ->
        def currentState = device.currentValue("switch")
        // Only send the command if the device isn't already in the target state
        if (currentState != targetState) {
            if (device.hasCommand(targetState)) {
                device."${targetState}"()
            } else {
                logger("error", "Device ${device.displayName} does not support '${targetState}'")
            }
        }
    }
}
