...
def timeStamp() { return "2026/08/28 10:40 AM" }
...
void setClosedContact(String stateStr) {
    logDebug "setClosedContact(): ${stateStr}"
    sendIfChanged([name: "closedContact", value: stateStr, descriptionText: "closed contact sensor is ${stateStr}"])
    evaluateDoorState()
}

void setOpenContact(String stateStr) {
    logDebug "setOpenContact(): ${stateStr}"
    sendIfChanged([name: "openContact", value: stateStr, descriptionText: "open contact sensor is ${stateStr}"])
    evaluateDoorState()
}

void updateContactStatus(String closedState, String openState) {
    logDebug "updateContactStatus(): closed = ${closedState}, open = ${openState}"
    sendIfChanged([name: "closedContact", value: closedState, descriptionText: "closed contact sensor is ${closedState}"])
    sendIfChanged([name: "openContact", value: openState, descriptionText: "open contact sensor is ${openState}"])
    evaluateDoorState()
}

// State Truth Table Evaluator
private void evaluateDoorState() {
    String closedSensor = device.currentValue("closedContact") ?: "open"
    String openSensor = device.currentValue("openContact") ?: "open"
    ...
    if (closedSensor == "closed" && openSensor == "open") {
        ...
        // Transitional state: Both contact sensors are open
        if (currentDoorState == "closed" || currentDoorState == "opening") {
            sendIfChanged([name: "door", value: "opening", descriptionText: "garage door is opening"])
        } else if (currentDoorState == "open" || currentDoorState == "closing") {
            sendIfChanged([name: "door", value: "closing", descriptionText: "garage door is closing"])
        } else {
            logWarn "Door is in motion between sensors but current state is ${currentDoorState}."
        }
    } 
