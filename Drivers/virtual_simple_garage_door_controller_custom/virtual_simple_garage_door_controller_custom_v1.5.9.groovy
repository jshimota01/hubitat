...
def timeStamp() { return "2026/08/28 04:30 PM" }
...
private void checkAndCreateChildShadowDevice() {
    ...
    if (shadowEnabled && !childDevice) {
        ...
        try {
            childDevice = addChildDevice("hubitat", "Virtual Lock", childNetworkId, [
            ...
            sendEvent(name: "shadowLockState", value: "LOCKED")
        } catch (Exception e) {
            logError "Failed to create Child Virtual Lock with 'Virtual Lock': ${e.message}. Attempting 'Generic Virtual Lock' fallback..."
            try {
                ...
                logInfo "Child Alexa Shadow Lock created successfully using fallback driver."
                sendEvent(name: "shadowLockState", value: "LOCKED")
            } catch (Exception ex) {
                logError "Failed to create Child Virtual Lock fallback: ${ex.message}."
            }
        }
        ...
        sendEvent(name: "shadowLockState", value: "DISABLED")
    } else if (shadowEnabled && childDevice) {
        String currentChildState = childDevice.currentValue("lock")?.toUpperCase() ?: "LOCKED"
        sendEvent(name: "shadowLockState", value: currentChildState)
    } else {
        sendEvent(name: "shadowLockState", value: "DISABLED")
    }
...
// Subscribes explicitly to child device events so parent driver acts when Alexa commands dev:1027 directly
private void setupChildSubscriptions() {
    String childNetworkId = "${device.deviceNetworkId}-ShadowLock"
    def childDevice = getChildDevice(childNetworkId)
    if (childDevice) {
        unsubscribe()
        subscribe(childDevice, "lock", "childLockHandler")
        logDebug "Subscribed parent driver to child lock events on ${childDevice.displayName}"
    }
}

// Intercept Lock/Unlock events from Child Device
void childLockHandler(evt) {
    logInfo "Child Alexa Lock Event Intercepted: lock=${evt.value}"
    sendEvent(name: "shadowLockState", value: evt.value.toUpperCase(), isStateChange: true)
    
    if (evt.value == "unlocked") {
        if (device.currentValue("door") != "open" && device.currentValue("door") != "opening") {
            logInfo "Alexa unlocked child device -> Executing parent open()..."
            open()
        }
    } else if (evt.value == "locked") {
        if (device.currentValue("door") != "closed" && device.currentValue("door") != "closing") {
            logInfo "Alexa locked child device -> Executing parent close()..."
            close()
        }
    }
}

// Native Hubitat Component Commands called when child device is targeted
void componentLock(def childDevice) {
    logInfo "Component command received via Child Shadow Lock: LOCK -> Executing close()"
    close()
}

void componentUnlock(def childDevice) {
    logInfo "Component command received via Child Shadow Lock: UNLOCK -> Executing open()"
    open()
}

// Strictly Deduplicated Child Lock Synchronization
private void syncChildShadowLock(String lockState) {
    String childNetworkId = "${device.deviceNetworkId}-ShadowLock"
    def childDevice = getChildDevice(childNetworkId)
    String upperState = lockState?.toUpperCase() ?: "LOCKED"
    
    if (device.currentValue("shadowLockState") != upperState) {
        sendEvent(name: "shadowLockState", value: upperState, isStateChange: true)
    }

    if (childDevice) {
        String currentChildLock = childDevice.currentValue("lock")
        if (currentChildLock != lockState) {
            logDebug "Syncing Child Alexa Shadow Lock state to: ${lockState}"
            if (lockState == "locked" && childDevice.hasCommand("lock")) {
                childDevice.lock()
            } else if (lockState == "unlocked" && childDevice.hasCommand("unlock")) {
                childDevice.unlock()
            } else {
                childDevice.sendEvent(name: "lock", value: lockState, isStateChange: true, descriptionText: "Alexa Shadow Lock set to ${lockState}")
            }
        } else {
            logDebug "Child Alexa Shadow Lock is already in target state '${lockState}'. Suppressing redundant event emission."
        }
    }
}
...
void updateContactStatus(String closedState, String openState) {
    ...
    if (cleanClosed == "closed" && cleanOpen == "open") {
        syncChildShadowLock("locked")
    } else if (cleanClosed == "open" && cleanOpen == "closed") {
        syncChildShadowLock("unlocked")
    }
