...
def timeStamp() { return "2026/08/28 04:23 PM" }
...
void close() {
    ...
    if (currentDoorState != "closed" && currentDoorState != "closing") {
        ...
        logDebug "close() requested but door is already close or closing."
    }
}
...
// Subscribes to child device events so parent driver acts when Alexa commands dev:1027 directly
...
void childLockHandler(evt) {
    logInfo "Child Alexa Lock Event Intercepted: lock=${evt.value} (source: ${evt.type ?: 'external'})"
...
// Handlers called when Child Device commands are triggered via component model
