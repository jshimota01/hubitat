...
void updateContactStatus(String closedState, String openState) {
    ...
    // Clear any active transition timers during forced status repair
...
// Internal Responsive HTML Generator Engine with Center-Aligned Alexa Badge
private void renderNotificationTile(String reason = "", String doorOverride = null, String closedOverride = null, String openOverride = null) {
    ...
    // Alexa Badge Styling (Centered Column)
    String alexaBgColor = alexaConfirm == "SETTLED" ? "#2e7d32" : "#ef6c00"
    String alexaText = alexaConfirm == "SETTLED" ? "SETTLED" : "PENDING"
    String alexaBadge = "<span style='background:${alexaBgColor}; color:#fff; padding:1px 4px; border-radius:3px; font-weight:bold; font-size:0.85em; letter-spacing:0.2px;'>${alexaText}</span>"
    ...
    String htmlOutput = """<div style='height:100%; width:100%; box-sizing:border-box; padding:4px 6px; font-family:sans-serif; background:#181818; color:#fff; border-radius:4px; overflow:hidden; display:flex; flex-direction:column; justify-content:space-between;'>
        ...
        <div style='display:flex; justify-content:space-between; align-items:flex-end; font-size:${cSize}; color:#ccc; margin:2px 0;'>
            ...
            <div style='display:flex; flex-direction:column; align-items:center; gap:2px;'>
                <span style='color:#888; font-size:0.75em;'>Alexa</span>
                ${alexaBadge}
            </div>
...
private void evaluateDoorState(String closedOverride = null, String openOverride = null) {
    ...
    if (closedSensor == "closed" && openSensor == "open") {
        ...
        // Only send stateChange event if door wasn't already marked closed by optimistic timer
        if (currentDoorState != "closed") {
        ...
        // Only send stateChange event if door wasn't already marked open by optimistic timer
        if (currentDoorState != "open") {
