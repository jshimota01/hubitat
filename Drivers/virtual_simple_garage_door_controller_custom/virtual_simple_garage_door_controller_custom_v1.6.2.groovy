 ...
 * Notes: Dual-contact state calculation driver with explicit command intent architecture
 ...
 *  and explicit command intent architecture to decouple physical sensors from reported door state.
 *
 *  Instructions:
...
def timeStamp() { return "2026/08/29 01:50 PM" }
...
// Capability Commands - Explicit Command Intent Invocation
void open() {
    ...
    if (currentDoorState != "open") {
        state.commandIntent = "open"
        logInfo "open() command received. Establishing command intent OPEN, setting door state to OPENING, and pulsing relay."
...
void close() {
    ...
    if (currentDoorState != "closed") {
        state.commandIntent = "close"
        logInfo "close() command received. Establishing command intent CLOSE, setting door state to CLOSING, and pulsing relay."
        logDebug "Emitting relayTrigger event -> pulse"
...
private void evaluateDoorState(String closedOverride = null, String openOverride = null) {
    ...
    String commandIntent = state.commandIntent

    logTrace "evaluateDoorState() evaluation started -> ClosedContact: ${closedSensor}, OpenContact: ${openSensor}, CurrentDoor: ${currentDoorState}, CommandIntent: ${commandIntent}"
    ...
    if (closedSensor == "closed" && openSensor == "open") {
        ...
        // Determine mid-travel direction via explicit command intent first, falling back to current door state if manually triggered
        if (commandIntent == "open" || (!commandIntent && (currentDoorState == "closed" || currentDoorState == "opening"))) {
            ...
            if (currentDoorState != "opening") {
                ...
                logDebug "Door state is already reported as OPENING. Suppressing duplicate event emission."
            }
        } 
        else if (commandIntent == "close" || (!commandIntent && (currentDoorState == "open" || currentDoorState == "closing"))) {
            ...
            if (currentDoorState != "closing") {
                ...
                logDebug "Door state is already reported as CLOSING. Suppressing duplicate event emission."
            }
        } 
