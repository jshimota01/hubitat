...
def processMotion(ZoneStatus status) {
    if (status.isAlarm1Set()) {
        ...
        // Fix: Only log and send event if changing state from inactive to active
        if (device.currentValue("motion") != "active") {
            ...
            if (debugLogging) log.debug "${device.displayName}: Motion is already active, suppressing duplicate event."
        }

        // Always refresh/extend the auto-reset timer on new motion reports
