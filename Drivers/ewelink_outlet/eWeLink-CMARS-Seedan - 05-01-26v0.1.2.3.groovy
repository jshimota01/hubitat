 ...
 *
 *      ORIGINAL SOURCE
 *      2020          markus-li     0.1.0.1.1123b    Copyright 2020 Markus Liljergren (https://oh-lalabs.com)
 *                                                   https://raw.githubusercontent.com/markus-li/Hubitat/development/drivers/expanded/zigbee-generic-outlet-expanded.groovy
 *
 *      Date          Source        Version          What
 *      ----          ------        -------          ----
 *      2021-10-07    jshimota      0.1.0.2          Fork of original source
 *      2021-10-07    jshimota      0.1.0.3          Fingerprint for eWeLink added
 *      2021-10-08    jshimota      0.1.0.4          Removed Presence capability
 *      2021-10-09    jshimota      0.1.0.5          Added Flash command
 *      2021-10-09    jshimota      0.1.0.6          Added Toggle command
 *      2021-10-10    jshimota      0.1.0.7          Changed toggle to use x06 01/x06 00
 *      2021-10-15    jshimota      0.1.0.8          Modified developer attribution text
 ...
 *      2023-04-20    jshimota      0.1.1.4          Added 2nd eWeLink fingerprint
 *      2023-09-08    jshimota      0.1.1.5          Fixed logging clarity
 *      2023-09-09    jshimota      0.1.1.6          More logging fixes
 *      2023-10-17    jshimota      0.1.1.7          More logging updates
 *      2024-01-28    jshimota      0.1.1.8          Changed skipping ping to debug logging
 *      2024-01-28    jshimota      0.1.1.9          Fixed logging debug typo
 *      2024-02-22    jshimota      0.1.2.0          Added new fingerprint for gmachado
 *      2024-02-23    jshimota      0.1.2.1          Cleaned up fingerprint list per gmachado
 *      2024-03-17    jshimota      0.1.2.2          Added fingerprint for peanut device
 *      2024-01-26    jshimota      0.1.2.3          Optimized: fixed duplicate clusterId case, flash return type,
 *                                                   aOff lost commands, autoOff not cancelled on manual off,
 *                                                   hexToASCII +30 offset bug, endpoint ignored in sendOnOffEvent
 */
...
static String version() { return '0.1.2.3' }

metadata {
    definition (name: "Zigbee eWeLink-CMARS-Seedan Outlet/Switch", namespace: "jshimota", author: "James Shimota",
                filename: "eWeLink-CMARS-Seedan.groovy",
                importUrl: "https://raw.githubusercontent.com/jshimota01/hubitat/main/Drivers/ewelink_outlet/eWeLink-CMARS-Seedan.groovy") {
        capability "Sensor"
        ...
        fingerprint model:"BASICZBR3",           manufacturer:"SONOFF",          profileId:"0104", endpointId:"01", inClusters:"0000,0003,0004,0005,0006",                  outClusters:"0000", application:"05"
        ...
        fingerprint model:"SA-003-Zigbee",       manufacturer:"eWeLink",         profileId:"C05E", endpointId:"01", inClusters:"0000,0003,0004,0005,0006",                  outClusters:"0000", application:"05,08"
        ...
        fingerprint model:"SWITCH-ZR02",         manufacturer:"eWeLink",         profileId:"0104", endpointId:"01", inClusters:"0000,0003,0004,0005,0006,FC57",             outClusters:"0019", application:"02"
        ...
        fingerprint model:"null",                manufacturer:"Securifi Ltd.W´# YÝ", profileId:"0104", endpointId:"01", inClusters:"0000,0001,0003,0004,0005,0006,0B04,0B05", outClusters:"0000,0001,0003,0004,0005,0006,0019,0B04,0B05"
    }
    ...
    preferences {
        ...
        input(name: "presenceEnable",       type: "bool",   title: styling_addTitleDiv("Enable Presence"),                                 description: styling_addDescriptionDiv("Enables Presence to indicate if the device has sent data within the last 3 hours (REQUIRES at least one Checkin option enabled)"), defaultValue: true)
        ...
        input(name: "recoveryMode",         type: "enum",   title: styling_addTitleDiv("Recovery Mode"),                                   description: styling_addDescriptionDiv("Select Recovery mode type (default: Slow)<br/>NOTE: \"Insane\" and \"Suicidal\" modes may destabilize your mesh!"), options: ["Disabled","Slow","Normal","Insane","Suicidal"], defaultValue: "Slow")
        input(name: "enablePing",           type: "bool",   title: styling_addTitleDiv("Enable Automatic Ping"),                           description: styling_addDescriptionDiv("Sends an infrequent ping to the device to track Presence (default: enabled)"), defaultValue: true)
        ...
        input name: "autoOffOn", type: "enum", description: "Automatically turns off the device after selected time.", title: "Enable Auto-Off-On",
              options: [[0:"Disabled"],[1:"1 second"],[2:"2 seconds"],[5:"5 seconds"],[10:"10 seconds"],[15:"15 seconds"],[20:"20 seconds"],
                        [30:"30 seconds"],[45:"45 seconds"],[60:"1 minute"],[120:"2 minutes"],[180:"3 minutes"],[300:"5 minutes"],
                        [600:"10 minutes"],[900:"15 minutes"],[1200:"20 minutes"],[1800:"30 minutes"],[2700:"45 minutes"],
                        [3600:"1 hour"],[5400:"1.5 hours"]], defaultValue: 0
    }
}

// BEGIN:getDeviceInfoFunction()
String getDeviceInfoByName(String infoName) {
    Map deviceInfo = [
        'name':      'Zigbee - Generic Outlet (with Presence)',
        'namespace': 'oh-lalabs.com',
        'author':    'Markus Liljergren',
        'filename':  'zigbee-generic-outlet',
        'importUrl': 'https://raw.githubusercontent.com/markus-li/Hubitat/development/drivers/expanded/zigbee-generic-outlet-expanded.groovy'
    ]
...
void ping() {
    if (enablePing == false) {
        ...
        cmd.each { if (!it.startsWith("delay")) logging(it, 1) }
        sendZigbeeCommands(cmd)
    } else {
    ...
    if (description.indexOf('encoding: 4C') >= 0) {
        ...
        if (description.indexOf('encoding: 42') >= 0) {
            ...
            if (zeroIndex > -1) {
                ...
                msgMap["additionalAttrs"] = [["encoding": "41", "value": parseXiaomiStruct(values.join(), isFCC0=false, hasLength=true)]]
            } else {
        ...
        default:
            switch (msgMap["clusterId"]) {
                case "0006":
                    ...
                    // FIX: now passes endpoint value through to sendOnOffEvent
                ...
                case "8021":
                    ...
                    sendlastCheckinEvent(minimumMinutesToRepeat=25)
                    break
                // FIX: removed duplicate "8021" case (was dead code; second one shadowed the first)
                case "8001":
...
/**
 * FIX: endpoint parameter was previously ignored. Now forwarded to the single-arg overload.
 * Multi-endpoint devices will behave correctly when this is expanded further.
 */
void sendOnOffEvent(Integer endpoint, boolean onOff) {
    logging("sendOnOffEvent(endpoint=$endpoint, onOff=$onOff)", 1)
    sendOnOffEvent(onOff)
}
...
ArrayList<String> off() {
    ...
    state.flashing = false
    sendEvent(name: "flashing", value: state.flashing)
    // FIX: cancel any pending auto-off so it doesn't fire after a manual off
    unschedule('aOff')
    return zigbeeCommand(0x006, 0x00)
}

/**
 * Toggle the switch by reading current state.
 * FIX: changed from `def` to typed `void`.
 */
void toggle() {
...
/**
 * Flash the outlet at the configured rate.
 * FIX: was declared ArrayList<String> but had bare `return` (void returns) on early-exit paths,
 *      which causes a type mismatch. Changed to void; flash commands are sent via sendZigbeeCommands.
 */
void flash() {
    if (state.flashing) {
        state.flashing = false
        ...
        if (infoLogging) log.info "${device.getDisplayName()} was set to flash with a rate of ${flashRate ?: 750} milliseconds"
        state.flashing = true
        sendEvent(name: "flashing", value: state.flashing)
...
void flashOn() {
    ...
    // FIX: send the command immediately rather than returning it into the void
    sendZigbeeCommands(zigbeeCommand(0x006, 0x01))
}
...
void flashOff() {
    ...
    // FIX: send the command immediately rather than returning it into the void
    sendZigbeeCommands(zigbeeCommand(0x006, 0x00))
}

/**
 * Auto-off handler scheduled by sendOnOffEvent.
 * FIX: the original returned the result of zigbeeCommand() but runIn() discards return values,
 *      so the Zigbee command was silently dropped. Now uses sendZigbeeCommands() to actually send it.
 */
void aOff() {
...
/* ── Library: default functions ─────────────────────────────────────────────── */

// BEGIN:getDefaultFunctions()
private String getDriverVersion() {
    String comment = "Works with Generic Outlets"
    if (comment != "") state.comment = comment
    String additionalComment = """Original driver by Markus Liljergren, customized to support eWeLink SA-003.<br>Also, AutoOff, Flash and Toggle capabilities added."""
    if (additionalComment != "") state.additionalComment = additionalComment
    String version = "v0.1.2.3"
...
private boolean logging(message, level) {
    ...
    if (infoLogging == null || infoLogging == true) { logLevelLocal = 100 }
    if (debugLogging == true) { logLevelLocal = 1 }

    if (logLevelLocal != 0) {
        switch (logLevelLocal) {
            case 1:
                if (level >= 1 && level < 99) { log.debug "$message"; didLogging = true }
                else if (level == 100)         { log.info  "$message"; didLogging = true }
                break
            case 100:
                if (level == 100) { log.info "$message"; didLogging = true }
                break
        }
    }
    return didLogging
...
private Integer getCLUSTER_BASIC()           { 0x0000 }
private Integer getCLUSTER_POWER()           { 0x0001 }
private Integer getCLUSTER_WINDOW_COVERING() { 0x0102 }
private Integer getCLUSTER_WINDOW_POSITION() { 0x000d }
private Integer getCLUSTER_ON_OFF()          { 0x0006 }
private Integer getBASIC_ATTR_POWER_SOURCE() { 0x0007 }
private Integer getPOWER_ATTR_BATTERY_PERCENTAGE_REMAINING() { 0x0021 }
private Integer getPOSITION_ATTR_VALUE()     { 0x0055 }
private Integer getCOMMAND_OPEN()            { 0x00 }
private Integer getCOMMAND_CLOSE()           { 0x01 }
private Integer getCOMMAND_PAUSE()           { 0x02 }
private Integer getENCODING_SIZE()           { 0x39 }
...
ArrayList<String> zigbeeCommand(Integer endpoint, Integer cluster, Integer command, Map additionalParams, int delay = 204, String... payload) {
    ...
    String cmdArgs = "0x${device.deviceNetworkId} 0x${HexUtils.integerToHexString(endpoint, 1)} 0x${HexUtils.integerToHexString(cluster, 2)} " +
            ...
            "{$finalPayload}" +
            "$mfgCode"
    return ["he cmd $cmdArgs", "delay $delay"]
}
...
ArrayList<String> zigbeeReadAttribute(Integer endpoint, Integer cluster, Integer attributeId, int delay = 206) {
    return ["he rattr 0x${device.deviceNetworkId} ${endpoint} 0x${HexUtils.integerToHexString(cluster, 2)} 0x${HexUtils.integerToHexString(attributeId, 2)} {}", "delay $delay"]
}
...
void sendZigbeeCommands(ArrayList<String> cmd) {
    ...
    cmd.each { allActions.add(new hubitat.device.HubAction(it, hubitat.device.Protocol.ZIGBEE)) }
...
String setCleanModelName(String newModelToSet=null, List<String> acceptedModels=null) {
    ...
    if (acceptedModels != null) {
        acceptedModels.each {
            if (!found && newModel.startsWith(it)) { newModel = it; found = true }
        }
    }
...
void parseAndSendBatteryStatus(BigDecimal vCurrent) {
    ...
    BigDecimal bat = (vMax - vMin > 0) ? ((vCurrent - vMin) / (vMax - vMin)) * 100.0 : 100
    bat = bat.setScale(0, BigDecimal.ROUND_HALF_UP).min(100)
...
Map unpackStructInMap(Map msgMap, String originalEncoding="4C") {
    ...
    while (values != []) {
        ...
        List ret = zigbee_generic_convertStructValueToList(values, cType)
        r += ret[0]
        values = ret[1]
    }
...
Map parseXiaomiStruct(String xiaomiStruct, boolean isFCC0=false, boolean hasLength=false) {
    Map tags = [
        '01':'battery','03':'deviceTemperature','04':'unknown1','05':'RSSI_dB','06':'LQI',
        '07':'unknown2','08':'unknown3','09':'unknown4','0A':'routerid','0B':'unknown5',
        '0C':'unknown6','6429':'temperature','6410':'openClose','6420':'curtainPosition',
        '6521':'humidity','6510':'switch2','66':'pressure','6E':'unknown10','6F':'unknown11',
        '95':'consumption','96':'voltage','98':'power','9721':'gestureCounter1',
        '9739':'consumption','9821':'gestureCounter2','9839':'power','99':'gestureCounter3',
        '9A21':'gestureCounter4','9A20':'unknown7','9A25':'accelerometerXYZ','9B':'unknown9',
    ]
    if (isFCC0 == true) { tags['05'] = 'numBoots'; tags['6410'] = 'onOff'; tags['95'] = 'current' }

    List<String> values = xiaomiStruct.split("(?<=\\G..)")
    if (hasLength == true) values = values.drop(1)
    Map r = [:]; r["raw"] = [:]
    while (values != []) {
        String cTag = values.take(1)[0]; values = values.drop(1)
        String cTypeStr = values.take(1)[0]; values = values.drop(1)
        Integer cType = Integer.parseInt(cTypeStr, 16)
        String cKey = tags.containsKey(cTag+cTypeStr) ? tags[cTag+cTypeStr] :
                      tags.containsKey(cTag)           ? tags[cTag] :
                      "unknown${cTag}${cTypeStr}"
        if (cKey.startsWith("unknown")) log.warn("PLEASE REPORT TO DEV - The Xiaomi Struct used an unrecognized tag: 0x$cTag (type: 0x$cTypeStr) (struct: $xiaomiStruct)")
        List ret = zigbee_generic_convertStructValue(r, values, cType, cKey, cTag)
        r = ret[0]; values = ret[1]
    }
    return r
}
...
Map parseAttributeStruct(List data, boolean hasLength=false) {
    Map tags = [
        '0000':'ZCLVersion','0001':'applicationVersion','0002':'stackVersion','0003':'HWVersion',
        '0004':'manufacturerName','0005':'dateCode','0006':'modelIdentifier','0007':'powerSource',
        '0010':'locationDescription','0011':'physicalEnvironment','0012':'deviceEnabled',
        '0013':'alarmMask','0014':'disableLocalConfig','4000':'SWBuildID',
    ]
    List<String> values = data
    if (hasLength == true) values = values.drop(1)
    Map r = [:]; r["raw"] = [:]
    while (values != []) {
        String cTag = values.take(2).reverse().join(); values = values.drop(2)
        values = values.drop(1)
        String cTypeStr = values.take(1)[0]; values = values.drop(1)
        Integer cType = Integer.parseInt(cTypeStr, 16)
        String cKey
        if (tags.containsKey(cTag+cTypeStr))  { cKey = tags[cTag+cTypeStr] }
        else if (tags.containsKey(cTag))       { cKey = tags[cTag] }
        else throw new Exception("The Xiaomi Struct used an unrecognized tag: 0x$cTag (type: 0x$cTypeStr)")
        List ret = zigbee_generic_convertStructValue(r, values, cType, cKey, cTag)
        r = ret[0]; values = ret[1]
    }
    return r
...
List zigbee_generic_convertStructValue(Map r, List values, Integer cType, String cKey, String cTag) {
    ...
    switch (cType) {
        case 0x10:
            r["raw"][cKey] = values.take(1)[0]; r[cKey] = Integer.parseInt(r["raw"][cKey], 16) != 0; values = values.drop(1); break
        case 0x18: case 0x20:
            r["raw"][cKey] = values.take(1)[0]; r[cKey] = Integer.parseInt(r["raw"][cKey], 16); values = values.drop(1); break
        case 0x19: case 0x21:
            r["raw"][cKey] = values.take(2).reverse().join(); r[cKey] = Integer.parseInt(r["raw"][cKey], 16); values = values.drop(2); break
        case 0x1A: case 0x22:
            r["raw"][cKey] = values.take(3).reverse().join(); r[cKey] = Integer.parseInt(r["raw"][cKey], 16); values = values.drop(3); break
        case 0x1B: case 0x23:
            r["raw"][cKey] = values.take(4).reverse().join(); r[cKey] = Long.parseLong(r["raw"][cKey], 16); values = values.drop(4); break
        case 0x1C: case 0x24:
            r["raw"][cKey] = values.take(5).reverse().join(); r[cKey] = Long.parseLong(r["raw"][cKey], 16); values = values.drop(5); break
        case 0x1D: case 0x25:
            r["raw"][cKey] = values.take(6).reverse().join(); r[cKey] = Long.parseLong(r["raw"][cKey], 16); values = values.drop(6); break
        case 0x1E: case 0x26:
            r["raw"][cKey] = values.take(7).reverse().join(); r[cKey] = Long.parseLong(r["raw"][cKey], 16); values = values.drop(7); break
        case 0x1F: case 0x27:
            r["raw"][cKey] = values.take(8).reverse().join(); r[cKey] = new BigInteger(r["raw"][cKey], 16); values = values.drop(8); break
        case 0x28:
            r["raw"][cKey] = values.take(1).reverse().join(); r[cKey] = convertToSignedInt8(Integer.parseInt(r["raw"][cKey], 16)); values = values.drop(1); break
        case 0x29:
            r["raw"][cKey] = values.take(2).reverse().join(); r[cKey] = (Integer)(short)Integer.parseInt(r["raw"][cKey], 16); values = values.drop(2); break
        case 0x2B:
            r["raw"][cKey] = values.take(4).reverse().join(); r[cKey] = (Integer)Long.parseLong(r["raw"][cKey], 16); values = values.drop(4); break
        case 0x30:
            r["raw"][cKey] = values.take(1)[0]; r[cKey] = Integer.parseInt(r["raw"][cKey], 16); values = values.drop(1); break
        case 0x31:
            r["raw"][cKey] = values.take(2).reverse().join(); r[cKey] = Integer.parseInt(r["raw"][cKey], 16); values = values.drop(2); break
        case 0x39:
            r["raw"][cKey] = values.take(4).reverse().join(); r[cKey] = parseSingleHexToFloat(r["raw"][cKey]); values = values.drop(4); break
        case 0x42:
            Integer strLength = Integer.parseInt(values.take(1)[0], 16); values = values.drop(1)
            r["raw"][cKey] = values.take(strLength)
            r[cKey] = r["raw"][cKey].collect { (char)(int)Integer.parseInt(it, 16) }.join()
            values = values.drop(strLength); break
        default:
...
ArrayList<String> zigbeeReadAttributeList(Integer cluster, List<Integer> attributeIds, Map additionalParams = [:], int delay = 211) {
    ...
    String rattrArgs = "0x${device.deviceNetworkId} 1 0x01 0x${integerToHexString(cluster, 2)} {000000${attributeIdsString.join()}}"
    ArrayList<String> cmd = ["he raw $rattrArgs", "delay $delay"]
    logging("zigbeeReadAttributeList cmd=$cmd", 1)
    return cmd
}
...
String integerToHexString(Integer value, Integer minBytes, boolean reverse=false) {
    return reverse ? HexUtils.integerToHexString(value, minBytes).split("(?<=\\G..)").reverse().join()
                   : HexUtils.integerToHexString(value, minBytes)
}

String bigIntegerToHexString(BigInteger value, Integer minBytes, boolean reverse=false) {
    return reverse ? value.toString(16).reverse().join()
                   : String.format("%0${minBytes*2}x", value)
}

BigInteger hexStringToBigInteger(String hexString, boolean reverse=false) {
    return reverse ? new BigInteger(hexString.split("(?<=\\G..)").reverse().join(), 16)
                   : new BigInteger(hexString, 16)
}

Integer miredToKelvin(Integer mired) {
    Integer t = mired.clamp(153, 500)
    return Math.round(1000000 / t).toInteger().clamp(2000, 6536)
}

Integer kelvinToMired(Integer kelvin) {
    Integer t = kelvin.clamp(2000, 6536)
    return Math.round(1000000 / t).toInteger().clamp(153, 500)
}
...
Integer getMaximumMinutesBetweenEvents(BigDecimal forcedMinutes=null) {
    if (forcedMinutes == null && (state.forcedMinutes == null || state.forcedMinutes == 0)) {
        return MINUTES_BETWEEN_EVENTS == null ? 90 : MINUTES_BETWEEN_EVENTS
    }
    return forcedMinutes != null ? forcedMinutes.intValue() : state.forcedMinutes.intValue()
}

void reconnectEvent(BigDecimal forcedMinutes=null) { recoveryEvent(forcedMinutes) }

void disableRecoveryDueToBug() {
    log.warn("Stopping Recovery feature due to Platform bug! Disabling in Preferences.")
    unschedule('recoveryEvent'); unschedule('reconnectEvent')
    device.updateSetting('recoveryMode', 'Disabled')
}

void recoveryEvent(BigDecimal forcedMinutes=null) {
    try { recoveryEventDeviceSpecific() } catch (Exception e) {
    ...
    } catch (Exception e) { disableRecoveryDueToBug() }
}

void scheduleRecoveryEvent(BigDecimal forcedMinutes=null) {
    Random rnd = new Random()
    switch (recoveryMode) {
        case "Suicidal": schedule("${rnd.nextInt(15)}/15 * * * * ? *", 'recoveryEvent'); break
        case "Insane":   schedule("${rnd.nextInt(30)}/30 * * * * ? *", 'recoveryEvent'); break
        case "Slow":     schedule("${rnd.nextInt(59)} ${rnd.nextInt(3)}/3 * * * ? *", 'recoveryEvent'); break
        default:         schedule("${rnd.nextInt(59)} ${rnd.nextInt(2)}/2 * * * ? *", 'recoveryEvent'); break
    }
    ...
    if (recoveryMode == "Disabled") {
        ...
        try {
            if (hasCorrectCheckinEvents(maximumMinutesBetweenEvents=mbe) == false) {
                ...
                if (displayWarnings && (presenceWarningEnable == null || presenceWarningEnable == true)) {
                    log.warn("Event interval INCORRECT, recovery mode ($recoveryMode) ACTIVE!")
                }
...
void startCheckEventInterval() {
    ...
    if (recoveryMode != "Disabled") {
        ...
        unschedule('checkEventInterval'); unschedule('recoveryEvent'); unschedule('reconnectEvent')
    }
}
...
void updateManufacturer(String manufacturer) {
    if (getDataValue("manufacturer") == null) updateDataValue("manufacturer", manufacturer)
}

void updateApplicationId(String application) {
    if (getDataValue("application") == null) updateDataValue("application", application)
}
...
Map parseSimpleDescriptorData(List<String> data) {
    ...
    if (data[1] == "00") {
        ...
        if (icn > 0) {
            (1..icn).each { b ->
                Integer cPos = pos + ((b-1)*2)
                d["inClusters"] += data[cPos..cPos+1].reverse().join()
                if (b < icn) d["inClusters"] += ","
            }
        }
        pos += icn*2
        Integer ocn = Integer.parseInt(data[pos], 16); pos += 1
        d["outClusters"] = ""
        if (ocn > 0) {
            (1..ocn).each { b ->
                Integer cPos = pos + ((b-1)*2)
                d["outClusters"] += data[cPos..cPos+1].reverse().join()
                if (b < ocn) d["outClusters"] += ","
            }
        }
...
void getInfo(boolean ignoreMissing=false, Map<String,String> sdi = [:]) {
    ...
    String endpointId   = device.getEndpointId() ?: getDataValue("endpointId")
    ...
    if (sdi != [:]) {
        endpointId  = endpointId  ?: sdi['endpointId']
        profileId   = profileId   ?: sdi['profileId']
        inClusters  = inClusters  ?: sdi['inClusters']
        outClusters = outClusters ?: sdi['outClusters']
    }
    String extraFingerPrint = application != null ? ", application:\"$application\"" : ""
    boolean missing = false
    String requestingFromDevice = ignoreMissing ? ". Try again." : ", requesting it from the device. If it is a sleepy device you may have to wake it up and run this command again."

    if (manufacturer == null) { missing = true; log.warn("Manufacturer name is missing for the fingerprint$requestingFromDevice"); if (!ignoreMissing) sendZigbeeCommands(zigbee.readAttribute(CLUSTER_BASIC, 0x0004)) }
    log.trace("Manufacturer: $manufacturer")
    if (model == null) { missing = true; log.warn("Model name is missing for the fingerprint$requestingFromDevice"); if (!ignoreMissing) sendZigbeeCommands(zigbee.readAttribute(CLUSTER_BASIC, 0x0005)) }
    log.trace("Model: $model")
    if (application == null) { log.info("NOT IMPORTANT: Application ID is missing for the fingerprint$requestingFromDevice"); if (!ignoreMissing) sendZigbeeCommands(zigbee.readAttribute(CLUSTER_BASIC, 0x0001)) }
    log.trace("Application: $application")
    if (profileId == null || endpointId == null || inClusters == null || outClusters == null) {
        missing = true
        String endpointIdTemp = endpointId ?: "01"
        log.warn("One or multiple pieces of data needed for the fingerprint is missing$requestingFromDevice")
        if (!ignoreMissing) sendZigbeeCommands(["he raw ${device.deviceNetworkId} 0 0 0x0004 {00 ${zigbee.swapOctets(device.deviceNetworkId)} $endpointIdTemp} {0x0000}"])
    }
    profileId = profileId ?: "0104"
    String fp = "fingerprint model:\"$model\", manufacturer:\"$manufacturer\", profileId:\"$profileId\", endpointId:\"$endpointId\", inClusters:\"$inClusters\", outClusters:\"$outClusters\"$extraFingerPrint"
    if (missing) {
        log.info("INCOMPLETE - DO NOT SUBMIT THIS - TRY AGAIN: $fp")
    } else {
        log.info("COPY AND PASTE THIS ROW TO USER JSHIMOTA IN THE HUBITAT COMMUNITY - DO NOT SEND TO MARKUS! $fp")
    }
}
...
boolean isDriver() {
    try { getDeviceDataByName('_unimportant'); return true } catch (MissingMethodException e) { return false }
}

void deviceCommand(String cmd) {
    def jsonSlurper = new JsonSlurper()
    def cmds = jsonSlurper.parseText(cmd)
    def r = this."${cmds['cmd']}"(*cmds['args'])
...
void setLogsOffTask(boolean noLogWarning=false) {
    if (debugLogging == true) {
        if (!noLogWarning) {
            log.warn(runReset != "DEBUG" ? "Debug logging will be disabled in 30 minutes..." : "Debug logging will NOT BE AUTOMATICALLY DISABLED!")
        }
        runIn(1800, "logsOff")
    }
}
...
void logsOff() {
    if (runReset != "DEBUG") {
        ...
        if (isDriver()) {
            device.clearSetting("logLevel"); device.removeSetting("logLevel"); device.updateSetting("logLevel", "0")
            state?.settings?.remove("logLevel")
            device.clearSetting("debugLogging"); device.removeSetting("debugLogging"); device.updateSetting("debugLogging", "false")
            state?.settings?.remove("debugLogging")
        } else {
            app.removeSetting("logLevel"); app.updateSetting("logLevel", "0")
            app.removeSetting("debugLogging"); app.updateSetting("debugLogging", "false")
        }
    } else {
...
def getEnvironmentObject() { return isDriver() ? device : app }

private String getFilteredDeviceDriverName() {
    String n = getDeviceInfoByName('name')
    return n.toLowerCase().endsWith(' (parent)') ? n.substring(0, n.length()-9) : n
}

private String getFilteredDeviceDisplayName() {
    return device.displayName.replace(' (parent)', '').replace(' (Parent)', '')
}

BigDecimal round2(BigDecimal number, Integer scale) {
    Integer pow = 10
    for (Integer i = 1; i < scale; i++) pow *= 10
    BigDecimal tmp = number * pow
    return ((Float)((Integer)(((tmp - (Integer)tmp) >= 0.5f) ? tmp + 1 : tmp))) / pow
}

String generateMD5(String s) {
    return s != null ? MessageDigest.getInstance("MD5").digest(s.bytes).encodeHex().toString() : "null"
}
...
/**
 * FIX: original added `+ 30` to every character code — a clear copy-paste bug.
 *      Correct ASCII conversion is a direct cast with no offset.
 */
...
String styling_addTitleDiv(title)           { return '<div class="preference-title">' + title + '</div>' }
String styling_addDescriptionDiv(description) { return '<div class="preference-description">' + description + '</div>' }

String styling_makeTextBold(s)   { return isDriver() ? "<b>$s</b>" : "$s" }
String styling_makeTextItalic(s) { return isDriver() ? "<i>$s</i>" : "$s" }

String styling_getDefaultCSS(boolean includeTags=true) {
    String defaultCSS = '''
    div.mdl-card__title div.mdl-grid div.mdl-grid .mdl-cell p::after {
        visibility: visible; position: absolute; left: 50%; transform: translate(-50%, 0%);
        width: calc(100% - 20px); padding-left: 5px; padding-right: 5px; margin-top: 0px;
    }
    h3, h4, .property-label { font-weight: bold; }
    .preference-title { font-weight: bold; }
    .preference-description { font-style: italic; }
    '''
    return includeTags ? "<style>$defaultCSS </style>" : defaultCSS
}

String styling_getLogo() {
    String logoCSS = '#ohla_logo { display:block; width:200px; height:50px; position:absolute; top:10px; right:10px; } @media screen and (max-device-width:450px),screen and (max-width:450px) { #ohla_logo { width:120px; top:55px; } }'
...
void refresh(String cmd) { deviceCommand(cmd) }

def installedDefault() {
    logging("installedDefault()", 100)
    try { tasmota_installedPreConfigure() } catch (MissingMethodException e) {}
    try { installedAdditional() } catch (MissingMethodException e) {}
}

def configureDefault() {
    logging("configureDefault()", 100)
    try { return configureAdditional() } catch (MissingMethodException e) {}
    try { getDriverVersion() } catch (MissingMethodException e) {}
}

void configureDelayed() { runIn(10, "configure"); runIn(30, "refresh") }
...
void stopSchedules() { unschedule(); log.info("Stopped ALL Device Schedules!") }
...
boolean isValidDate(String dateFormat, String dateString) {
    try { Date.parse(dateFormat, dateString) } catch (e) { return false }
    return true
}

Integer retrieveMinimumMinutesToRepeat(Integer minimumMinutesToRepeat=55) {
    if (state.forcedMinutes == null || state.forcedMinutes == 0) return minimumMinutesToRepeat
    return state.forcedMinutes - 1 < 1 ? 1 : state.forcedMinutes.intValue() - 1
}
...
boolean sendlastCheckinEvent(Integer minimumMinutesToRepeat=55) {
    ...
    if (lastCheckinEnable == true || lastCheckinEnable == null) {
        ...
        if (lastCheckinVal == null || isValidDate('yyyy-MM-dd HH:mm:ss', lastCheckinVal) == false ||
                now() >= Date.parse('yyyy-MM-dd HH:mm:ss', lastCheckinVal).getTime() + (mmr * 60 * 1000)) {
            r = true
    ...
    if (lastCheckinEpochEnable == true) {
        if (device.currentValue('lastCheckinEpoch') == null ||
                now() >= device.currentValue('lastCheckinEpoch').toLong() + (mmr * 60 * 1000)) {
            r = true
...
Long secondsSinceLastCheckinEvent() {
    ...
    if (lastCheckinEnable == true || lastCheckinEnable == null) {
        String lastCheckinVal = device.currentValue('lastCheckin')
        if (lastCheckinVal == null || isValidDate('yyyy-MM-dd HH:mm:ss', lastCheckinVal) == false) {
            logging("No VALID lastCheckin event available!", 100); r = -1
        } else {
    ...
    if (lastCheckinEpochEnable == true) {
        if (device.currentValue('lastCheckinEpoch') == null) {
            logging("No VALID lastCheckin event available!", 100); r = r == null ? -1 : r
        } else {
...
boolean hasCorrectCheckinEvents(Integer maximumMinutesBetweenEvents=90, boolean displayWarnings=true) {
    ...
    if (secondsSinceLastCheckin != null && secondsSinceLastCheckin > maximumMinutesBetweenEvents * 60) {
        if (displayWarnings && (presenceWarningEnable == null || presenceWarningEnable == true)) {
            log.warn("One or several EXPECTED checkin events have been missed! Minutes since last checkin: ${Math.round(secondsSinceLastCheckin / 60)} (maximum expected $maximumMinutesBetweenEvents)")
        }
        return false
    }
    return true
...
boolean checkPresence(boolean displayWarnings=true) {
    ...
    String lastCheckinVal = device.currentValue('lastCheckin')
    if ((lastCheckinEnable == true || lastCheckinEnable == null) && isValidDate('yyyy-MM-dd HH:mm:ss', lastCheckinVal)) {
    ...
    if (lastCheckinTime != null && lastCheckinTime >= now() - (3 * 60 * 60 * 1000)) {
        ...
        if (displayWarnings) {
            ...
            if (presenceWarningEnable == null || presenceWarningEnable == true) {
                log.warn("No event seen from the device for over 3 hours! (consecutive events: $numNotPresent)")
            }
        }
    }
    return isPresent
