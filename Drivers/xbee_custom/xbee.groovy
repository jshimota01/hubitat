...
private handleTemperatureEvent(cluster) {
    ...
    if (logDesc) log.info "Temperature C: ${temperature_c}"
    if (logDesc) log.info "Temperature F: ${temperature_f}"
    def linkText = getLinkText(device)
    descriptionText = "${linkText} XBee chip temperature C is ${temperature_c}C"
    descriptionText = "${linkText} XBee chip temperature F is ${temperature_f}F"
    def eventMap = [
        name: 'temperature',
        value: temperature_c,
        unit: "C",
    ...
    if (logDebug) log.debug "Creating temperature event for temperature=${temperature_f}F: ${linkText} ${eventMap2.name} is ${eventMap2.value}F"
    sendEvent(eventMap2)      
}
/**
