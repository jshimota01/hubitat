...
def timeStamp() { return "2026/08/28 10:10 AM" }
...
metadata {
    definition (
        ...
        importUrl: "https://raw.githubusercontent.com/jshimota/Hubitat/Drivers/sonoff_zigbee_garage_door_opener_relay_custom/sonoff_GDO_relay.groovy", 
        singleThreaded: true
    ) {
        capability "Actuator"
    ...
    preferences {
        input name: "healthCheckInterval", type: "enum", title: "Health Check Schedule", description: "Automatically ping device on a regular interval to verify presence.", options: ["0":"Disabled", "1":"Every Hour", "3":"Every 3 Hours", "6":"Every 6 Hours", "12":"Every 12 Hours", "24":"Every 24 Hours"], defaultValue: "3", required: true
    ...
    if (description?.startsWith('catchall:') || description?.startsWith('read attr -')) {
        ...
        if (descMap?.clusterInt == CLUSTER_SONOFF) {
            ...
            if (descMap?.command in ["00", "01", "02"]) {
                ...
                logWarn "UNPROCESSED COMMAND Sonoff cmd ${descMap?.command} : dp=${descMap?.data[2]} value=${descMap?.data[3]} descMap.data = ${descMap?.data}"
            }
        } 
...
def sendSwitchEvent(state, isDigital=false) {
    ...
    sendIfChanged([name: "switch", value: state, type: typeStr, descriptionText: "switch is ${state} (${typeStr})"])
}
