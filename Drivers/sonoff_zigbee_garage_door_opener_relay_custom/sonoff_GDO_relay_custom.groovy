 ...
 * ver  1.2.1  2026-08-27 jshimota - bug fixes: corrected cron syntax for hourly schedule, updated unschedule method call, robust attrInt checking in parse
 * ver  1.2.2  2026-08-27 jshimota - updated ping and refresh logging to write to info logs when txtEnable is true
 * ver  1.2.3  2026-08-27 jshimota - added lastPing and pingStatus attributes with dynamic timeout tracking
 */

def version() { "1.2.3" }
def timeStamp() {"2026/08/27 10:00 AM"}
...
def parse(String description) {
    ...
    if (description?.startsWith('catchall:') || description?.startsWith('read attr -')) {
        ...
        if (descMap?.clusterInt == CLUSTER_SONOFF) {
            ...
            sendEvent(name: "pingStatus", value: "Responded")
            if (txtEnable) log.info "${device.displayName} Health check response / presence confirmed."
        }
...
def refresh() {
    if (txtEnable) log.info "${device.displayName} refresh() requested"
    return ping()
}

def ping() {
    String timestamp = new Date().format("yyyy-MM-dd HH:mm:ss", location.timeZone)
    sendEvent(name: "lastPing", value: timestamp)
    sendEvent(name: "pingStatus", value: "Pending")
    
    if (txtEnable) log.info "${device.displayName} sending health check ping (Basic Cluster read)..."
...
def pingTimeout() {
    sendEvent(name: "pingStatus", value: "Timeout")
    if (logEnable) log.warn "${device.displayName} health check ping timed out (no response received)."
}
...
void scheduleHealthCheck() {
    ...
    switch (interval) {
        ...
        case "12":
            ...
            break
        case "24":
            schedule("0 0 3 ? * * *", healthCheck) // Daily at 3:00 AM
