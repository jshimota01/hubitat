...
metadata {
    definition(
        ...
        // Includes "fanCirculate" as an option for platform-supported drop-downs
...
def initialize() {
    ...
    // UI presentation list context - adding fanCirculate to dropdown options
    def fanOptionsList = ["fanOn", "fanAuto", "fanOff", "fanCirculate"]
...
def getFanModeMap() { ["00":"fanOff", "04":"fanOn", "05":"fanAuto"] }
...
def fanOn() {
    ...
    unschedule(runCirculateCycle) // Break circulation scheduler loop if explicit mode requested
...
def fanCirculate() {
    if (txtEnable) log.info "${device.displayName} received 'Fan Circulate' request. Beginning 30m On/30 min Off cycle. (Device does not support hardware Circulate command, using on/off routine)"
    sendEvent(name: "thermostatFanMode", value: "fanCirculate", descriptionText: "${device.displayName} fan mode is set to fanCirculate")
    runCirculateCycle("on")
}

// Software scheduler worker loop
def runCirculateCycle(String targetState = null) {
    // If our mode was changed externally out of fanCirculate, terminate execution thread safely
    if (device.currentValue("thermostatFanMode") != "fanCirculate") {
        if (dbgEnable) log.debug "Circulation cycle stopped because fan mode is no longer set to fanCirculate."
        return
    }

    // Determine state change toggle if no explicit parameter was passed by the execution sequence
    if (targetState == null) {
    ...
    if (targetState == "on") {
        if (txtEnable) log.info "${device.displayName} Circulation Cycle: Turning fan ON for 30 minutes."
        // Write attribute 4 (fanOn) directly via device connection
        ...
        if (txtEnable) log.info "${device.displayName} Circulation Cycle: Turning fan OFF/Auto for 30 minutes."
        // Write attribute 5 (fanAuto) directly via device connection so the HVAC safely drops to idle
