...
def timeStamp() { return "2026/08/31 09:15 PM" }
...
void setThermostatFanMode(String value) { 
    ...
    if (normValue == "ext") {
        logInfo "Thermostat Fan Mode set to 'ext' (External app ownership active)."
...
List<String> fanOn() {
    ...
    // PRESERVE 'ext' ATTRIBUTE IF ACTIVE
    if (device.currentValue("thermostatFanMode") != "ext") {
        updateAttribute("thermostatFanMode", "on")
    } else {
        logDebug "thermostatFanMode attribute preserved as 'ext' during external fanOn execution."
    }
...
List<String> fanAuto() {
    ...
    if (device.currentValue("thermostatFanMode") != "ext") {
        ...
        logDebug "thermostatFanMode attribute preserved as 'ext' during external fanAuto execution."
    }
...
List<String> fanOff() {
    ...
    if (device.currentValue("thermostatFanMode") != "ext") {
        ...
        logDebug "thermostatFanMode attribute preserved as 'ext' during external fanOff execution."
    }
        ...
        if (description.startsWith("read attr -") || description.startsWith("catchall:")) {
            ...
            switch(clusterInt) {
                ...
                case 0x0202: // Fan Control Cluster
                    if (attrInt == 0x0000) {
                        // PRESERVE 'ext' ATTRIBUTE IF ACTIVE DURING PHYSICAL INBOUND REPORTING
                        if (device.currentValue("thermostatFanMode") != "circulate" && device.currentValue("thermostatFanMode") != "ext") {
                            ...
                            logDebug "Inbound Zigbee fan report acknowledged; thermostatFanMode attribute preserved as '${device.currentValue("thermostatFanMode")}'."
                        }
                    }
                    break
