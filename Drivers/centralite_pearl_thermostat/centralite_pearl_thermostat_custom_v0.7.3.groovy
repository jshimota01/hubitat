 ...
 *
 * ARCHITECTURAL NOTE - LOCKED 'AUTO' DASHBOARD DISPLAY:
 * Includes a preference option ('lockDashboardToAuto') that locks the 'thermostatMode' attribute 
 * to 'auto'. This prevents Dashboard tiles from collapsing into single-setpoint heat/cool views when 
 * MEM or external routines command explicit physical modes. The physical hardware state is tracked 
 * via 'physicalThermostatMode'.
 **/

static String version() { return '0.7.3' }
def timeStamp() { return "2026/09/01 09:35 AM" }
...
metadata {
    ...
    preferences {
        input name: "lockDashboardToAuto", type: "bool", title: "<b>Lock Dashboard Tile to 'Auto' Mode</b>", description: "<i>When enabled, locks thermostatMode attribute to 'auto' so dashboard tiles preserve the dual-setpoint view even when MEM changes physical hardware modes.</i>", defaultValue: true, required: true
