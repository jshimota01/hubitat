...
def timeStamp() { return "2026/09/01 09:30 AM" }
...
metadata {
    ...
    preferences {
        ...
        input name: "tempPrecision", type: "enum", title: "<b>Device Temperature Display Precision</b>", options: ["0": "0 Decimals (Whole Number - e.g., 72)", "1": "1 Decimal (e.g., 71.8)", "2": "2 Decimals (e.g., 71.82)"], defaultValue: "1", description: "<i>Controls decimal precision for reported device ambient temperature attribute only (e.g., for dashboard tile display). Setpoints remain whole integers.</i>"
...
void initialize(Boolean isInstall = false) {
    ...
    // Exposes 'auto' to enable Dashboard dual-setpoint tiles and MEM thermostat controller orchestration.
...
List<String> auto() {
    ...
    // Dynamically evaluate active setpoint based on current ambient temperature
...
private List<String> processSetpoint(degrees, int attributeId) { 
    ...
    if (currentMode == "auto" || (attrName == "heatingSetpoint" && currentMode in ["heat", "emergencyHeat"]) || (attrName == "coolingSetpoint" && currentMode == "cool")) {
            ...
            switch(clusterInt) {
                case 0x0201: // Thermostat Cluster
                    if (attrInt == 0x0000) {
                        ...
                        if (device.currentValue("thermostatMode") in ["cool", "auto"]) {
