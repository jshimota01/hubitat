...
def timeStamp() { return "2026/08/31 04:58 PM" }
...
metadata {
    definition(
        ...
        command "setThermostatFanMode", [[name: "fanMode*", type: "ENUM", description: "Thermostat Fan Mode (Only Functional if preference setting is enabled. Check preferences when using external app or controller.)", constraints: ["on", "auto", "off", "circulate"]]]
    ...
    preferences {
        ...
        input name: "enableFanCommands", type: "bool", title: "<b>Enable Thermostat Fan Mode Commands</b>", description: "<i>Turn OFF to disable local driver fan execution and dump fan commands when managed by an external app like MEM.</i>", defaultValue: true, required: true
        input name: "fanModeNote", type: "paragraph", title: "<b>Fan Mode Operational Note</b>", description: "<i>Check preferences when using an external app or controller. When utilizing Multiperiod Environment Manager (MEM), disable fan execution above or leave physical fan mode in <b>Auto</b>.</i>"
...
void setThermostatFanMode(String value) { 
    ...
    if (!getSettingBool("enableFanCommands", true)) {
        logInfo "Thermostat Fan Mode command '${value}' ignored (Fan Mode disabled in preferences)."
        return
    }
