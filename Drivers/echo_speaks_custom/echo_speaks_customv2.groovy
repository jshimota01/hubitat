...
def installed() {
    ...
    if(!isZone()) {
		...
		// JAS Modded 12-21-2024
		// sendEvent(name: "deviceTemp", value: 0)
    }
...
def initialize() {
    ...
    stateCleanup()
	// modded JAS 12-21-2024
    if(minVersionFailed()) { logError("CODE UPDATE required to RESUME operation.  No Device Events will be updated."); return }
...
Boolean isCommandTypeAllowed(String type, Boolean noLogs=false, Boolean ignoreDoNotDisturb=false) {
	...
	// JAS Modded 12-22-2024
	// if(!(String)state.temperatureSensor) { if(!noLogs) { logWarn("DeviceType Temperature Value Missing: ${(String)state.temperatureSensor}", true) }; return false }
...
void updateDeviceStatus(Map devData) {
    ...
    if(devData.size()) {
		...
		//JAS Modded 12-22-2024
        // String deviceTemp = (String)devData.temperatureSensor
        // state.temperatureSensor = deviceTemp
		...
		// Modded JAS 12-21-2024
		String devIcon = "<img style='width:64px;height:64px;' src='https://raw.githubusercontent.com/jshimota01/hubitat/main/Drivers/echo_speaks_custom/resources/icons/${isZone() ? "es_groups.png" : "${(String)deviceStyle?.i ?: "echo_gen1"}.png"}'/>"
		...
		// Modded JAS 12/22/2024 Uncommented
...
void playbackStateHandler(Map playerInfo, Boolean isGroupResponse=false) {
    ...
    //Track Art Image
    String trackImg = playerInfo.mainArt?.url ?: "https://raw.githubusercontent.com/jshimota01/hubitat/main/Drivers/echo_speaks_custom/resources/icons/na.png"
