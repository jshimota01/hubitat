...
def timeStamp() { return "2026/09/23 12:20 PM" }
...
metadata {
    ...
    preferences {
        ...
        input name: "basedOn", type: "enum", title: "<b>Publish Primary AQI Based On</b>", description: "<i>Select parameter basis for Primary AQI evaluation.</i>", required: true, defaultValue: "1", options: [1: "O3 Ozone", 2: "PM2.5 Particle", 3: "PM10 Particle", 4: "Worst AQI"]
...
void pollHandler(resp, data) {
    ...
    if (resp.getStatus() == 200 || resp.getStatus() == 207) {
        ...
        def isBasis = aqiBasis[settings.basedOn ? settings.basedOn as Integer : 1]
        ...
        aqi.each { obs ->
            if ((obs.AQI >= 0) && (obs.AQI <= 2000)) {
                ...
                sendIfChanged(paramName, obs.AQI, null, "info", "${paramName} AQI")
