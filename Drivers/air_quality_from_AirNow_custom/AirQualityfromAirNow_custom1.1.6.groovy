...
def timeStamp() { return "2026/09/23 01:30 PM" }
...
void pollHandler(resp, data) {
    ...
    if (resp.getStatus() == 200 || resp.getStatus() == 207) {
        ...
        aqi.each { obs ->
            if ((obs.AQI >= 0) && (obs.AQI <= 2000)) {
                ...
                if (isBasis == paramName) {
                    String primarySummary = "AirNow Air Quality Index is now ${obs.AQI} for ${paramName} pollutant"
                    sendIfChanged("airQualityIndex", obs.AQI, null, "info", "airQualityIndex", primarySummary)
        ...
        if (isBasis == "maxAQI" && maxAQI >= 0) {
            String primarySummary = "AirNow Air Quality Index is now ${maxAQI} for ${dominantName} pollutant"
            sendIfChanged("airQualityIndex", maxAQI, null, "info", "airQualityIndex", primarySummary)
