...
metadata {
    definition (name: "TZ Garage Door Opener-Custom", namespace: "jshimota", author: "Krassimir Kossev", importUrl: "https://raw.githubusercontent.com/jshimota/Hubitat/Drivers/tuya_zigbee_garage_door_opener_custom/tz_gdo_v1.1.0.b.groovy", singleThreaded: true ) {
        capability "Actuator"
...
def open() {
    if (device?.currentState('contact')?.value != "open") {
        if (dbgEnable) log.debug "${device.displayName} opening (door was ${device?.currentState('door')?.value} , contact was ${device?.currentState('contact')?.value})"
...
def close() {
    if (device?.currentState('contact')?.value != "closed") {
        if (dbgEnable) log.debug "${device.displayName} closing (door was ${device?.currentState('door')?.value} , contact was ${device?.currentState('contact')?.value})"
...
void initializeVars( boolean fullInit = true ) {
    ...
    if (device?.currentState('powerRetValue')?.value == null ) {
        sendEvent(name : "powerRetValue",	value : "?", isStateChange : true)
    }
}

def initialize() {
    if (txtEnable==true) log.info "${device.displayName} Initialize()..."
    if (txtEnable==true) log.info "${device.displayName} Door assumed CLOSED."
    unschedule()
    initializeVars()
