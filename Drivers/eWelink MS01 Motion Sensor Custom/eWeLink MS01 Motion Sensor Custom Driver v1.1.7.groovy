...
static String timeStamp() { "2026/08/10 11:43 AM" }
...
void configure() {
    ...
    // 1. IAS CIE Address Write
    String hubIeee = swapEndianHex(device.hub.zigbeeId)
    cmds += "he wattr 0x${device.deviceNetworkId} 0x01 0x0500 0x0010 0xF0 {${hubIeee}}"
    cmds += "delay 300"

    // 2. IAS Zone Enroll Response
    cmds += "he raw 0x${device.deviceNetworkId} 1 0x01 0x0500 {01 23 00 00 00}"
    cmds += "delay 300"

    // 3. IAS Zone Status / Enrolled State
    cmds += "he raw 0x${device.deviceNetworkId} 1 0x01 0x0500 {10 00 00 00 00}"
    cmds += "delay 300"

    // 4. Configure Battery Reporting
    cmds += zigbee.configureReporting(0x0001, 0x0021, 0x20, 30, 21600, 0x01)
    cmds += "delay 500"

    // 5. Read initial battery attributes
...
void parse(final String description) {
    ...
    if (descMap.clusterInt == 0x0500) {
        ...
        // IAS Zone Auto-Enrollment Request / Handshake Trigger (Command 01 or Command 0B)
        if ((descMap.command == '01' && descMap.attrId == null) || descMap.command == '0B') {
            logInfo "IAS Zone Handshake / Enroll Request received (Command ${descMap.command}). Completing enrollment sequence."
            List enrollCmds = [
                "he raw 0x${device.deviceNetworkId} 1 0x01 0x0500 {01 23 00 00 00}",
                "delay 300",
                "he raw 0x${device.deviceNetworkId} 1 0x01 0x0500 {10 00 00 00 00}"
            ]
