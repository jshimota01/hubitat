...
static String timeStamp() { "2026/08/10 11:52 AM" }
...
void configure() {
    ...
    // 1. Write Hub IEEE Address as IAS CIE Target (Cluster 0x0500, Attr 0x0010, Type 0xF0)
    String hubIeee = swapEndianHex(device.hub.zigbeeId)
    cmds += zigbee.writeAttribute(0x0500, 0x0010, 0xF0, hubIeee)

    // 2. Send Standard Zone Enroll Response (Command 0x00 on Cluster 0x0500: 00 = Success, 00 = Zone ID)
    cmds += zigbee.command(0x0500, 0x00, "0000")

    // 3. Query IAS Zone State (Attr 0x0000) & Zone Type (Attr 0x0001)
    cmds += zigbee.readAttribute(0x0500, 0x0000)
    cmds += zigbee.readAttribute(0x0500, 0x0001)

    // 4. Configure Battery Reporting (Cluster 0x0001, Attr 0x0021, Min 30s, Max 6h, Delta 1%)
    cmds += zigbee.configureReporting(0x0001, 0x0021, 0x20, 30, 21600, 0x01)

    // 5. Query initial battery attributes
...
void parse(final String description) {
    ...
    if (descMap.clusterInt == 0x0500) {
        ...
        // Command 01 (Zone Enroll Request): Device explicitly requesting enrollment
        if (descMap.command == '01' && descMap.attrId == null) {
            logInfo "IAS Zone Enroll Request received. Responding with Enroll Response Success."
            List enrollCmds = zigbee.command(0x0500, 0x00, "0000")
            sendZigbeeCommands(enrollCmds)
            return
        }

        // Command 0B (Default Response): Log and do NOT bounce back response to avoid loops
        if (descMap.command == '0B') {
            logTrace "MS01 returned ZCL Default Response (0B). Data: ${descMap.data}"
            return
        }
        ...
        if (descMap.attrId == "0000" && descMap.value) {
            ...
            if (zoneState == 1) {
                logInfo "MS01 IAS Zone status verified: ENROLLED."
            } else {
                logWarn "MS01 IAS Zone enrollment state reported un-enrolled (${zoneState}). Sending configuration..."
                configure()
            }
            return
        }
        ...
        // Command 04: Write Attribute Response acknowledgment
        if (descMap.command == "04") {
            logDebug "MS01 IAS Zone CIE Address write acknowledged successfully."
            return
        }

        // Actual IAS Zone Status Change Notification (Cluster-specific Command 00 or 02)
