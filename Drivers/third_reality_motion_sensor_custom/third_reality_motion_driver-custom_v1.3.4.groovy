 ...
 * parsing, motion deduplication, battery voltage calculation, passive network health tracking,
 * and hardware-level cool-down (motion reset delay) attribute configuration.
 *
 * Notes:
 ...
 *
 * Proprietary Cluster Configuration (Firmware 1.00.79+)
 * - Hardware Cool-Down Period: Cluster 0xFF01, Attr 0x0001 (UINT16, MfrCode 0x1233).
 *   Range: 5 to 3600 seconds. Default: 30 seconds.
 **/
/**
...
/**
 * Changelog:
 * v1.3.4    09/19/26    jshimota    Promoted hardware-level Motion Cool-Down Time (Cluster 0xFF01, Attr 0x0001) implementation to production release.
 * v1.4.1-dev 09/19/26    jshimota    Updated coolDownTimeSetting preference description to note that hardware changes apply only on initialization or battery removal.
 * v1.4.0-dev 09/19/26    jshimota    Added hardware-level Motion Cool-Down Time (Cluster 0xFF01, Attr 0x0001) configuration command and preferences.
 * v1.3.3    09/14/26    jshimota    Enforced lastActivity refresh in parse() on all incoming Zigbee messages (isStateChange: false).
 * v1.3.2    08/31/26    jshimota    Stripped legacy MFR state variables, DataUpdate flags, and HTML icon/donation routines.
...
def timeStamp() { return "2026/09/19 09:05 AM" }
...
metadata {
    definition (
        ...
        attribute "coolDownTime", "number"

        // Custom Commands
        command "resetDriver"
        command "updateFirmware"
        command "setCoolDownTime", [[name: "Seconds", type: "NUMBER", description: "Motion cool-down delay in seconds (5-3600)"]]

        fingerprint profileId: "0104", endpointId: "01", inClusters: "0000,0001,0500,FF01", outClusters: "0019", model: "3RMS16BZ", manufacturer: "Third Reality, Inc", deviceJoinName: "Third Reality Motion Sensor (Custom)"
    }

    preferences {
        input name: "coolDownTimeSetting", type: "number", title: "<b>Motion Cool-Down Period</b>", range: "5..3600", defaultValue: 30, required: true, description: "<i>Hardware motion delay before clearing active state (5 to 3600 seconds). Requires firmware 1.00.79+. Note: Setting changes will only update the physical device upon driver initialization or battery removal.</i>"
...
List<String> configure() {
    checkAndLogVersionDemarcation()
    logInfo "Configuring device reporting and hardware parameters..."
    initialize(false)
    ...
    // Write Third Reality Cool-Down Period preference (Cluster 0xFF01, Attr 0x0001, MfrCode 0x1233, UINT16)
    int coolDownVal = settings.coolDownTimeSetting != null ? settings.coolDownTimeSetting as Integer : 30
    coolDownVal = Math.min(3600, Math.max(5, coolDownVal))
    logInfo "Setting hardware cool-down delay to ${coolDownVal} seconds"
    cmds += zigbee.writeAttribute(0xFF01, 0x0001, DataType.UINT16, coolDownVal, [mfrCode: 0x1233])
    ...
    cmds += zigbee.readAttribute(0xFF01, 0x0001, [mfrCode: 0x1233]) // Read back coolDownTime
...
List<String> refresh() {
    ...
    cmds += zigbee.readAttribute(0xFF01, 0x0001, [mfrCode: 0x1233]) // Cool-Down Period
    return cmds
}
...
List<String> setCoolDownTime(BigDecimal seconds) {
    int coolDownVal = seconds != null ? seconds.intValue() : 30
    coolDownVal = Math.min(3600, Math.max(5, coolDownVal))
    logInfo "Executing setCoolDownTime(${coolDownVal}s)..."
    device.updateSetting("coolDownTimeSetting", [type: "number", value: coolDownVal])
    
    List<String> cmds = []
    cmds += zigbee.writeAttribute(0xFF01, 0x0001, DataType.UINT16, coolDownVal, [mfrCode: 0x1233])
    cmds += zigbee.readAttribute(0xFF01, 0x0001, [mfrCode: 0x1233])
    return cmds
}
...
void parse(String description) {
    ...
    if (description.startsWith("zone status")) {
        sendEvent(name: "healthStatus", value: "online", isStateChange: false, type: "physical")
    ...
    // Third Reality Proprietary Cluster (0xFF01)
    if (descMap.clusterInt == 0xFF01) {
        if (descMap.attrInt == 0x0001 && descMap.value != null) {
            sendEvent(name: "healthStatus", value: "online", isStateChange: false, type: "physical")
            int parsedSeconds = Integer.parseInt(descMap.value, 16)
            logDebug "Third Reality proprietary coolDownTime attribute reported: ${parsedSeconds}s"
            updateAttribute("coolDownTime", parsedSeconds, "s", "physical")
        }
        return
    }
}
