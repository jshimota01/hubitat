 ...
 * lux delta filtering, calibration offset, and in-memory debounce protection.
 *
 * Notes:
...
def timeStamp() { return "2026/09/24 11:30 AM" }
...
metadata {
    definition (
        ...
        // Fingerprints
        fingerprint profileId: "0104", endpointId: "01", inClusters: "0000,0004,0005,EF00", outClusters: "0019,000A", model: "TS0601", manufacturer: "_TZE200_3towulqd", controllerType: "ZIGBEE"
        fingerprint profileId: "0104", endpointId: "01", inClusters: "0000,0004,0005,EF00", outClusters: "0019,000A", model: "TS0601", manufacturer: "_TZE200_rhgsbacq", controllerType: "ZIGBEE"
    }

    preferences {
        input name: "pirSensitivity", type: "enum", title: "<b>PIR Sensitivity</b>", options: ["0": "Low", "1": "Medium", "2": "High"], defaultValue: "1"
        input name: "pirKeepTime", type: "enum", title: "<b>PIR Keep Time</b>", options: ["10": "10 Seconds", "30": "30 Seconds", "60": "60 Seconds", "120": "120 Seconds"], defaultValue: "30"
...
void parse(String description) {
    ...
    // Intercept and IGNORE Standard ZCL 0x0400 to prevent race conditions with Tuya EF00 DP 12
    ...
    // Process Tuya Cluster EF00 Commands
    if (description.contains("clusterId: EF00") || description.contains("cluster: EF00")) {
...
private void parseTuyaCluster(List<String> data) {
    ...
    switch (dp) {
        case 0x01: // Motion State (DP 1)
            String motionState = (rawVal == 1) ? "active" : "inactive"
...
void updated() {
    ...
    // Send Sensitivity setting (DP 0x09, Enum Type 0x04)
    if (settings.pirSensitivity != null) {
        cmds += sendTuyaCommand("09", "04", "01", zigbee.convertToHexString(settings.pirSensitivity.toInteger(), 2))
    }
    
    // Send Keep Time setting (DP 0x0A, Value Type 0x02)
    if (settings.pirKeepTime != null) {
        cmds += sendTuyaCommand("0A", "02", "04", zigbee.convertToHexString(settings.pirKeepTime.toInteger(), 8))
    }
...
private List<String> executeHealthCheck() {
    ...
    // Read Basic Cluster (0x0000) Attribute 0x0004 (Manufacturer Name) to verify network route
    return zigbee.readAttribute(0x0000, 0x0004)
}
