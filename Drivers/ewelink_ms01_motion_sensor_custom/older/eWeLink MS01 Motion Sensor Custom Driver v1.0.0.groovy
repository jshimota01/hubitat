/**
 *  eWeLink Motion Sensor Driver (MS01)
...
metadata {
    definition (
        name: 'eWeLink Motion Sensor MS01',
        importUrl: '',
        namespace: 'jshimota', 
...
void parse(final String description) {
    ...
    if (descMap.clusterInt == 0x0500) {
        if (descMap.command == '00' || descMap.command == '02') {
            ...
            String zoneStatus = descMap.value ?: (descMap.data ? descMap.data[0] : null)
            if (zoneStatus) {
                int statusInt = Integer.parseInt(zoneStatus, 16)
                boolean isMotion = (statusInt & 1) != 0 // Bit 0 is Alarm 1 (Motion)
                state.rawMotion = isMotion ? 1 : 0
                logDebug "Received IAS Zone report: raw status ${zoneStatus} (Motion: ${isMotion})"
                updateMotionFromRaw()
            }
        }
    }
