...
def timeStamp() { return "2026/08/29 10:15 AM" }
...
def configure() {
    ...
    cmds += zigbee.readAttribute(0x0000, 0x0002) // Read firmware ApplicationVersion
...
List<String> updateFirmware() {
    ...
    List<String> cmds = []
    cmds += zigbee.readAttribute(0x0000, 0x0002) // Query local version from sensor
    cmds += zigbee.updateFirmware()               // Trigger Hubitat OTA check
    return cmds
}
