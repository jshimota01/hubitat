/**
 * Alexa Garage Door Protocol Test Driver (Diagnostic Tool)
...
static String version() { return '1.0.0Test' }
def timeStamp() { return "2026/08/29 02:40 PM" }

metadata {
    definition (
        name: "Alexa Garage Door Protocol Test Driver", 
        namespace: "jshimota", 
    ...
    preferences {
        ...
        input name: "prefCustomCsvSequence", type: "text", title: "<b>Custom CSV Test Sequence String</b>", description: "Format: state,delay,state,delay... (e.g. closing,0,closing,2,closed)", defaultValue: "closing,10,closed", required: false
...
// -----------------------------------------------------------------------------
void open() {
    handleIncomingCommand("open")
}

void close() {
    handleIncomingCommand("close")
}
...
private void handleIncomingCommand(String cmd) {
    ...
    logInfo "TEST: COMMAND RECEIVED -> ${cmd}() [Total Commands: ${cCount}]"

    // Cancel any active running test sequences
    unschedule("executeScheduledEventStep")
...
// Core Event Emission Engine with Detailed Transcript Logging
private void emitTestDoorEvent(String targetState, String sourceStr) {
    ...
    sendEvent(
        ...
        descriptionText: "Test Event: garage door set to ${targetState} (Source: ${sourceStr})"
    )

    sendEvent(name: "lastDoorEvent", value: targetState, isStateChange: true)
    sendEvent(name: "lastDoorEventTime", value: nowStr, isStateChange: true)
    sendEvent(name: "doorEventCount", value: eCount, isStateChange: true)

    logInfo """
TEST: DOOR EVENT EMITTED
  ------------------------------------
