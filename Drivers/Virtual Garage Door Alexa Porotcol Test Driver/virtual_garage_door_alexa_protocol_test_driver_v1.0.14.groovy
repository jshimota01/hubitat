 ...
 * Capabilities: Lock, ContactSensor, Actuator, Configuration, Refresh (GarageDoorControl disabled for Lock schema testing)
 **/
/**
 ...
 *  
 *  Changelog:
 *  v1.0.14Test   09/07/26    jshimota    Commented out GarageDoorControl capability to force Alexa endpoint discovery as SMARTLOCK / Alexa.LockController
 *  v1.0.13Test   09/07/26    jshimota    Embedded capability "Lock" (lock/unlock) alongside ContactSensor and GarageDoorControl for Alexa payload schema testing
 *  v1.0.12Test   09/07/26    jshimota    Embedded capability "ContactSensor" and simultaneous dual-event emission for Alexa payload testing
 *  v1.0.11Test   08/30/26    jshimota    Moved lastEventEmitEpoch and epoch sampling immediately before primary sendEvent call; state validation executed first
 *  v1.0.10Test   08/30/26    jshimota    Clean refactor: map-driven presets applicator, consolidated queue engine, refined comments
...
static String version() { return '1.0.14Test' }
def timeStamp() { return "2026/09/07 11:42 AM" }
...
metadata {
    definition (
        ...
        // capability "GarageDoorControl" // Disabled to force Alexa SMARTLOCK (Alexa.LockController) endpoint registration
        capability "Lock"
        ...
        // Explicit Door Commands (Preserved for internal rules / dashboard tiles)
        command "open"
        command "close"
...
// -----------------------------------------------------------------------------
// Capability Commands: open(), close(), lock(), unlock()
// -----------------------------------------------------------------------------
void open()   { executeCommandWithInstrumentation("open") }
void close()  { executeCommandWithInstrumentation("close") }
void lock()   { executeCommandWithInstrumentation("close") }
void unlock() { executeCommandWithInstrumentation("open") }
...
private void handleIncomingCommand(String cmd) {
    ...
    List queue = []
    String intermediateState = (cmd == "open" || cmd == "unlock") ? "opening" : "closing"
    String finalState = (cmd == "open" || cmd == "unlock") ? "open" : "closed"
...
private void emitTestDoorEvent(String targetState, String sourceStr) {
    ...
    // Strict Garage Capability State Guard Validation First
    ...
    // Determine Contact Sensor & Lock state equivalents
    String contactState = (cleanTarget == "closed") ? "closed" : "open"
    String lockState    = (cleanTarget == "closed") ? "locked" : "unlocked"

    // Precision Timestamp Sampling Immediately Prior to Event Emission
    ...
    // Emit Dual Contact Sensor Event (Triggers Alexa.ContactSensor payload state updates)
    sendEvent(
        name: "contact", 
        value: contactState, 
        isStateChange: flagVal, 
        descriptionText: "Test Event: contact sensor set to ${contactState} (Source: ${sourceStr})"
    )

    // Emit Lock Event (Triggers Primary Alexa.LockController payload state updates)
    sendEvent(
        name: "lock", 
        value: lockState, 
        isStateChange: flagVal, 
        descriptionText: "Test Event: lock set to ${lockState} (Source: ${sourceStr})"
    )
    ...
    logInfo """
TEST: DOOR, CONTACT & LOCK EVENT TRANSCRIPT
  ------------------------------------------------------------------------
  Target Door State              : ${cleanTarget}
  Mapped Contact State           : ${contactState}
  Mapped Lock State              : ${lockState}
  Previous Door State            : ${prevDoorState}
