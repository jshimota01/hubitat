...
static String version() { return '1.0.4Test' }
def timeStamp() { return "2026/08/29 03:15 PM" }
...
private String getFormattedTime() {
    ...
    return new Date().format("yyyy-MM-dd HH:mm:ss.SSS", tz)
}
...
// -----------------------------------------------------------------------------
// Capability Commands: open() & close() with Instrumentation
// -----------------------------------------------------------------------------
void open() {
...
private void executeCommandWithInstrumentation(String cmd) {
    ...
    String entryTimeStr = getFormattedTime()
    ...
    logInfo "TEST: COMMAND ENTRY -> ${cmd}() @ ${entryTimeStr} [Epoch: ${entryEpoch} ms]"

    sendEvent(name: "lastCommandPhase", value: "ACTION", isStateChange: true)
    logInfo "TEST: COMMAND ACTION -> ${cmd}() processing sequence queue"
    
    handleIncomingCommand(cmd)

    Long exitEpoch = new Date().getTime()
    String exitTimeStr = getFormattedTime()
    ...
    logInfo "TEST: COMMAND EXIT -> ${cmd}() @ ${exitTimeStr} [Epoch: ${exitEpoch} ms | Execution Time: ${exitEpoch - entryEpoch} ms]"
}
...
// Core Event Emission Engine with Delta Instrumentation Logging
private void emitTestDoorEvent(String targetState, String sourceStr) {
    ...
    Long deltaMs = (cmdExitEpoch != null) ? (eventEpoch - cmdExitEpoch) : -1
    ...
    String timingRelation = (deltaMs < 0) ? "EMITTED BEFORE COMMAND EXIT (Inside Method)" : "${deltaMs} ms AFTER COMMAND EXIT"

    logInfo """
TEST: DOOR EVENT EMITTED
  ------------------------------------
  Previous State : ${prevDoorState}
  New State      : ${cleanTarget}
  Source         : ${sourceStr}
  isStateChange  : ${flagVal}
  Event Count    : ${eCount}
  Timestamp      : ${nowStr}
  Exit Delta     : ${timingRelation}
  ------------------------------------"""
}
