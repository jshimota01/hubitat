...
* 	 2025-11-28  jshimota 	    2.0.14 mods for color by JAS
* 								create5H became createMsgH
* 								last5 became lastMsg
* 								last5H became lastMsgH
*								string formatted also out to state variable
*								text fix - shinking to shrinking
*								mod to line to add my values for font-size, color, align
*/
...
void deviceNotification(notification){
		...
		msgFilled = state.msgCount.toInteger()
		String existing = device.currentValue("LastMsg")?.replace('<span class="LastMsg">', '')?.replace('</span>', '')?.trim()
		if (msgFilled > 0 && existing) {
            wkTile = '<span class="LastMsg">' + notification + '<br />' + existing + '</span>'
        } else {
            wkTile = '<span class="LastMsg">' + notification + '</span>'
        }
...
String colorizeNotification(String msg) {
    ...
    String formatted = "<span style='color:${color};text-align:${existingTileHorzWordPos};font-size:${existingTileFontSize}%;display:block'>${cleanedMsg}</span>"
	escformatted = "\\ + $formatted"
    state.escformatted = escformatted
    return formatted
}
