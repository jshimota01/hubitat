...
*    2022-11-08  jshimota 2.0.14     added HTML tile layout values and features
*    2022-11-08  jshimota 2.0.15   added date/time format to list
* 	 2025-04-03	 thebearmay 2.0.16  add time/date formats, lowered mininum message count to 1
*    2025-04-20  amithalp 2.0.17 add color options
*    2026-04-20  jshimota 2.0.18 added Pre: wrapper to fix bad text input for Battery Monitor 2 tile
*/
...
metadata {
    definition (
            name: "Notification Tile Custom with PRE",
            namespace: "jshimota",
...
void deviceNotification(notification){
    ...
    //      wkTile = '<span class="last5"><pre>' + notification + '<br />' + existing + '</span>'
        } else {
            wkTile = '<span class="last5">' + notification + '</span>'
    //      wkTile = '<span class="last5"><pre>' + notification + '</span>'
        }
    ...
    else
    {
        ...
        if (i != -1)
            ...
            wkTile = wkTile.substring(0, i) + '</span>';
    //      wkTile = wkTile.substring(0, i) + '</pre></span>';
    }
    ...
    while (wkLen > 1024 && msgFilled > 0)
    {
        ...
        {
            wkTile='<span class="last5"><pre></pre></span>'
            msgFilled=0
        }
        wkLen=wkTile.length()
...
String colorizeNotification(String msg) {
    ...
    String formatted = "<span style='color:${color};'><pre>${cleanedMsg}</pre></span>"
    return formatted
}
