...
static String version()    {  return '2.0.37'  }
...
preferences {
    ...
    input(name: "existingTileFontSize", type: "decimal", title: "HTML Tile Font Size (em)*", description: "Sets font scaling in relative em units (e.g., 1.0 = standard size, 0.85 = smaller text).", defaultValue: 1.0)
...
void configure() {
    ...
    sendIfChanged([name: "tileFontSize", value: (existingTileFontSize != null ? existingTileFontSize : 1.0).toString()])
}

String getTileStyles() {
    String alignment = existingTileHorzWordPos ?: "left"
    String fontSize = (existingTileFontSize != null ? existingTileFontSize : 1.0).toString()
    String preStyle = preAdd ? "white-space:pre-wrap;font-family:ui-monospace,'Courier New',monospace;" : ""
    
    return "<style>.last5{display:block;${preStyle}text-align:${alignment};font-size:${fontSize}em;}.sb{text-shadow:0 2px 2px rgba(0,0,0,.8);}.sw{text-shadow:0 2px 2px rgba(255,255,255,.6);}</style>"
}
...
def deviceNotification(String notification) {
    ...
    // Hubitat tile length limit is 1024. Account for CSS block overhead + 27 chars for <span> wrapper
...
def colorizeNotification(String tag, String body) {
    ...
    switch(tag) {
        ...
        case "[H]":
            icon = "⚠️"
            color = settings.colorH ?: "#FF6600"
            shadowClass = useShadow ? "sb" : ""
            break
        case "[L]":
            icon = "🔋"
            color = settings.colorL ?: "#FFD700"
            shadowClass = useShadow ? "sb" : ""
            break
        case "[N]":
            icon = "ℹ️"
    ...
    String classAttr = shadowClass ? "class='${shadowClass}' " : ""
    String iconSpacer = icon ? "${icon} " : ""
    return "<span ${classAttr}style='color:${color}'>${iconSpacer}${body}</span>"
}
...
private void sendIfChanged(Map args) {
    ...
    // Normalize comparison string to ignore platform entity encoding differences
