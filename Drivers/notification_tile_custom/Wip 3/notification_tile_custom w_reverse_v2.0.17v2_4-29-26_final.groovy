	...
	// V2.0.2 When converting from original version set state variables, adjust html in last5 to make it work with V2.0.0+	
		if (state?.msgCount == null)
			...
			if (x>0)										//if there is anything in tile, adjust for v2.0.0
				...
				sendEvent(name:"last5", value:wkTile + "top of updated ln 128")
...
void deviceNotification(notification){
    	...
    	if(preAdd) {
            existing = existing?.replace('<style type="text/css"> .last5 {display:block;white-space:pre-line;text-align:' + existingTileHorzWordPos + ';font-size:' + existingTileFontSize + '%;</style>>', '')?.trim()
        } else {
            existing = existing?.replace('<style type="text/css"> .last5 {display:block;text-align:' + existingTileHorzWordPos + ';font-size:' + existingTileFontSize + '%;</style>', '')?.trim()
        }
		...
		if(!revFill) {
    		if (msgFilled > 0 && existing) {
				...
				if(preAdd) {
					wkTile = '<span class="last5"><pre>' + notification + '</pre></span><br />' + existing
                } else {
					wkTile = '<span class="last5">' + notification + '</span><br />' + existing
                }	

        	} else {

				if(preAdd) {
   					wkTile = '<span class="last5"><pre>' + notification + '</pre></span>'
				} else {
			...
			if (msgFilled > 0 && existing) {
				...
				if(preAdd) {
                    wkTile = '<span class="last5"><pre>'+ existing + '</pre></span><br />' + notification
                } else {
                    wkTile = '<span class="last5">' + existing + '</span><br />' + notification
				}	
	...
	//	Ensure tile length is less than 1024 and hopefully stop loops
    	...
    	if(!revFill) {
    		while (wkLen > 1024 && msgFilled > 0) {
                ...
                log.debug ("wkTile length ${wkLen}> 1024 truncating msgCount: ${msgFilled}")
				int i = wkTile.lastIndexOf('<br />');
				if (i != -1) {
					wkTile = wkTile.substring(0, i) + '</span>';
					msgFilled--
				...
				log.debug("Truncated wkTile length ${wkLen}, msgCount: ${msgFilled}")
            }
        } else {
            wkLen=wkTile.length()
            while (wkLen > 1024 && msgFilled > 0) {
	...
	//	Update attributes and state
		sendEvent(name:"last5", value: tileWrap + wkTile)
		state.msgCount = msgFilled
		if (settings.create5H)
