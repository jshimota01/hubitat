...
static String version()    {  return '2.0.31'  }
...
preferences {
    input("logInfoEnable", "bool", title: "Enable info logging?", defaultValue: true)
    input("logDebugEnable", "bool", title: "Enable debug logging?", defaultValue: false)
    input("logTraceEnable", "bool", title: "Enable trace logging?", defaultValue: false)
    input("logWarnEnable", "bool", title: "Enable warning logging?", defaultValue: true)
    input("logErrorEnable", "bool", title: "Enable error logging?", defaultValue: true)
    ...
    input("colorL", "text", title: "Color for [L] Low", defaultValue: "#FFD700")
...
void configure() {
    ...
    sendIfChanged([name: "tileFontColor", value: existingTileFontColor])
    sendIfChanged([name: "tileHorzWordPos", value: existingTileHorzWordPos])
    sendIfChanged([name: "tileFontSize", value: existingTileFontSize])
}
...
def deviceNotification(String notification) {
    ...
    while (wkTile.length() > 1010 && state.msgList.size() > 1) { 
        if (!revFill) state.msgList.removeAt(state.msgList.size() - 1) else state.msgList.removeAt(0)
...
def colorizeNotification(String tag, String body) {
    ...
    switch(tag) {
        case "[E]":
            ...
            shadowClass = useShadow ? "sh-white" : ""
            break
        case "[H]":
            icon = "⚠️"
            color = settings.colorH ?: "#FF6600"
            shadowClass = useShadow ? "sh-black" : ""
            break
        case "[L]":
            icon = "🔋"
    ...
    return "<span ${classAttr}style='color:${color}'> ${icon} ${body}</span>"
}
