...
*    2022-11-08  jshimota 2.0.12     added HTML tile layout values and features
*    2022-11-08  jshimota 2.0.13     added date/time format to list
* 	 2024-01-07  jshimota 2.0.14     changed last5 refs to lastMsg (and last5H to lastMsgH)
* 	 2024-01-07  jshimota 2.0.15     changed lastLimit to lastMsgLimit
*/
...
metadata {
	definition (
			...
			attribute "lastMsg", "STRING"
			attribute "lastMsgH", "STRING"
	...
	preferences {
		...
		input("createMsgH", "bool", title: "Create horizontal message tile?")
	...
	// V2.0.2 When converting from original version set state variables, adjust html in last5 to make it work with V2.0.0+	
		if (state?.msgCount == null)
			{
			state.lastMsgLimit=5
			wkTile=device.currentValue("lastMsg")
			...
			if (x>0)										//if there is anything in tile, adjust for v2.0.0
				...
				sendEvent(name:"lastMsg", value:wkTile)
        ...
        if(lastMsgLimit == null) device.updateSetting("lastMsgLimit",[value:5,type:"number"])
	// V2.0.3 When new lastMsgLimit less than prior(state) lastMsgLimit adjust message and state values	
		if (state?.lastMsgLimit.toInteger()>settings.lastMsgLimit.toInteger())
			{
			wkTile=device.currentValue("lastMsg")
			msgFilled=state.msgCount.toInteger()
			if (debugEnable) log.debug "Shinking tile count lastMsgLimit ${state.lastMsgLimit} newLimit ${settings.lastMsgLimit} msgCount ${msgFilled}"
			int i = wkTile.lastIndexOf('<br />');
			while (i != -1 && msgFilled > settings.lastMsgLimit.toInteger())
				{
				wkTile = wkTile.substring(0, i) + '</span>';
				msgFilled--
		...
		if (!settings.create5H)
			sendEvent(name:"lastMsgH", value:'<span class="lastMsg"></span>')
		state.lastMsgLimit=settings.lastMsgLimit
	}
...
void tileHorzWordPos() {
    tileHorzWordPos = "left"
    if(existingTileHorzWordPos > " ") tileHorzWordPos = existingTileHorzWordPos
    sendEvent(name: "Tile Horizontal Word Position", value: tileHorzWordPos)
}
...
//		sendEvent(name:"lastMsg", value:'<span class="lastMsg"></span>')
		sendEvent(name:"lastMsg", value:'<style type="text/css">' +
				'.lastMsg {\n' +
				'     display: block;\n' +
...
void deviceNotification(notification){
	...
	//	insert new message at beginning	of lastMsg string
		msgFilled = state.msgCount.toInteger()
		if (msgFilled>0)
			wkTile=device.currentValue("lastMsg").replace('<span class="lastMsg">','<span class="lastMsg">' + notification + '<br />')
		else
			wkTile=device.currentValue("lastMsg").replace('<span class="lastMsg">','<span class="lastMsg">' + notification)
	...
	//	when msg count exceeds limit, purge last message
		...
		if (msgFilled < settings.lastMsgLimit.toInteger())
			msgFilled++
		else
			{
			int i = wkTile.lastIndexOf('<br />');
	...
	//	Ensure tile length is less than 1024 and hopefully stop loops
		...
		while (wkLen > 1024 && msgFilled > 0)
			...
			if (i != -1) 
				...
				{
				wkTile='<span class="lastMsg"></span>'
				msgFilled=0
				}
	...
	//	Update attributes and state
		...
		state.msgCount = msgFilled
		if (settings.create5H)
			sendEvent(name:"lastMsgH", value: " ** "+wkTile.replaceAll("<br />"," ** ")+" ** ")
	}    
