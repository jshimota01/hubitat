/**
 * Third Reality Temp & Humidity Sensor w/Display (Custom)
 * Third Reality Device Model 3RTHS0224Z with Display 
 * Device Driver for Hubitat Elevation
 *
 * Purpose:
 * Purpose-built custom driver for the Third Reality Temp & Humidity Sensor Lite (3RTHS0224Z).
...
static String version() { return '1.00' }
def timeStamp() { return "2026/09/05 09:30 AM" }
...
metadata {
    definition(
        ...
        importUrl: "https://raw.githubusercontent.com/jshimota01/hubitat/main/Drivers/third_reality_th_lite/third_reality_th_lite.groovy"
    ) {
        capability "Battery"
    ...
    preferences {
        input name: "tempOffset", type: "decimal", title: "<b>Temperature Offset</b>", description: "<i>Adjust temperature reading (-10.0 to +10.0 °F/°C).</i>", range: "-10.0..10.0", defaultValue: 0.0
        input name: "humidityOffset", type: "decimal", title: "<b>Humidity Offset</b>", description: "<i>Adjust relative humidity reading (-20 to +20 %).</i>", range: "-20..20", defaultValue: 0
        
        input name: "tempDelta", type: "decimal", title: "<b>Temperature Minimum Change Delta</b>", description: "<i>Minimum temperature change required to emit an event (0.1 to 5.0 °F/°C).</i>", range: "0.1..5.0", defaultValue: 0.2
        input name: "humidityDelta", type: "decimal", title: "<b>Humidity Minimum Change Delta</b>", description: "<i>Minimum humidity % change required to emit an event (1 to 10 %).</i>", range: "1..10", defaultValue: 1.0
...
private BigDecimal getSettingBigDecimal(String key, BigDecimal defaultVal = 0.0) {
...
List<String> configure() {
    ...
    // Battery voltage (0x0001, attr 0x0020): Min 300s, Max 21600s, Change 1 (0.1V)
    cmds += zigbee.configureReporting(0x0001, 0x0020, DataType.UINT8, 300, 21600, 1, [:], DELAY_MS)

    // Battery percentage (0x0001, attr 0x0021): Min 300s, Max 21600s, Change 2 (1%)
...
List<String> refresh() {
    ...
    cmds += zigbee.readAttribute(0x0001, 0x0020, [:], DELAY_MS) // Battery Voltage
...
void parsePowerCluster(final Map descMap) {
    ...
    switch (descMap.attrInt as Integer) {
        case 0x0020: // Battery Voltage (0.1V scale)
            BigDecimal volts = new BigDecimal(rawValue).divide(10G, 1, RoundingMode.HALF_UP)
            updateAttribute("batteryVoltage", volts, "V", "physical", "debug")
            break
...
void parseHumidityCluster(final Map descMap) {
    ...
    BigDecimal rawPct = new BigDecimal(rawHumidity).divide(100G, 1, RoundingMode.HALF_UP)

    // Apply preference offset
    BigDecimal offset = getSettingBigDecimal("humidityOffset", 0.0G)
    BigDecimal finalHumidity = Math.max(0G, Math.min(100G, rawPct + offset)).setScale(0, RoundingMode.HALF_UP)

    BigDecimal currentVal = device.currentValue("humidity") as BigDecimal
    BigDecimal minDelta = getSettingBigDecimal("humidityDelta", 1.0G)
