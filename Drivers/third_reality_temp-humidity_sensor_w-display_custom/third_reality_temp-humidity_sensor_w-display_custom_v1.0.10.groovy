 ...
 * Features read/write support for private cluster 0xFF01 manufacturer calibration attributes,
 * display unit state probing, driver-side delta filtering, software offset calibration, and phase-anchored health monitoring.
 *
 * Notes:
...
def timeStamp() { return "2026/09/05 12:05 PM" }
...
metadata {
    definition(
        ...
        command "writeCelsiusOffset", [[name: "Decimal Offset (°C)", type: "NUMBER", description: "e.g., 0.5 for +0.50°C (sends 50) or 1.0 for +1.00°C (sends 100)"]]
        command "writeFahrenheitOffset", [[name: "Decimal Offset (°F)", type: "NUMBER", description: "e.g., 0.5 for +0.50°F (sends 50) or 1.0 for +1.00°F (sends 100)"]]
        command "writeHumidityOffset", [[name: "Raw Value (INT16)", type: "NUMBER", description: "e.g., 10 for humidity scaling experiment"]]
...
List<String> queryPrivateCluster() {
    logInfo "Querying Private Cluster 0xFF01 calibration & display unit attributes..."
    List<String> cmds = []
    cmds += zigbee.readAttribute(0xFF01, 0x0010, [mfgCode: 0x1233], DELAY_MS) // Potential Display Unit Enum
    cmds += zigbee.readAttribute(0xFF01, 0x0011, [mfgCode: 0x1233], DELAY_MS) // Potential Display Unit Enum Alt
    ...
    // Force immediate protocol dispatch
    sendHubCommand(new hubitat.device.HubMultiAction(cmds, hubitat.device.Protocol.ZIGBEE))
    return cmds
}

List<String> writeCelsiusOffset(Object rawVal) {
    BigDecimal decVal = rawVal ? new BigDecimal(rawVal.toString()) : 0G
    int rawInt = (decVal * 100G).setScale(0, RoundingMode.HALF_UP).intValue()
    logInfo "Writing ${decVal}°C (raw INT16: ${rawInt}) to Private Cluster 0xFF01 Celsius Offset (attr 0x0031)..."
    
    List<String> cmds = []
    cmds += zigbee.writeAttribute(0xFF01, 0x0031, DataType.INT16, rawInt, [mfgCode: 0x1233], DELAY_MS)
...
List<String> writeFahrenheitOffset(Object rawVal) {
    BigDecimal decVal = rawVal ? new BigDecimal(rawVal.toString()) : 0G
    int rawInt = (decVal * 100G).setScale(0, RoundingMode.HALF_UP).intValue()
    logInfo "Writing ${decVal}°F (raw INT16: ${rawInt}) to Private Cluster 0xFF01 Fahrenheit Offset (attr 0x0033)..."
    
    List<String> cmds = []
    cmds += zigbee.writeAttribute(0xFF01, 0x0033, DataType.INT16, rawInt, [mfgCode: 0x1233], DELAY_MS)
...
List<String> writeHumidityOffset(Object rawVal) {
    int val = rawVal ? rawVal.toString().toInteger() : 0
    logInfo "EXPERIMENTAL: Writing ${val} to Private Cluster 0xFF01 Humidity Offset (attr 0x0032)..."
    
    List<String> cmds = []
    cmds += zigbee.writeAttribute(0xFF01, 0x0032, DataType.INT16, val, [mfgCode: 0x1233], DELAY_MS)
...
void parsePrivateCluster(final Map descMap) {
    ...
    switch (descMap.attrInt as Integer) {
        case 0x0010:
        case 0x0011: // Display Unit State Enum
            String unitStr = (rawVal == 1) ? "Fahrenheit (°F)" : "Celsius (°C)"
            state.deviceDisplayUnit = unitStr
            logInfo "Private Cluster 0xFF01 Display Unit Mode: ${unitStr} (attr: ${String.format('0x%04X', descMap.attrInt)}, raw: ${rawVal})"
            break
        case 0x0031: // Celsius Calibration Offset (Hundredths scale: 100 = 1.0°C)
        ...
        case 0x0032: // Humidity Calibration Offset
            ...
            logDebug "Private Cluster 0xFF01 Humidity Calibration Offset: ${rawVal} (raw: ${rawVal})"
            break
        case 0x0033: // Fahrenheit Calibration Offset (Hundredths scale: 100 = 1.0°F)
