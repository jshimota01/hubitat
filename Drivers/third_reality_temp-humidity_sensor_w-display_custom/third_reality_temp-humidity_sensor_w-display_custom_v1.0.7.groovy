 ...
 * Features verified hardware LCD screen calibration via private cluster 0xFF01, software delta filtering,
 * software offset calibration, and phase-anchored health monitoring.
 *
 * Notes:
...
metadata {
    ...
    preferences {
        ...
        input name: "tempOffset", type: "decimal", title: "<b>Driver Temperature Offset</b>", description: "<i>Adjust temperature reading in Hubitat software (-10.0 to +10.0 °${tempUnit}).</i>", range: "-10..10", defaultValue: 0.0
        ...
        input name: "humidityOffset", type: "number", title: "<b>Driver Humidity Offset</b>", description: "<i>Adjust relative humidity reading in Hubitat software (-20 to +20 %). Integers only.</i>", range: "-20..20", defaultValue: 0
        ...
        // Hardware LCD Screen Offset Settings (Cluster 0xFF01)
        input name: "lcdTempOffset", type: "decimal", title: "<b>Hardware LCD Temperature Offset</b>", description: "<i>Adjust offset stored directly on physical device LCD display (-10.0 to +10.0 °${tempUnit}). Wake device after changing.</i>", range: "-10..10", defaultValue: 0.0
        input name: "lcdHumidityOffset", type: "number", title: "<b>Hardware LCD Humidity Offset</b>", description: "<i>Adjust relative humidity stored directly on physical device LCD display (-20 to +20 %). Wake device after changing.</i>", range: "-20..20", defaultValue: 0
...
void updated() {
    ...
    // Sync Hardware LCD Offset Preferences if changed
...
private void syncHardwareLcdOffsets() {
    ...
    // Check if Hardware Temp Offset changed
    if (state.lastLcdTempOffset != targetLcdTempOffset) {
        int rawTempInt = (targetLcdTempOffset * 10G).setScale(0, RoundingMode.HALF_UP).intValue()
        int attrId = isFahrenheit ? 0x0033 : 0x0031
        logInfo "Updating Hardware LCD Temperature Offset to ${targetLcdTempOffset}°${isFahrenheit ? 'F' : 'C'} (raw: ${rawTempInt})..."
        ...
        state.lastLcdTempOffset = targetLcdTempOffset
    }

    // Check if Hardware Humidity Offset changed
    if (state.lastLcdHumidityOffset != targetLcdHumidityOffset) {
        logInfo "Updating Hardware LCD Humidity Offset to ${targetLcdHumidityOffset}%..."
        cmds += zigbee.writeAttribute(0xFF01, 0x0032, DataType.INT16, targetLcdHumidityOffset, [mfgCode: 0x1233], DELAY_MS)
        cmds += zigbee.readAttribute(0xFF01, 0x0032, [mfgCode: 0x1233], DELAY_MS)
        state.lastLcdHumidityOffset = targetLcdHumidityOffset
    }
...
List<String> refresh() {
    ...
    // Read 0xFF01 attributes for state tracking
...
void parsePrivateCluster(final Map descMap) {
    ...
    switch (descMap.attrInt as Integer) {
        case 0x0031: // Celsius Calibration Offset
            state.deviceCalCelsius = (rawVal / 10.0G).setScale(1, RoundingMode.HALF_UP)
        ...
        case 0x0032: // Humidity Calibration Offset
            ...
            logDebug "Private Cluster 0xFF01 Humidity Calibration Offset: ${rawVal}%"
            break
        case 0x0033: // Fahrenheit Calibration Offset
            state.deviceCalFahrenheit = (rawVal / 10.0G).setScale(1, RoundingMode.HALF_UP)
