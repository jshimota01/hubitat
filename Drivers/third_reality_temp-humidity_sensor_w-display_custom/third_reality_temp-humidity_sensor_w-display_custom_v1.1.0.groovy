 ...
 * driver-side software delta filtering, software offset calibration, and phase-anchored health monitoring.
 *
 * Notes:
...
def timeStamp() { return "2026/09/05 12:25 PM" }
...
metadata {
    ...
    preferences {
        ...
        input name: "lcdHumidityOffset", type: "number", title: "<b>Hardware LCD Humidity Offset</b>", description: "<i>Adjust offset stored directly on physical LCD screen (-20 to +20 %). Hardware uses hundredths scale (100 = 1.0% RH). Note that physical LCD screen rounds to nearest whole integer percentage. Updates within ~10 seconds or immediately upon pressing physical wake button.</i>", range: "-20..20", defaultValue: 0
...
void updated() {
    ...
    // Clear last evaluated state entries if software offset changed so next report passes delta filter naturally
    ...
    // Sync Hardware LCD Screen Offsets over Cluster 0xFF01
...
private void syncHardwareLcdOffsets() {
    ...
    // Sync Hardware LCD Temperature Offset (Hundredths scale: 100 = 1.0 deg)
    ...
    // Sync Hardware LCD Humidity Offset (Hundredths scale: 100 = 1.0 % RH)
    if (state.lastLcdHumidityOffset != targetLcdHumidityOffset) {
        int rawHumidityInt = (targetLcdHumidityOffset * 100).intValue()
...
List<String> refresh() {
    ...
    // Read Private Cluster 0xFF01 calibration attributes for state verification
...
void parse(final String description) {
    ...
    switch (descMap.clusterInt as Integer) {
        ...
        case 0x0405: // Relative Humidity Measurement
            ...
            break
        case 0xFF01: // Manufacturer Private Cluster
...
void parsePrivateCluster(final Map descMap) {
    ...
    switch (descMap.attrInt as Integer) {
        ...
        case 0x0031: // Celsius Calibration Offset (Hundredths scale: 100 = 1.0°C)
            ...
            break
        case 0x0032: // Humidity Calibration Offset (Hundredths scale: 100 = 1.0% RH)
...
void parseGeneralCommandResponse(final Map descMap) {
    ...
    // Check if this is a Read Attributes Response (command 01) on Private Cluster 0xFF01 (65281)
    if (descMap.clusterInt == 65281 && descMap.command == "01" && descMap.attrInt != null) {
        parsePrivateCluster(descMap)
        return
    }
