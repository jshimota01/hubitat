...
def timeStamp() { return "2026/09/05 12:30 PM" }
...
private void syncHardwareLcdOffsets() {
    ...
    // Note: State variable is updated ONLY when confirmed back via read attribute response
...
void parseGeneralCommandResponse(final Map descMap) {
    ...
    // Check if this is a Read Attributes Response (command 01) on Private Cluster 0xFF01
    if (descMap.clusterInt == 0xFF01 && descMap.command == "01" && descMap.attrInt != null) {
        parsePrivateCluster(descMap)
        return
    }
