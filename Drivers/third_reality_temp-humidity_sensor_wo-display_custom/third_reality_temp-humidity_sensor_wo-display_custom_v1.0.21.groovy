 ...
 * tile visualization attributes and passive activity watchdog monitoring.
 **/
/**
...
/**
 * Changelog:
 * v1.1.21   09/23/26   jshimota    Replaced active polling health check with passive activity watchdog in checkIn() and added lastActivity attribute.
...
List<String> refresh() {
    ...
    cmds += zigbee.readAttribute(0x0402, 0x0000, [:], DELAY_MS)
...
void parseTemperatureCluster(final Map descMap) {
    ...
    if (isDelta(finalTemp, currentVal, minDelta)) {
        ...
        updateAttribute("temperature", finalTemp, unit, "physical", "trace")
        updateAttribute("temperatureText", "${finalTemp} ${unit}", null, "physical", "info", "temperature")
    }
}
...
void parseHumidityCluster(final Map descMap) {
    ...
    if (isDelta(finalHumidity, currentVal, minDelta)) {
        updateAttribute("humidity", finalHumidity, "%", "physical", "trace")
        updateAttribute("humidityText", "${finalHumidity}%", null, "physical", "info", "humidity")
    }
}
