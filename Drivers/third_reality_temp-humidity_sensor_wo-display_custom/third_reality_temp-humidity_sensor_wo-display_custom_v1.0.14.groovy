 ...
 * - OTA Firmware Update support (updateFirmware) is included via standard Zigbee cluster commands 
 *   but remains UNVERIFIED on Hubitat for this specific device model.
 **/
/**
...
static String version() { return '1.14' }
def timeStamp() { return "2026/09/05 10:30 AM" }
...
metadata {
    ...
    preferences {
        ...
        // Humidity Settings (Integer)
        input name: "humidityOffset", type: "number", title: "<b>Humidity Offset</b>", description: "<i>Adjust relative humidity reading (-20 to +20 %).</i>", range: "-20..20", defaultValue: 0
        input name: "humidityDelta", type: "number", title: "<b>Humidity Minimum Change Delta</b>", description: "<i>Minimum humidity % change required to emit an event (1 to 10 %).</i>", range: "1..10", defaultValue: 1
...
// NPE-Safe Integer Preference Conversion Helper
private Integer getSettingInteger(String key, Integer defaultVal = 0) {
    Object val = settings[key]
    if (val == null) return defaultVal
    try {
        return val.toString().toInteger()
    } catch (Exception e) {
...
void updated() {
    ...
    // Flag next reading to bypass delta check if offset was modified
...
List<String> configure() {
    ...
    // Battery percentage (0x0001, attr 0x0021 ONLY): Min 300s, Max 21600s, Change 2 (1%)
...
// UNVERIFIED: Experimental Zigbee OTA Firmware update command execution for Hubitat Elevation
List<String> updateFirmware() {
    logInfo "Checking for firmware updates (Experimental)..."
...
void parseTemperatureCluster(final Map descMap) {
    ...
    boolean forceEvent = (state.forceNextTempEvent == true)

    if (forceEvent || isDelta(finalTemp, currentVal, minDelta)) {
        state.forceNextTempEvent = false
...
void parseHumidityCluster(final Map descMap) {
    ...
    int rawPct = Math.round(rawHumidity / 100.0f)

    // Apply integer preference offset
    Integer offset = getSettingInteger("humidityOffset", 0)
    int calculated = rawPct + offset
    
    if (calculated < 0) calculated = 0
    if (calculated > 100) calculated = 100

    BigDecimal finalHumidity = new BigDecimal(calculated)
    ...
    boolean forceEvent = (state.forceNextHumidityEvent == true)

    if (forceEvent || isDelta(finalHumidity, currentVal, minDelta)) {
        state.forceNextHumidityEvent = false
