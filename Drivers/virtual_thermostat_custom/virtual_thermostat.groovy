...
metadata {
	definition (name: "Advanced vThermostat Device", 
		...
		command "coolUp"
		command "coolDown"
		command "setMaxUpdateIntervalMinutes", ["number"]
...
def installed() {
	...
	sendEvent(name: "maxUpdateInterval", value: 180)
	sendEvent(name: "lastTempUpdate", value: new Date() )
	sendEvent(name: "supportedThermostatModes", value: ["heat", "cool", "auto", "off", "emergency_stop"])
...
//************************************************************
def configure() {
	sendEvent(name: "supportedThermostatModes", value: '["auto","cool","heat","off", "emergency_stop"]')
...
*/	if (maxInterval > 180) maxinterval = 180
	...
	// JAS Get maxInterval, if higher than the safety preset, set to preset of 1200 minutes (20 hours)
	def maxInterval = device.currentValue("maxUpdateInterval") ?: 1200
	// If fetched maxInternal from user is higher than 1200, set to 1200
	if (maxInterval > 1200) maxinterval = 1200
	if (maxInterval > 1200) sendEvent(name: "maxUpdateInterval", value: 1200)
	...
	if (current == "idle" && now - lastUpdate >= maxIntervalMili) {
        	logger("info", "now-lastUpdate = $now - $lastUpdate")
        	logger("info", "maxInterval = $maxInterval")
        logger("info", "maxIntervalMili = $maxIntervalMili")        
		...
		//return
	} else if (! ((mode in ["emergency_stop", "off"]) || current == "idle") && now - lastUpdate >= maxIntervalMili) {
            logger("info", "now-lastUpdate = $now - $lastUpdate")
        	logger("info", "maxIntervalMili = $maxIntervalMili")
		logger("info", "Temp sensor(s) maximum update time interval exceeded ($maxInterval minutes). Setting emergency_stop mode until temp sensor(s) starts reporting again!")
        logger("error", "Temp sensor(s) maximum update time interval exceeded ($maxInterval minutes). Setting emergency_stop mode until temp sensor(s) starts reporting again!")
      	logger("info", "Converting string $value to type double")
		sendEvent(name: "preEmergencyMode", value: mode)
		sendEvent(name: "thermostatMode", value: "emergency_stop")
		runIn(2, 'evaluateMode') // Re-evaluate to set the state to idle ASAP
		return
	} else if (mode == "emergency_stop" && now - lastUpdate < maxIntervalMili && device.currentValue("preEmergencyMode")) {
		logger("warn", "Temp sensor started reporting again, Autorecovered from emergency_stop. Setting to previous mode.")
...
//************************************************************
// setMaxUpdateIntervalMinutes
//     Max interval between two reports of the temp sensors
//     This is for SAFETY reasons
// Signature(s)
//     setMaxUpdateIntervalMinutes(BigDecimal minutes)
// Parameters
//     minutes :
// Returns
//     None
//************************************************************
def setMaxUpdateIntervalMinutes(BigDecimal minutes) {
	if (value != device.currentValue("maxUpdateInterval")) {
		logger("trace", "setMaxUpdateIntervalMinutes($minutes) - sendEvent")
		sendEvent(name: "maxUpdateInterval", value: minutes)
		runIn(2,'evaluateMode')
	} else {
		logger("trace", "setMaxUpdateIntervalMinutes($minutes) - already set")
	}
}
...
//************************************************************
def emergencyStop() {
	if (device.currentValue("thermostatMode") != "emergency_stop") {
...
def setTemperature(value) {
	...
	logger("info", "setTemperature($value) - sendEvent")
