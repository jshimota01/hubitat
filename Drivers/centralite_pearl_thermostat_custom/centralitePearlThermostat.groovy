 ...
 * 2026-07-10    jshimota      0.4.4	   Repair critical error on amazon gateway failure
 * 2026-07-10    jshimota      0.4.5	   Driver hardening
 */
...
def raiseHeatingSetpointLevel() { changeSetpoint("heatingSetpoint", 1) }
def lowerHeatingSetpointLevel() { changeSetpoint("heatingSetpoint", -1) }
def raiseCoolingSetpointLevel() { changeSetpoint("coolingSetpoint", 1) }
def lowerCoolingSetpointLevel() { changeSetpoint("coolingSetpoint", -1) }
...
def parse(String description) {
	...
	if (map) {
        if (map.name in ["coolingSetpoint", "heatingSetpoint"]) {
            ...
            map.descriptionText = "${device.displayName} ${map.name} received from device is ${map.value} degrees ${fullScaleName}"
        } else if (map.unit == "F" || map.unit == "C") {
            map.descriptionText = "${device.displayName} ${map.name} received from device is ${map.value}°${map.unit}"
        } else {
            map.descriptionText = "${device.displayName} ${map.name} received from device is ${map.value}${map.unit ?: ''}"
        }
       
        // --- Only log if the state is genuinely changing or different from current value ---
        boolean isNewValue = device.currentValue(map.name) != map.value
        if (txtEnable && isNewValue) {
            log.info map.descriptionText
        } else if (dbgEnable && !isNewValue) {
            log.debug "Filtered duplicate reporting log entry for ${map.name}: ${map.value}"
        }
...
def off() {
    if (txtEnable) log.info "${device.displayName} mode sent to Off."
    sendEvent(name: "thermostatMode", value: "off")
    zigbee.writeAttribute(0x0201, 0x1C, 0x30, 0)
}
 
def cool() {
    if (txtEnable) log.info "${device.displayName} mode sent to Cool."
    sendEvent(name: "thermostatMode", value: "cool")
    zigbee.writeAttribute(0x0201, 0x1C, 0x30, 3)
}
 
def heat() {
    if (txtEnable) log.info "${device.displayName} mode sent to Heat."
    sendEvent(name: "thermostatMode", value: "heat")
    zigbee.writeAttribute(0x0201, 0x1C, 0x30, 4)
}
 
def emergencyHeat() {
    if (txtEnable) log.info "${device.displayName} mode sent to Emergency Heat."
...
def fanOn() {
    if (txtEnable) log.info "${device.displayName} fan sent to On."
...
def fanAuto() {
    if (txtEnable) log.info "${device.displayName} fan sent to Auto."
    unschedule(runCirculateCycle) 
...
def fanOff() {
    if (txtEnable) log.info "${device.displayName} fan shut sent to Off."
    unschedule(runCirculateCycle) 
...
def fanCirculate() {
    if (txtEnable) log.info "${device.displayName} fan sent to Circulate (30m Loop)."
    sendEvent(name: "thermostatFanMode", value: "circulate")
    runCirculateCycle([targetState: "on"])
}

def runCirculateCycle(Map data = [:]) {
    ...
    if (targetState == "on") {
        if (dbgEnable) log.debug "Circulation Loop: Turning Fan ON"
        ...
        if (dbgEnable) log.debug "Circulation Loop: Setting Fan to AUTO"
...
private def processSetpoint(degrees, int attributeId) { 
    ...
    if (txtEnable) log.info "${device.displayName} ${attrName} sent to ${degreesInteger}."
