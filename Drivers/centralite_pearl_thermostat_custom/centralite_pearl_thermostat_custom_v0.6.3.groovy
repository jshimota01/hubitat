 ...
 * v0.5.9    08/31/26    jshimota    Added GUI guidance note under preferences documenting Auto fan mode requirement when managed by MEM application.
 * v0.5.8    08/31/26    jshimota    NPE bug audit: Hardened scheduleHealthCheck cast guards, processSetpoint string-to-number casting, setThermostatHoldMode null checks, and settings map references.
 * v0.5.7    08/31/26    jshimota    Added defensive parse exception guard, renamed clearAllDeviceAttributes(), and documented fan circulate & whole-degree setpoint design contracts.
 * v0.5.6    08/31/26    jshimota    Fixed integer division bug in getBatteryLevel(), hardened setpoint updates on mode transitions, updated health timeout wording, and documented protocol mode vs supported capability modes.
...
def timeStamp() { return "2026/08/31 05:10 PM" }
...
metadata {
    definition(
        ...
        command "setThermostatFanMode", [[name: "fanMode*", type: "ENUM", description: "Thermostat Fan Mode (Select 'ext' when fan modes are managed externally by MEM application).", constraints: ["on", "auto", "off", "circulate", "ext"]]]
    ...
    preferences {
        ...
        // Fan Mode Preference & Application Guidance
        input name: "enableFanCommands", type: "bool", title: "<b>Enable Thermostat Fan Mode Commands</b>", description: "<i>Turn Off to disable driver Thermostat Fan Modes - fan commands will only be available through external sources.</i>", defaultValue: true, required: true
        input name: "fanModeNote", type: "paragraph", title: "<b>Fan Mode Operational Note</b>", description: "<i>Check preferences when using an external app or controller. Select <b>Ext</b> in the Thermostat Fan Mode dropdown when fan modes are managed by Multiperiod Environment Manager (MEM).</i>"
...
void initialize(Boolean isInstall = false) {
    ...
    // Include 'ext' as a supported fan mode token
    def fanOptionsList = ["on", "auto", "off", "circulate", "ext"] 
...
void setThermostatFanMode(String value) { 
    ...
    if (normValue == "ext") {
        logInfo "Thermostat Fan Mode set to 'ext' (GUI fan control handed off to external application)."
        unschedule("runCirculateCycle")
        updateAttribute("thermostatFanMode", "ext")
        return
    }

    if (!getSettingBool("enableFanCommands", true) || device.currentValue("thermostatFanMode") == "ext") {
        logInfo "Thermostat Fan Mode command '${value}' ignored (External mode active or fan commands disabled in preferences)."
        return
    }
...
List<String> fanOn() {
    if (!getSettingBool("enableFanCommands", true)) {
        logInfo "fanOn command ignored (Fan Mode disabled in preferences)."
        return []
    }
...
List<String> fanAuto() {
    if (!getSettingBool("enableFanCommands", true)) {
        logInfo "fanAuto command ignored (Fan Mode disabled in preferences)."
        return []
    }
...
List<String> fanOff() {
    if (!getSettingBool("enableFanCommands", true)) {
        logInfo "fanOff command ignored (Fan Mode disabled in preferences)."
        return []
    }
...
void fanCirculate() {
    if (!getSettingBool("enableFanCommands", true)) {
        logInfo "fanCirculate command ignored (Fan Mode disabled in preferences)."
        return
    }
...
void runCirculateCycle(Map data = [:]) {
    if (!getSettingBool("enableFanCommands", true) || device.currentValue("thermostatFanMode") != "circulate") return
        ...
        if (description.startsWith("read attr -") || description.startsWith("catchall:")) {
            ...
            switch(clusterInt) {
                ...
                case 0x0202: // Fan Control Cluster
                    if (attrInt == 0x0000) {
                        // Passive inbound physical updates retain current mode if set to 'ext'
                        if (device.currentValue("thermostatFanMode") != "circulate" && device.currentValue("thermostatFanMode") != "ext") {
