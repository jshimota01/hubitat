 ...
 * - Uses phase-anchored randomized daily scheduling and timeout guards to monitor ping/pong responses
...
def timeStamp() { return "2026/08/31 01:20 PM" }
...
metadata {
    ...
    preferences {
        input name: "HealthCheckInterval", type: "enum", title: "<b>Health Check (Ping/Pong) Interval</b>", options: HealthCheckIntervalOpts.options, defaultValue: HealthCheckIntervalOpts.defaultValue, description: "<i>Changes how often the driver sends a Health Check Ping to verify device online status.</i>"
...
private void changeSetpoint(String attributeName, int delta) {
    if (isHoldOn()) {
        logWarn "Thermostat is in Hold Mode. Setpoint change ignored."
        return
    }
...
void parse(String description) {
    ...
    if (description.startsWith("read attr -") || description.startsWith("catchall:")) {
        ...
        // Passive network health assertion: Any valid parsed packet proves device is online
        ...
        switch(clusterInt) {
            ...
            case 0x0000: // Basic Cluster (Pong verification)
                if (attrInt == 0x0007) {
...
private BigDecimal getTemperature(String value) {
    ...
    return Math.round(tempVal) as BigDecimal
}
