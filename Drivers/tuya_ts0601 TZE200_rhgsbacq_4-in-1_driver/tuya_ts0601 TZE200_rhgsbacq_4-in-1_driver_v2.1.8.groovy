...
static String timeStamp() { "2026/08/09 6:00 PM" }
...
void handleTemperatureCelsius(Float tempC) {
    BigDecimal tempF = (((tempC * 1.8f) + 32.0f) as BigDecimal).setScale(1, BigDecimal.ROUND_HALF_UP)
    sendIfChanged(name: 'temperature', value: tempF, unit: '°F')
    scheduleFormatAttrib()
}

void handleHumidityEvent(Float hum) {
    sendIfChanged(name: 'humidity', value: hum, unit: '%RH')
    scheduleFormatAttrib()
}
