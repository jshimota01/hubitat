 ...
 *  ------------------------------------------------------------------------------------------------------
 *  Changelog:
 *  v1.3.2 (2026-08-28) - Added explicit debug logging inside battery parsing functions.
...
private String version() { return "1.3.2" }
...
def parse(String description) {
    ...
    if (description?.startsWith("zone status")) {
        ...
        if (logEnable) log.debug "${device.displayName}: Parsed description map -> ${descMap}"
        
        if (descMap.cluster == "0001" && descMap.attrId == "0021") {
            if (logEnable) log.debug "${device.displayName}: Processing battery percentage attribute (0x0021)"
            map = parseBattery(descMap.value)
        }
    } else if (description?.startsWith("enroll request")) {
        if (logEnable) log.debug "${device.displayName}: Handling IAS zone enrollment request"
    ...
    if (map) {
        ...
        if (logEnable) log.debug "${device.displayName}: Emitting event -> ${map}"
        sendEvent(map)
    }
...
private Map parseBattery(String hexValue) {
    if (logEnable) log.debug "${device.displayName}: parseBattery() raw hex -> ${hexValue}"
    Integer rawValue = Integer.parseInt(hexValue, 16)
    Integer pct = Math.round(rawValue / 2)
    pct = Math.min(100, Math.max(0, pct))
    if (logEnable) log.debug "${device.displayName}: Calculated battery percentage -> ${pct}% (raw: ${rawValue})"
...
def setupSchedule() {
    ...
    if (logEnable) log.debug "${device.displayName}: Setting check-in schedule interval to ${interval} hour(s)"
    switch(interval) {
        case "1":
...
def configure() {
    ...
    if (logEnable) log.debug "${device.displayName}: configure() sending Zigbee payload -> ${cmds}"
    return cmds
}
