...
/**
 * Changelog:
 * v1.6.6    09/14/26    jshimota    Enforced lastActivity refresh in parse() on all incoming Zigbee messages (isStateChange: false).
 * v1.6.5    09/12/26    jshimota    Added contactInverted boolean attribute to explicitly report state inversion status for Rule Machine and Dashboard tiles.
 * v1.6.4    09/12/26    jshimota    Merged invertContact preference option from reversible test fork into core production driver and fixed resetDriver to invoke clearAllAttributes.
 * v1.6.3    08/31/26    jshimota    Updated parseBattery to quietly update lastBatteryReport via sendEvent, eliminating redundant info log spam.
...
def timeStamp() { return "2026/09/14 09:50 AM" }
...
metadata {
    definition (
        ...
        // Attributes
        attribute "contactInverted", "enum", ["true", "false"]
    ...
    preferences {
        ...
        input name: "logInfoEnable", type: "bool", title: "Logging - Enable Info Logging", description: "Enable to output normal activity to log<br>Default: <b>On</b>", defaultValue: true, required: true, width: 6
        input name: "logErrorEnable", type: "bool", title: "Logging - Enable Error Logging", description: "Enable to output error activity to log<br>Default: <b>On</b>", defaultValue: true, required: true, width: 6
        input name: "logWarnEnable", type: "bool", title: "Logging - Enable Warning Logging", description: "Enable to output warning activity to log<br>Default: <b>On</b>", defaultValue: true, required: true, width: 6
        input name: "logDebugEnable", type: "bool", title: "Logging - Enable Debug Logging", description: "Enable to output debugging activity to log<br>Default: <b>Off</b><br>(Is turned on for 30 minutes after Initialized or first installed)", defaultValue: false, required: true, width: 6
        input name: "logTraceEnable", type: "bool", title: "Logging - Enable Trace Logging", description: "Enable to output tracing activity to log<br>Default: <b>Off</b>", defaultValue: false, required: true, width: 6
    }
}
...
private void initialize(Boolean isInstall = false) {
    ...
    // Synchronize healthStatus and contactInverted attributes
    if (device.currentValue("healthStatus") == null) {
        sendEvent(name: "healthStatus", value: "unknown")
    }

    Boolean isInverted = getSettingBool("invertContact", false)
    updateAttribute("contactInverted", isInverted ? "true" : "false")
...
void parse(String description) {
    ...
    // Any incoming valid packet confirms network presence and drives Hubitat lastActivity update
...
private void parseIasZoneStatus(String description) {
    ...
    updateAttribute("contactInverted", invert ? "true" : "false")
...
void clearAllAttributes() {
    ...
    device.supportedAttributes.each { attr ->
        device.deleteCurrentState(attr.name)
    }
