...
def parse(String description) {
    ...
    if (description?.startsWith("zone status")) {
        ...
        String descText = "${device.displayName} is ${val}"
        
        if (txtEnable) log.info descText
        sendEvent(name: "contact", value: val, descriptionText: descText)
    } 
    // Parse Power Configuration Cluster (0x0001) - Battery Percentage
        ...
        if (descMap.cluster == "0001" && descMap.attrId == "0021") {
            ...
            String descText = "${device.displayName} battery check-in: ${batteryVal}%"
            if (txtEnable) log.info descText
