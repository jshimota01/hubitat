...
def timeStamp() { return "2026/09/04 03:50 PM" }
...
def queryClusterB00D() {
    logInfo "Executing Protocol Archaeology: Probing Cluster 0xB00D attributes 0x0000..0x0003..."
    List<String> cmds = []
    
    // Probing Cluster 0xB00D with Manufacturer Code 0x1124 (Jasco Products)
    cmds += zigbee.readAttribute(0xB00D, 0x0000, [mfgCode: 0x1124])
    cmds += zigbee.readAttribute(0xB00D, 0x0001, [mfgCode: 0x1124])
    
    // Probing without Manufacturer Code
    cmds += zigbee.readAttribute(0xB00D, 0x0000)
    cmds += zigbee.readAttribute(0xB00D, 0x0001)
    
    // Raw ZCL probe to Endpoint 02, Cluster 0xB00D
    cmds += ["he raw ${device.deviceNetworkId} 1 0x02 0xB00D {00 00 00 00 00}"]
