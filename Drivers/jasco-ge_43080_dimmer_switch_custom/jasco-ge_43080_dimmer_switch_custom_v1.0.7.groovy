 ...
 * background custom Health Check monitoring, and surgical ZDO protocol archaeology routines.
 *
 * Notes:
...
/**
 * Changelog:
 * v1.0.7    09/04/26    jshimota    Corrected proprietary cluster target from 0xB00D to 0xB005 based on verified ZDO Simple Descriptor payloads. Added probeClusterB005 routine.
...
def timeStamp() { return "2026/09/04 04:20 PM" }
...
metadata {
    definition (
        ...
        command "resetDriver"
        command "discoverActiveEndpoints"
        command "describeEndpoint1"
        command "describeEndpoint2"
        command "probeClusterB005"

        // Fingerprints
        fingerprint profileId: "0104", inClusters: "0000,0003,0004,0005,0006,0008,B005", outClusters: "000A,0019", manufacturer: "Jasco Products", model: "43080", deviceJoinName: "Enbrighten Zigbee In-Wall Smart Dimmer (43080)"
    }
...
private void parseDescMap(Map descMap) {
    ...
    // ZDO Cluster 0x8004 (Simple Descriptor Response)
    if (descMap.clusterInt == 32772 || descMap.clusterId == "8004") {
    ...
    if (descMap.clusterInt == 32773 || descMap.clusterId == "8005") {
        if (descMap.data && descMap.data.size() >= 5) {
            int count = Integer.parseInt(descMap.data[4], 16)
            List<String> eps = descMap.data[5..-1].collect { "0x" + it }
            logInfo "*** ARCHAEOLOGY: Active Endpoints discovered (${count}): ${eps.join(', ')} ***"
        } else {
    ...
    // Cluster 0xB005: Jasco Proprietary Cluster
    if (descMap.clusterInt == 45061 || descMap.clusterId == "B005" || descMap.clusterId == "b005") {
        logInfo "*** ARCHAEOLOGY: Cluster 0xB005 Response received *** raw: ${descMap}"
        return
    }
...
/**
 * Parser Helper for ZDO 0x8004 Simple Descriptor Responses
 **/
private void parseSimpleDescriptorResponse(Map descMap) {
    List<String> data = descMap.data
    if (!data || data.size() < 10) {
        logWarn "Simple Descriptor payload too short: ${data}"
        return
    }
    
    String ep = "0x" + data[4]
    String profile = "0x" + data[6] + data[5]
    String deviceId = "0x" + data[8] + data[7]
    
    int inCount = Integer.parseInt(data[10], 16)
    int idx = 11
    List<String> inClusters = []
    for (int i = 0; i < inCount && idx + 1 < data.size(); i++) {
        inClusters.add("0x" + data[idx+1] + data[idx])
        idx += 2
    }
    
    int outCount = (idx < data.size()) ? Integer.parseInt(data[idx], 16) : 0
    idx++
    List<String> outClusters = []
    for (int i = 0; i < outCount && idx + 1 < data.size(); i++) {
        outClusters.add("0x" + data[idx+1] + data[idx])
        idx += 2
    }

    logInfo "*** ARCHAEOLOGY: Simple Descriptor for Endpoint ${ep} *** Profile: ${profile}, DeviceID: ${deviceId}, InClusters: ${inClusters}, OutClusters: ${outClusters}"
}
...
def describeEndpoint1() {
    logInfo "Executing Protocol Archaeology: Requesting Simple Descriptor for Endpoint 0x01 (ZDO 0x0004)..."
    List<String> cmds = ["he raw ${device.deviceNetworkId} 0 0 0x0004 {00 ${zigbee.swapOctets(device.deviceNetworkId)} 01} {0000}"]
    sendHubCommand(new hubitat.device.HubMultiAction(cmds, hubitat.device.Protocol.ZIGBEE))
}

def describeEndpoint2() {
    logInfo "Executing Protocol Archaeology: Requesting Simple Descriptor for Endpoint 0x02 (ZDO 0x0004)..."
    List<String> cmds = ["he raw ${device.deviceNetworkId} 0 0 0x0004 {00 ${zigbee.swapOctets(device.deviceNetworkId)} 02} {0000}"]
    sendHubCommand(new hubitat.device.HubMultiAction(cmds, hubitat.device.Protocol.ZIGBEE))
}

def probeClusterB005() {
    logInfo "Executing Protocol Archaeology: Probing Cluster 0xB005 attributes 0x0000..0x0004 with MfgCode 0x1124..."
    List<String> cmds = []
    
    // Probe specific candidate attribute IDs 0x0000..0x0004 on Endpoint 1 for Cluster 0xB005
    cmds += zigbee.readAttribute(0xB005, 0x0000, [mfgCode: 0x1124])
    cmds += zigbee.readAttribute(0xB005, 0x0001, [mfgCode: 0x1124])
    cmds += zigbee.readAttribute(0xB005, 0x0002, [mfgCode: 0x1124])
    cmds += zigbee.readAttribute(0xB005, 0x0003, [mfgCode: 0x1124])
    cmds += zigbee.readAttribute(0xB005, 0x0004, [mfgCode: 0x1124])
