...
def timeStamp() { return "2026/09/10 07:25 AM" }
...
private void parseDescMap(Map descMap) {
    ...
    // Strict Health Check Validation (Requires valid cluster ID to avoid false positives)
    if (isHealthFrame && descMap.clusterId != null) {
        if (state.healthCheckPending == true) {
            logInfo "Valid Health Check response verified on cluster 0x${descMap.clusterId}"
