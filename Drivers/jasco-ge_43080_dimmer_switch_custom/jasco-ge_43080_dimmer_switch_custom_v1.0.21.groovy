/**
 * Jasco/GE 43080 Dimming Switch (Custom)
...
/**
 * Changelog:
 * v1.0.21   09/14/26    jshimota    Optimized parseDescMap to emit isStateChange: false to refresh lastActivity without event history log clutter.
 * v1.0.20   09/10/26    jshimota    Fixed platform 'Last Activity' timestamp refresh by forcing healthStatus state change on health check verification.
 * v1.0.19   09/10/26    jshimota    Updated driver header comments and metadata definition name to explicitly include model 43080.
 * v1.0.18   09/10/26    jshimota    Fixed clusterId check in parseDescMap to use descMap.cluster and descMap.clusterInt.
 * v1.0.17   09/10/26    jshimota    Restored missing getSettingBool helper function.
 * v1.0.16   09/10/26    jshimota    Added null cluster checks to Health Check parsing validation and enforced logWarn output on command timeout failure.
 * v1.0.15   09/10/26    jshimota    Promoted Health Check execution and response logging outputs to logInfo.
 * v1.0.14   09/04/26    jshimota    Enforced List<String> return types across command methods, added Double-safe casting to setLevel, added immediate OnOff read when setLevel is executed while off, and cleared healthCheckPending state on initialize.
 * v1.0.13   09/04/26    jshimota    Integrated updateFirmware command routine from power plug driver template.
 * v1.0.12   09/04/26    jshimota    Removed Endpoint 0x02 Direct Binding informational block from driver preferences.
...
def timeStamp() { return "2026/09/14 10:25 AM" }

import groovy.transform.Field

metadata {
    definition (
        name: "Jasco/GE 43080 Dimming Switch (Custom)", 
        namespace: "jshimota", 
...
private void parseDescMap(Map descMap) {
    ...
    if (descMap.clusterInt == 32801 || descMap.cluster == "8021") {
    ...
    if (descMap.clusterInt == 6 || descMap.cluster == "0006") {
    ...
    else if (descMap.clusterInt == 8 || descMap.cluster == "0008") {
        if (descMap.attrInt == 0 || descMap.attrId == "0000") {
            isHealthFrame = true
...
List<String> setLevel(level, duration = null) {
    int targetLevel = 0
    try {
        targetLevel = (level as Double).round().toInteger()
    } catch (Exception e) {
        logWarn "Invalid level parameter passed to setLevel: ${level}"
        return []
    }
    ...
    logInfo "setLevel(${targetLevel}, ${duration}) requested"
    List<String> cmds = []
    ...
    // Force rapid state sync for switch attribute if currently off
...
List<String> stopLevelChange() {
    ...
    return zigbee.command(0x0008, 0x03)
}
