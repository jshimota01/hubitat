...
0.1.8 - JAS added AirQualityIndex and Sensor so it shows up when sensors scanned
...
void installed() {
    schedule("0 0/30 * 1/1 * ?", "pollAQI")      // Every 30 mins on the hour/half-hour
    schedule("5 0/30 * 1/1 * ?", "pollCityName") // 5 seconds past every 30 mins
}
...
void refresh() {
    if (dbgEnable) log.debug "Refreshing schedules and polling data..."
    unschedule()
    installed()
    pollData()
}
