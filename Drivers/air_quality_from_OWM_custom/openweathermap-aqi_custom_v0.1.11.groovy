...
def timeStamp() { return "2026/09/23 12:45 PM" }
...
void pollAQIHandler(resp, data) {
    if (resp.getStatus() == 200 || resp.getStatus() == 207) {
        ...
        // Primary AQI & Dominant Pollutant Events
        logInfo "OpenWeatherMap Air Quality Index is now ${aQI} for ${aQIfactor} pollutant"
        sendIfChanged("airQualityIndex", aQI, null, "info", "airQualityIndex")
...
void pollCityNameHandler(resp, data) {
    if (resp.getStatus() == 200 || resp.getStatus() == 207) {
        ...
        if (geoData && geoData[0]?.name) {
            ...
            sendIfChanged("AirNowCity", cityName, null, "info", "AirNowCity")
        }
    } else {
