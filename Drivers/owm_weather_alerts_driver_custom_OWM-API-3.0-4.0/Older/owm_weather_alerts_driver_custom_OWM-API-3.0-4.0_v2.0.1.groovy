...
metadata {
    ...
    preferences {
        ...
        input name: "overrideCity", type: "text", title: "Override - City", description: "Optional - e.g., Newberg, OR or London, UK. Overrides Latitude/Longitude parameters configured below.", required: false
        input name: "latitude", type: "text", title: "Override - Latitude", description: "Optional - Leave blank to use Hub location", required: false
        input name: "longitude", type: "text", title: "Override - Longitude", description: "Optional - Leave blank to use Hub location", required: false
        
        // Display Selector Options
        input name: "useImperial", type: "bool", title: "Display - Imperial Base Units?", description: "Turn ON for Fahrenheit (°F)/inHg, OFF for Celsius (°C)/hPa", defaultValue: true, required: true
        input name: "precisionPrecip", type: "enum", title: "Display - Precipitation Decimal Precision", options: ["0": "0 Places", "1": "1 Place", "2": "2 Places"], defaultValue: "2", required: true
        input name: "precisionPressure", type: "enum", title: "Display - Pressure Decimal Precision", options: ["0": "0 Places", "1": "1 Place", "2": "2 Places"], defaultValue: "2", required: true
        input name: "precisionTemp", type: "enum", title: "Display - Temperature Decimal Precision", options: ["0": "0 Places", "1": "1 Place", "2": "2 Places"], defaultValue: "2", required: true
        input name: "precisionWind", type: "enum", title: "Display - Wind Speed Decimal Precision", options: ["0": "0 Places", "1": "1 Place", "2": "2 Places"], defaultValue: "2", required: true
        input name: "windUnit", type: "enum", title: "Display - Wind Speed Unit", options: ["mph": "Miles per Hour (mph)", "kmh": "Kilometers per Hour (km/h)", "kt": "Knots (kt)", "ms": "Meters per Second (m/s)"], defaultValue: "mph", required: true
...
private def parsePayload(Map json) {
    ...
    def rawWindSpeed = current.wind_speed != null ? current.wind_speed.toBigDecimal() : null
    ...
    // Dynamic Wind Speed Unit Conversions
    def windSpd = 0
    if (rawWindSpeed != null) {
        // OWM baseline returns: Imperial = mph, Metric = m/s
        String baseWindUnit = (useImperial == true) ? "mph" : "ms"
        String targetWindUnit = windUnit ?: "mph"
        
        if (baseWindUnit == "mph") {
            if (targetWindUnit == "mph")  windSpd = rawWindSpeed
            if (targetWindUnit == "kmh")  windSpd = rawWindSpeed * 1.609344
            if (targetWindUnit == "kt")   windSpd = rawWindSpeed * 0.868976
            if (targetWindUnit == "ms")   windSpd = rawWindSpeed * 0.44704
        } else { // base is m/s
            if (targetWindUnit == "mph")  windSpd = rawWindSpeed * 2.236936
            if (targetWindUnit == "kmh")  windSpd = rawWindSpeed * 3.6
            if (targetWindUnit == "kt")   windSpd = rawWindSpeed * 1.943844
            if (targetWindUnit == "ms")   windSpd = rawWindSpeed
        }
