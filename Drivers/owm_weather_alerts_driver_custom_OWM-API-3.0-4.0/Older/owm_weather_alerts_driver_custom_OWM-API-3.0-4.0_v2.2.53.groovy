...
void scheduledTextValue(List dataList) {
    ...
    if (dataList && dataList.size() >= 9) {
        BigDecimal lux   = dataList[0] != null ? dataList[0].toBigDecimal() : null
        BigDecimal temp  = dataList[1] != null ? dataList[1].toBigDecimal() : null
        BigDecimal press = dataList[2] != null ? dataList[2].toBigDecimal() : null
        BigDecimal wind  = dataList[3] != null ? dataList[3].toBigDecimal() : null
        BigDecimal hum   = dataList[4] != null ? dataList[4].toBigDecimal() : null
        Map cur          = dataList[5] instanceof Map ? dataList[5] : [:]
        Map today        = dataList[6] instanceof Map ? dataList[6] : [:]
        Map tom          = dataList[7] instanceof Map ? dataList[7] : [:]
        Map tda          = dataList[8] instanceof Map ? dataList[8] : [:]
        
        calcTextValue(lux, temp, press, wind, hum, cur, today, tom, tda)
    } else {
...
// --- FIX PATH: If initializing, force a brief 2-second out-of-band delayed execution
    if (state.isInitializing == true) {
        ...
        // Define the variables locally so they can be bundled into the delayed schedule execution safely
        ...
        // Pass the safely instantiated local values to the out-of-band execution thread
...
private void sendOWMData(Map current, Map today, Map tom, Map tda) {
    ...
    // ==========================================
    if (current) {
        // Handle the hourly rain and snow values if provided by OWM API[cite: 1]
        def rawRain = current.calculatedRain ?: 0.00
        def rawSnow = current.calculatedSnow ?: 0.00

        // Process through convertPrecip() which handles unit conversion and precision[cite: 1]
        def rainVal = convertPrecip(rawRain)
        def snowVal = convertPrecip(rawSnow)

        // Handle 'none' display unit preference gracefully[cite: 1]
        String preUnit = (settings.precipUnit == "none" || settings.precipUnit == null) ? "" : "${settings.precipUnit}"

        // Dynamic spacing based on precipUnit selection[cite: 1]
        String spaceStr = (preUnit == '"') ? "" : " "

        // Cleaned up sendIfChanged calls[cite: 1]
        ...
        if (current.temp != null) {
            ...
            sendIfChanged(name: "temperature", value: calcTemp) // Map to Core Capability[cite: 1]
        }
        ...
        if (current.humidity != null) {
            ...
            sendIfChanged(name: "humidity", value: calcHumidity) // Map to Core Capability[cite: 1]
        }
        if (current.pressure != null) {
            ...
            sendIfChanged(name: "pressure", value: calcPressure) // Map to Core Capability[cite: 1]
        }
        if (current.uvi != null) {
            sendIfChanged(name: "currentUVI", value: current.uvi)
            sendIfChanged(name: "ultravioletIndex", value: current.uvi) // Map to Core Capability[cite: 1]
        }
        ...
        // Nested weather condition arrays[cite: 1]
    ...
    // ==========================================
    // 2. REFRACTORED FORECAST DISPATCHES (Today, Tom, Tda)[cite: 1]
    // ==========================================
    def forecastDays = [
    ...
    forecastDays.each { day ->
        ...
        if (data) {
            ...
            // Nested Temperature structure loop[cite: 1]
            if (data.temp) {
            ...
            // Nested Feels Like structure loop[cite: 1]
            if (data.feels_like) {
            ...
            // Weather & Atmospheric metrics[cite: 1]
    ...
    // ==========================================
    // 3. ILLUMINANCE & OUT-OF-BAND MATH EXECUTION[cite: 1]
    // ==========================================
    // Trigger the illuminance calculation right before concluding the dispatch[cite: 1]
    BigDecimal currentAlt = state.sunAltitude != null ? state.sunAltitude.toBigDecimal() : (device.currentValue("altitude")?.toBigDecimal() ?: 0.0)
        
    // Pass the in-memory current cloud layer to avoid initialization null-outs[cite: 1]
    def liveClouds = current?.clouds != null ? current.clouds : null
    BigDecimal freshLux = calcCurrentIlluminance(currentAlt, liveClouds)
	
    // Extract localized in-memory blocks to bypass async DB lag during initialization[cite: 1]
    ...
    // Pass local variables to keep text fields populated instantly[cite: 1]
    calcTextValue(freshLux, freshTemp, freshPress, freshWind, freshHum, current, today, tom, tda)
    calcMoonPhaseIconWithText(today, tom, tda) // Uses active API responses directly[cite: 1]
...
private void calcTextValue(BigDecimal freshLux = null, BigDecimal freshTemp = null, BigDecimal freshPress = null, BigDecimal freshWind = null, BigDecimal freshHum = null, Map currentMap = [:], Map todayMap = [:], Map tomMap = [:], Map tdaMap = [:]) {
    ...
    // 3. Wind Speed Text Formatting (Current, Today, Tomorrow, Day After)
    logDebug "Section 3: Formatting Wind Speed Text values ..."
    ...
    // Today Wind
    def todayWindVal = (todayMap && todayMap.wind_speed != null) ? convertWindSpeed(todayMap.wind_speed) : device.currentValue("todayWindSpeed")
    if (todayWindVal != null) {
        sendIfChanged(name: "todayWindSpeedText", value: "${todayWindVal}${wUnit}")
    }

    // Tomorrow Wind
    def tomWindVal = (tomMap && tomMap.wind_speed != null) ? convertWindSpeed(tomMap.wind_speed) : device.currentValue("tomWindSpeed")
    if (tomWindVal != null) {
        sendIfChanged(name: "tomWindSpeedText", value: "${tomWindVal}${wUnit}")
    }

    // Day After Wind
    def tdaWindVal = (tdaMap && tdaMap.wind_speed != null) ? convertWindSpeed(tdaMap.wind_speed) : device.currentValue("tdaWindSpeed")
    if (tdaWindVal != null) {
        sendIfChanged(name: "tdaWindSpeedText", value: "${tdaWindVal}${wUnit}")
    }
    ...
    sendIfChanged(name: "currentWindSpeedDescText", value: getBeaufortText(curWindRaw))
    
    // Today Wind Speed
    def todayWindRaw = (todayMap && todayMap.wind_speed != null) ? todayMap.wind_speed : device.currentValue("todayWindSpeed")
    sendIfChanged(name: "todayWindSpeedDescText", value: getBeaufortText(todayWindRaw))
    
    // Tomorrow Wind Speed
    def tomWindRaw = (tomMap && tomMap.wind_speed != null) ? tomMap.wind_speed : device.currentValue("tomWindSpeed")
    sendIfChanged(name: "tomWindSpeedDescText", value: getBeaufortText(tomWindRaw))
    ...
    sendIfChanged(name: "tdaWindSpeedDescText", value: getBeaufortText(tdaWindRaw))

	// 12. Current Wind Summary Text Formatter
    logDebug "Section 11b: Formatting Current Wind Summary Text ..."
    try {
        wUnit = (settings.windSpeedUnit == "none") ? "" : " ${settings.windSpeedUnit ?: 'mph'}"
        ...
        String windSummary = "${currentWindSpeedDescText} from the ${windDirFull} at ${windSpeedText}"
        
        sendIfChanged(name: "currentWindSummaryText", value: windSummary)
    } catch (Exception e) {
    ...
    try {
        ...
        // DateTime Formats
        lastUpdatedEpoch = device.currentValue("lastUpdatedDateTime")
        chosenDateForm = settings.DateForm ?: "1"
        chosenTimeForm = settings.TimeForm ?: "1"
        ...
        String alertSentence = "No active alerts exist for this area."
        if (alertVal != "No active alerts" && alertVal != "N/A" && alertVal != "") {
            alertSentence = "Active alerts for ${alertVal} exist for this area."
        }
