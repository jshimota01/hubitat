...
metadata {
    definition(name: "OpenWeatherMap Multi-Version Weather Driver", namespace: "jshimota", author: "James Shimota") {
        ...
        // Weather Condition IDs and Icon Names
        attribute "condition_code", "number"
        attribute "condition_icon", "string"
        attribute "condition_icon_name", "string"
    }
...
private def parsePayload(Map json) {
    ...
    // Fallback block: Moved the img to a new trailing line after the description text
    String alertTile = "<div style='text-align:center;'>No Active Weather Alerts</div><div style='text-align:center; margin-top:5px;'><img src='${alertIconUrl}' style='max-width:25px; max-height:25px; vertical-align:middle;'/></div>"
    ...
    if (alerts.size() > 0) {
        ...
        // Active alert block: Placed the img inside a wrapper row following alertDescr
        alertTile = "<div style='color:red; font-weight:bold; text-align:center;'>${alertActive}</div><div style='font-size:0.8em;'>${alertDescr}</div><div style='text-align:center; margin-top:5px;'><img src='${alertIconUrl}' style='height:2em'/></div>"
    }
    ...
    sendEvent(name: "condition_code", value: conditionId)
    sendEvent(name: "condition_icon", value: formattedConditionIcon)
    sendEvent(name: "condition_icon_name", value: iconNameValue)
    ...
    sendEvent(name: "cloudPctCurrent", value: clouds)
    sendEvent(name: "cloudPctToday", value: (daily.size() > 0 ? daily[0].clouds ?: 0 : 0))
    sendEvent(name: "cloudPctTomorrow", value: cloudTom)
    sendEvent(name: "cloudPctDayAfterTomorrow", value: cloudDat)
    ...
    // --- Auto-trigger sun position calculations on execution schedule ---
    updateSunPosition()
}
...
private void updateSunPosition() {
    ...
    // --- Process custom "betwixt" slicing logic dynamically using computed angles and times ---
    calcBetwixtState(altitudeDeg)
}
