...
private void sendOWMData(Map current, Map today, Map tom, Map tda) {
    ...
    if (tom) {
        ...
        if (tom.moonrise != null) sendIfChanged(name: "todayMoonrise", value: tom.moonrise)
        if (tom.moonset != null) sendIfChanged(name: "todayMoonset", value: tom.moonset)
...
private BigDecimal convertIlluminance(BigDecimal rawLux) {
    ...
    BigDecimal finalValue = convertedValue.setScale(precision, RoundingMode.HALF_UP)
...
private BigDecimal convertKelvin(def kelvinVal) {
    ...
    return converted.setScale(precision, RoundingMode.HALF_UP)
}
