...
private void updateDynamicSchedules(long sunriseEpoch, long sunsetEpoch) {
    ...
    if (nightInterval != "manual" && nightInterval) {
        ...
        String nightCronStr = ""
        if (mins < 60) {
            // Minutes less than 60 work fine with standard ranges (e.g., 0/30)
            nightCronStr = "0 0/${mins} ${sunsetHour}-23,0-${endNightHour} * * ?"
        } else {
            // For hourly/multi-hour intervals, dynamically build an explicit list of hours
            int hours = mins / 60
            List<Integer> nightHours = []
            int currentHour = sunsetHour
            
            while (true) {
                nightHours.add(currentHour)
                currentHour += hours
                if (currentHour >= 24) currentHour -= 24
                
                // Break condition: check if the next step falls outside the nighttime window
                if (sunsetHour <= endNightHour) {
                    if (currentHour < sunsetHour || currentHour > endNightHour) break
                } else {
                    if (currentHour > endNightHour && currentHour < sunsetHour) break
                }
                if (currentHour == sunsetHour) break // Prevent infinite loop if intervals span 24h
            }
            
            nightCronStr = "0 0 ${nightHours.join(',')} * * ?"
        }
