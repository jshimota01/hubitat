...
void generateTiles(Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    // =========================================================================
    ...
    // =========================================================================
    // SECTION 2: 3DayForecastTile (In-Memory Fix)
    // =========================================================================
    ...
    def buildForecastCol = { dayLabel, prefix, Map sourceMap ->
        // Pull values directly from live payload map, converting Kelvin dynamically if present
        ...
        def fPop = sourceMap.pop != null ? sourceMap.pop : (device.currentValue("${prefix}POP") ?: 0)
        def fSummary = sourceMap.summary != null ? sourceMap.summary : (device.currentValue("${prefix}Summary") ?: "Clear")
...
private void parseOWMData(Map json) {
    ...
    // --- FIXED: Pass live forecast maps directly into the loop to solve duplicate updates ---
    generateTiles(data0, data1, data2) 
...
private void calcBetwixtState(BigDecimal altitude) {
    ...
    if (sunriseEpoch > 0 && sunsetEpoch > 0) { 
		...
		try {
			// Correct Hubitat safe way to format an epoch timestamp
			String noonTimeStr = (midDayEpoch * 1000).toString().toDateTime().format("HH:mm", location.timeZone)
