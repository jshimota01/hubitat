...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    // =========================================================================
    String moonPhaseText = device.currentValue("todayMoonPhaseText") ?: "Unknown"
    String moonEmoji = device.currentValue("todayMoonPhaseEmojiIcon") ?: "🌑"
    String moonSvg = device.currentValue("todayMoonPhaseSVGIcon") ?: ""
    String moonPngUrl = device.currentValue("todayMoonPhaseIcon") ?: ""

    // Build the graphic visualization block
    ...
    // CSS layer setup creating a 3D-feeling card overlay (Title header removed completely)
    String moonBodyHtml = "<div style='display:flex; flex-direction:column; justify-content:space-between; height:100%; padding:8px; box-sizing:border-box; background:rgba(30,30,40,0.65); border-radius:6px; color:#fff; font-family:sans-serif; position:relative; text-align:center; overflow:hidden;'>" +
                          "<div style='position:relative; flex-grow:1; display:flex; align-items:center; justify-content:center; z-index:1;'>" +
                          graphicDisplay +
                          "</div>" +
                          "<div style='font-size:0.85em; font-weight:bold; color:#fff; text-shadow:1px 1px 3px rgba(0,0,0,0.9), -1px -1px 3px rgba(0,0,0,0.9); padding:2px 4px; background:rgba(0,0,0,0.3); border-radius:4px; z-index:2; margin-top:2px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;'>${moonPhaseText}</div>"
