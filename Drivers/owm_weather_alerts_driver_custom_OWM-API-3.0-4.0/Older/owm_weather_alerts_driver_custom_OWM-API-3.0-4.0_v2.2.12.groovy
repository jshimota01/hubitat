...
// Trigger the illuminance calculation right before concluding the lifecycle dispatch
	...
	// --- FIXED: Pass the local variables to keep the text fields populated instantly on install ---
...
private Map convertWindDirectionState(degrees) {
    ...
    // Construct the standard naming scheme for the 16-point wind graphics file payload
...
private void calcBetwixtState(BigDecimal altitude) {
    ...
    if (sunriseEpoch > 0 && sunsetEpoch > 0) { 
		...
		try {
			// location.timeZone is already a native TimeZone object, so we can apply it directly
