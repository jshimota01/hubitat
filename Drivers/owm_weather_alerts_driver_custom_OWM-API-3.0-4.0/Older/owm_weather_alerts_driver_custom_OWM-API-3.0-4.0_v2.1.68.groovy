...
private void sendOWMData(Map current, Map today, Map tom, Map tda) {
	...
	// --- Rain and Snow Cleanup ---
    def rawRain = resp.current?.rain?.'1h' ?: 0.00
    def rawSnow = resp.current?.snow?.'1h' ?: 0.00
    ...
    sendIfChanged("currentRain", value: "${rainVal}")
    sendIfChanged("currentSnow", value: "${snowVal}")
    sendIfChanged("currentRainText", value: "${rainVal ${precipUnit}")
    sendIfChanged("currentSnowText", value: "${snowVal $(precipUnit}")
