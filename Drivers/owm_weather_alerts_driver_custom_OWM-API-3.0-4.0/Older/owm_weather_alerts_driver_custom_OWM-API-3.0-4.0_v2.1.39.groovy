...
void calcSunPosition() {
    ...
    if (state.usedLatitude != null && state.usedLongitude != null) {
        ...
        azimuth = 0.0
        altitude = 0.0
        sendEvent(name: "azimuthText", value: "0.0 °")
        sendEvent(name: "altitudeText", value: "0.0 °")
        return
    }
    ...
    try {
        ...
        // 3. Round and store numeric values into state variables
        ...
        sendEvent(name: "azimuth", value: "${azimuth}")
        sendEvent(name: "altitude", value: "${altitude}")

        // 4. Update the requested text attributes with space and degree symbol
        sendEvent(name: "azimuthText", value: "${azimuth} °")
        sendEvent(name: "altitudeText", value: "${altitude} °")

        logDebug "calcSunPosition: Calculated Sun Azimuth: ${azimuth}, Altitude: ${altitude} in degrees"

    } catch (Exception e) {
        logError "calcSunPosition: Error performing solar calculations, setting values to 0.0: ${e.message}"
        azimuth = 0.0
        altitude = 0.0
        sendEvent(name: "azimuth", value: "${azimuth}")
        sendEvent(name: "altitude", value: "${altitude}")
