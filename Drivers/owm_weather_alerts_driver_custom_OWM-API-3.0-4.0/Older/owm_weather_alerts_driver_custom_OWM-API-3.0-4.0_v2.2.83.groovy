...
// =========================================================================
    ...
    if (settings.displayTileMoonPhaseSVGEnable == true && moonSvg != "") {
        // Use clean sizing adjustments for nested SVG rendering
        graphicDisplay = moonSvg.replace("<svg ", "<svg style='width:90px; height:90px; margin: auto; display:block;' ")
    } else if (moonPngUrl != "") {
        graphicDisplay = "<img src='${moonPngUrl}' style='width:75px; height:75px; object-fit:contain; margin:10px auto; display:block;'>"
    } else {
        String moonEmoji = device.currentValue("currentMoonPhaseEmojiIcon") ?: "🌑"
        graphicDisplay = "<div style='font-size:4.5em; line-height:1; margin:8px 0;'>${moonEmoji}</div>"
    }
    ...
    String moonBodyHtml = "<div style='display:flex; flex-direction:column; justify-content:center; align-items:center; height:100%; padding:8px; box-sizing:border-box; background:rgba(30,30,40,0.65); border-radius:6px; color:#fff; font-family:sans-serif; position:relative; text-align:center; overflow:hidden;'>" +
                          "<div style='position:relative; display:flex; align-items:center; justify-content:center; z-index:1; width:90px; height:90px;'>" +
                          graphicDisplay +
                          "</div>" +
...
String calcMoonPhaseSvgMask(double phase, String path) {
    // Explicitly handle New Moon boundary to prevent zero-area path overlap
    if (phase <= 0.02 || phase >= 0.98) {
        return """<svg width="256" height="256" viewBox="0 0 140 140">
      <circle cx="70" cy="70" r="65" fill="#121214"/>
    </svg>"""
    }
    ...
    // Dynamic graphic generation pulling directly from path setup (xmlns declaration removed as requested)
	return """<svg width="256" height="256" viewBox="0 0 140 140">
