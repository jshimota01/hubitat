...
metadata {
    definition(name: "OpenWeatherMap Multi-Version Weather Driver", namespace: "jshimota", author: "James Shimota") {
        ...
        attribute "conditionIconUrlTom", "string"
        attribute "conditionIconUrlDayAfterTom", "string"

        // Solar / Ephemeris Coordinates
        attribute "twBegin", "string"
        attribute "sunriseTime", "string"
        attribute "noonTime", "string"
        attribute "sunsetTime", "string"
        attribute "twEnd", "string"
        ...
        // Polling Timestamps
        attribute "lastPollForcast", "string"
        attribute "lastObservationForecase", "string"
    ...
    preferences {
        ...
        input name: "apiSelection", type: "enum", title: "API Version", options: ["2.5": "One Call 2.5 (Obsolete!)","3.0": "One Call 3.0", "4.0": "One Call 4.0"], defaultValue: "3.0", description: "Select your API Key version here<br><b>Required by OpenWeatherMaps</b><br><i>*Note: 2.5 API is now obsolete as of June 2024.</i>", required: true
        ...
        input name: "precisionPrecip", type: "enum", title: "Display Decimal Precision - Precipitation", options: ["0": "0 Places", "1": "1 Place", "2": "2 Places"], description: "Choice of decimal precision  for rainfall readings in logging and tiles<br>Default: <b>2</b><br>EG: 1, 1.5, 1.55", defaultValue: "2", required: true
        input name: "precisionPressure", type: "enum", title: "Display Decimal Precision - Pressure", options: ["0": "0 Places", "1": "1 Place", "2": "2 Places"], description: "Choice of decimal precision  for barometer readings in logging and tiles<br>Default: <b>2</b><br>EG: 30 mb, 30.5 mb, 30.55 mb", defaultValue: "2", required: true
        input name: "precisionTemp", type: "enum", title: "Display Decimal Precision - Temperature", options: ["0": "0 Places", "1": "1 Place", "2": "2 Places"], description: "Choice of decimal precision  for temperature readings in logging and tiles<br>Default: <b>2</b><br>EG: 70 °F, 70.3 °F, 70.55 °F", defaultValue: "2", required: true
        input name: "precisionWind", type: "enum", title: "Display Decimal Precision - Wind Speed", options: ["0": "0 Places", "1": "1 Place", "2": "2 Places"], description: "Choice of decimal precision  for wind speed readings in logging and tiles<br>Default: <b>2</b><br>EG: 12 mph, 12.7 mph, 12.77 mph", defaultValue: "2", required: true
		...
		input name: "temperatureUnit", type: "enum", title: "Display Unit - Temperature", options: ["f": "Fahrenheit (°F)", "c": "Celsius (°C)", "none": "None (No Unit Suffix)"], description: "Choice of temperature unit formatting used in tiles and logging<br>Default: <b>Fahrenheit (°F)</b>", defaultValue: "f", required: true

		input name: "pressureUnit", type: "enum", title: "Display Unit - Barometric Pressure", options: ["inHg": "Mercury (inHg)", "hPa": "Hectopascals (hPa)", "mb": "Millibar (mb)", "none": "None"], description: "Choice of barometer unit used in tiles and logging<br>Default: <b>Mercury (inHg)</b>", defaultValue: "inHg", required: true
		input name: "windUnit", type: "enum", title: "Display Unit - Wind Speed", options: ["mph": "Miles per Hour (mph)", "kmh": "Kilometers per Hour (km/h)", "kt": "Knots (kt)", "ms": "Meters per Second (m/s)", "none": "None"], description: "Choice of wind speed unit used in tiles and logging<br>Default: <b>Miles per Hour(mph)</b>", defaultValue: "mph", required: true		
        ...
        input name: "nightInterval", type: "enum", title: "Polling - Nighttime Interval", options: ["manual": "Manual Only (via pollOWM command)", "15": "15 Minutes", "30": "30 Minutes", "60": "1 Hour", "180": "3 Hours"], description: "Polling frequency of OWM during nightime (between sunset and sunrise)<br>Default: <b>1 Hour</b>", defaultValue: "60", required: true
...
private void requestWeatherData(def lat, def lon) {
	...
	if (apiSelection == "2.5") {
		url = "https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=${owmUnits}&exclude=minutely,hourly&appid=${apiKey}"
	} else if (apiSelection == "3.0") {
    ...
    sendIfChanged(name: "lastPollForcast", value: new Date().format("yyyy-MM-dd HH:mm:ss", location.timeZone))
...
private def parsePayload(Map json) {
    ...
    if (val == null) return null
		...
		// Convert from metric baseline if Fahrenheit is requested
		if (tUnit == "f") { 
			v = (v * 1.8) + 32.0
		}
		// Note: If tUnit is "c" or "none", it remains standard Celsius values coming from OWM
        ...
        if (daily.size() > 2) {
            ...
            if (d2.weather ? d2.weather[0]?.icon : null) { 
                ...
                if (settings.altIconsEnable == true && altImg2) {
                    IconUrlDayAfterTom = iconBasePath.endsWith("/") ? "${iconBasePath}${altImg2}" : "${iconBasePath}/${altImg2}"
                } else {
                    IconUrlDayAfterTom = iconBasePath.endsWith("/") ? "${iconBasePath}${iCode2}.png" : "${iconBasePath}/${iCode2}.png"
                }
            }
        }
    }
    ...
    if (tempValue != null) sendIfChanged(name: "temperature", value: tempValue, unit: (settings.temperatureUnit ? "°F" : "°C"))
    ...
    sendIfChanged(name: "conditionIconUrlTom", value: IconUrlTom)
    sendIfChanged(name: "conditionIconUrlDayAfterTom", value: IconUrlDayAfterTom)
    ...
    sendIfChanged(name: "conditionIconToday", value: formattedConditionIcon)
    sendIfChanged(name: "conditionIconToday_name", value: iconNameValue)
    ...
    sendIfChanged(name: "twBegin", value: srTime)
    sendIfChanged(name: "twEnd", value: ssTime)
    ...
    sendIfChanged(name: "cloudPctToday", value: clouds)
