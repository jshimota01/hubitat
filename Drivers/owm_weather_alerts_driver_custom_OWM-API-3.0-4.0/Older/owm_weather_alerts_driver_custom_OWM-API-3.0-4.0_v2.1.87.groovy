...
void generateTiles() {
    ...
    def temp = device.currentValue("currentTemperature")?.getAt(0) ?: "--"
    def cond = device.currentValue("currentConditionType") ?: "Clear"
    def hi   = device.currentValue("todayTempMax")?.getAt(0) ?: "--"
    def lo   = device.currentValue("todayTempMin")?.getAt(0) ?: "--"
    ...
    String tileHtml = "<div style='display:flex;flex-direction:column;justify-content:space-between;height:100%;padding:8px;box-sizing:border-box;background:rgba(30,30,40,0.65);border-radius:12px;color:#fff;font-family:sans-serif;line-height:1.2;text-align:center'><div style='display:flex;align-items:center;justify-content:space-around;width:100%'><img src='https://openweathermap.org/img/wn/${icon}@2x.png' style='width:38%;max-height:60px;object-fit:contain'><div style='font-size:1.8em;font-weight:bold;letter-spacing:-1px'>${temp}${tUnit}</div></div><div style='font-size:0.9em;font-weight:600;color:#ddd;text-transform:capitalize'>${cond}</div><div style='font-size:0.75em;color:#aaa;display:flex;justify-content:center;gap:10px'><span>H: ${hi}°</span><span>L: ${lo}°</span></div><div style='display:flex;justify-content:space-between;border-top:1px solid rgba(255,255,255,0.15);padding-top:4px;font-size:0.7em;color:#bbb'><span>💧 ${hum}%</span><span>💨 ${wind} ${wUnit}</span></div></div>"
