...
private BigDecimal convertPrecip(def precipVal) {
    if (precipVal == null) return 0.0
    
    // Ensure we are working with a BigDecimal representation
    BigDecimal precip = (precipVal instanceof BigDecimal) ? precipVal : new BigDecimal(precipVal.toString())
    
    // Grab precision selection from preferences (defaulting to 2 if not found)
    int precision = settings.precisionPrecip != null ? settings.precisionPrecip.toInteger() : 2
    
    // Round precipValue using HALF_UP strategy
