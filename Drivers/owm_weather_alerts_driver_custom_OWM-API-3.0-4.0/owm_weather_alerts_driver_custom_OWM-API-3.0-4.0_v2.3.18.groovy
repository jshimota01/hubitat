...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    String alertBodyHtml = "<div style='display:flex;flex-direction:column;justify-content:space-between;height:90%;padding:10px 4px;;border-radius:6px;color:#fff;line-height:1.2;text-align:center'>"

    if (alertActive != "No active alerts" && alertActive != "N/A" && alertActive != "") {
        // --- DYNAMIC 1024-CHARACTER BOUNDARY CALCULATION ---
        String headerHtml = "<div style='font-size:.85em;color:#ff5555;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'><b>⚠️ ${alertActive}</b></div><div style='font-size:.5em;color:#ddd;padding:2px 0;text-align:left;flex-grow:1;overflow:hidden;'>"
        String footerHtml = "</div><div style='display:flex;justify-content:space-between;border-top:1px solid rgba(255,255,255,0.15);padding-top:4px;font-size:.6em;color:#bbb'><span>👤 ${alertSender.take(12)}</span><span>⏰ ${lastPollTime}</span></div>"
        ...
        alertBodyHtml += "<div style='font-size:0.85em;font-weight:bold;color:#55ff55;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-bottom:2px;'>✅ ${cityName}</br>Clear Weather</div><div style='font-size:0.8em;color:#aaa;padding:8px 4px;'>No active weather alerts from OpenWeatherMap or their sources</div><div style='display:flex;justify-content:center;border-top:1px solid rgba(255,255,255,0.15);padding-top:4px;font-size:.6em;color:#bbb'><span>Updated ${lastPollTime}</span></div>"
    }
    ...
    if (rawMoonPhase != null && rawMoonPhase.toString().isNumber()) {
        ...
        else if (val >= (0.25 - tolerance) && val <= (0.25 + tolerance)) moonPhaseText = "First Quarter"
        ...
        else if (val >= (0.75 - tolerance) && val <= (0.75 + tolerance)) moonPhaseText = "Last Quarter"
    ...
    String moonBodyHtml = "<div style='display:flex;flex-direction:column;justify-content:center;align-items:center;height:100%;padding:8px;box-sizing:border-box;border-radius:6px;color:#fff;font-family:sans-serif;text-align:center;overflow:hidden;'>" +
