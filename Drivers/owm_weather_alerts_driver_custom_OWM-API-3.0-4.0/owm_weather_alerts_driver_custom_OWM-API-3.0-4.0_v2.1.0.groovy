...
private def parsePayload(Map json) {
	...
	int conditionId = (current.weather && current.weather.size() > 0 && current.weather[0]?.id != null) ? current.weather[0].id.toInteger() : 0
    String iconCode = current.weather?.getAt(0)?.icon ?: "" 
    ...
    long currentSunriseTime = (current.sunrise != null) ? (current.sunrise.toLong() * 1000) : 0
    long currentSunsetTime = (current.sunset != null) ? (current.sunset.toLong() * 1000) : 0
	...
	String luxUnitLabel = settings.illuminanceUnit ?: "lx"
	if (luxUnitLabel == "none") luxUnitLabel = ""
    ...
    // Updated to custom attribute currentPressure
    if (pressureValue != null) sendIfChanged(name: "currentPressure", value: pressureValue, unit: pressureUnitLabel)
    sendIfChanged(name: "ultravioletIndex", value: uvi ?: 0)
    sendIfChanged(name: "illuminance", value: estimatedLux, unit: "lux")

    sendIfChanged(name: "todaySummary", value: description)
    sendIfChanged(name: "weather", value: mainWeather)
    
    sendIfChanged(name: "currentFeelsLike", value: feelsLikeValue ?: tempValue, unit: tempUnitLabel)
    ...
    sendIfChanged(name: "currentWindGust", value: windSpd)
    sendIfChanged(name: "currentWindSpeed", value: windSpd)
    sendIfChanged(name: "currentWindDeg", value: windDir)

    sendIfChanged(name: "currentSunriseTime", value: srTime)
    sendIfChanged(name: "currentSunsetTime", value: ssTime)
    sendIfChanged(name: "currentNoonTime", value: "12:00")
    sendIfChanged(name: "currentTwilightBegin", value: srTime)
    sendIfChanged(name: "currentTwilightEnd", value: ssTime)
    
    sendIfChanged(name: "todayMoonrise", value: moonRiseStr)
    sendIfChanged(name: "todayMoonset", value: moonSetStr)
    sendIfChanged(name: "todayMoonPhase", value: moonPhase)
    ...
    sendIfChanged(name: "rainTom", value: rainTom)
    ...
    String activeWindUnit = settings.windUnit ?: "mph"
    String activeTempLabel = tempUnitLabel ?: "°F"
    String cleanDesc = description ? description.split(' ').collect { it.capitalize() }.join(' ') : "N/A"
	String formattedPress = (pressureValue != null) ? "${pressureValue} ${pressureUnitLabel}" : "N/A"
    
    // Explicitly appended activeTempLabel string variables into the Feels Like dashboard tile grid array row		 
    String finalFeelsLikeStr = feelsLikeValue != null ? "${feelsLikeValue}${activeTempLabel}" : "${tempValue}${activeTempLabel}"

    String currentTileHtml = """<div style='font-family: system-ui, -apple-system, sans-serif; padding: 12px; background: rgba(15, 23, 42, 0.65); border-radius: 12px; border: 1px solid rgba(255, 255, 255, 0.1); color: #f8fafc; max-width: 320px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06); margin: auto;'>
  <div style='display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;'>
    <div>
      <div style='font-size: 1.15rem; font-weight: 700; letter-spacing: -0.025em; line-height: 1.25;'>${calculatedCityAttr}</div>
      <div style='font-size: 0.8rem; opacity: 0.8; margin-top: 2px;'>${cleanDesc}</div>
    </div>
    <div style='display: flex; align-items: center; gap: 4px; text-align: right;'>
      <img src='${currentIconUrl}' style='width: 48px; height: 48px; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.2));' alt='Weather Icon'/>
      <div style='font-size: 1.85rem; font-weight: 800; tracking: -0.05em; line-height: 1;'>${curTempStr}<span style='font-size: 1.1rem; font-weight: 500; vertical-align: top;'>${activeTempLabel}</span></div>
    </div>
  </div>
  
  <div style='display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; border-top: 1px solid rgba(255, 255, 255, 0.1); padding-top: 10px; font-size: 0.75rem; text-align: center;'>
    <div>
      <div style='opacity: 0.6; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 2px;'>Feels Like</div>
      <div style='font-weight: 600; font-size: 0.85rem;'>${finalFeelsLikeStr}</div>
    </div>
    <div>
      <div style='opacity: 0.6; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 2px;'>Humidity</div>
      <div style='font-weight: 600; font-size: 0.85rem;'>${humidityValue}%</div>
    </div>
    <div>
      <div style='opacity: 0.6; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 2px;'>Wind</div>
      <div style='font-weight: 600; font-size: 0.85rem; white-space: nowrap;'>${windSpd} ${activeWindUnit}</div>
    </div>
    <div>
      <div style='opacity: 0.6; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 2px;'>Pressure</div>
      <div style='font-weight: 600; font-size: 0.85rem; white-space: nowrap;'>${formattedPress}</div>
    </div>
    <div>
      <div style='opacity: 0.6; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 2px;'>UV Index</div>
      <div style='font-weight: 600; font-size: 0.85rem;'>${uvi ?: 0}</div>
    </div>
    <div>
      <div style='opacity: 0.6; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 2px;'>Lux</div>
      <div style='font-weight: 600; font-size: 0.85rem; white-space: nowrap;'>${estimatedLux}</div>
    </div>
  </div>
  
  <div style='margin-top: 10px; display: flex; justify-content: space-between; font-size: 0.7rem; border-top: 1px solid rgba(255, 255, 255, 0.1); padding-top: 8px; opacity: 0.75;'>
    <div style='display: flex; align-items: center; gap: 8px;'>
      <span>☀️ ${srTime}</span>
      <span>🌙 ${ssTime}</span>
    </div>
    <div style='font-size: 0.65rem; font-style: italic;'>Refreshed ${new Date().format("HH:mm", location.timeZone)}</div>
  </div>
</div>""".toString()
