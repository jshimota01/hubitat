...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
	...
	// =========================================================================
    // SECTION 1: currentTile (Optimized & Compact Dashboard Layout)
    // =========================================================================
    ...
    // Tightened styling: reduced temperature to 1.6em, decreased line-height, and minimized paddings/margins
    String currentBodyHtml = "<div style='background:#666;border-radius:6px;padding:4px 6px;color:#fff;font-family:sans-serif;text-align:center;line-height:1.15;box-sizing:border-box;height:100%;display:flex;flex-direction:column;justify-content:space-between;'><div style='font-weight:bold;font-size:0.95em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'>${cityName}</div><div style='display:flex;align-items:center;justify-content:center;gap:4px;margin:-2px 0;'><img src='https://openweathermap.org/img/wn/${icon}.png' style='width:32px;height:32px;'> <span style='font-size:1.6em;font-weight:bold;'>${temp}${tUnit}</span></div><div style='color:#ddd;font-size:0.85em;text-transform:capitalize;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'>${cond}</div><div style='color:#bbb;font-size:0.85em;margin-bottom:2px;'>H: ${hi}° | L: ${lo}°</div><div style='display:flex;justify-content:space-between;border-top:1px solid rgba(255,255,255,0.15);padding-top:3px;font-size:0.8em;color:#bbb;'><span>💧 ${hum}%</span><span>${windIconDisplay}${wind}${wUnit}</span></div>"
    
    String currentTileHtml = ""
    if (settings.debugTileEnable == true) {
        String currentSuffixTemplate = "<div style='font-size:0.65em;color:#777;'>Char length: 0000</div></div>"
        int currentLenEstimate = currentBodyHtml.length() + currentSuffixTemplate.length()
        currentTileHtml = currentBodyHtml + "<div style='font-size:0.65em;color:#777;'>Char length: ${currentLenEstimate}</div></div>"
    } else {
