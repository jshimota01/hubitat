...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    String currentBodyHtml = "<div align='center' style='background:#888;color:#fff;border-radius:6px;padding:4px'><div>Currently in ${cityName}</div><div><img src='https://openweathermap.org/img/wn/${icon}.png' style='width:30px;height:30px;vertical-align:middle'><b>${sT}${tUnit}</b></div><div>${cond}</div><div>H:${sH}° L:${sL}°</div><div><span>💧${hum}%</span>&nbsp;<span>${windIconDisplay}${sW}${wUnit}</span></div></div>"
    ...
    if (settings.debugTileEnable == true) {
        ...
        currentTileHtml = currentBodyHtml + "<div align='center' style='font-size:.6em;color:#555'>Len:${currentLenEstimate}</div></div>"
    } else {
