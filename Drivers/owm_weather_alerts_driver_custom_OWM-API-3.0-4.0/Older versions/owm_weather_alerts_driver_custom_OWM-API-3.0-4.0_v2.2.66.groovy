...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    // Dynamic character calculation toggled by settings.debugTileEnable
...
private void calcAlertsState(Map json, String calculatedCityAttr, String iconBasePath) {
    ...
    if (alerts.size() > 0) {
        ...
        String alertBody = "<div style='${baseStyle}'><div style='font-size:0.85em;font-weight:bold;color:#ff5555;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-bottom:2px;'>⚠️ ${alertActive}</div><div style='font-size:0.8em;font-weight:600;color:#ddd;padding:4px;overflow:hidden;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;'>${currentAlertDesc}</div><div style='display:flex;justify-content:space-between;border-top:1px solid rgba(255,255,255,0.15);padding-top:4px;font-size:0.7em;color:#bbb'><span>👤 ${currentAlertSender.take(12)}</span><span>⏰ ${lastPollTime}</span></div>"
        ...
        String noAlertBody = "<div style='${baseStyle}'><div style='font-size:0.85em;font-weight:bold;color:#55ff55;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-bottom:2px;'>✅ Your Local Area</br>Clear Weather</div><div style='font-size:0.8em;font-weight:600;color:#aaa;padding:8px 4px;'>No active weather alerts from OpenWeatherMap or their sources</div><div style='display:flex;justify-content:center;border-top:1px solid rgba(255,255,255,0.15);padding-top:4px;font-size:0.7em;color:#bbb'><span>Updated ${lastPollTime}</span></div>"
