...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    // Hyper-compressed HTML template using condensed CSS tags
    String currentBodyHtml = "<div style='background:#666;border-radius:6px;padding:4px;color:#fff;font-family:sans-serif;text-align:center;line-height:1.1;box-sizing:border-box;height:100%;display:flex;flex-direction:column;justify-content:space-between;'><div style='font-weight:bold;font-size:0.9em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'>${cityName}</div><div style='display:flex;align-items:center;justify-content:center;gap:2px;margin:-4px 0;'><img src='https://openweathermap.org/img/wn/${icon}.png' style='width:30px;height:30px;'> <span style='font-size:1.5em;font-weight:bold;'>${sT}${tUnit}</span></div><div style='color:#ddd;font-size:0.8em;text-transform:capitalize;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'>${cond}</div><div style='color:#bbb;font-size:0.8em;'>H:${sH}° L:${sL}°</div><div style='display:flex;justify-content:space-between;border-top:1px solid rgba(255,255,255,0.15);padding-top:2px;font-size:0.75em;color:#bbb;'><span>💧${hum}%</span><span>${windIconDisplay}${sW}${wUnit}</span></div>"
    
    String currentTileHtml = ""
    if (settings.debugTileEnable == true) {
        String currentSuffixTemplate = "<div style='font-size:0.6em;color:#777;'>Len:000</div></div>"
        int currentLenEstimate = currentBodyHtml.length() + currentSuffixTemplate.length()
        currentTileHtml = currentBodyHtml + "<div style='font-size:0.6em;color:#777;'>Len:${currentLenEstimate}</div></div>"
    } else {
