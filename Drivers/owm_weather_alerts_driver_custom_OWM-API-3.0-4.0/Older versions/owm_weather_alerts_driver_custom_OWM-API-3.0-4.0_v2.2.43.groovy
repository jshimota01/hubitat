...
def installed() {
    ...
    if (settings.visibilityUnit == null) device.updateSetting("visibilityUnit", [type: "enum", value: "miles"])
    ...
    sendIfChanged(name: "visibilityUnit", value: "miles")
...
def updated() {
    ...
    sendIfChanged(name: "visibilityUnit", value: settings.visibilityUnit ?: "miles")	
...
// Preset placeholder strings to ensure clean execution bounds
    sendIfChanged(name: "todayMoonPhaseText", value: "--")
    sendIfChanged(name: "tomMoonPhaseText", value: "--")
    sendIfChanged(name: "tdaMoonPhaseText", value: "--")
...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    if (currentData && todayData) {
        ...
        cond = currentData.weather && currentData.weather[0] ? currentData.weather[0].description : "Clear Sky"
        ...
        cond = device.currentValue("currentConditionTypeFull") ?: "Clear Sky"
    ...
    String forecastHtml = forecastBodyHtml + "<span style='font-size:0.6em;color:#888;'>${forecastLenEstimate} chars long</span></div></div>"
...
private void parseOWMData(Map json) {
    ...
    if (state.isInitializing == true) {
        ...
        // PASS THE LOCAL MAPS TO THE DELAYED RUNS TO BE SURE TEXT VALUES ARE BUILT ON INITIALIZATION
        runIn(2, "calcTextValue", [data: [freshLux, freshTemp, freshPress, freshWind, freshHum, currentData, data0, data1, data2]])
...
// 0. handle the hourly rain and snow values if provided by OWM Api
       ...
       if (today.wind_gust != null) {
        ...
        if (today.wind_speed != null) {
         ...
         if (today.wind_deg != null) {
            ...
            sendIfChanged(name: "todayWindDirIcon", value: wDir.iconUrl)
        } 
       ...
       if (tom.wind_gust != null) {
        ...
        if (tom.wind_speed != null) {
         ...
         if (tom.wind_deg != null) {
            ...
            sendIfChanged(name: "tomWindDirIcon", value: wDir.iconUrl)
        } 
       ...
       if (tda.wind_gust != null) {
        ...
        if (tda.wind_speed != null) {
         ...
         if (tda.wind_deg != null) {
            ...
            sendIfChanged(name: "tdaWindDirIcon", value: wDir.iconUrl)
        } 
	...
	calcTextValue(freshLux, freshTemp, freshPress, freshWind, freshHum)
	calcMoonPhaseIconWithText(today, tom, tda) // uses api response data so it works on initialization
...
private BigDecimal convertVisibilityDistance(def rawVisibility) {
    if (rawVisibility == null) return 0.0
...
private Map convertWindDirectionState(degrees) {
    ...
    // Determine the base path cleanly using your fallback-safe runtime state property
    ...
    if (settings.altIconsEnable == false) {
        filename = "wb1.png"
    } else {
        filename = "wind-${token.toLowerCase()}.png"
    }
    
    // Safety Fallback Check: If your path is pointing straight to OWM standard images, 
    // it won't have the wind-*.png repository, so we point directly to the Hubitat Community hub asset tree.
    if (basePath.contains("openweathermap.org")) {
        finalIconUrl = "https://raw.githubusercontent.com/HubitatCommunity/WeatherIcons/master/${filename}"
    } else {
...
void calcMoonPhaseIconWithText(Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    phases.each { phase ->
        ...
        // Ensure the value is both non-null and safely numeric before processing
        if (rawVal != null && rawVal.toString().isNumber()) {
            ...
            String filename = "mp${fileNum}.png"
            String basePath = state.iconBasePath ?: "https://tinyurl.com/icnqz/"
            String finalIconUrl = "${basePath}${filename}"
            
            sendIfChanged(name: phase.iconAttr, value: finalIconUrl)

            // 2. Determine phase name using small range tolerances (+/- 0.02) around major quarter marks
            String phaseName = "Unknown"
            double tolerance = 0.02
            
            if (val <= tolerance || val >= (1.0 - tolerance)) {
                phaseName = "New Moon"
            } else if (val > tolerance && val < (0.25 - tolerance)) {
                phaseName = "Waxing Crescent"
            } else if (val >= (0.25 - tolerance) && val <= (0.25 + tolerance)) {
                phaseName = "First Quarter Moon"
            } else if (val > (0.25 + tolerance) && val < (0.5 - tolerance)) {
                phaseName = "Waxing Gibbous"
            } else if (val >= (0.5 - tolerance) && val <= (0.5 + tolerance)) {
                phaseName = "Full Moon"
            } else if (val > (0.5 + tolerance) && val < (0.75 - tolerance)) {
                phaseName = "Waning Gibbous"
            } else if (val >= (0.75 - tolerance) && val <= (0.75 + tolerance)) {
                phaseName = "Last Quarter Moon"
            } else if (val > (0.75 + tolerance) && val < (1.0 - tolerance)) {
                phaseName = "Waning Crescent"
            }
            
            sendIfChanged(name: phase.textAttr, value: phaseName)
        } else {
            logDebug "Phase value for ${phase.valAttr} (${rawVal}) is non-numeric or missing. Leaving existing state intact."
        }
    }
}
...
private void calcAlertsState(Map json, String calculatedCityAttr, String iconBasePath) {
    ...
    if (alerts.size() > 0) {
        ...
        String alertBody = "<div style='${baseStyle}'>" +
                           ...
                           "</div><div style='font-size:0.6em;color:#888;margin-top:2px;'>"
        ...
        String noAlertBody = "<div style='${baseStyle}'>" +
...
private void calcBetwixtState(BigDecimal altitude, long liveSunrise = 0, long liveSunset = 0) {
    ...
    // Use the live method arguments if provided, otherwise fall back to cached states
...
private void calcTextValue(BigDecimal freshLux = null, BigDecimal freshTemp = null, BigDecimal freshPress = null, BigDecimal freshWind = null, BigDecimal freshHum = null, Map currentMap = [:], Map todayMap = [:], Map tomMap = [:], Map tdaMap = [:]) {
    ...
    // 7. DateTime Attributes Auto-Formatting
...
// 10. Time-Only Attributes Auto-Formatting
    ...
    device.properties.supportedAttributes.each { attr ->
        ...
        if (attrName.endsWith("Time") && !attrName.contains("DateTime")) {
            ...
            else if (attrName == "currentTwilightBeginTime") {
    ...
    // 11. Beaufort Wind Scale Formatter
    logDebug "Section 11: Formatting Beaufort Wind Scale values ..."
    
    // Current Wind Speed (using in-memory freshWind if passed, falling back to OWM raw structure or DB value)
    def curWindRaw = (currentMap && currentMap.wind_speed != null) ? currentMap.wind_speed : device.currentValue("currentWindSpeed")
    sendIfChanged(name: "currentWindSpeedText", value: getBeaufortText(curWindRaw))
    
    // Today Wind Speed
    def todayWindRaw = (todayMap && todayMap.wind_speed != null) ? todayMap.wind_speed : device.currentValue("todayWindSpeed")
    sendIfChanged(name: "todayWindSpeedText", value: getBeaufortText(todayWindRaw))
    
    // Tomorrow Wind Speed
    def tomWindRaw = (tomMap && tomMap.wind_speed != null) ? tomMap.wind_speed : device.currentValue("tomWindSpeed")
    sendIfChanged(name: "tomWindSpeedText", value: getBeaufortText(tomWindRaw))
    
    // Day After Tomorrow Wind Speed
    def tdaWindRaw = (tdaMap && tdaMap.wind_speed != null) ? tdaMap.wind_speed : device.currentValue("tdaWindSpeed")
    sendIfChanged(name: "tdaWindSpeedText", value: getBeaufortText(tdaWindRaw))

// 12. Dynamic Current Weather Summary Text
    logDebug "Section 12: Generating currentWeatherSummaryText ..."
    
    // Extract localized dependencies (prioritizing in-memory params, falling back to DB/State)
    String city = state.usedCity ?: "Local Area"
    String tempUnit = settings.temperatureUnit ?: "°F"
    
    // --- Optimized Date/Time Extraction (Use fresh variables if available) ---
    String lastUpdatedDate = device.currentValue("todayDateText") ?: "--"
    String lastUpdatedTime = "--"
    
    // Reuse local/state epoch rather than query the DB repeatedly
    lastUpdatedEpoch = freshLux != null ? (new Date().getTime() / 1000L) : device.currentValue("lastUpdatedDateTime")
    if (lastUpdatedEpoch != null) {
        // Reuse the 'chosenTimeForm' variable already declared in Section 10 of this method
        if (chosenTimeForm == "10") {
            lastUpdatedTime = "${lastUpdatedEpoch}"
        } else {
            Map formattedMap = convertDateTimeFormat(lastUpdatedEpoch, chosenTimeForm)
            lastUpdatedTime = formattedMap.time
        }
    }
    
    // --- Nested Weather Condition Extraction ---
    String currentConditionFull = "clear sky"
    if (currentMap && currentMap.weather && currentMap.weather[0]) {
        currentConditionFull = currentMap.weather[0].description ?: "clear sky"
    } else {
        currentConditionFull = device.currentValue("currentConditionTypeFull") ?: "clear sky"
    }
    
    // --- Temperatures (Prioritize in-memory Kelvin values to avoid DB read lags) ---
    def currentTempVal = (freshTemp != null) ? freshTemp : device.currentValue("currentTemperature")
    String currentTempStr = currentTempVal != null ? "${currentTempVal}${tempUnit}" : "--"
    
    def feelsLikeVal = (currentMap && currentMap.feels_like != null) ? convertKelvin(currentMap.feels_like) : device.currentValue("currentFeelsLike")
    String feelsLikeStr = feelsLikeVal != null ? "${feelsLikeVal}${tempUnit}" : "--"
    
    def todayTempMaxVal = (todayMap && todayMap.temp?.max != null) ? convertKelvin(todayMap.temp.max) : device.currentValue("todayTempMax")
    String todayTempMaxStr = todayTempMaxVal != null ? "${todayTempMaxVal}${tempUnit}" : "--"
    
    def todayTempMinVal = (todayMap && todayMap.temp?.min != null) ? convertKelvin(todayMap.temp.min) : device.currentValue("todayTempMin")
    String todayTempMinStr = todayTempMinVal != null ? "${todayTempMinVal}${tempUnit}" : "--"
    
    // --- Humidity ---
    String currentHumidityTextStr = "--"
    if (freshHum != null) {
        String hUnit = settings.humidityUnit ?: "%"
        if (hUnit == "none") hUnit = ""
        currentHumidityTextStr = "${freshHum}${hUnit}"
    } else {
        currentHumidityTextStr = device.currentValue("currentHumidityText") ?: "--"
    }
    
    // --- Wind (Utilize memory references) ---
    String windStrengthText = getBeaufortText(curWindRaw)
    String currentWindDirFullStr = device.currentValue("currentWindDirFull") ?: "unknown direction"
    
    String currentWindSpeedTextStr = ""
    if (freshWind != null) {
        String wUnit = settings.windSpeedUnit ?: "mph"
        currentWindSpeedTextStr = "${freshWind} ${wUnit}"
    } else {
        currentWindSpeedTextStr = device.currentValue("currentWindSpeedText") ?: ""
    }
    
    // --- Precipitation Probability (POP) ---
    def rawPop = (todayMap && todayMap.pop != null) ? todayMap.pop : device.currentValue("todayPOP")
    int popPct = 0
    if (rawPop != null) {
        try {
            popPct = (rawPop.toBigDecimal() * 100).intValue()
        } catch (Exception e) {
            logError "Failed to calculate POP percentage: ${e.message}"
        }
    }
    String todayPopStr = "${popPct}%"
    
    // --- Visibility ---
    String currentVisibilityTextStr = device.currentValue("currentVisibilityText") ?: "--"
    
    // --- Alerts ---
    String alertVal = device.currentValue("currentAlert") ?: "No active alerts"
    String alertSentence = (alertVal == "No active alerts" || alertVal == "N/A" || alertVal == "") ? 
        "No active alerts exist for this area." : "Active alerts for ${alertVal} exist for this area."
    
    // --- Construct Summary String ---
    String summaryText = "Weather summary for ${city} updated at ${lastUpdatedTime} on ${lastUpdatedDate}. " +
                         "${currentConditionFull.capitalize()} with a high of ${todayTempMaxStr} and a low of ${todayTempMinStr}. " +
                         "Humidity is ${currentHumidityTextStr} and the current temperature is ${currentTempStr}. " +
                         "The temperature feels like it is ${feelsLikeStr}. " +
                         "Wind: ${windStrengthText} from the ${currentWindDirFullStr} at ${currentWindSpeedTextStr}. " +
                         "There is a ${todayPopStr} chance of precipitation. " +
                         "Visibility is around ${currentVisibilityTextStr}. " +
                         "${alertSentence}"
                         
    sendIfChanged(name: "currentWeatherSummaryText", value: summaryText)
}

private String getBeaufortText(def rawMsVal) {
    if (rawMsVal == null || rawMsVal == "") return "Calm"
    
    double ms = 0.0
    try {
        ms = rawMsVal.toDouble()
    } catch (Exception e) {
        logError "Failed to parse wind speed for Beaufort conversion (${rawMsVal}): ${e.message}"
        return "Calm"
    }
    
    // Beaufort scale bounds based on m/s [source: 1]
