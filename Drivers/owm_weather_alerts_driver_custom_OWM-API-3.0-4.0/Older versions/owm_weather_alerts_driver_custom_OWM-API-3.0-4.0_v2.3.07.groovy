...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
	...
	String currentBodyHtml = "<div align='center' style='background:#888;color:#fff;border-radius:6px;padding:4px;height:100%;display:flex;flex-direction:column;justify-content:center'><div>Currently in ${cityName}</div><div><img src='https://openweathermap.org/img/wn/${icon}.png' style='width:30px;height:30px;vertical-align:middle'><b>${sT}${tUnit}</b></div><div>${cond}</div><div>H:${sH}° L:${sL}°</div><div><span>💧${hum}%</span>&nbsp;<span>${windIconDisplay}${sW}${wUnit}</span></div>"
    
    String currentTileHtml = ""
    if (settings.debugTileEnable == true) {
        String debugWrapPrefix = "<hr style='width:100%'><div align='center' style='font-size:.6em;color:#555'>Len:"
        int baseLen = currentBodyHtml.length() + debugWrapPrefix.length() + 6
        ...
        currentTileHtml = currentBodyHtml + debugWrapPrefix + "${currentLenEstimate}</div></div>"
    } else {
    ...
    if (alertActive != "No active alerts" && alertActive != "N/A" && alertActive != "") {
        ...
        if (settings.debugTileEnable == true) {
            String footerStart = "<hr style='width:100%'><div style='font-size:0.6em;color:#888;'>"
    ...
    if (settings.debugTileEnable == true) {
        String footerStart = "<hr style='width:100%'><div style='font-size:1em;color:#4accf9;z-index:12;'>"
