...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    // =========================================================================
    // SECTION 1: currentTile (Hyper-Compressed Layout - Scrollbar Fix Applied)
    // =========================================================================
    ...
    String currentBodyHtml = "<div align='center' style='color:#fff;border-radius:6px;padding:4px;height:100%;box-sizing:border-box;overflow:hidden;display:flex;flex-direction:column;justify-content:center'><div>Currently in ${cityName}</div><div><img src='https://openweathermap.org/img/wn/${icon}.png' style='width:30px;height:30px;vertical-align:middle'><b>${sT}${tUnit}</b></div><div>${cond}</div><div>H:${sH}° L:${sL}°</div><div><span>💧${hum}%</span>&nbsp;<span>${windIconDisplay}${sW}${wUnit}</span></div>"
    sendIfChanged(name: "currentTile", value: appendTileDebug(currentBodyHtml))

// =========================================================================
    // SECTION 2: 3DayForecastTile (Minimized, 4-Column Layout, EM Units, Scope Fix)
    // =========================================================================
    long epochMsNow = now()
    String day1Name = new Date(epochMsNow + 86400000L).format("EEE", location.timeZone)
    String day2Name = new Date(epochMsNow + 172800000L).format("EEE", location.timeZone)
    ...
    // Helper to render a single column in em units
    def buildCol = { label, iconUrl, desc, colHi, colLo, popVal, precipVal ->
        int popPct = (popVal.toBigDecimal() * 100).intValue()
        String precipStr = (precipVal != null && precipVal.toBigDecimal() > 0) ? "${precipVal}${pUnit}" : ""
        String popStr = popPct > 0 ? "💧${popPct}%${precipStr ? ' ' + precipStr : ''}" : "☀️0%"

        return "<div style='flex:1;display:flex;flex-direction:column;justify-content:space-between;align-items:center;padding:0 .2em;min-width:0;'><div style='font-size:.75em;font-weight:bold;'>${label}</div><img src='${iconUrl}' style='height:1.8em;width:1.8em;object-fit:contain;'><div style='font-size:.65em;text-transform:capitalize;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%;'>${desc}</div><div style='font-size:.8em;font-weight:bold;'>${colHi}°<span style='font-size:.8em;opacity:.7;font-weight:normal;'>/${colLo}°</span></div><div style='font-size:.65em;opacity:.85;'>${popStr}</div></div>"
    }

    // 1. Current Column Data
    ...
    String cDesc = currentData.weather ? currentData.weather[0].description : (device.currentValue("currentConditionTypeDesc") ?: "--")
    def cTemp    = currentData.temp != null ? convertKelvin(currentData.temp) : (device.currentValue("currentTemperature") ?: "--")
    def cFeels   = currentData.feels_like != null ? convertKelvin(currentData.feels_like) : (device.currentValue("currentFeelsLike") ?: "--")
    String cTStr = (cTemp instanceof BigDecimal) ? cTemp.setScale(0, java.math.RoundingMode.HALF_UP) : cTemp
    String cFStr = (cFeels instanceof BigDecimal) ? cFeels.setScale(0, java.math.RoundingMode.HALF_UP) : cFeels
    def cRain    = currentData.calculatedRain ?: 0.0
    def cSnow    = currentData.calculatedSnow ?: 0.0
    def cPrecip  = (cRain.toBigDecimal() > 0) ? convertPrecip(cRain) : convertPrecip(cSnow)

    // 2. Forecast Columns Data Lookup
    def getForecastColData = { label, prefix, Map sourceMap ->
        ...
        return buildCol(label, iconUrl, desc, sHi, sLo, pop, precip)
    }

    String divLine = "<div style='border-left:1px solid rgba(255,255,255,0.15);height:100%;'></div>"

    String forecastBodyHtml = "<div style='display:flex;flex-direction:row;justify-content:space-between;align-items:stretch;height:100%;padding:.4em .2em;box-sizing:border-box;color:#fff;font-family:sans-serif;line-height:1.1;text-align:center;overflow:hidden;'>" +
                          buildCol("Now", cIcon, cDesc, cTStr, cFStr, 0, cPrecip) +
                          divLine +
                          getForecastColData("Today", "today", todayData) +
                          divLine +
                          getForecastColData(day1Name, "tom", tomData) +
                          divLine +
                          getForecastColData(day2Name, "tda", tdaData) +
                          "</div>"
    ...
    if (alertActive != "No active alerts" && alertActive != "N/A" && alertActive != "") {
        ...
        alertBodyHtml = "${containerStart}<div style='font-size:0.85em;font-weight:bold;color:#5f5;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-bottom:2px;'>✅ ${cityName}<br>Clear Weather</div><div style='font-size:0.8em;color:#aaa;padding:4px 4px;'>No active weather alerts from OpenWeatherMap or their sources</div><div style='display:flex;justify-content:center;border-top:1px solid rgba(255,255,255,0.15);padding-top:4px;font-size:.6em;color:#bbb'><span>Updated ${lastPollTime}</span></div></div>"
    }
    ...
    String moonSvg = device.currentValue("todayMoonPhaseSvgImage") ?: ""
    String moonPngUrl = device.currentValue("todayMoonPhasePngImageUrl") ?: ""
