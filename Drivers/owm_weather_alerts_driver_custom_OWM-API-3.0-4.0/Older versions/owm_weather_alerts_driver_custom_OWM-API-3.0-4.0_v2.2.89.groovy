...
metadata {
    definition(name: "OpenWeatherMap Multi-Version Weather Driver 2.0", namespace: "jshimota", author: "James Shimota") {
		...
		attribute "currentMoonPhaseSvgImage", "string"
...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    if (rawMoonPhase != null && rawMoonPhase.toString().isNumber()) {
        ...
        moonSvg = device.currentValue("currentMoonPhaseSvgImage") ?: ""
...
String calcMoonPhaseSvgMask(double phase, String path) {
    if (phase <= 0.02 || phase >= 0.98) {
        return """<svg viewBox="0 0 256 256"><circle cx="128" cy="128" r="127" fill="#121214"/></svg>"""
    }
    // Set horizontal and vertical masking constraints to match your requested 127px radius
...
void calcMoonPhaseSvgImage(Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    def targets = [
        ...
        [sourceMap: todayData, apiKey: "moon_phase", valAttr: "todayMoonPhase", targetAttr: "currentMoonPhaseSvgImage"], // Map to current
