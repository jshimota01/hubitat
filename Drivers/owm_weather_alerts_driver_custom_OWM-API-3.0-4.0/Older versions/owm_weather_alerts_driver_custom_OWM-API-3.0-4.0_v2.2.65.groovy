...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    // Exact dynamic character calculation and rendering to fit inside the end div
    ...
    String currentTileHtml = currentBodyHtml + "<div style='font-size:0.75em;color:#777;margin-top:3px;'>Char length: ${currentLenEstimate}</div></div>"
	...
	// Exact dynamic character calculation and rendering to fit inside the end footer span
    String forecastSuffixTemplate = "<span style='font-size:0.6em;color:#888;'>Char length: 0000</span></div></div>"
    int forecastLenEstimate = forecastBodyHtml.length() + forecastSuffixTemplate.length()
    String forecastHtml = forecastBodyHtml + "<span style='font-size:0.6em;color:#888;'>Char length: ${forecastLenEstimate}</span></div></div>"
...
private void calcAlertsState(Map json, String calculatedCityAttr, String iconBasePath) {
    ...
    if (alerts.size() > 0) {
        ...
        // Exact character calculation including dynamic suffix inside the wrapper
        String alertSuffixTemplate = "Char length: 0000</div></div>"
        int alertLenEstimate = noAlertBody.length() + alertSuffixTemplate.length()
        currentAlertTile = noAlertBody + "Char length: ${alertLenEstimate}</div></div>"
    }
