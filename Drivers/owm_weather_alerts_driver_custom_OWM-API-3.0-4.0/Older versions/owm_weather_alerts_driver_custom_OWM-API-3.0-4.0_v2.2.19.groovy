...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    // =========================================================================
    // SECTION 1: currentTile (Optimized & Shortened)
    // =========================================================================
    ...
    // Minified HTML structure to radically drop character payload weights
    String currentBodyHtml = "<div style='display:flex;flex-direction:column;justify-content:space-between;height:100%;padding:6px;box-sizing:border-box;background:rgba(30,30,40,.65);border-radius:12px;color:#fff;font:11px sans-serif;line-height:1.2;text-align:center'><div style='font-size:1.1em;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'>${cityName} - currently</div><div style='display:flex;align-items:center;justify-content:space-around;width:100%'><img src='https://openweathermap.org/img/wn/${icon}@2x.png' style='width:35%;max-height:55px;object-fit:contain'><div style='font-size:2.2em;font-weight:700;letter-spacing:-1px'>${temp}${tUnit}</div></div><div style='color:#ddd;font-weight:600;text-transform:capitalize;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'>${cond}</div><div style='color:#aaa;display:flex;justify-content:center;gap:10px'><span>H: ${hi}°</span><span>L: ${lo}°</span></div><div style='display:flex;justify-content:space-between;border-top:1px solid rgba(255,255,255,.15);padding-top:4px;color:#bbb;align-items:center;'><span>💧 ${hum}%</span><span style='display:flex;align-items:center;'>${windIconDisplay}${wind} ${wUnit}</span></div>"
    
    int currentLenEstimate = currentBodyHtml.length() + "<div style='font-size:.8em;color:#888;margin-top:2px;'>999 chars long</div></div>".length()
    String currentTileHtml = currentBodyHtml + "<div style='font-size:.8em;color:#888;margin-top:2px;'>${currentLenEstimate} chars long</div></div>"
...
private void sendOWMData(Map current, Map today, Map tom, Map tda) {
	...
	calcCurrentText(freshLux, freshTemp, freshPress, freshWind, freshHum)
	calcMoonPhaseIcon()
...
void calcMoonPhaseIcon() {
    ...
    // Define the list of phase attributes to iterate through
    def phases = [
        [valAttr: "todayMoonPhase", iconAttr: "todayMoonPhaseIcon"],
        [valAttr: "tomMoonPhase",   iconAttr: "tomMoonPhaseIcon"],
        [valAttr: "tdaMoonPhase",   iconAttr: "tdaMoonPhaseIcon"]
    ]
    
    phases.each { phase ->
        def rawVal = device.currentValue(phase.valAttr)
        if (rawVal != null) {
            ...
            // Map 0.0-1.0 into 8 distinct bins (0 to 7)
            // An offset of 0.5/8 ensures values close to 0.5 align with index 4 (mp5.png)
            ...
            logDebug "Skipping icon calculation for ${phase.valAttr}: Value is null."
        }
    }
}
...
private void calcCurrentText(BigDecimal freshLux = null, BigDecimal freshTemp = null, BigDecimal freshPress = null, BigDecimal freshWind = null, BigDecimal freshHum = null) {
