...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    if (alertActive != "No active alerts" && alertActive != "N/A" && alertActive != "") {
        alertBodyHtml = "<div><div>⚠️ ${alertActive}&nbsp;<br>&nbsp;</div><div style='font-size:75%;'>${alertDesc}</div><div><span>👤 ${alertSender.take(12)}</span><span>⏰ ${lastPollTime}</span></div>"
    } else {
        alertBodyHtml = "<div><div>✅ ${cityName}</br>Clear Weather</div><div>No active weather alerts from OpenWeatherMap or their sources</div><div><span>Updated ${lastPollTime}</span></div>"
    }
