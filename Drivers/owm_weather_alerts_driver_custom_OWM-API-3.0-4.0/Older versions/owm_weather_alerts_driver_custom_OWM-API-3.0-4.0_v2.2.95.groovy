...
def initialize() {
    ...
    // Initialize MoonPhaseImagePath fallback path safely as a persistent state
    state.moonPhaseImagePath = calcMoonPhaseImagePath(settings.altMoonPhaseImagePath)
...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    // Refactored to map text directly into an absolute container sharing the graphic box space
    String moonBodyHtml = "<div style='display:flex;flex-direction:column;justify-content:center;align-items:center;height:100%;padding:8px;box-sizing:border-box;background:rgba(30,30,40,0.65);border-radius:6px;color:#fff;font-family:sans-serif;text-align:center;overflow:hidden;'>" +
...
private Map convertWindDirectionState(degrees) {
    ...
    // Wind directional matching emojis mapping sequentially alongside the 16 compass headings
    ...
    String finalIconUrl = ""
    String basePath = state.windDirectionImagePath ?: "None"

    if (basePath != "None") {
        // Option A: If a preference path is specified, stitch together the full image asset URL filename
        String filename = "wind-${token.toLowerCase()}.png"
        finalIconUrl = "${basePath}${filename}"
    } else {
        // Option B: If empty, default back to the corresponding directional wind emoji symbol
        finalIconUrl = directionEmojis[index]
    }
...
private String calcMoonPhaseImagePath(String altMPLoc) {
    ...
    String base = altMPLoc ? altLocMP.trim() : "https://raw.githubusercontent.com/thebearmay/hubitat/main/moonPhaseRes/"
    if (!base.endsWith("/")) base += "/"
    logDebug "Calculated Moon Phase Image base path resolved to: ${base}"
    return base
}

private String calcWinDirImagePath(String altWindDirectionImageLoc) {
    // Validate user preference override string, fall back to 'None' if empty
    String base = altWindDirectionImageLoc ? altWindDirectionImageLoc.trim() : "None"
