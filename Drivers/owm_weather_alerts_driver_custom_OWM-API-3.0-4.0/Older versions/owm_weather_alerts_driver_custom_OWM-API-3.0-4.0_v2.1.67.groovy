...
private void sendOWMData(Map current, Map today, Map tom, Map tda) {
    ...
    if (current) {
		...
		// Text string variants honoring the user selected precipUnit format
       String pUnit = settings.precipUnit ?: "inHr"
       sendIfChanged(name: "currentRainText", value: "${current.calculatedRain != null ? current.calculatedRain : 0} ${pUnit}")
       sendIfChanged(name: "currentSnowText", value: "${current.calculatedSnow != null ? current.calculatedSnow : 0} ${pUnit}")
