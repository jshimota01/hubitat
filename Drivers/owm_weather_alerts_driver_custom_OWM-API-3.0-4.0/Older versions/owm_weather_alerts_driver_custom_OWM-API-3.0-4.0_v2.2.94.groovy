...
metadata {
    definition(name: "OpenWeatherMap Multi-Version Weather Driver 2.0", namespace: "jshimota", author: "James Shimota") {
        ...
        attribute "currentWindDirImageUrul", "string"
        ...
        attribute "todayWindDirImageUrul", "string"
        attribute "tomWindDirImageUrul", "string"
        attribute "tdaWindDirImageUrul", "string"
...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    if (settings.altIconsEnable == true) {
        String windIconUrl = device.currentValue("currentWindDirImageUrul") ?: ""
        if (windIconUrl != "") {
...
private void sendOWMData(Map current, Map today, Map tom, Map tda) {
    ...
    if (current) {
        ...
        if (current.wind_deg != null) {
            ...
            sendIfChanged(name: "currentWindDirImageUrul", value: wDir.iconUrl)
        }       
    ...
    forecastDays.each { day ->
        ...
        if (data) {
            ...
            if (data.wind_deg != null) {
                ...
                sendIfChanged(name: "${prefix}WindDirImageUrul", value: wDir.iconUrl)
            }
