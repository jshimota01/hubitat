/* OpenWeatherMap Weather-Alerts Driver (Custom OWM API 3.0/4.0))                    
	...
	V1.0.16		06/27/2026		JAS		Added initialize and configure commands
	V1.0.15		06/27/2026		JAS		Icon Location - another fix
	V1.0.14		06/27/2026		JAS		Added icon location attribute - it was broken. fixed duplicate tZ
	V1.0.13		06/26/2026		JAS		Added pollOWMl repairs for hub long/lat
	V1.0.12		06/26/2026		JAS		Added pollOWM repairs to use hub better
	...
	V1.0.4		06/25/2026		JAS		Bug fixes - equality issues = should be == in two places, cleaned bracket and alerts loop memory leak
...
metadata {
	definition (name: 'OpenWeatherMap Weather-Alerts Driver (Custom OWM API 3.0/4.0)',
		namespace: 'jshimota',
...
//icon storage
       attribute 'DEFAULT_ICON_LOCATION', sSTR
...
//	https://tinyurl.com/icnqz/ points to https://raw.githubusercontent.com/HubitatCommunity/WeatherIcons/master/
 // https://raw.githubusercontent.com/jshimota01/hubitat/main/Drivers/owm_weather_alerts_driver_custom_OWM-API-3.0-4.0/owm-icons/
			// input 'iconLocation', 'text', defaultValue: 'https://tinyurl.com/icnqz/', title: 'Alternative Icon Location:<br><i>blank for default location</i>',  required: false
			...
			if (altCoord) {
				input 'altLat', 'text', title: 'Latitude Override (Optional)', description: 'Leave blank to use Hub Location Latitude', required: false
				input 'altLon', 'text', title: 'Longitude Override (Optional)', description: 'Leave blank to use Hub Location Longitude', required: false
				// input 'altLat', sSTR, title: 'Override location Latitude', required: false, defaultValue: location?.latitude ? location.latitude.toString() : "0.0", description: '<br>Enter location Latitude<br>'
				//input 'altLon', sSTR, title: 'Override location Longitude', required: false, defaultValue: location?.longitude ? location.longitude.toString() : "0.0", description: '<br>Enter location Longitude<br>'
			}
...
void pollOWM() {
    // 1. Get override or fallback safely
    ...
    // 2. Prevent empty API submissions if everything is missing
    if (!currentLat || !currentLon) {
        log.error "Coordinates are completely empty! Please assign values in Device Preferences or your Hub Location configurations."
    ...
    // 3. Build the URI params using the validated 'currentLat' and 'currentLon' strings
    Map ParamsOWM = [
        uri: 'https://api.openweathermap.org/data/' + (apiVer==true ? '4.0' : '3.0') + '/onecall?lat=' + currentLat + '&lon=' + currentLon + '&exclude=minutely,hourly&mode=json&units=imperial&appid=' + (String)apiKey,
        timeout: 20
    ]
...
void pollOWMHandler(resp, data) {
	// GUARD CLAUSE: Stop immediately if the API request didn't return a valid payload
	if (resp == null || resp.getStatus() != 200) {
		String errMsg = resp ? "HTTP Status ${resp.getStatus()}" : "Response object is null"
        log.error "OpenWeatherMap API Error: ${errMsg}. Please verify your API Key and coordinates."
        sendEvent(name: "status", value: "API Error")
        return
    }
	// Fix for the NullPointerException: Extract JSON data safely
    def owmNPE = resp.getJson()
    if (owmNPE == null) {
        log.error "OpenWeatherMap API returned empty or invalid JSON data payload."
        return
    }
	...
	// Change this line inside pollOWMHandler if it fails:
		LOGWARN('Calling https://api.openweathermap.org/data/' + (apiVer==true ? '4.0' : '3.0') + '/onecall?lat=' + (settings.altLat ?: location.latitude?.toString()) + '&lon=' + (settings.altLon ?: location.longitude?.toString()) + '&exclude=minutely,hourly&mode=json&units=imperial&appid=' + (String)apiKey)
		...
		if(owm.toString()==sNULL) {
            sendEvent(name: 'status', value: 'Empty Response')
            sendEvent(name: 'lastChecked', value: timestamp)
			pauseExecution(1000)
			pollOWM()
			return
...
// Track valid active connection status
        sendEvent(name: 'status', value: 'Online')
        sendEvent(name: 'lastChecked', value: timestamp)
		
// ... rest of processing code down to PostPoll() ...
...
// >>>>>>>>>> End Process Standard Weather-Station Variables (Regardless of Forecast Selection)  <<<<<<<<<<
	    ...
	    if (tma >= 0.8125 && tma < 0.9375) {mPhase = 'Waning Crescent'}
...
//			String tmpImg0= myGetData(sICON) + getImgName((!owmDaily[0].weather[0].id ? 999 : owmDaily[0].weather[0].id.toInteger()), sTRU) + imgT1
		...
		if(cloudExtendedPublish){
			...
			device.deleteCurrentState('cloudDayAfterTomorrow')
		}
...
//	String altLon = "-68.735892" //"-81.6934446" // "-75.43" //"-90.199402" //-88.0398912"
			...
			if((alertSource==sZERO) || (!owm.alerts && alertSource==sONE) || (myGetData('curAl')==sNCWA && alertSource==sTWO)) {
				...
				// HOISTED STATE: Declared safely above all logical branches to ensure availability
				Integer alertCnt = 0
				...
				if(alertSource==sONE) {
					if(curAl==sNCWA) {
						...
						// Safely evaluate prior NWS configurations before committing modifications
...
/*  'Required for Dashboards' Data Elements */
    ...
    // added as addon from comment in Community 6-10-26
    def rainTodayPublish = true
...
/*  Selected optional Data Elements */
	...
	if((alertSource==sZERO) || (alertSource==sONE) || (myGetData('curAl')==sNCWA && alertSource==sTWO)) {
		...
		OWMIcon = '<a href="https://forecast.weather.gov/MapClick.php?lat=' + altLat + '&lon=' + altLon + '" target="_blank">' + sIMGS5 + myGetData(sICON) + 'NWS_240px.png style="height:2em"></a> @ ' + myGetData(sSUMLST)
		OWMText = '<a href="https://forecast.weather.gov/MapClick.php?lat=' + altLat + '&lon=' + altLon + '" target="_blank">Weather.gov</a> @ ' + myGetData(sSUMLST)
	}
//  <<<<<<<<<< Begin Built 3dayfcstTile >>>>>>>>>>
	if(threedayTilePublish) {
		...
		// Changed as to make tile meet my needs JAS
		// my3day = '<style type="text/css">.cI{height:45%}.cIb{height:80%}</style>'
		// my3day += '<table style="text-align:center;display:inline">'
		
		my3day = '<style type="text/css">.cI{width:55%}.cIb{height:80%}tr:nth-child(3){vertical-align:bottom}tr:first-child{font-size:.6rem}td:first-child{font-size:.5rem}tr:last-child{font-size:.4rem}</style>'
		my3day += '<table style="text-align:center;display:inline-table;width: 100%;height: 92%;font-size:.6rem">'
		my3day += sTR
		...
		// changed to meet my needs JAS
		// my3day += '<tr style="font-size:85%">' + '<td  colspan="4">'

		my3day += '<tr>' + '<td colspan="4" style="font-size: 6px;">'
		my3day += 'HTML length = ' + (my3day.length() + OWMIcon.length()+13) + ' chrs'

		// commented out to shorten overall html char count JAS
		...
		// orig if((my3day.length() + OWMIcon.length()+8) < 1025) {
		// orig 	my3day += OWMIcon
		// orig }else if((my3day.length() + OWMText.length()+8) < 1025) {
		// orig 	my3day += OWMText
		// orig } else {// orig 
		// orig 	my3day += 'OpenWeatherMap.org'
		// orig }
		my3day += '</td></tr></table>'
...
//  >>>>>>>>>> End Build 3dayfcstTile <<<<<<<<<<
//  >>>>>>>>>> Start Build Weather Summary text <<<<<<<<<<
...
//  >>>>>>>>>> End Build Weather Summary text <<<<<<<<<<
...
void refresh() {
	updateLux(true)
	pollOWM()
	LOGINFO("Refresh command executed()")
 }
...
void initialize() {
    ...
    initMe() // Directs to your existing setup logic
}

void configure() {
    LOGINFO("Configuring device updates...")
    updated() // Directs to your existing schedule logic
}
...
void uninstalled() {
    String mc = device.id.toString()
    dataStoreFLD.remove(mc)
    verFLD.remove(mc)
    device.deleteCurrentState('status')
    device.deleteCurrentState('lastChecked')
    LOGINFO("Cleaned global memory footprints for device ${mc}")
}
...
void updated(){
	...
	String mc=device.id.toString()
	verFLD[mc]=version()
	if (!settings.altLat && location.latitude) {
        device.updateSetting('altLat', [value: location.latitude.toString(), type: 'string'])
    }
    if (!settings.altLon && location.longitude) {
        device.updateSetting('altLon', [value: location.longitude.toString(), type: 'string'])
    }
	initMe()
	updateCheck()
	runIn(5,finishSched)
...
void pollOWMl() {
    ...
    // 1. Establish robust local variables using the hub location fallback logic
    ...
    // 2. Safely abort if neither override nor hub coordinates are set anywhere
    if (!activeLat || !activeLon) {
        log.error "OWM Poll Aborted: Latitude and Longitude coordinates could not be resolved!"
        sendEvent(name: "status", value: "Config Error")
        return
    }

    // 3. Construct the network payload utilizing the safe local strings
Map ParamsOWM = [
        uri: 'https://api.openweathermap.org/data/' + (apiVer==true ? '4.0' : '3.0') + 
             '/onecall?lat=' + activeLat + '&lon=' + activeLon + 
             '&exclude=minutely,hourly&mode=json&units=imperial&appid=' + (String)settings.apiKey,
        requestContentType: 'application/json',
        contentType: 'application/json',
        timeout: 20
    ]
...
@Field final Map<String,Map> attributesMap = [
		'city':						[t: 'Display City', d: 'Display City name?', ty: sBLK, defa: sFLS],
		'threedayTile':				[t: 'Three Day Forecast Tile', d: 'Display Three Day Forecast Tile?', ty: sSTR, defa: sFLS],
		'alert':					[t: 'Weather Alert', d: 'Display any weather alert?', ty: sBLK, defa: sFLS],
		...
		'cloudExtended':			[t: 'Cloud Forecast', d: 'Display cloud coverage forecast?', ty: sBLK, defa: sFLS],
		...
		'dashHubitatOWM':			[t: 'Dash - Hubitat and OpenWeatherMap', d: 'Display attributes required by Hubitat and OpenWeatherMap dashboards?', ty: sBLK, defa: sFLS],
		'dashSmartTiles':			[t: 'Dash - SmartTiles', d: 'Display attributes required by SmartTiles dashboards?', ty: sBLK, defa: sFLS],
		'dashSharpTools':			[t: 'Dash - SharpTools.io', d: 'Display attributes required by SharpTools.io?', ty: sBLK, defa: sFLS],
		'dewpoint':					[t: 'Dewpoint (in default unit)', d: 'Display the dewpoint?', ty: sNUM, defa: sFLS],
		'fcstHighLow':				[t: 'Forecast High/Low Temperatures:', d: 'Display forecast High/Low temperatures?', ty: sBLK, defa: sFLS],
		...
		'localSunrise':				[t: 'Local SunRise and SunSet', d: 'Display the Group of Time of Local Sunrise and Sunset, with and without Dashboard text?', ty: sBLK, defa: sFLS],
		...
		'precipExtended':			[t: 'Precipitation Forecast', d: 'Display precipitation forecast?', ty: sBLK, defa: sFLS],
		'obspoll':					[t: 'Observation time', d: 'Display Observation and Poll times?', ty: sBLK, defa: sFLS],
		'suncalc':					[t: 'Sun calculations', d: 'Display Altitude and Azuimuth of Sun?', ty: sBLK, defa: sFLS],
...
void updateCheckHandler(resp, data) {
	...
	if (resp.getStatus() == 200 || resp.getStatus() == 207) {
		...
		String Ver = respUD.driver?."${state.InternalName}"?.ver
