...
metadata {
    definition(name: "OpenWeatherMap Multi-Version Weather Driver 2.0", namespace: "jshimota", author: "James Shimota") {
		...
		attribute "todayMoonPhaseTile", "string"
    ...
    preferences {
        ...
        input name: "altWindDirectionImageLoc", type: "text", title: "Base Override - Wind Direction Image Location", description: "Optional - Wind Direction Image Source Location:<br><i>If not used, Wind Direction ioons are used", required: false
...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    // =========================================================================
    // SECTION 1: currentTile (Hyper-Compressed Layout)
    // =========================================================================
    ...
    if (settings.altIconsEnable == true) {
        ...
        if (windIconUrl != "") {
            // Compressed style payload for the wind image tag
            windIconDisplay = "<img src='${windIconUrl}' style='height:1em;vertical-align:middle;margin-right:2px'>"
        }
    }

    // Dynamic numeric formatting safely shortens down long decimal floats to fit smaller space strings
    ...
    // Hyper-compressed HTML template using CSS variables and flex-flow shorthands
    String currentBodyHtml = "<div style='box-sizing:border-box;height:100%;padding:4px;border-radius:6px;background:#666;color:#fff;font:1.1/1.1 sans-serif;text-align:center;display:flex;flex-flow:column;justify-content:space-between;--o:hidden;--t:ellipsis;--w:nowrap'><div style='font-weight:bold;font-size:.9em;overflow:var(--o);text-overflow:var(--t);white-space:var(--w)'>${cityName}</div><div style='display:flex;place-content:center;align-items:center;gap:2px;margin:-4px 0'><img src='https://openweathermap.org/img/wn/${icon}.png' style='width:30px;height:30px'><b style='font-size:1.5em'>${sT}${tUnit}</b></div><div style='color:#ddd;font-size:.8em;text-transform:capitalize;overflow:var(--o);text-overflow:var(--t);white-space:var(--w)'>${cond}</div><div style='color:#bbb;font-size:.8em'>H:${sH}° L:${sL}°</div><div style='display:flex;justify-content:space-between;border-top:1px solid rgba(255,255,255,.15);padding-top:2px;font-size:.75em;color:#bbb'><span>💧${hum}%</span><span>${windIconDisplay}${sW}${wUnit}</span></div>"
    
    String currentTileHtml = ""
    if (settings.debugTileEnable == true) {
        // Calculate exact length footprint by matching the final appended format string length
        int baseLen = currentBodyHtml.length() + 45 // Length of static debug wrap characters without the number
        int currentLenEstimate = baseLen + String.valueOf(baseLen).length()
        // If string length boundary jumps a digit (e.g. 99 to 100), adjust estimate up by 1
        if(String.valueOf(currentLenEstimate).length() > String.valueOf(baseLen).length()) { currentLenEstimate++ }
        
        currentTileHtml = currentBodyHtml + "<div style='font-size:.6em;color:#777'>Len:${currentLenEstimate}</div></div>"
    } else {
    ...
    String forecastBodyHtml = "<div style='display:flex; flex-direction:column; justify-content:space-between; height:100%; padding:10px 6px; box-sizing:border-box; background:#666; border-radius:6px; color:#fff; font-family:sans-serif; line-height:1.2; text-align:center;'><div style='display:flex; flex-direction:row; justify-content:space-between; gap:4px; flex-grow:1;'>" +
    ...
    String baseStyle = "display:flex;flex-direction:column;justify-content:space-between;height:100%;padding:8px;box-sizing:border-box;background:#666;border-radius:6px;color:#fff;font-family:sans-serif;line-height:1.2;text-align:center"
    ...
    // =========================================================================
    // SECTION 4: todayMoonPhaseTile (Centered Overlay Layout Optimization)
    // =========================================================================
    ...
    if (rawMoonPhase != null && rawMoonPhase.toString().isNumber()) {
        ...
        moonPhaseText = device.currentValue("todayMoonPhaseText") ?: "Unknown"
        moonSvg = device.currentValue("todayMoonPhaseSvgImage") ?: ""
        moonPngUrl = device.currentValue("todayMoonPhasePngImageUrl") ?: ""
    }
    ...
    String moonBodyHtml = "<div style='display:flex;flex-direction:column;justify-content:center;align-items:center;height:100%;padding:8px;box-sizing:border-box;background:#666;border-radius:6px;color:#fff;font-family:sans-serif;text-align:center;overflow:hidden;'>" +
                          "<div style='position:relative;display:flex;align-items:center;justify-content:center;width:100%;height:100%;min-height:0;'>" +
                          graphicDisplay +
                          "<div style='position:absolute;width:100%;left:0;right:0;bottom:45%;font-size:2em;font-weight:bold;-webkit-text-stroke:1px #000;text-shadow:2px 2px 2px #000;z-index:10;pointer-events:none;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'>${moonPhaseText}</div>" +
                          "</div>"

    String todayMoonPhaseTile = ""
    if (settings.debugTileEnable == true) {
        String footerStart = "<div style='font-size:1em;color:#4accf9;margin-top:2px;z-index:12;'>"
        ...
        todayMoonPhaseTile = moonBodyHtml + footerStart + "Char length: ${moonLenEstimate}</div></div>"
    } else {
        todayMoonPhaseTile = moonBodyHtml + "</div>"
    }
    sendIfChanged(name: "todayMoonPhaseTile", value: todayMoonPhaseTile)
}
...
private Map convertWindDirectionState(degrees) {
    if (degrees == null) {
        ...
        return [cardinal: "Unknown", full: "Unknown", iconUrl: "None entered", icon: "💨"]
    }
    double deg = 0.0
    try {
    ...
    // Icon ALWAYS displays the actual direction emoji text asset
    String finalIcon = directionEmojis[index]

    // Check if the preference field itself is filled out only to build the alternative URL asset state 
    if (settings.altWindDirectionImageLoc && settings.altWindDirectionImageLoc.trim() != "") {
        String cleanedPath = settings.altWindDirectionImageLoc.trim()
        if (!cleanedPath.endsWith("/")) {
            cleanedPath += "/"
        }
        String filename = "wind-${token.toLowerCase()}.png"
        finalIconUrl = "${cleanedPath}${filename}"
    } else {
        finalIconUrl = "None entered"
    }
    
    return [cardinal: token, full: word, iconUrl: finalIconUrl, icon: finalIcon]
}
...
private String calcWinDirImagePath(String altWDLoc) {
    // Validate user preference override string, fall back to 'None' if empty
    String base = altWDLoc ? altWDLoc.trim() : "None"
    
    // Enforce trailing slash constraint if a valid path was provided
    if (base != "None" && !base.endsWith("/")) {
        base += "/"
    }
