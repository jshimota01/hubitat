...
void generateTiles(Map currentData = [:], Map todayData = [:], Map tomData = [:], Map tdaData = [:]) {
    ...
    String forecastHtml = ""
    if (settings.debugTileEnable == true) {
        String forecastDivider = "<hr style='width:100%'><div>"
...
// =========================================================================
    ...
    if (alertActive != "No active alerts" && alertActive != "N/A" && alertActive != "") {
        String alertBody = "<div align='center' style='background:#888;color:#fff;border-radius:6px;padding:4px;height:100%;display:flex;flex-direction:column;justify-content:center'><div>⚠️ ${alertActive}<br>&nbsp;</div><div style='font-size:75%;'>${alertDesc}</div><div><span>👤 ${alertSender.take(12)}</span><span>⏰ ${lastPollTime}</span></div>"
        
        if (settings.debugTileEnable == true) {
            String footerStart = "<hr style='width:100%'><div>"
        ...
        String noAlertBody = "<div align='center' style='background:#888;color:#fff;border-radius:6px;padding:4px;height:100%;display:flex;flex-direction:column;justify-content:center'><div>✅ ${cityName}</br>Clear Weather<br>&nbsp;</div><div>No active weather alerts from OpenWeatherMap or their sources</div><div><span>Updated ${lastPollTime}</span></div>"
