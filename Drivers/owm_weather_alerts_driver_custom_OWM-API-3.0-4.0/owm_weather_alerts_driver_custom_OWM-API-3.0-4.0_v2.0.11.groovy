...
metadata {
    definition(name: "OpenWeatherMap Multi-Version Weather Driver", namespace: "jshimota", author: "James Shimota") {
        ...
        attribute "cloudCurrent", "string"
        attribute "cloudToday", "string"
        attribute "cloudTomorrow", "string"
        attribute "cloudDayAfterTomorrow", "string"
...
private def parsePayload(Map json) {
    ...
    def iconNameValue = iconCode ? iconCode.toString() : ""
    
    // Determine base URL path for the current condition tile
    String iconBasePath = (altIconLoc && altIconLoc.trim() != "") ? altIconLoc.trim() : "https://openweathermap.org/img/wn/"
    ...
    // Wraps the icon in an HTML img tag to force rendering as a dashboard image tile
    def formattedConditionIcon = iconCode ? "<img src='https://openweathermap.org/img/wn/${iconCode}.png' style='max-width:50px; max-height:50px;'/>" : "N/A"
    def clouds = current.clouds ?: 0
    ...
    sendEvent(name: "cloudCurrent", value: clouds, unit: " %")
    sendEvent(name: "cloudToday", value: (daily.size() > 0 ? daily[0].clouds ?: 0 : 0), unit: " %")
    sendEvent(name: "cloudTomorrow", value: cloudTom, unit: " %")
    sendEvent(name: "cloudDayAfterTomorrow", value: cloudDat, unit: " %")
