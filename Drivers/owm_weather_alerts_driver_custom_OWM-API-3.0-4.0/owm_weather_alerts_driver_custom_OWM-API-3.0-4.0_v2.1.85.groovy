...
private void pollOWMAPI() {
	...
	} catch (hubitat.execution.HttpResponseException e) {
		logError "HTTP Error: ${e.statusCode} - ${e.response?.data}"
		sendIfChanged(name: "lastResponseCode", value: e.statusCode.toString())
	} catch (Exception e) {
		logError "Generic Exception during OWM API Call: ${e.message}"
	}
}
