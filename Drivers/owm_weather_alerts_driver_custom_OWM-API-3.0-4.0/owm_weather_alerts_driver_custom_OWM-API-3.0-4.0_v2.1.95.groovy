...
private BigDecimal convertHumidity(BigDecimal rawHumidity) {
    if (rawHumidity == null) return 0.0
    ...
    // Safely round to 2 decimal places and return as a BigDecimal
    return BigDecimal.valueOf(calculatedValue).setScale(2, java.math.RoundingMode.HALF_UP)
}
...
private void calcCurrentText(BigDecimal freshLux = null) { // <-- Accept parameter with null fallback
    ...
    if (humVal != null) {
        sendIfChanged(name: "currentHumidityText", value: "${humVal}°")
    }
}
