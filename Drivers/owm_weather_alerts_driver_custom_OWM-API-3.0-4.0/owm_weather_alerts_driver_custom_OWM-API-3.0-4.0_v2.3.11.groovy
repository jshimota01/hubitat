...
// =========================================================================
    ...
    if (alertActive != "No active alerts" && alertActive != "N/A" && alertActive != "") {
        String alertBody = "<div style='flex:1; display:flex; flex-direction:column; justify-content:space-between; align-items:center; padding:0 4px; min-width:0;'><div>⚠️ ${alertActive}</div><div>${alertDesc}</div><div><span>👤 ${alertSender.take(12)}</span><span>⏰ ${lastPollTime}</span></div>"
        
        if (settings.debugTileEnable == true) {
            String footerStart = "<hr><div>"
        ...
        String noAlertBody = "<div style='flex:1; display:flex; flex-direction:column; justify-content:space-between; align-items:center; padding:0 4px; min-width:0;'><div>✅ ${cityName}</br>Clear Weather</div><div>No active weather alerts from OpenWeatherMap or their sources</div><div><span>Updated ${lastPollTime}</span></div>"
