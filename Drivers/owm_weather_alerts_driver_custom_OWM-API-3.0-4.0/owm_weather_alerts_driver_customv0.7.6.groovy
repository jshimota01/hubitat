 ...
 * 06-23-2026		v0.7.4  jshimota	Stripped v2.5 and v3.0 due to API change at vendor
 * 06-20-2026		v0.7.3b	jshimota	Gemini modernization
*/

static String version()    {  return '0.7.6'  }
import groovy.transform.Field

// Define the static map BEFORE metadata so the parser can see it
@Field static final Map attributesMap = [
    ...
    // If you have more attributes inside your original attributesMap, add them here
]
...
void pollOWMData() {
    ...
    Map ParamsOWM
    ParamsOWM = [ uri: 'https://api.openweathermap.org/data/4.0/onecall?lat=' + (String)altLat + '&lon=' + (String)altLon + '&exclude=minutely,hourly&mode=json&units=imperial&appid=' + (String)apiKey, timeout: 20 ]
...
void pollOWMHandler(resp, data) {
    ...
    if(resp.getStatus() != 200 && resp.getStatus() != 207) {
        ...
        LOGWARN('Calling https://api.openweathermap.org/data/4.0/onecall?lat=' + (String)altLat + '&lon=' + (String)altLon + '&exclude=minutely,hourly&mode=json&units=imperial&appid=' + (String)apiKey)
        ...
        if(owm.toString()==sNULL) {
            ...
            pauseExecution(1000)
            pollOWMData() // <-- FIXED: Now correctly references the updated method name
            return
        }
