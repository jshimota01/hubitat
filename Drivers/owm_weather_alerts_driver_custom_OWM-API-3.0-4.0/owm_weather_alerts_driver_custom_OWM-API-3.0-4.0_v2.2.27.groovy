...
private void calcCurrentText(BigDecimal freshLux = null, BigDecimal freshTemp = null, BigDecimal freshPress = null, BigDecimal freshWind = null, BigDecimal freshHum = null) {
    ...
    // 7. Twilight Text Formatting (Explicit Fix)
    String chosenTimeForm = settings.TimeForm ?: "1"
    
    def twiBegin = device.currentValue("currentTwilightBeginTime")
    if (twiBegin != null) {
        if (chosenTimeForm == "10") {
            sendIfChanged(name: "currentTwilightBeginTimeText", value: "${twiBegin}")
        } else {
            Map formattedMap = convertDateTimeFormat(twiBegin, chosenTimeForm)
            sendIfChanged(name: "currentTwilightBeginTimeText", value: formattedMap.time)
        }
    }

    def twiEnd = device.currentValue("currentTwilightEndTime")
    if (twiEnd != null) {
        if (chosenTimeForm == "10") {
            sendIfChanged(name: "currentTwilightEndTimeText", value: "${twiEnd}")
        } else {
            Map formattedMap = convertDateTimeFormat(twiEnd, chosenTimeForm)
            sendIfChanged(name: "currentTwilightEndTimeText", value: formattedMap.time)
        }
    }

    // 8 DateTime Attributes Auto-Formatting
    logDebug "Section 7: Formatting all properties containing 'DateTime' via convertDateTimeFormat..."
    ...
    logDebug "Section 8: Formatting all properties ending in 'Date' via convertDateTimeFormat..."
    ...
    logDebug "Section 9: Formatting all properties ending in 'Time' via convertDateTimeFormat..."
    
    device.properties.supportedAttributes.each { attr ->
        String attrName = attr.name
        // Filter out variables that don't conform to standard epoch numeric conversions
        if (attrName.endsWith("Time") && !attrName.contains("DateTime") && attrName != "currentSolarNoonTime" && !attrName.contains("Twilight")) {
            def epochValue = device.currentValue(attrName)
            if (epochValue != null) {
                String targetTextAttribute = "${attrName}Text"
