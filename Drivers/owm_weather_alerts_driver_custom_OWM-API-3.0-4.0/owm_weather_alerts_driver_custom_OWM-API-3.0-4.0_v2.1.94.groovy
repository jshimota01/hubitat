...
metadata {
    definition(name: "OpenWeatherMap Multi-Version Weather Driver 2.0", namespace: "jshimota", author: "James Shimota") {
		...
		attribute "currentWindDirectionText", "string"
		...
		attribute "currentWindDirectionText", "string"
		attribute "todayWindDirectionText", "string"
		attribute "tomWindDirectionText", "string"
		attribute "tdaWindDirectionText", "string"
    ...
    preferences {
		...
		input name: "humidityUnit", type: "enum", title: "Display Unit - Humidity", options: ["%": "Percent (%)", "%RH": "Percent RH (%RH)", "g/m³": "Absolute Hunidity (g/m³)", "g/kg": "Mixing Ratio (g/kg³)", "none": "None (No Unit Suffix)"], description: "Choice of humidity unit formatting used in tiles and logging<br>Default: <b>Humidity ($RH)</b>", defaultValue: "%RH", required: true
...
def updated() {
    ...
    sendIfChanged(name: "humidityUnit", value: settings.pressureUnit ?: "%RH")
...
// start currentTile 
    ...
    // String futureTileHtml = "..."
    // sendIfChanged(name: "futureTile", value: futureTileHtml)
...
private void sendOWMData(Map current, Map today, Map tom, Map tda) {
    ...
    //  EXAMPLE sendIfChanged(name: "currentIlluminanceText", value: "${luxVal} ${iUnit}")
        ...
        if (current.wind_gust != null) {
         ...
         if (current.wind_deg != null) {
            sendIfChanged(name: "currentWindDirectionText", value: convertWindDirectionState(current.wind_deg))
        }       
    ...
    if (today) {
        ...
        if (today.humidity != null) {
            sendIfChanged(name: "todayHumidity", value: today.humidity)
        }
       ...
       if (today.wind_gust != null) {
        ...
        if (today.wind_speed != null) {
         ...
         if (today.wind_deg != null) {
            sendIfChanged(name: "todayWindDirectionText", value: convertWindDirectionState(today.wind_deg))
        } 
	...
	logTrace "sendOWMData tom section started"		
        ...
        if (tom.humidity != null) {
            sendIfChanged(name: "tomHumidity", value: tom.humidity)
        }
       ...
       if (tom.wind_gust != null) {
        ...
        if (tom.wind_speed != null) {
         ...
         if (tom.wind_deg != null) {
            sendIfChanged(name: "tomWindDirectionText", value: convertWindDirectionState(tom.wind_deg))
        } 
    ...
    if (tda) {
        ...
        if (tda.humidity != null) {
            sendIfChanged(name: "tdaHumidity", value: tda.humidity)
        }
       ...
       if (tda.wind_gust != null) {
        ...
        if (tda.wind_speed != null) {
         ...
         if (tda.wind_deg != null) {
            sendIfChanged(name: "tdaWindDirectionText", value: convertWindDirectionState(tda.wind_deg))
        } 
...
/**
 * Calculates and formats the derived humidity text string based on selected humidityUnit.
 * Uses options: ["%": "Percent (%)", "%RH": "Percent RH (%RH)", "g/m³": "Absolute Humidity", "g/kg": "Mixing Ratio", "none": "None"]
 */
/**
 * Calculates and formats the derived humidity text string based on selected humidityUnit.
 * Handles automatic unit conversions for %, %RH, g/m³, and g/kg.
 */
void convertHumidityState(BigDecimal rawHumidity = null) {
    // 1. Fall back to the current attribute value if no explicit numeric argument was provided
    def hVal = rawHumidity != null ? rawHumidity : device.currentValue("currentHumidity")
    
    if (hVal == null) {
        logDebug "convertHumidityState: No humidity data available to format."
        sendIfChanged(name: "currentHumidityText", value: "--")
        return
    }

    // 2. Fetch the preference selection (fallback to standard percentage representation if unset)
    String hUnit = settings.humidityUnit ?: "%RH"
    Double calculatedValue = hVal.toDouble()
    
    // 3. Perform conversions if specialized unit metrics are chosen
    if (hUnit == "g/m³" || hUnit == "g/kg") {
        // Fetch current temperature and pressure attributes needed for advanced math 
        def tempVal = device.currentValue("currentTemperature")
        def pressVal = device.currentValue("currentPressure") // Expected in hPa / millibars by default from API
        
        if (tempVal != null) {
            // Convert temperature to Celsius if the driver is currently storing Fahrenheit
            ...
            // Calculate Saturation Vapor Pressure (Es) using the August-Roche-Magnus approximation
            Double es = 6.112 * Math.exp((17.67 * tempC) / (tempC + 243.5))
            // Actual Vapor Pressure (E)
            Double e = (calculatedValue / 100.0) * es
            
            if (hUnit == "g/m³") {
                // Absolute Humidity Formula: (e * 100 * 18.02) / (8.314 * (tempC + 273.15))
                calculatedValue = (e * 216.7) / (tempC + 273.15)
            } 
            else if (hUnit == "g/kg") {
                // Mixing Ratio requires Pressure. Fallback to standard baseline 1013.25 hPa if attribute is missing
                Double pressureHpa = (pressVal != null) ? pressVal.toDouble() : 1013.25
                
                // If the user preferred pressure unit is inHg, convert back to hPa for the formula
                ...
                // Mixing ratio formula (grams of water vapor per kilogram of dry air)
                calculatedValue = 621.99 * e / (pressureHpa - e)
            }
            
            // Round the converted values to a clean 2 decimal places for presentation layout
            calculatedValue = Math.round(calculatedValue * 100.0) / 100.0
        } else {
            logWarn "convertHumidityState: Cannot convert to ${hUnit} because currentTemperature is missing. Defaulting to relative percentage."
            hUnit = "%RH"
        }
    }

    // 4. Format the final output string
    String suffix = hUnit == "none" ? "" : " ${hUnit}"
    String formattedText = "${calculatedValue}${suffix}".trim()
    
    logDebug "calcHumidityState: Formatted humidity string to '${formattedText}' based on selection [${settings.humidityUnit}]."
    
    // 5. Update the derived text attribute
    sendIfChanged(name: "currentHumidityText", value: formattedText)
}
...
private String convertWindDirectionState(degrees) {
    if (degrees == null) {
        logDebug "convertWindDirectionState received null value."
        return "Unknown direction"
    }
    
    // Safely cast or convert value to a double
    double deg = 0.0
    try {
    ...
    // Normalize degrees to be strictly within 0.0 to 359.99...
    deg = (deg % 360 + 360) % 360

    // 16-point Compass Cardinal mapping array (each slice covers 22.5 degrees)
    def cardinals = [
        "N", "NNE", "NE", "ENE",
        "E", "ESE", "SE", "SSE",
        "S", "SSW", "SW", "WSW",
    ...
    // Calculate the index segment: adding 11.25 centers the degree value to the nearest slice
