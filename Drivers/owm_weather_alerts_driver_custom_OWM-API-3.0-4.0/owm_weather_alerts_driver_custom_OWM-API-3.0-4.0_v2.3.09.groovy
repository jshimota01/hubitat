...
void scheduledTextValue(List dataList) {
    ...
    if (dataList && dataList.size() >= 9) {
        ...
        Map rawJson = (dataList.size() >= 10 && dataList[9] instanceof Map) ? dataList[9] : [:]
        ...
        // --- RACE CONDITION FIX: Run Alert & Moon Phase calculations synchronously before tile rendering ---
        if (rawJson) {
            String calculatedCityAttr = state.usedCity ?: "Local Area"
            String iconBasePath = state.iconBasePath ?: "https://tinyurl.com/icnqz/"
            calcAlertsState(rawJson, calculatedCityAttr, iconBasePath)
        }
        
        calcMoonPhaseValue(todayData, tomData, tdaData)
        calcMoonPhaseSvgImage(todayData, tomData, tdaData)
        
        // --- Run tile generation ONLY after synchronously completing calculations ---
