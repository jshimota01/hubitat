 ...
 * 2026-97-18	jshimota	0.2.7	bug hunt, problems with date storage payloads
 *
 */
...
def sunRiseSetHandler(resp, data) {
    if(resp.getStatus() == 200 || resp.getStatus() == 207) {
        ...
        // Sync Data to Driver State Storage
        state.localSunrise = displayFormat.format(sRise)
        state.localSunset  = displayFormat.format(sSet)
        state.usedTwilightBegin = displayFormat.format(usedTwilightBegin)
        state.usedTwilightEnd = displayFormat.format(usedTwilightEnd)
        ...
        state.localDayLength = formattedDay_Length

        // Attempting a direct write to Global variables fails in drivers. 
        // Best Practice fallback: Warn user or handle via a linked app connector interface if needed.
        if (settings.debugEnable) {
            log.debug "Calculated Twilight Values: Begin=${state.usedTwilightBegin}, End=${state.usedTwilightEnd}. Note: Drivers cannot set Global Variables directly."
        }

        // Fire stringified clean event values to device dashboard interfaces
        ...
        sendEvent(name: 'localSolarNoon', value: displayFormat.format(sNoon))
        sendEvent(name: 'localCivilTwilightBegin', value: displayFormat.format(civBeg))
        sendEvent(name: 'localCivilTwilightEnd', value: displayFormat.format(civEnd))
        sendEvent(name: 'localNauticalTwilightBegin', value: displayFormat.format(nautBeg))
        sendEvent(name: 'localNauticalTwilightEnd', value: displayFormat.format(nautEnd))
        sendEvent(name: 'localAstronomicalTwilightBegin', value: displayFormat.format(astroBeg))
        sendEvent(name: 'localAstronomicalTwilightEnd', value: displayFormat.format(astroEnd))
        ...
        sendEvent(name: 'usedTwilightBegin', value: state.usedTwilightBegin)
        sendEvent(name: 'usedTwilightEnd', value: state.usedTwilightEnd)
