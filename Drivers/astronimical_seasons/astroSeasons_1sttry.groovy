...
def sendAsynchttpGet() {
    def getParams = [
		uri: "https://aa.usno.navy.mil/api/seasons?year=2023&tz=-8&dst=true",
	...
	asynchttpGet('myCallbackMethod', getParams, [dataitem1: "datavalue1"])
}

def myCallbackMethod(response, data) {
    if(data["dataitem1"] == "datavalue1")
    	log.debug "data was passed successfully"
    log.debug "status of get call is: ${response.status}"
	if(response.getStatus() == 200 || response.getStatus() == 207) {
		obs = parseJson(response.data)
		doJSONParse(obs)		// parse the data returned by USNavy
	} else {
		log.error "US Navy site returned data: $response"
	}
}

def doJSONParse() {
    String data1day = response.data.Status[0]
    sendEvent(name: "data 1 day is ", value: "${data1day}")
}
