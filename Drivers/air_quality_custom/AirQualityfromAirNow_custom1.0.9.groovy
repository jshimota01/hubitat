...
void pollAirNow() {
    if (apiKey == null) {
        ...
        sendEvent(name: "status", value: "Fail", descriptionText: "${device.displayName} status is Fail (Missing API Key)")
        sendEvent(name: "lastChecked", value: new Date().format("MM/dd/yyyy h:mm a", location.timeZone), descriptionText: "${device.displayName} lastChecked timestamp updated")
        return
    }
...
void pollHandler(resp, data) {
    ...
    if (resp.getStatus() == 200 || resp.getStatus() == 207) {
        ...
        log.warn "AirNow API lookup failed with status code: ${resp.getStatus()}"
        sendEvent(name: "status", value: "Fail", descriptionText: "${device.displayName} status is Fail (HTTP ${resp.getStatus()})")
    }
}
